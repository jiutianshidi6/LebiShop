...
namespace Shop.SQLDataAccess
{
	...
	public interface Lebi_Type_interface
	{
		...
		int Add(Lebi_Type model);
		void Update(Lebi_Type model);
		void Delete(int id);
		void Delete(string strWhere);
		void Delete(SQLPara para);
		Lebi_Type GetModel(int id);
		Lebi_Type GetModel(string strWhere);
		Lebi_Type GetModel(SQLPara para);
		List<Lebi_Type> GetList(string strWhere, string strFieldOrder, int PageSize, int page);
		List<Lebi_Type> GetList(SQLPara para, int PageSize, int page);
		List<Lebi_Type> GetList(string strWhere, string strFieldOrder);
		List<Lebi_Type> GetList(SQLPara para);
		Lebi_Type BindForm(Lebi_Type model);
		Lebi_Type SafeBindForm(Lebi_Type model);
		Lebi_Type ReaderBind(IDataReader dataReader);
	}

	/// <summary>
	/// 数据访问类D_Lebi_Type。
	/// </summary>
	public class D_Lebi_Type
	{
		static Lebi_Type_interface _Instance;
		public static Lebi_Type_interface Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            if (BaseUtils.BaseUtilsInstance.DBType == "access")
		                _Instance = new access_Lebi_Type();
		            else
		                _Instance = new sqlserver_Lebi_Type();
		        }
		        return _Instance;
		    }
		    set
		...
		{}
		#region  成员方法
	class sqlserver_Lebi_Type : Lebi_Type_interface
	{
		/// <summary>
		...
		public string GetValue(string colName,string strWhere)
		{
			...
			try
			{
				...
				strSql.Append("select " + colName + " from [Lebi_Type]");
				if(strWhere.Trim()!="")
				{
		...
		public int Counts(string strWhere)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from [Lebi_Type]");
			if(strWhere.Trim()!="")
				strSql.Append(" where "+strWhere);
		...
		public int GetMaxID(string strWhere)
		{
			...
			StringBuilder strSql = new StringBuilder();
			strSql.Append("select max(id) from [Lebi_Type]");
			if(strWhere.Trim()!="")
				strSql.Append(" where "+strWhere);
			return Convert.ToInt32(SqlUtils.SqlUtilsInstance.TextExecuteScalar(strSql.ToString()));
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from [Lebi_Type] ");
			strSql.Append(" where @id=id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_Type GetModel(string strWhere)
		{
			if (strWhere.IndexOf("lbsql{") > 0)
			{
				SQLPara para = new SQLPara(strWhere, "", "");
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_Type> GetList(string strWhere, string strFieldOrder, int PageSize, int page)
		{
			if (strWhere.IndexOf("lbsql{") > 0)
			{
		...
		public List<Lebi_Type> GetList(SQLPara para, int PageSize, int page)
		{
			string strTableName = "[Lebi_Type]";
			string strFieldKey = "id";
			StringBuilder strSql=new StringBuilder();
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_Type> GetList(string strWhere,string strFieldOrder)
		{
			if (strWhere.IndexOf("lbsql{") > 0)
			{
				SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_Type> GetList(SQLPara para)
		{
			string strTableName = "[Lebi_Type]";
			StringBuilder strSql=new StringBuilder();
		...
		public Lebi_Type BindForm(Lebi_Type model)
		{
			...
			if (HttpContext.Current.Request["Class"] != null)
				model.Class=Shop.Tools.RequestTool.RequestString("Class");
			...
			if (HttpContext.Current.Request["Color"] != null)
				model.Color=Shop.Tools.RequestTool.RequestString("Color");
				return model;
		}
		/// <summary>
		/// 安全方式绑定对象表单
	...
	class access_Lebi_Type : Lebi_Type_interface
	{
		/// <summary>
		/// 根据字段名，where条件获取一个值,返回字符串
		/// </summary>
		...
		public int Counts(string strWhere)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(*) from [Lebi_Type]");
			if(strWhere.Trim()!="")
				strSql.Append(" where "+strWhere);
