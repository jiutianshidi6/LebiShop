...
namespace Shop.SQLDataAccess
{
	...
	public interface Lebi_ImageSize_interface
	{
		...
		int Add(Lebi_ImageSize model);
		void Update(Lebi_ImageSize model);
		void Delete(int id);
		void Delete(string strWhere);
		void Delete(SQLPara para);
		Lebi_ImageSize GetModel(int id);
		Lebi_ImageSize GetModel(string strWhere);
		Lebi_ImageSize GetModel(SQLPara para);
		List<Lebi_ImageSize> GetList(string strWhere, string strFieldOrder, int PageSize, int page);
		List<Lebi_ImageSize> GetList(SQLPara para, int PageSize, int page);
		List<Lebi_ImageSize> GetList(string strWhere, string strFieldOrder);
		List<Lebi_ImageSize> GetList(SQLPara para);
		Lebi_ImageSize BindForm(Lebi_ImageSize model);
		Lebi_ImageSize SafeBindForm(Lebi_ImageSize model);
		Lebi_ImageSize ReaderBind(IDataReader dataReader);
	}

	/// <summary>
	/// 数据访问类D_Lebi_ImageSize。
	/// </summary>
	public class D_Lebi_ImageSize
	{
		static Lebi_ImageSize_interface _Instance;
		public static Lebi_ImageSize_interface Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            if (BaseUtils.BaseUtilsInstance.DBType == "access")
		                _Instance = new access_Lebi_ImageSize();
		            else
		                _Instance = new sqlserver_Lebi_ImageSize();
		        }
		        return _Instance;
		    }
		    set
		...
		public D_Lebi_ImageSize()
		{}
		#region  成员方法
	class sqlserver_Lebi_ImageSize : Lebi_ImageSize_interface
	{
		/// <summary>
		...
		public string GetValue(string colName,string strWhere)
		{
			...
			try
			{
				...
				strSql.Append("select " + colName + " from [Lebi_ImageSize]");
				if(strWhere.Trim()!="")
				{
		...
		public int Counts(string strWhere)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from [Lebi_ImageSize]");
			if(strWhere.Trim()!="")
				strSql.Append(" where "+strWhere);
		...
		public int GetMaxID(string strWhere)
		{
			...
			StringBuilder strSql = new StringBuilder();
			strSql.Append("select max(id) from [Lebi_ImageSize]");
			if(strWhere.Trim()!="")
				strSql.Append(" where "+strWhere);
			return Convert.ToInt32(SqlUtils.SqlUtilsInstance.TextExecuteScalar(strSql.ToString()));
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from [Lebi_ImageSize] ");
			strSql.Append(" where @id=id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_ImageSize GetModel(SQLPara para)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select top 1 * from [Lebi_ImageSize] ");
			if (para.Where != "")
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_ImageSize> GetList(string strWhere, string strFieldOrder, int PageSize, int page)
		{
			if (strWhere.IndexOf("lbsql{") > 0)
			{
			...
			string strTableName = "[Lebi_ImageSize]";
		...
		public List<Lebi_ImageSize> GetList(SQLPara para, int PageSize, int page)
		{
			string strTableName = "[Lebi_ImageSize]";
			string strFieldKey = "id";
			StringBuilder strSql=new StringBuilder();
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_ImageSize> GetList(string strWhere,string strFieldOrder)
		{
			if (strWhere.IndexOf("lbsql{") > 0)
			{
				SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
			...
			strSql.Append(" FROM [Lebi_ImageSize] ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public List<Lebi_ImageSize> GetList(SQLPara para)
		{
			string strTableName = "[Lebi_ImageSize]";
			StringBuilder strSql=new StringBuilder();
	...
	class access_Lebi_ImageSize : Lebi_ImageSize_interface
	{
		/// <summary>
		/// 根据字段名，where条件获取一个值,返回字符串
		/// </summary>
		...
		public int Counts(string strWhere)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(*) from [Lebi_ImageSize]");
			if(strWhere.Trim()!="")
				strSql.Append(" where "+strWhere);
