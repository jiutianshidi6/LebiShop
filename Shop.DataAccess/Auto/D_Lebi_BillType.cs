...
namespace Shop.SQLDataAccess
{
	...
	public interface Lebi_BillType_interface
	{
		...
		int Add(Lebi_BillType model);
		void Update(Lebi_BillType model);
		void Delete(int id);
		void Delete(string strWhere);
		void Delete(SQLPara para);
		Lebi_BillType GetModel(int id);
		Lebi_BillType GetModel(string strWhere);
		Lebi_BillType GetModel(SQLPara para);
		List<Lebi_BillType> GetList(string strWhere, string strFieldOrder, int PageSize, int page);
		List<Lebi_BillType> GetList(SQLPara para, int PageSize, int page);
		List<Lebi_BillType> GetList(string strWhere, string strFieldOrder);
		List<Lebi_BillType> GetList(SQLPara para);
		Lebi_BillType BindForm(Lebi_BillType model);
		Lebi_BillType SafeBindForm(Lebi_BillType model);
		Lebi_BillType ReaderBind(IDataReader dataReader);
	}

	/// <summary>
	/// 数据访问类D_Lebi_BillType。
	/// </summary>
	public class D_Lebi_BillType
	{
		static Lebi_BillType_interface _Instance;
		public static Lebi_BillType_interface Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            if (BaseUtils.BaseUtilsInstance.DBType == "access")
		                _Instance = new access_Lebi_BillType();
		            else
		                _Instance = new sqlserver_Lebi_BillType();
		        }
		        return _Instance;
		    }
		    set
		...
		public D_Lebi_BillType()
		{}
		#region  成员方法
	class sqlserver_Lebi_BillType : Lebi_BillType_interface
	{
		/// <summary>
		...
		public string GetValue(string colName,string strWhere)
		{
			...
			try
			{
				...
				strSql.Append("select " + colName + " from [Lebi_BillType]");
				if(strWhere.Trim()!="")
				{
		...
		public int Counts(string strWhere)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from [Lebi_BillType]");
			if(strWhere.Trim()!="")
				strSql.Append(" where "+strWhere);
		...
		public int GetMaxID(string strWhere)
		{
			...
			StringBuilder strSql = new StringBuilder();
			strSql.Append("select max(id) from [Lebi_BillType]");
			if(strWhere.Trim()!="")
				strSql.Append(" where "+strWhere);
			return Convert.ToInt32(SqlUtils.SqlUtilsInstance.TextExecuteScalar(strSql.ToString()));
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from [Lebi_BillType] ");
			strSql.Append(" where @id=id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_BillType GetModel(SQLPara para)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select top 1 * from [Lebi_BillType] ");
			if (para.Where != "")
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_BillType> GetList(string strWhere, string strFieldOrder, int PageSize, int page)
		{
			if (strWhere.IndexOf("lbsql{") > 0)
			{
			...
			string strTableName = "[Lebi_BillType]";
		...
		public List<Lebi_BillType> GetList(SQLPara para, int PageSize, int page)
		{
			string strTableName = "[Lebi_BillType]";
			string strFieldKey = "id";
			StringBuilder strSql=new StringBuilder();
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_BillType> GetList(string strWhere,string strFieldOrder)
		{
			if (strWhere.IndexOf("lbsql{") > 0)
			{
				SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
			...
			strSql.Append(" FROM [Lebi_BillType] ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public List<Lebi_BillType> GetList(SQLPara para)
		{
			string strTableName = "[Lebi_BillType]";
			StringBuilder strSql=new StringBuilder();
	...
	class access_Lebi_BillType : Lebi_BillType_interface
	{
		/// <summary>
		/// 根据字段名，where条件获取一个值,返回字符串
		/// </summary>
		...
		public int Counts(string strWhere)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(*) from [Lebi_BillType]");
			if(strWhere.Trim()!="")
				strSql.Append(" where "+strWhere);
