...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> euCXs=Languages();RecordCount=euCXs.Count;int euCX_index=1;
foreach (Shop.Model.Lebi_Language euCX in euCXs){%>
            <dd <%if (euCX_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=euCX.id%>,'<%=euCX.Code%>','<%=euCX.Path%>');"><%if (euCX.ImageUrl!=""){%><img src="<%=Image(euCX.ImageUrl) %>" /><%}%><%=euCX.Name%></a></dd>
            <%euCX_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int FLjt_index=1;
List<Lebi_Currency> FLjts = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency FLjt in FLjts){%>
            <dd <%if (FLjt_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=FLjt.id%>,'<%=FLjt.Code%>',<%=FLjt.ExchangeRate%>,'<%=FLjt.Msige%>','<%=FLjt.DecimalLength%>');"><%=FLjt.Code%></a></dd>
            <%FLjt_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int LnJa_index=1;
List<Lebi_Searchkey> LnJas = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey LnJa in LnJas){%><%=Lang(LnJa.Name)%><%LnJa_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int CuSw_index=1;
List<Lebi_Searchkey> CuSws = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey CuSw in CuSws){%>
<%if (CuSw.Type==1){ %><a href="<%=URL("P_Search",""+Lang(CuSw.Name)+"") %>"><%}else{ %><a href="<%=CuSw.URL%>" target="_blank"><%} %><span><%=Lang(CuSw.Name)%></span></a>
<%CuSw_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int YVvu_index=1;
List<Lebi_Page> YVvus = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page YVvu in YVvus){%>
<a class="menu" href="<%=URL("","",YVvu.url)%>"><span><%=YVvu.Name%></span></a>
<%YVvu_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Bnat_index=1;
List<Lebi_FriendLink> Bnats = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Bnat in Bnats){%>
            
                <% if (Bnat.Logo != "" && Bnat.IsPic == 1){ %><a href="<%=Bnat.Url%>" target="_blank"><img src="<%=Image(Bnat.Logo) %>" alt="<%=Bnat.Name%>" /></a><%}else{%><a href="<%=Bnat.Url%>" target="_blank"><%=Bnat.Name%></a><%} %>     
