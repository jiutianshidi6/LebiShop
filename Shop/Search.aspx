...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_Search"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> ldbMs=Languages();RecordCount=ldbMs.Count;int ldbM_index=1;
foreach (Shop.Model.Lebi_Language ldbM in ldbMs){%>
            <dd <%if (ldbM_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=ldbM.id%>,'<%=ldbM.Code%>','<%=ldbM.Path%>');"><%if (ldbM.ImageUrl!=""){%><img src="<%=Image(ldbM.ImageUrl) %>" /><%}%><%=ldbM.Name%></a></dd>
            <%ldbM_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int bkrO_index=1;
List<Lebi_Currency> bkrOs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency bkrO in bkrOs){%>
            <dd <%if (bkrO_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=bkrO.id%>,'<%=bkrO.Code%>',<%=bkrO.ExchangeRate%>,'<%=bkrO.Msige%>','<%=bkrO.DecimalLength%>');"><%=bkrO.Code%></a></dd>
            <%bkrO_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Jhgi_index=1;
List<Lebi_Searchkey> Jhgis = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Jhgi in Jhgis){%><%=Lang(Jhgi.Name)%><%Jhgi_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int cPVI_index=1;
List<Lebi_Searchkey> cPVIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey cPVI in cPVIs){%>
<%if (cPVI.Type==1){ %><a href="<%=URL("P_Search",""+Lang(cPVI.Name)+"") %>"><%}else{ %><a href="<%=cPVI.URL%>" target="_blank"><%} %><span><%=Lang(cPVI.Name)%></span></a>
<%cPVI_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int mloh_index=1;
List<Lebi_Page> mlohs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page mloh in mlohs){%>
<a class="menu" href="<%=URL("","",mloh.url)%>"><span><%=mloh.Name%></span></a>
<%mloh_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Asfh_index=1;
List<Lebi_Page> Asfhs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Asfh in Asfhs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",Asfh.id,Asfh.url)%>" target="<%=Asfh.target%>" title="<%=Asfh.Name%>"><span style="color:<%=Asfh.NameColor%>"><%=Asfh.Name%></span></a></li>
        <%Asfh_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int PdgW_index=1;
List<Lebi_FriendLink> PdgWs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink PdgW in PdgWs){%>
            
                <% if (PdgW.Logo != "" && PdgW.IsPic == 1){ %><a href="<%=PdgW.Url%>" target="_blank"><img src="<%=Image(PdgW.Logo) %>" alt="<%=PdgW.Name%>" /></a><%}else{%><a href="<%=PdgW.Url%>" target="_blank"><%=PdgW.Name%></a><%} %>     
