...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_NewsDetails"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> mjKgs=Languages();RecordCount=mjKgs.Count;int mjKg_index=1;
foreach (Shop.Model.Lebi_Language mjKg in mjKgs){%>
            <dd <%if (mjKg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=mjKg.id%>,'<%=mjKg.Code%>','<%=mjKg.Path%>');"><%if (mjKg.ImageUrl!=""){%><img src="<%=Image(mjKg.ImageUrl) %>" /><%}%><%=mjKg.Name%></a></dd>
            <%mjKg_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int AiuD_index=1;
List<Lebi_Currency> AiuDs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency AiuD in AiuDs){%>
            <dd <%if (AiuD_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=AiuD.id%>,'<%=AiuD.Code%>',<%=AiuD.ExchangeRate%>,'<%=AiuD.Msige%>','<%=AiuD.DecimalLength%>');"><%=AiuD.Code%></a></dd>
            <%AiuD_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int YFUV_index=1;
List<Lebi_Searchkey> YFUVs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey YFUV in YFUVs){%><%=Lang(YFUV.Name)%><%YFUV_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int IsFP_index=1;
List<Lebi_Searchkey> IsFPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey IsFP in IsFPs){%>
<%if (IsFP.Type==1){ %><a href="<%=URL("P_Search",""+Lang(IsFP.Name)+"") %>"><%}else{ %><a href="<%=IsFP.URL%>" target="_blank"><%} %><span><%=Lang(IsFP.Name)%></span></a>
<%IsFP_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int PwRi_index=1;
List<Lebi_Page> PwRis = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page PwRi in PwRis){%>
<a class="menu" href="<%=URL("","",PwRi.url)%>"><span><%=PwRi.Name%></span></a>
<%PwRi_index++;}%>
...
<ul class="text">
    <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int bXDk_index=1;
List<Lebi_Brand> bXDks = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand bXDk in bXDks){%>
    <li><a href="<%=URL("P_Brand",bXDk.id)%>" title="<%=Lang(bXDk.Name) %>"><%=Lang(bXDk.Name) %></a></li>
    <%bXDk_index++;}%>
</ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int emPr_index=1;
List<Lebi_FriendLink> emPrs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink emPr in emPrs){%>
            
                <% if (emPr.Logo != "" && emPr.IsPic == 1){ %><a href="<%=emPr.Url%>" target="_blank"><img src="<%=Image(emPr.Logo) %>" alt="<%=emPr.Name%>" /></a><%}else{%><a href="<%=emPr.Url%>" target="_blank"><%=emPr.Name%></a><%} %>     
