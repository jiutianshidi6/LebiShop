...
<% LoadPage("ljq@lebi.cn_43",15,"CN","P_Help"); %>
...
<div class="mhead clearfix">
	...
	<h2 id="pagename"><%=ThemePageMeta("P_Help","title")%></h2>
...
<script type="text/javascript">
    ...
    function ($) {
        $("img.contentlazy").lazyload({
            placeholder: "/Theme/system/wap/images/lazy.gif",
            effect: "fadeIn"
        });
    });
</script>
...
<div class="mc">
    ...
    if (type !=""){
        ...
        %>
    <%Table="Lebi_Page";Where=""+strwhere+"";PageSize=999;pageindex=Rint("page");RecordCount=B_Lebi_Page.Counts(Where);int uMGn_index=1;
List<Lebi_Page> uMGns = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page uMGn in uMGns){%>
    <div class="nodeitem clearfix" id="help<%=uMGn.id%>">
    <div class="mt">
        <h2><font style="color:<%=uMGn.NameColor%>"><%=uMGn.Name%></font></h2>
    </div>
    <%if (uMGn.Description != ""){ %><div class="tips"><%=uMGn.Description%></div><%} %>
    <div class="content">
    <%=ImgSrc(uMGn.Content,CurrentLanguage.IsLazyLoad) %>
    </div>
    </div>
    <%uMGn_index++;}%>
    <%} %>
