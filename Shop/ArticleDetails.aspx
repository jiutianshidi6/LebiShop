<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.P_ArticleDetails" validateRequest="false"%>


<%@ Import Namespace="Shop.Bussiness" %>
<%@ Import Namespace="Shop.Model" %>
<%@ Import Namespace="System.Collections.Generic" %>
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_ArticleDetails"); %>
...
<title><%=ThemePageMeta("P_ArticleDetails","title")%></title>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Language" content="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0;" />
<meta name="keywords" content="<%=ThemePageMeta("P_ArticleDetails","keywords")%>" />
<meta name="description" content="<%=ThemePageMeta("P_ArticleDetails","description")%>" />
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> GFmWs=Languages();RecordCount=GFmWs.Count;int GFmW_index=1;
foreach (Shop.Model.Lebi_Language GFmW in GFmWs){%>
            <dd <%if (GFmW_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=GFmW.id%>,'<%=GFmW.Code%>','<%=GFmW.Path%>');"><%if (GFmW.ImageUrl!=""){%><img src="<%=Image(GFmW.ImageUrl) %>" /><%}%><%=GFmW.Name%></a></dd>
            <%GFmW_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int qVRJ_index=1;
List<Lebi_Currency> qVRJs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency qVRJ in qVRJs){%>
            <dd <%if (qVRJ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=qVRJ.id%>,'<%=qVRJ.Code%>',<%=qVRJ.ExchangeRate%>,'<%=qVRJ.Msige%>','<%=qVRJ.DecimalLength%>');"><%=qVRJ.Code%></a></dd>
            <%qVRJ_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int cfOe_index=1;
List<Lebi_Searchkey> cfOes = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey cfOe in cfOes){%><%=Lang(cfOe.Name)%><%cfOe_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int WEKP_index=1;
List<Lebi_Searchkey> WEKPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey WEKP in WEKPs){%>
<%if (WEKP.Type==1){ %><a href="<%=URL("P_Search",""+Lang(WEKP.Name)+"") %>"><%}else{ %><a href="<%=WEKP.URL%>" target="_blank"><%} %><span><%=Lang(WEKP.Name)%></span></a>
<%WEKP_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int lpaz_index=1;
List<Lebi_Page> lpazs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page lpaz in lpazs){%>
<a class="menu" href="<%=URL("","",lpaz.url)%>"><span><%=lpaz.Name%></span></a>
<%lpaz_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id = "+ node.id +" and Language like '%" + CurrentLanguage.Code +"%' and id <> "+ id +"";Order="id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Wsxz_index=1;
List<Lebi_Page> Wsxzs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Wsxz in Wsxzs){%>
        <%if (Wsxz.ImageSmall !=""){ %>
        <li class="image"><div class="image"><p><a href="<%=URL("P_ArticleDetails",Wsxz.id,Wsxz.url)%>" target="_blank"><img <%if (CurrentLanguage.IsLazyLoad==1){ %>class="lazy" src="/Theme/system/images/lazy.gif" data-original<%}else{ %>src<%} %>="<%=Image(Wsxz.ImageSmall) %>" title="<%=Lang(Wsxz.Name) %>" /></a></p></div><div class="title"><a href="<%=URL("P_ArticleDetails",Wsxz.id,Wsxz.url)%>" title="<%=Wsxz.Name%>" target="_blank"><span style="color:<%=Wsxz.NameColor%>"><%=Wsxz.Name%></span></a></div></li>
        <%} else{%>
        <li class="text"><div class="title"><a href="<%=URL("P_ArticleDetails",Wsxz.id,Wsxz.url)%>" title="<%=Wsxz.Name%>" target="_blank"><span style="color:<%=Wsxz.NameColor%>"><%=Wsxz.Name%></span></a></div></li>
        <%} %>
        <%Wsxz_index++;}%>
    </ul>
</div>
...
<div class="nodecontent">
    ...
    <div class="mc clearfix">
        ...
        <div class="content">
            <%=ImgSrc(page.Content,CurrentLanguage.IsLazyLoad) %>
        </div>
    </div>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int hXkf_index=1;
List<Lebi_FriendLink> hXkfs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink hXkf in hXkfs){%>
            
                <% if (hXkf.Logo != "" && hXkf.IsPic == 1){ %><a href="<%=hXkf.Url%>" target="_blank"><img src="<%=Image(hXkf.Logo) %>" alt="<%=hXkf.Name%>" /></a><%}else{%><a href="<%=hXkf.Url%>" target="_blank"><%=hXkf.Name%></a><%} %>     
