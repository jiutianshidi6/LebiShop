...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_NewsDetails"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> pHuFs=Languages();RecordCount=pHuFs.Count;int pHuF_index=1;
foreach (Shop.Model.Lebi_Language pHuF in pHuFs){%>
            <dd <%if (pHuF_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=pHuF.id%>,'<%=pHuF.Code%>','<%=pHuF.Path%>');"><%if (pHuF.ImageUrl!=""){%><img src="<%=Image(pHuF.ImageUrl) %>" /><%}%><%=pHuF.Name%></a></dd>
            <%pHuF_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int rnai_index=1;
List<Lebi_Currency> rnais = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency rnai in rnais){%>
            <dd <%if (rnai_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=rnai.id%>,'<%=rnai.Code%>',<%=rnai.ExchangeRate%>,'<%=rnai.Msige%>','<%=rnai.DecimalLength%>');"><%=rnai.Code%></a></dd>
            <%rnai_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ARKV_index=1;
List<Lebi_Searchkey> ARKVs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ARKV in ARKVs){%><%=Lang(ARKV.Name)%><%ARKV_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int KWEx_index=1;
List<Lebi_Searchkey> KWExs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey KWEx in KWExs){%>
<%if (KWEx.Type==1){ %><a href="<%=URL("P_Search",""+Lang(KWEx.Name)+"") %>"><%}else{ %><a href="<%=KWEx.URL%>" target="_blank"><%} %><span><%=Lang(KWEx.Name)%></span></a>
<%KWEx_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int wVRx_index=1;
List<Lebi_Page> wVRxs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page wVRx in wVRxs){%>
<a class="menu" href="<%=URL("","",wVRx.url)%>"><span><%=wVRx.Name%></span></a>
<%wVRx_index++;}%>
...
<ul class="text">
    <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int ewOP_index=1;
List<Lebi_Brand> ewOPs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand ewOP in ewOPs){%>
    <li><a href="<%=URL("P_Brand",ewOP.id)%>" title="<%=Lang(ewOP.Name) %>"><%=Lang(ewOP.Name) %></a></li>
    <%ewOP_index++;}%>
</ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int brpM_index=1;
List<Lebi_FriendLink> brpMs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink brpM in brpMs){%>
            
                <% if (brpM.Logo != "" && brpM.IsPic == 1){ %><a href="<%=brpM.Url%>" target="_blank"><img src="<%=Image(brpM.Logo) %>" alt="<%=brpM.Name%>" /></a><%}else{%><a href="<%=brpM.Url%>" target="_blank"><%=brpM.Name%></a><%} %>     
