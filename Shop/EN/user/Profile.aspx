...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> hYSds=Languages();RecordCount=hYSds.Count;int hYSd_index=1;
foreach (Shop.Model.Lebi_Language hYSd in hYSds){%>
            <dd <%if (hYSd_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=hYSd.id%>,'<%=hYSd.Code%>','<%=hYSd.Path%>');"><%if (hYSd.ImageUrl!=""){%><img src="<%=Image(hYSd.ImageUrl) %>" /><%}%><%=hYSd.Name%></a></dd>
            <%hYSd_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int rIdl_index=1;
List<Lebi_Currency> rIdls = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency rIdl in rIdls){%>
            <dd <%if (rIdl_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=rIdl.id%>,'<%=rIdl.Code%>',<%=rIdl.ExchangeRate%>,'<%=rIdl.Msige%>','<%=rIdl.DecimalLength%>');"><%=rIdl.Code%></a></dd>
            <%rIdl_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int fOGb_index=1;
List<Lebi_Searchkey> fOGbs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey fOGb in fOGbs){%><%=Lang(fOGb.Name)%><%fOGb_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int VHyu_index=1;
List<Lebi_Searchkey> VHyus = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey VHyu in VHyus){%>
<%if (VHyu.Type==1){ %><a href="<%=URL("P_Search",""+Lang(VHyu.Name)+"") %>"><%}else{ %><a href="<%=VHyu.URL%>" target="_blank"><%} %><span><%=Lang(VHyu.Name)%></span></a>
<%VHyu_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int LWNj_index=1;
List<Lebi_Page> LWNjs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page LWNj in LWNjs){%>
<a class="menu" href="<%=URL("","",LWNj.url)%>"><span><%=LWNj.Name%></span></a>
<%LWNj_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int dFRm_index=1;
List<Lebi_FriendLink> dFRms = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink dFRm in dFRms){%>
            
                <% if (dFRm.Logo != "" && dFRm.IsPic == 1){ %><a href="<%=dFRm.Url%>" target="_blank"><img src="<%=Image(dFRm.Logo) %>" alt="<%=dFRm.Name%>" /></a><%}else{%><a href="<%=dFRm.Url%>" target="_blank"><%=dFRm.Name%></a><%} %>     
