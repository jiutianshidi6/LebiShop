...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserReturnDetails"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> Xodxs=Languages();RecordCount=Xodxs.Count;int Xodx_index=1;
foreach (Shop.Model.Lebi_Language Xodx in Xodxs){%>
            <dd <%if (Xodx_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Xodx.id%>,'<%=Xodx.Code%>','<%=Xodx.Path%>');"><%if (Xodx.ImageUrl!=""){%><img src="<%=Image(Xodx.ImageUrl) %>" /><%}%><%=Xodx.Name%></a></dd>
            <%Xodx_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int kohU_index=1;
List<Lebi_Currency> kohUs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency kohU in kohUs){%>
            <dd <%if (kohU_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=kohU.id%>,'<%=kohU.Code%>',<%=kohU.ExchangeRate%>,'<%=kohU.Msige%>','<%=kohU.DecimalLength%>');"><%=kohU.Code%></a></dd>
            <%kohU_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int rpeR_index=1;
List<Lebi_Searchkey> rpeRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey rpeR in rpeRs){%><%=Lang(rpeR.Name)%><%rpeR_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int PQQM_index=1;
List<Lebi_Searchkey> PQQMs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey PQQM in PQQMs){%>
<%if (PQQM.Type==1){ %><a href="<%=URL("P_Search",""+Lang(PQQM.Name)+"") %>"><%}else{ %><a href="<%=PQQM.URL%>" target="_blank"><%} %><span><%=Lang(PQQM.Name)%></span></a>
<%PQQM_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int JqnM_index=1;
List<Lebi_Page> JqnMs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page JqnM in JqnMs){%>
<a class="menu" href="<%=URL("","",JqnM.url)%>"><span><%=JqnM.Name%></span></a>
<%JqnM_index++;}%>
...
<div id="orderstate">
    <div class="mt">
        <div class="left">
            <strong>Order ID：<%=order.Code %>&nbsp;&nbsp;&nbsp;&nbsp;Status：<span class="ftx14"><%=ReturnStatus(order)%></span></strong>
        </div>
        <div class="right">
            <div class="toolbar">
            </div>
    ...
    <div id="orderinfo" class="clearfix">
        ...
        <div class="mc">
            <dl class="fore clearfix">
                ...
                <dd>
                    <ul>
                        <li>Express Company：<%=t_o.Transport_Name %></li>
                        <li>Shipping Track Number：<%=t_o.Code %></li>
                        <li>Status：<%=TypeName(t_o.Type_id_TransportOrderStatus) %></li>
                    </ul>
                </dd>
        ...
        <div class="total">
            <ul>
                <li><span>Total Products Amount：</span><%=FormatMoney(order.Money_Product) %></li>
                <li><span>+ Shipping Fees：</span><%=FormatMoney(order.Money_Transport) %></li>
                <li><span>+ Taxes：</span><%=FormatMoney(order.Money_Bill) %></li>
            </ul>
            <span class="clear"></span>
            <div class="extra">
                Refund：<span class="red"><b><%=FormatMoney(order.Money_Give) %></b></span></div>
        </div>
    </div>
</div>
<script type="text/javascript">
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int dBHi_index=1;
List<Lebi_FriendLink> dBHis = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink dBHi in dBHis){%>
            
                <% if (dBHi.Logo != "" && dBHi.IsPic == 1){ %><a href="<%=dBHi.Url%>" target="_blank"><img src="<%=Image(dBHi.Logo) %>" alt="<%=dBHi.Name%>" /></a><%}else{%><a href="<%=dBHi.Url%>" target="_blank"><%=dBHi.Name%></a><%} %>     
