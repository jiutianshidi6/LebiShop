...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserReturn"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> Czaas=Languages();RecordCount=Czaas.Count;int Czaa_index=1;
foreach (Shop.Model.Lebi_Language Czaa in Czaas){%>
            <dd <%if (Czaa_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Czaa.id%>,'<%=Czaa.Code%>','<%=Czaa.Path%>');"><%if (Czaa.ImageUrl!=""){%><img src="<%=Image(Czaa.ImageUrl) %>" /><%}%><%=Czaa.Name%></a></dd>
            <%Czaa_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int WVzL_index=1;
List<Lebi_Currency> WVzLs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency WVzL in WVzLs){%>
            <dd <%if (WVzL_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=WVzL.id%>,'<%=WVzL.Code%>',<%=WVzL.ExchangeRate%>,'<%=WVzL.Msige%>','<%=WVzL.DecimalLength%>');"><%=WVzL.Code%></a></dd>
            <%WVzL_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int sBln_index=1;
List<Lebi_Searchkey> sBlns = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey sBln in sBlns){%><%=Lang(sBln.Name)%><%sBln_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int pjHd_index=1;
List<Lebi_Searchkey> pjHds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey pjHd in pjHds){%>
<%if (pjHd.Type==1){ %><a href="<%=URL("P_Search",""+Lang(pjHd.Name)+"") %>"><%}else{ %><a href="<%=pjHd.URL%>" target="_blank"><%} %><span><%=Lang(pjHd.Name)%></span></a>
<%pjHd_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int vDdf_index=1;
List<Lebi_Page> vDdfs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page vDdf in vDdfs){%>
<a class="menu" href="<%=URL("","",vDdf.url)%>"><span><%=vDdf.Name%></span></a>
<%vDdf_index++;}%>
...
<div class="user">
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        ...
        <tr>
            ...
            <td><%if(o.IsShipped==1){ %><a href="<%=URL("P_UserReturnDetails",o.id)%>" target="_blank">View</a><%}else{ %><a href="<%=URL("P_UserReturnShip",o.id)%>" target="_blank">View</a><%} %>
            </td>
        </tr>
        <%} %>
    </table>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int HTuU_index=1;
List<Lebi_FriendLink> HTuUs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink HTuU in HTuUs){%>
            
                <% if (HTuU.Logo != "" && HTuU.IsPic == 1){ %><a href="<%=HTuU.Url%>" target="_blank"><img src="<%=Image(HTuU.Logo) %>" alt="<%=HTuU.Name%>" /></a><%}else{%><a href="<%=HTuU.Url%>" target="_blank"><%=HTuU.Name%></a><%} %>     
