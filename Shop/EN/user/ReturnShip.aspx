<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.P_UserReturnShip" validateRequest="false"%>


<%@ Import Namespace="Shop.Bussiness" %>
<%@ Import Namespace="Shop.Model" %>
<%@ Import Namespace="System.Collections.Generic" %>
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserReturnShip"); %>
...
<title><%=ThemePageMeta("P_UserReturnShip","title")%></title>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Language" content="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0;" />
<meta name="keywords" content="<%=ThemePageMeta("P_UserReturnShip","keywords")%>" />
<meta name="description" content="<%=ThemePageMeta("P_UserReturnShip","description")%>" />
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> KmRFs=Languages();RecordCount=KmRFs.Count;int KmRF_index=1;
foreach (Shop.Model.Lebi_Language KmRF in KmRFs){%>
            <dd <%if (KmRF_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=KmRF.id%>,'<%=KmRF.Code%>','<%=KmRF.Path%>');"><%if (KmRF.ImageUrl!=""){%><img src="<%=Image(KmRF.ImageUrl) %>" /><%}%><%=KmRF.Name%></a></dd>
            <%KmRF_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int TfJe_index=1;
List<Lebi_Currency> TfJes = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency TfJe in TfJes){%>
            <dd <%if (TfJe_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=TfJe.id%>,'<%=TfJe.Code%>',<%=TfJe.ExchangeRate%>,'<%=TfJe.Msige%>','<%=TfJe.DecimalLength%>');"><%=TfJe.Code%></a></dd>
            <%TfJe_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int QOAS_index=1;
List<Lebi_Searchkey> QOASs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey QOAS in QOASs){%><%=Lang(QOAS.Name)%><%QOAS_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int cOfQ_index=1;
List<Lebi_Searchkey> cOfQs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey cOfQ in cOfQs){%>
<%if (cOfQ.Type==1){ %><a href="<%=URL("P_Search",""+Lang(cOfQ.Name)+"") %>"><%}else{ %><a href="<%=cOfQ.URL%>" target="_blank"><%} %><span><%=Lang(cOfQ.Name)%></span></a>
<%cOfQ_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int mjwe_index=1;
List<Lebi_Page> mjwes = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page mjwe in mjwes){%>
<a class="menu" href="<%=URL("","",mjwe.url)%>"><span><%=mjwe.Name%></span></a>
<%mjwe_index++;}%>
...
<div class="userbox">
    ...
    <dl>
        <dt>Express Company：</dt>
        <dd><input name="Transport_Name" id="Transport_Name" type="text" class="input" shop="true" /></dd>
    </dl>
    <dl>
        <dt>Tracking Number：</dt>
        <dd><input name="Code" id="Code" type="text" class="input" shop="true" /></dd>
    </dl>
    <dl class="dl-btn">
        <dt></dt>
...
<script type="text/javascript" >
    function submit() {
        ...
        var url = path+"/ajax/ajax_order.aspx?__Action=torder_shipping&order_id=<%=order.id %>";
        RequestAjax(url, postData, function () { MsgBox(1, "Operation Successful", "<%=URL("P_UserReturn","") %>") });
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int luHd_index=1;
List<Lebi_FriendLink> luHds = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink luHd in luHds){%>
            
                <% if (luHd.Logo != "" && luHd.IsPic == 1){ %><a href="<%=luHd.Url%>" target="_blank"><img src="<%=Image(luHd.Logo) %>" alt="<%=luHd.Name%>" /></a><%}else{%><a href="<%=luHd.Url%>" target="_blank"><%=luHd.Name%></a><%} %>     
