...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserCenter"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> mPKss=Languages();RecordCount=mPKss.Count;int mPKs_index=1;
foreach (Shop.Model.Lebi_Language mPKs in mPKss){%>
            <dd <%if (mPKs_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=mPKs.id%>,'<%=mPKs.Code%>','<%=mPKs.Path%>');"><%if (mPKs.ImageUrl!=""){%><img src="<%=Image(mPKs.ImageUrl) %>" /><%}%><%=mPKs.Name%></a></dd>
            <%mPKs_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int QNaz_index=1;
List<Lebi_Currency> QNazs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency QNaz in QNazs){%>
            <dd <%if (QNaz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=QNaz.id%>,'<%=QNaz.Code%>',<%=QNaz.ExchangeRate%>,'<%=QNaz.Msige%>','<%=QNaz.DecimalLength%>');"><%=QNaz.Code%></a></dd>
            <%QNaz_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int NJVs_index=1;
List<Lebi_Searchkey> NJVss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey NJVs in NJVss){%><%=Lang(NJVs.Name)%><%NJVs_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MnUB_index=1;
List<Lebi_Searchkey> MnUBs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MnUB in MnUBs){%>
<%if (MnUB.Type==1){ %><a href="<%=URL("P_Search",""+Lang(MnUB.Name)+"") %>"><%}else{ %><a href="<%=MnUB.URL%>" target="_blank"><%} %><span><%=Lang(MnUB.Name)%></span></a>
<%MnUB_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int VxOy_index=1;
List<Lebi_Page> VxOys = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page VxOy in VxOys){%>
<a class="menu" href="<%=URL("","",VxOy.url)%>"><span><%=VxOy.Name%></span></a>
<%VxOy_index++;}%>
...
<div class="userbox">
    <div class="mt">
        <h2><%=CurrentUser.NickName %>&nbsp;Welcome</h2>
    </div>
    <div class="mc default-info">
        ...
        <dl>
            ...
            <dd><%=FormatMoney(CurrentUser.Money) %> <a href="<%=URL("P_Recharge","") %>"  class="click">Recharge</a></dd>
        </dl>
        <dl>
            <dt>Point：</dt>
...
<div class="userbox">
    ...
    <div class="mc default-static">
        ...
        <dl>
            <dt>My Order：</dt>
            <dd>Total：<a href="<%=URL("P_UserOrders","")%>"><strong><%=Shop.Bussiness.Order.GetCount_Order("","","User_id = "+ CurrentUser.id +" and Type_id_OrderType = 211") %></strong></a>&nbsp;&nbsp;Awaiting Payment：<a href="<%=URL("P_UserOrders","")%>?status=1"><strong><%=Shop.Bussiness.Order.GetCount_Order("","","User_id = "+ CurrentUser.id +" and Type_id_OrderType = 211 and IsPaid = 0") %></strong></a>&nbsp;&nbsp;Pending Goods Receiving：<a href="<%=URL("P_UserOrders","")%>?status=2"><strong><%=Shop.Bussiness.Order.GetCount_Order("","","User_id = "+ CurrentUser.id +" and Type_id_OrderType = 211 and IsShipped = 1 and IsReceived = 0") %></strong></a>&nbsp;&nbsp;Received：<a href="<%=URL("P_UserOrders","")%>?status=3"><strong><%=Shop.Bussiness.Order.GetCount_Order("","","User_id = "+ CurrentUser.id +" and Type_id_OrderType = 211 and IsReceived = 1") %></strong></a>&nbsp;&nbsp;Completed：<a href="<%=URL("P_UserOrders","")%>?status=2"><strong><%=Shop.Bussiness.Order.GetCount_Order("","","User_id = "+ CurrentUser.id +" and Type_id_OrderType = 211 and IsCompleted = 1") %></strong></a></dd>
        </dl>
        <dl>
            <dt>My  like：</dt>
            <dd>Total：<a href="<%=URL("P_UserLike","")%>"><strong><%=Like_Product_Count() %></strong></a></dd>
        </dl>
        <dl>
            <dt>Reviews：</dt>
            <dd>Total：<a href="<%=URL("P_UserLike","")%>"><strong><%=Count_Comment(0)+Count_Comment(1) %></strong></a>&nbsp;&nbsp;been evaluated：<a href="<%=URL("P_UserComment","0","")%>"><strong><%=Count_Comment(1) %></strong></a>&nbsp;&nbsp;to be evaluated：<%if(Count_Comment(0)>0){%><a href="<%=URL("P_UserComment","1","")%>"><strong style="color:#ff0000"><%=Count_Comment(0) %></strong></a><%}else{ %>0<%} %></dd>
        </dl>
        <dl>
            <dt>Comment：</dt>
            <dd>Total：<a href="<%=URL("P_UserAsk","")%>"><strong><%=Count_ProductAsk(0)+Count_ProductAsk(1) %></strong></a>&nbsp;&nbsp;Replied：<a href="<%=URL("P_UserAsk","","")%>?status=283"><strong><%=Count_ProductAsk(1) %></strong></a>&nbsp;&nbsp;
                        Unreplied：<%if(Count_ProductAsk(0)>0){%><a href="<%=URL("P_UserAsk","","")%>?status=282"><strong style="color:#ff0000"><%=Count_ProductAsk(0) %></strong></a><%}else{ %>0<%} %></dd>
        </dl>
        <dl>
            <dt>Messages：</dt>
            <dd>Total：<a href="<%=URL("P_UserMessage","0")%>"><strong><%=Shop.Bussiness.Message.GetCount_Message("", "", "User_id_To = "+ CurrentUser.id +" or User_id_From = "+ CurrentUser.id +"")%></strong></a>&nbsp;&nbsp;Inbox：<a href="<%=URL("P_UserMessage","0")%>"><strong><%=Shop.Bussiness.Message.GetCount_Message("", "", "User_id_To = "+ CurrentUser.id +"")%></strong></a>&nbsp;&nbsp;Outbox：<a href="<%=URL("P_UserMessage","1")%>"><strong><%=Shop.Bussiness.Message.GetCount_Message("", "", "User_id_From = "+ CurrentUser.id +"")%></strong></a>&nbsp;&nbsp;new information：<%if(Count_Message(0)>0){%><a href="<%=URL("P_UserMessage","0")%>"><strong style="color:#ff0000"><%=Count_Message(0) %></strong></a><%}else{ %>0<%} %></dd>
        </dl>
        </div>
    </div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int ecYf_index=1;
List<Lebi_FriendLink> ecYfs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink ecYf in ecYfs){%>
            
                <% if (ecYf.Logo != "" && ecYf.IsPic == 1){ %><a href="<%=ecYf.Url%>" target="_blank"><img src="<%=Image(ecYf.Logo) %>" alt="<%=ecYf.Name%>" /></a><%}else{%><a href="<%=ecYf.Url%>" target="_blank"><%=ecYf.Name%></a><%} %>     
