...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserAccount"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> oPnVs=Languages();RecordCount=oPnVs.Count;int oPnV_index=1;
foreach (Shop.Model.Lebi_Language oPnV in oPnVs){%>
            <dd <%if (oPnV_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=oPnV.id%>,'<%=oPnV.Code%>','<%=oPnV.Path%>');"><%if (oPnV.ImageUrl!=""){%><img src="<%=Image(oPnV.ImageUrl) %>" /><%}%><%=oPnV.Name%></a></dd>
            <%oPnV_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int PAIM_index=1;
List<Lebi_Currency> PAIMs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency PAIM in PAIMs){%>
            <dd <%if (PAIM_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=PAIM.id%>,'<%=PAIM.Code%>',<%=PAIM.ExchangeRate%>,'<%=PAIM.Msige%>','<%=PAIM.DecimalLength%>');"><%=PAIM.Code%></a></dd>
            <%PAIM_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int FQtR_index=1;
List<Lebi_Searchkey> FQtRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey FQtR in FQtRs){%><%=Lang(FQtR.Name)%><%FQtR_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MOfl_index=1;
List<Lebi_Searchkey> MOfls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MOfl in MOfls){%>
<%if (MOfl.Type==1){ %><a href="<%=URL("P_Search",""+Lang(MOfl.Name)+"") %>"><%}else{ %><a href="<%=MOfl.URL%>" target="_blank"><%} %><span><%=Lang(MOfl.Name)%></span></a>
<%MOfl_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int oXvp_index=1;
List<Lebi_Page> oXvps = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page oXvp in oXvps){%>
<a class="menu" href="<%=URL("","",oXvp.url)%>"><span><%=oXvp.Name%></span></a>
<%oXvp_index++;}%>
...
<div class="userbox">
    ...
    <dl>
        ...
        <dd><%if(CurrentUser.bind_qq_id=="") {%>
            <a href="<%=Shop.Platform.QQ.Instance.LoginURL(backurl) %>" >Bind</a>
            <%}else{ %>
            <a href="javascript:void(0);" onclick="cancalbind('qq');" >Unbind</a>
            <%} %></dd>
    </dl>
    <%} %>
    ...
    <dl>
        ...
        <dd><%if(CurrentUser.bind_weibo_id=="") {%>
            <a href="<%=Shop.Platform.Weibo.Instance.LoginURL(backurl) %>" >Bind</a>
            <%}else{ %>
            <a href="javascript:void(0);" onclick="cancalbind('weibo');" >Unbind</a>
            <%} %></dd>
    </dl>
    <%} %>
    ...
    <dl>
        ...
        <dd><%if(CurrentUser.bind_taobao_id=="") {%>
            <a href="<%=Shop.Platform.Taobao.Instance.LoginURL(backurl) %>" >Bind</a>
            <%}else{ %>
            <a href="javascript:void(0);" onclick="cancalbind('taobao');" >Unbind</a>
            <%} %></dd>
    </dl>
    <%} %>
    ...
    <dl>
        ...
        <dd><%if(CurrentUser.bind_facebook_id=="") {%>
            <a href="<%=Shop.Platform.Facebook.Instance.LoginURL(backurl) %>" >Bind</a>
            <%}else{ %>
            <a href="javascript:void(0);" onclick="cancalbind('facebook');" >Unbind</a>
            <%} %></dd>
    </dl>
    <%} %>
    ...
    <dl>
        ...
        <dd><%if(CurrentUser.bind_weixin_id=="") {%>
            ...
            <%}else{ %>
            <a href="javascript:void(0);" onclick="cancalbind('weixin');" >Unbind</a>
            <%} %></dd>
    </dl>
    <%} %>
    </div>
...
<div class="userbox">
    ...
    <dl class="dl-btn">
        <dt></dt>
        <dd><a href="javascript:void(0)" onclick="userloginsubmit();" class="btn btn-11"><s></s>Submit</a></dd>
    </dl>
    </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    function userloginsubmit() {
        ...
        RequestAjax(url, postData, function (res) { MsgBox(1, "Operation Successful", "?") });
    }
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int OrEt_index=1;
List<Lebi_FriendLink> OrEts = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink OrEt in OrEts){%>
            
                <% if (OrEt.Logo != "" && OrEt.IsPic == 1){ %><a href="<%=OrEt.Url%>" target="_blank"><img src="<%=Image(OrEt.Logo) %>" alt="<%=OrEt.Name%>" /></a><%}else{%><a href="<%=OrEt.Url%>" target="_blank"><%=OrEt.Name%></a><%} %>     
