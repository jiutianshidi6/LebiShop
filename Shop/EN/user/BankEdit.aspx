...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserBankEdit"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> nxbgs=Languages();RecordCount=nxbgs.Count;int nxbg_index=1;
foreach (Shop.Model.Lebi_Language nxbg in nxbgs){%>
            <dd <%if (nxbg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=nxbg.id%>,'<%=nxbg.Code%>','<%=nxbg.Path%>');"><%if (nxbg.ImageUrl!=""){%><img src="<%=Image(nxbg.ImageUrl) %>" /><%}%><%=nxbg.Name%></a></dd>
            <%nxbg_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int uaVa_index=1;
List<Lebi_Currency> uaVas = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency uaVa in uaVas){%>
            <dd <%if (uaVa_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=uaVa.id%>,'<%=uaVa.Code%>',<%=uaVa.ExchangeRate%>,'<%=uaVa.Msige%>','<%=uaVa.DecimalLength%>');"><%=uaVa.Code%></a></dd>
            <%uaVa_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ORfB_index=1;
List<Lebi_Searchkey> ORfBs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ORfB in ORfBs){%><%=Lang(ORfB.Name)%><%ORfB_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MeAK_index=1;
List<Lebi_Searchkey> MeAKs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MeAK in MeAKs){%>
<%if (MeAK.Type==1){ %><a href="<%=URL("P_Search",""+Lang(MeAK.Name)+"") %>"><%}else{ %><a href="<%=MeAK.URL%>" target="_blank"><%} %><span><%=Lang(MeAK.Name)%></span></a>
<%MeAK_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int kaSB_index=1;
List<Lebi_Page> kaSBs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page kaSB in kaSBs){%>
<a class="menu" href="<%=URL("","",kaSB.url)%>"><span><%=kaSB.Name%></span></a>
<%kaSB_index++;}%>
...
<div class="userbox">
    ...
    <dl>
        ...
        <dd>
        <label><input type="radio" name="PayType" value="bank" shop="true" onclick="SetType();" <%=model.PayType == "bank"?"":"checked"%> />Bank transfer</label>
        <label><input type="radio" name="PayType" value="alipay" shop="true" onclick="SetType();" <%=model.PayType == "alipay"?"checked":""%> />Alipay</label>
        </dd>
    </dl>
    <dl id="DIVUserName">
        <dt>Name：</dt>
...
<script type="text/javascript">
    function submit() {
        ...
        RequestAjax(url, postData, function () { MsgBox(1, "Operation Successful", "<%=URL("P_UserBank","") %>") });
    }
    function SetType()
    {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int sCXN_index=1;
List<Lebi_FriendLink> sCXNs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink sCXN in sCXNs){%>
            
                <% if (sCXN.Logo != "" && sCXN.IsPic == 1){ %><a href="<%=sCXN.Url%>" target="_blank"><img src="<%=Image(sCXN.Logo) %>" alt="<%=sCXN.Name%>" /></a><%}else{%><a href="<%=sCXN.Url%>" target="_blank"><%=sCXN.Name%></a><%} %>     
