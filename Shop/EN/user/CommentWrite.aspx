...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserCommentWrite"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> psxOs=Languages();RecordCount=psxOs.Count;int psxO_index=1;
foreach (Shop.Model.Lebi_Language psxO in psxOs){%>
            <dd <%if (psxO_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=psxO.id%>,'<%=psxO.Code%>','<%=psxO.Path%>');"><%if (psxO.ImageUrl!=""){%><img src="<%=Image(psxO.ImageUrl) %>" /><%}%><%=psxO.Name%></a></dd>
            <%psxO_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Apwm_index=1;
List<Lebi_Currency> Apwms = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Apwm in Apwms){%>
            <dd <%if (Apwm_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Apwm.id%>,'<%=Apwm.Code%>',<%=Apwm.ExchangeRate%>,'<%=Apwm.Msige%>','<%=Apwm.DecimalLength%>');"><%=Apwm.Code%></a></dd>
            <%Apwm_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int EbDk_index=1;
List<Lebi_Searchkey> EbDks = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey EbDk in EbDks){%><%=Lang(EbDk.Name)%><%EbDk_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Bbzb_index=1;
List<Lebi_Searchkey> Bbzbs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Bbzb in Bbzbs){%>
<%if (Bbzb.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Bbzb.Name)+"") %>"><%}else{ %><a href="<%=Bbzb.URL%>" target="_blank"><%} %><span><%=Lang(Bbzb.Name)%></span></a>
<%Bbzb_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int OCFA_index=1;
List<Lebi_Page> OCFAs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page OCFA in OCFAs){%>
<a class="menu" href="<%=URL("","",OCFA.url)%>"><span><%=OCFA.Name%></span></a>
<%OCFA_index++;}%>
...
<div class="nbbox clearfix">
    <div class="user">
        <ul class="tablist">
            <li><a href="<%=URL("P_UserComment","0","")%>"><span>Product had been commented</span></a></li>
            <li class="selected"><a href="<%=URL("P_UserComment","1","")%>"><span>Awaiting comment<em>(<%=Count_Comment(0) %>)</em></span></a></li>
        </ul>
        ...
        <dl class="dl-btn">
            <dt></dt>
            <dd><a href="javascript:void(0)" onclick="submit();" class="btn btn-11"><s></s>Write a Review</a><input type="hidden" id="Images" name="Images" value="" shop="true"/><input type="hidden" id="ImagesSmall" name="ImagesSmall" value="" shop="true"/></dd>
        </dl>
        </div>
    </div>
</div>
<script type="text/javascript">
    function submit() {
        ...
        RequestAjax(url, postData, function () { MsgBox(1, "Operation Successful", "<%=URL("P_UserComment","0","")%>") });
    }
    ...
    function insertimage(image,small)
    {
        ...
        str+='<span class="image-manage" ><a onclick="deleteimage(\''+image+'\',\''+small+'\');" href="javascript:void(0)">Delete</a></span>';
        str+='</li>'
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int uySx_index=1;
List<Lebi_FriendLink> uySxs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink uySx in uySxs){%>
            
                <% if (uySx.Logo != "" && uySx.IsPic == 1){ %><a href="<%=uySx.Url%>" target="_blank"><img src="<%=Image(uySx.Logo) %>" alt="<%=uySx.Name%>" /></a><%}else{%><a href="<%=uySx.Url%>" target="_blank"><%=uySx.Name%></a><%} %>     
