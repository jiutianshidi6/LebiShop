...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserMoney"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> urgvs=Languages();RecordCount=urgvs.Count;int urgv_index=1;
foreach (Shop.Model.Lebi_Language urgv in urgvs){%>
            <dd <%if (urgv_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=urgv.id%>,'<%=urgv.Code%>','<%=urgv.Path%>');"><%if (urgv.ImageUrl!=""){%><img src="<%=Image(urgv.ImageUrl) %>" /><%}%><%=urgv.Name%></a></dd>
            <%urgv_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int SyMs_index=1;
List<Lebi_Currency> SyMss = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency SyMs in SyMss){%>
            <dd <%if (SyMs_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=SyMs.id%>,'<%=SyMs.Code%>',<%=SyMs.ExchangeRate%>,'<%=SyMs.Msige%>','<%=SyMs.DecimalLength%>');"><%=SyMs.Code%></a></dd>
            <%SyMs_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Qmno_index=1;
List<Lebi_Searchkey> Qmnos = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Qmno in Qmnos){%><%=Lang(Qmno.Name)%><%Qmno_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Drim_index=1;
List<Lebi_Searchkey> Drims = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Drim in Drims){%>
<%if (Drim.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Drim.Name)+"") %>"><%}else{ %><a href="<%=Drim.URL%>" target="_blank"><%} %><span><%=Lang(Drim.Name)%></span></a>
<%Drim_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int KoSO_index=1;
List<Lebi_Page> KoSOs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page KoSO in KoSOs){%>
<a class="menu" href="<%=URL("","",KoSO.url)%>"><span><%=KoSO.Name%></span></a>
<%KoSO_index++;}%>
...
<div class="user">
    <div class="searchbox clearfix">
        ...
        <a href="<%=URL("P_Recharge","") %>"  class="btn btn-12"><s></s>Recharge</a>
        <a href="<%=URL("P_UserCash","") %>"  class="btn btn-12"><s></s>Cash out</a>
        <div style="padding:10px 0 0 0;">
         Account Balance：<%=FormatMoney(CurrentUser.Money) %>
        </div>
    </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int ptTK_index=1;
List<Lebi_FriendLink> ptTKs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink ptTK in ptTKs){%>
            
                <% if (ptTK.Logo != "" && ptTK.IsPic == 1){ %><a href="<%=ptTK.Url%>" target="_blank"><img src="<%=Image(ptTK.Logo) %>" alt="<%=ptTK.Name%>" /></a><%}else{%><a href="<%=ptTK.Url%>" target="_blank"><%=ptTK.Name%></a><%} %>     
