<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.P_UserCard" validateRequest="false"%>


<%@ Import Namespace="Shop.Bussiness" %>
<%@ Import Namespace="Shop.Model" %>
<%@ Import Namespace="System.Collections.Generic" %>
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserCard"); %>
...
<title><%=ThemePageMeta("P_UserCard","title")%></title>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Language" content="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0;" />
<meta name="keywords" content="<%=ThemePageMeta("P_UserCard","keywords")%>" />
<meta name="description" content="<%=ThemePageMeta("P_UserCard","description")%>" />
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> iEXPs=Languages();RecordCount=iEXPs.Count;int iEXP_index=1;
foreach (Shop.Model.Lebi_Language iEXP in iEXPs){%>
            <dd <%if (iEXP_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=iEXP.id%>,'<%=iEXP.Code%>','<%=iEXP.Path%>');"><%if (iEXP.ImageUrl!=""){%><img src="<%=Image(iEXP.ImageUrl) %>" /><%}%><%=iEXP.Name%></a></dd>
            <%iEXP_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int PtOt_index=1;
List<Lebi_Currency> PtOts = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency PtOt in PtOts){%>
            <dd <%if (PtOt_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=PtOt.id%>,'<%=PtOt.Code%>',<%=PtOt.ExchangeRate%>,'<%=PtOt.Msige%>','<%=PtOt.DecimalLength%>');"><%=PtOt.Code%></a></dd>
            <%PtOt_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int XuUk_index=1;
List<Lebi_Searchkey> XuUks = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey XuUk in XuUks){%><%=Lang(XuUk.Name)%><%XuUk_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Nbbx_index=1;
List<Lebi_Searchkey> Nbbxs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Nbbx in Nbbxs){%>
<%if (Nbbx.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Nbbx.Name)+"") %>"><%}else{ %><a href="<%=Nbbx.URL%>" target="_blank"><%} %><span><%=Lang(Nbbx.Name)%></span></a>
<%Nbbx_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int ywId_index=1;
List<Lebi_Page> ywIds = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page ywId in ywIds){%>
<a class="menu" href="<%=URL("","",ywId.url)%>"><span><%=ywId.Name%></span></a>
<%ywId_index++;}%>
...
<div class="user">
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        <tr>
            ...
            <th style="width:80px;">Nominal Value</th>
            <th style="width:80px;">Valid</th>
            <th style="width:80px;">Status</th>
            <th>Remarks</th>
        </tr>
        <%foreach (Shop.Model.Lebi_Card uc in cards)
        {%>
        <tr>
            <td><%=CardName(uc.CardOrder_id) %></td>
            <td><%=uc.Code%></td>
            <td><%=uc.Money%></td>
            <td><%=FormatDate(uc.Time_End)%></td>
            <td><%=TypeName(uc.Type_id_CardStatus)%></td>
            <td><%=CardRemark(uc) %></td>
        </tr>
        <%} %>
    </table>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int poEA_index=1;
List<Lebi_FriendLink> poEAs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink poEA in poEAs){%>
            
                <% if (poEA.Logo != "" && poEA.IsPic == 1){ %><a href="<%=poEA.Url%>" target="_blank"><img src="<%=Image(poEA.Logo) %>" alt="<%=poEA.Name%>" /></a><%}else{%><a href="<%=poEA.Url%>" target="_blank"><%=poEA.Name%></a><%} %>     
