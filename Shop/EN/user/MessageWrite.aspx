<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.P_UserMessageWrite" validateRequest="false"%>


<%@ Import Namespace="Shop.Bussiness" %>
<%@ Import Namespace="Shop.Model" %>
<%@ Import Namespace="System.Collections.Generic" %>
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserMessageWrite"); %>
...
<title><%=ThemePageMeta("P_UserMessageWrite","title")%></title>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Language" content="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0;" />
<meta name="keywords" content="<%=ThemePageMeta("P_UserMessageWrite","keywords")%>" />
<meta name="description" content="<%=ThemePageMeta("P_UserMessageWrite","description")%>" />
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> vEaHs=Languages();RecordCount=vEaHs.Count;int vEaH_index=1;
foreach (Shop.Model.Lebi_Language vEaH in vEaHs){%>
            <dd <%if (vEaH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=vEaH.id%>,'<%=vEaH.Code%>','<%=vEaH.Path%>');"><%if (vEaH.ImageUrl!=""){%><img src="<%=Image(vEaH.ImageUrl) %>" /><%}%><%=vEaH.Name%></a></dd>
            <%vEaH_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int TUGf_index=1;
List<Lebi_Currency> TUGfs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency TUGf in TUGfs){%>
            <dd <%if (TUGf_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=TUGf.id%>,'<%=TUGf.Code%>',<%=TUGf.ExchangeRate%>,'<%=TUGf.Msige%>','<%=TUGf.DecimalLength%>');"><%=TUGf.Code%></a></dd>
            <%TUGf_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int dhpb_index=1;
List<Lebi_Searchkey> dhpbs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey dhpb in dhpbs){%><%=Lang(dhpb.Name)%><%dhpb_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int YeTi_index=1;
List<Lebi_Searchkey> YeTis = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey YeTi in YeTis){%>
<%if (YeTi.Type==1){ %><a href="<%=URL("P_Search",""+Lang(YeTi.Name)+"") %>"><%}else{ %><a href="<%=YeTi.URL%>" target="_blank"><%} %><span><%=Lang(YeTi.Name)%></span></a>
<%YeTi_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int fCfM_index=1;
List<Lebi_Page> fCfMs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page fCfM in fCfMs){%>
<a class="menu" href="<%=URL("","",fCfM.url)%>"><span><%=fCfM.Name%></span></a>
<%fCfM_index++;}%>
...
<div class="user">
    <ul class="tablist">
        <li><a href="<%=URL("P_UserMessage","0","")%>"><span>Inbox<em>(<%=Count_Message(0) %>)</em></span></a></li>
        <li><a href="<%=URL("P_UserMessage","1","")%>"><span>Outbox</span></a></li>
        <li class="selected"><a href="<%=URL("P_UserMessageWrite","","")%>"><span>Send Message</span></a></li>
    </ul>
    <div class="mc clearfix">
    <div class="dl-table clearfix">
    <dl>
        <dt>Type：</dt>
        <dd><select name="Message_Type_id" shop="true">
            <%foreach (Shop.Model.Lebi_Message_Type model in message_types)
            {%>
    ...
    <dl>
        <dt>Subject：</dt>
        <dd><input type="text" id="Title" name="Title" class="input" style="width: 80%;" value="" shop="true" /></dd>
    </dl>
    <dl>
        <dt>Content：</dt>
    ...
    <dl class="dl-btn">
        <dt></dt>
        <dd><a href="javascript:void(0)" onclick="submit();" class="btn btn-11"><s></s>Send</a></dd>
    </dl>
    </div>
    </div>
</div>
...
<script type="text/javascript">
    function submit() {
        ...
        var url = path+"/Ajax/Ajax_userin.aspx?__Action=Message_Write";
        RequestAjax(url, postData, function () { MsgBox(1, "Send successful", "<%=URL("P_UserMessage","1","")%>") });
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int PgON_index=1;
List<Lebi_FriendLink> PgONs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink PgON in PgONs){%>
            
                <% if (PgON.Logo != "" && PgON.IsPic == 1){ %><a href="<%=PgON.Url%>" target="_blank"><img src="<%=Image(PgON.Logo) %>" alt="<%=PgON.Name%>" /></a><%}else{%><a href="<%=PgON.Url%>" target="_blank"><%=PgON.Name%></a><%} %>     
