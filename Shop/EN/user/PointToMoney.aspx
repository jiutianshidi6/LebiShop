<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.P_UserPointToMoney" validateRequest="false"%>


<%@ Import Namespace="Shop.Bussiness" %>
<%@ Import Namespace="Shop.Model" %>
<%@ Import Namespace="System.Collections.Generic" %>
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserPointToMoney"); %>
...
<title><%=ThemePageMeta("P_UserPointToMoney","title")%></title>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Language" content="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0;" />
<meta name="keywords" content="<%=ThemePageMeta("P_UserPointToMoney","keywords")%>" />
<meta name="description" content="<%=ThemePageMeta("P_UserPointToMoney","description")%>" />
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> wQJos=Languages();RecordCount=wQJos.Count;int wQJo_index=1;
foreach (Shop.Model.Lebi_Language wQJo in wQJos){%>
            <dd <%if (wQJo_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=wQJo.id%>,'<%=wQJo.Code%>','<%=wQJo.Path%>');"><%if (wQJo.ImageUrl!=""){%><img src="<%=Image(wQJo.ImageUrl) %>" /><%}%><%=wQJo.Name%></a></dd>
            <%wQJo_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int SEpx_index=1;
List<Lebi_Currency> SEpxs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency SEpx in SEpxs){%>
            <dd <%if (SEpx_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=SEpx.id%>,'<%=SEpx.Code%>',<%=SEpx.ExchangeRate%>,'<%=SEpx.Msige%>','<%=SEpx.DecimalLength%>');"><%=SEpx.Code%></a></dd>
            <%SEpx_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int qklw_index=1;
List<Lebi_Searchkey> qklws = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey qklw in qklws){%><%=Lang(qklw.Name)%><%qklw_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int LRvL_index=1;
List<Lebi_Searchkey> LRvLs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey LRvL in LRvLs){%>
<%if (LRvL.Type==1){ %><a href="<%=URL("P_Search",""+Lang(LRvL.Name)+"") %>"><%}else{ %><a href="<%=LRvL.URL%>" target="_blank"><%} %><span><%=Lang(LRvL.Name)%></span></a>
<%LRvL_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int jtja_index=1;
List<Lebi_Page> jtjas = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page jtja in jtjas){%>
<a class="menu" href="<%=URL("","",jtja.url)%>"><span><%=jtja.Name%></span></a>
<%jtja_index++;}%>
...
<div class="userbox">
    ...
    %>
    <dl>
        <dt><input type="radio" name="ptom" value="<%=v %>" checked /><%=val[0] %>Point：</dt>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int huGG_index=1;
List<Lebi_FriendLink> huGGs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink huGG in huGGs){%>
            
                <% if (huGG.Logo != "" && huGG.IsPic == 1){ %><a href="<%=huGG.Url%>" target="_blank"><img src="<%=Image(huGG.Logo) %>" alt="<%=huGG.Name%>" /></a><%}else{%><a href="<%=huGG.Url%>" target="_blank"><%=huGG.Name%></a><%} %>     
