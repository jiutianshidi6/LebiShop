...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserCash"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> bANDs=Languages();RecordCount=bANDs.Count;int bAND_index=1;
foreach (Shop.Model.Lebi_Language bAND in bANDs){%>
            <dd <%if (bAND_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=bAND.id%>,'<%=bAND.Code%>','<%=bAND.Path%>');"><%if (bAND.ImageUrl!=""){%><img src="<%=Image(bAND.ImageUrl) %>" /><%}%><%=bAND.Name%></a></dd>
            <%bAND_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int PUiB_index=1;
List<Lebi_Currency> PUiBs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency PUiB in PUiBs){%>
            <dd <%if (PUiB_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=PUiB.id%>,'<%=PUiB.Code%>',<%=PUiB.ExchangeRate%>,'<%=PUiB.Msige%>','<%=PUiB.DecimalLength%>');"><%=PUiB.Code%></a></dd>
            <%PUiB_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int aMjU_index=1;
List<Lebi_Searchkey> aMjUs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey aMjU in aMjUs){%><%=Lang(aMjU.Name)%><%aMjU_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int bCCL_index=1;
List<Lebi_Searchkey> bCCLs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey bCCL in bCCLs){%>
<%if (bCCL.Type==1){ %><a href="<%=URL("P_Search",""+Lang(bCCL.Name)+"") %>"><%}else{ %><a href="<%=bCCL.URL%>" target="_blank"><%} %><span><%=Lang(bCCL.Name)%></span></a>
<%bCCL_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int mQRM_index=1;
List<Lebi_Page> mQRMs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page mQRM in mQRMs){%>
<a class="menu" href="<%=URL("","",mQRM.url)%>"><span><%=mQRM.Name%></span></a>
<%mQRM_index++;}%>
...
<div class="user">
    ...
    <dl>
        ...
        <dd>
        <label><input type="radio" name="PayType" value="bank" shop="true" onclick="SetType();" checked />Bank transfer</label>
        <label><input type="radio" name="PayType" value="alipay" shop="true" onclick="SetType();" />Alipay</label>
        </dd>
    </dl>
    <dl>
        <dt>Name：</dt>
    ...
    <dl>
        <dt>Withdrawal Amount(<%=DefaultCurrency.Msige %>)：</dt>
        ...
        <em>Account Balance(<%=DefaultCurrency.Msige %>):<%=CurrentUser.Money-CurrentUser.Money_fanxian %> &nbsp;&nbsp;Min cash out amount(<%=DefaultCurrency.Msige %>):<%=SYS.TakeMoneyLimit %></em></dd>
    </dl>
    <dl>
        <dt>Pay-Password：</dt>
        <dd><input name="Pay_Password" id="Pay_Password" size="30" type="password" shop="true" min="notnull" class="input" /> <a href="<%=URL("P_UserQuestion","1")%>" target="_blank">Forget Password</a></dd>
    </dl>
    <%if(SubmitShow){ %>
    <dl class="dl-btn">
        <dt></dt>
        <dd><a href="javascript:void(0)" onclick="submit();" class="btn btn-11"><s></s>Submit</a></dd>
    </dl>
    <%}else{ %>
    <dl class="dl-btn">
        <dt></dt>
        <dd>Min cash out amount(<%=DefaultCurrency.Msige %>):<%=SYS.TakeMoneyLimit %></dd>
    </dl>
    <%} %>
    </div>
    </div>
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        ...
        <tr>
            ...
            <td><%=ub.Code%></td>
            <td><%=ub.Name%></td>
            <td><a href="javascript:void(0)" onclick="SetBank('<%=ub.PayType %>','<%=ub.UserName %>','<%=ub.Code %>','<%=ub.Name %>');">Select</a></td>
        </tr>
        <%} %>
    </table>
    <%
    ...
    <div>
        <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
            <tr>
                ...
                <th style="width:120px;">Sum/Fee</th>
...
<script type="text/javascript">
    function submit() {
        ...
        if (RMoney == "" || RMoney<=0) {
            CheckNO("RMoney", "The amount must be greater than 0");
            return false;
        }
        var postData = GetFormJsonData("shop");;
        var url = path+"/ajax/ajax_userin.aspx?__Action=TackMoney";
        RequestAjax(url, postData, function (res) { MsgBox(1, "Operation Successful", "?");$("#CashAccount_Name").val("");$("#CashAccount_Code").val("");$("#CashAccount_Bank").val("");$("#RMoney").val("<%=CurrentUser.Money %>"); });
    }
    function SetType() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Ktfr_index=1;
List<Lebi_FriendLink> Ktfrs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Ktfr in Ktfrs){%>
            
                <% if (Ktfr.Logo != "" && Ktfr.IsPic == 1){ %><a href="<%=Ktfr.Url%>" target="_blank"><img src="<%=Image(Ktfr.Logo) %>" alt="<%=Ktfr.Name%>" /></a><%}else{%><a href="<%=Ktfr.Url%>" target="_blank"><%=Ktfr.Name%></a><%} %>     
