<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.P_UserCardGet" validateRequest="false"%>


<%@ Import Namespace="Shop.Bussiness" %>
<%@ Import Namespace="Shop.Model" %>
<%@ Import Namespace="System.Collections.Generic" %>
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserCardGet"); %>
...
<title><%=ThemePageMeta("P_UserCardGet","title")%></title>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Language" content="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0;" />
<meta name="keywords" content="<%=ThemePageMeta("P_UserCardGet","keywords")%>" />
<meta name="description" content="<%=ThemePageMeta("P_UserCardGet","description")%>" />
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> Ykees=Languages();RecordCount=Ykees.Count;int Ykee_index=1;
foreach (Shop.Model.Lebi_Language Ykee in Ykees){%>
            <dd <%if (Ykee_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Ykee.id%>,'<%=Ykee.Code%>','<%=Ykee.Path%>');"><%if (Ykee.ImageUrl!=""){%><img src="<%=Image(Ykee.ImageUrl) %>" /><%}%><%=Ykee.Name%></a></dd>
            <%Ykee_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int VgMI_index=1;
List<Lebi_Currency> VgMIs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency VgMI in VgMIs){%>
            <dd <%if (VgMI_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=VgMI.id%>,'<%=VgMI.Code%>',<%=VgMI.ExchangeRate%>,'<%=VgMI.Msige%>','<%=VgMI.DecimalLength%>');"><%=VgMI.Code%></a></dd>
            <%VgMI_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int TDWT_index=1;
List<Lebi_Searchkey> TDWTs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey TDWT in TDWTs){%><%=Lang(TDWT.Name)%><%TDWT_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int iDeL_index=1;
List<Lebi_Searchkey> iDeLs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey iDeL in iDeLs){%>
<%if (iDeL.Type==1){ %><a href="<%=URL("P_Search",""+Lang(iDeL.Name)+"") %>"><%}else{ %><a href="<%=iDeL.URL%>" target="_blank"><%} %><span><%=Lang(iDeL.Name)%></span></a>
<%iDeL_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int DvCh_index=1;
List<Lebi_Page> DvChs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page DvCh in DvChs){%>
<a class="menu" href="<%=URL("","",DvCh.url)%>"><span><%=DvCh.Name%></span></a>
<%DvCh_index++;}%>
...
<div class="userbox">
    <div class="mt">
        <h2><%=Lang(model.Name)%></h2>
    </div>
    <div class="mc default-info">
        <div class="dl-table clearfix">
        <dl>
            <dt>Nominal Value：</dt>
            <dd><%=FormatMoney(model.Money) %></dd>
        </dl>
        <dl>
            <dt>Minimum order：</dt>
            <dd><%=FormatMoney(model.Money_Buy) %></dd>
        </dl>
        <%if (Shop.Bussiness.EX_Product.TypeNames(model.Pro_Type_ids,CurrentLanguage.Code)!=""){ %>
        <dl>
            <dt>Limit Catergories：</dt>
            <dd><%=Shop.Bussiness.EX_Product.TypeNames(model.Pro_Type_ids,CurrentLanguage.Code) %></dd>
        </dl>
        <%} %>
        <dl>
            <dt>Valid：</dt>
            <dd><%=FormatDate(model.Time_Begin) %> - <%=FormatDate(model.Time_End) %></dd>
        </dl>
        <dl>
            <dt>Security Code：</dt>
            <dd><input name="verifycode" type="text" shop="true" id="verifycode" shop="true" size="6"
class="input" /> <img class="img_c" align="absmiddle" style="width: 80px; height: 25px;" title="Click to refresh Security Code" id="img1" src="<%=WebPath%>/code.aspx" language="javascript" onclick="this.src=this.src+'?'"></dd>
        </dl>
        <dl class="dl-btn">
            <dt></dt>
            <dd><a href="javascript:void(0)" onclick="submit();" class="btn btn-11"><s></s>Receive</a></dd>
        </dl>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    function submit() {
        ...
        var url = path+"/Ajax/Ajax_userin.aspx?__Action=CardGet&id=<%=id %>";
        RequestAjax(url, postData, function () { MsgBox(1, "Operation Successful", "<%=URL("P_UserCard","","")%>") });
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int JXlX_index=1;
List<Lebi_FriendLink> JXlXs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink JXlX in JXlXs){%>
            
                <% if (JXlX.Logo != "" && JXlX.IsPic == 1){ %><a href="<%=JXlX.Url%>" target="_blank"><img src="<%=Image(JXlX.Logo) %>" alt="<%=JXlX.Name%>" /></a><%}else{%><a href="<%=JXlX.Url%>" target="_blank"><%=JXlX.Name%></a><%} %>     
