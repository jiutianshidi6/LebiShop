...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserBank"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> FNohs=Languages();RecordCount=FNohs.Count;int FNoh_index=1;
foreach (Shop.Model.Lebi_Language FNoh in FNohs){%>
            <dd <%if (FNoh_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=FNoh.id%>,'<%=FNoh.Code%>','<%=FNoh.Path%>');"><%if (FNoh.ImageUrl!=""){%><img src="<%=Image(FNoh.ImageUrl) %>" /><%}%><%=FNoh.Name%></a></dd>
            <%FNoh_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int rJmQ_index=1;
List<Lebi_Currency> rJmQs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency rJmQ in rJmQs){%>
            <dd <%if (rJmQ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=rJmQ.id%>,'<%=rJmQ.Code%>',<%=rJmQ.ExchangeRate%>,'<%=rJmQ.Msige%>','<%=rJmQ.DecimalLength%>');"><%=rJmQ.Code%></a></dd>
            <%rJmQ_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int gueX_index=1;
List<Lebi_Searchkey> gueXs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey gueX in gueXs){%><%=Lang(gueX.Name)%><%gueX_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int KNNe_index=1;
List<Lebi_Searchkey> KNNes = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey KNNe in KNNes){%>
<%if (KNNe.Type==1){ %><a href="<%=URL("P_Search",""+Lang(KNNe.Name)+"") %>"><%}else{ %><a href="<%=KNNe.URL%>" target="_blank"><%} %><span><%=Lang(KNNe.Name)%></span></a>
<%KNNe_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int nUEa_index=1;
List<Lebi_Page> nUEas = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page nUEa in nUEas){%>
<a class="menu" href="<%=URL("","",nUEa.url)%>"><span><%=nUEa.Name%></span></a>
<%nUEa_index++;}%>
...
<div class="user">
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        ...
        <tr>
            ...
            <td><%=ub.Code%></td>
            <td><%=ub.Name%></td>
            <td><a href="<%=URL("P_UserBankEdit",ub.id,"")%>">Edit</a></td>
        </tr>
        <%} %>
    </table>
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>Delete</a><a href="<%=URL("P_UserBankEdit","0","")%>" class="btn btn-7"><s></s>Add</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript">
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int bDuu_index=1;
List<Lebi_FriendLink> bDuus = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink bDuu in bDuus){%>
            
                <% if (bDuu.Logo != "" && bDuu.IsPic == 1){ %><a href="<%=bDuu.Url%>" target="_blank"><img src="<%=Image(bDuu.Logo) %>" alt="<%=bDuu.Name%>" /></a><%}else{%><a href="<%=bDuu.Url%>" target="_blank"><%=bDuu.Name%></a><%} %>     
