...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserMessage"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> gFyRs=Languages();RecordCount=gFyRs.Count;int gFyR_index=1;
foreach (Shop.Model.Lebi_Language gFyR in gFyRs){%>
            <dd <%if (gFyR_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=gFyR.id%>,'<%=gFyR.Code%>','<%=gFyR.Path%>');"><%if (gFyR.ImageUrl!=""){%><img src="<%=Image(gFyR.ImageUrl) %>" /><%}%><%=gFyR.Name%></a></dd>
            <%gFyR_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int PoUj_index=1;
List<Lebi_Currency> PoUjs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency PoUj in PoUjs){%>
            <dd <%if (PoUj_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=PoUj.id%>,'<%=PoUj.Code%>',<%=PoUj.ExchangeRate%>,'<%=PoUj.Msige%>','<%=PoUj.DecimalLength%>');"><%=PoUj.Code%></a></dd>
            <%PoUj_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hqXC_index=1;
List<Lebi_Searchkey> hqXCs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hqXC in hqXCs){%><%=Lang(hqXC.Name)%><%hqXC_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int gbrY_index=1;
List<Lebi_Searchkey> gbrYs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey gbrY in gbrYs){%>
<%if (gbrY.Type==1){ %><a href="<%=URL("P_Search",""+Lang(gbrY.Name)+"") %>"><%}else{ %><a href="<%=gbrY.URL%>" target="_blank"><%} %><span><%=Lang(gbrY.Name)%></span></a>
<%gbrY_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int jAPo_index=1;
List<Lebi_Page> jAPos = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page jAPo in jAPos){%>
<a class="menu" href="<%=URL("","",jAPo.url)%>"><span><%=jAPo.Name%></span></a>
<%jAPo_index++;}%>
...
<div class="user">
    <ul class="tablist">
        <li <%if (type == 0){Response.Write("class=\"selected\"");} %>><a href="<%=URL("P_UserMessage","0","")%>"><span>Inbox<em>(<%=Count_Message(0) %>)</em></span></a></li>
        <li <%if (type == 1){Response.Write("class=\"selected\"");} %>><a href="<%=URL("P_UserMessage","1","")%>"><span>Outbox</span></a></li>
        <li><a href="<%=URL("P_UserMessageWrite","","")%>"><span>Send Message</span></a></li>
    </ul>
    <div class="searchbox clearfix">
        <select id="type_id">
            ...
            <%} %>
            <option value="0" <%if (type_id == "0"){%>selected<%} %>>System News</option>
        </select>
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        <tr>
            <th style="width:30px;" class="selectAll"><a href="javascript:void(0);" onclick="$('input[name=\'id\']').attr('checked',!$(this).attr('checked'));$(this).attr('checked',!$(this).attr('checked'));">All</a></th>
            <th style="width:30px;">Status</th>
            <th style="width:120px;">Time</th>
            <th style="width:*;">Subject</th>     
        </tr>
        ...
        <tr class="list">
            ...
            <td style="text-align:center"><%if (model.IsRead == 0){%><img src="/Theme/system/images/icon/email.png" title="Unread" /><%}else{ %><img src="/Theme/system/images/icon/email_open.png" title="Read" /><%} %></td>
            <td><%=FormatTime(model.Time_Add) %></td>
            <td><a href="<%=URL("P_UserMessageDetails",model.id)%>"><%=model.Title %></a></td>
        </tr>
        <%} %>
    </table>
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>Delete</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript">
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int NjKc_index=1;
List<Lebi_FriendLink> NjKcs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink NjKc in NjKcs){%>
            
                <% if (NjKc.Logo != "" && NjKc.IsPic == 1){ %><a href="<%=NjKc.Url%>" target="_blank"><img src="<%=Image(NjKc.Logo) %>" alt="<%=NjKc.Name%>" /></a><%}else{%><a href="<%=NjKc.Url%>" target="_blank"><%=NjKc.Name%></a><%} %>     
