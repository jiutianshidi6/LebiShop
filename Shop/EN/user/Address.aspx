...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserAddress"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> iiKys=Languages();RecordCount=iiKys.Count;int iiKy_index=1;
foreach (Shop.Model.Lebi_Language iiKy in iiKys){%>
            <dd <%if (iiKy_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=iiKy.id%>,'<%=iiKy.Code%>','<%=iiKy.Path%>');"><%if (iiKy.ImageUrl!=""){%><img src="<%=Image(iiKy.ImageUrl) %>" /><%}%><%=iiKy.Name%></a></dd>
            <%iiKy_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int LpbD_index=1;
List<Lebi_Currency> LpbDs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency LpbD in LpbDs){%>
            <dd <%if (LpbD_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=LpbD.id%>,'<%=LpbD.Code%>',<%=LpbD.ExchangeRate%>,'<%=LpbD.Msige%>','<%=LpbD.DecimalLength%>');"><%=LpbD.Code%></a></dd>
            <%LpbD_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int AeKm_index=1;
List<Lebi_Searchkey> AeKms = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey AeKm in AeKms){%><%=Lang(AeKm.Name)%><%AeKm_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int SzsA_index=1;
List<Lebi_Searchkey> SzsAs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey SzsA in SzsAs){%>
<%if (SzsA.Type==1){ %><a href="<%=URL("P_Search",""+Lang(SzsA.Name)+"") %>"><%}else{ %><a href="<%=SzsA.URL%>" target="_blank"><%} %><span><%=Lang(SzsA.Name)%></span></a>
<%SzsA_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int YehK_index=1;
List<Lebi_Page> YehKs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page YehK in YehKs){%>
<a class="menu" href="<%=URL("","",YehK.url)%>"><span><%=YehK.Name%></span></a>
<%YehK_index++;}%>
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        <tr>
            <th style="width:30px;" class="selectAll">
                <a href="javascript:void(0);" onclick="$('input[name=\'id\']').attr('checked',!$(this).attr('checked'));$(this).attr('checked',!$(this).attr('checked'));">All</a>
            </th>
        ...
        <tr>
            ...
            <td><%if (CurrentUser.User_Address_id == ua.id){ %>Default<%}else{ %>&nbsp;<%} %></td>
            <td><a href="<%=URL("P_UserAddressEdit",ua.id,"")%>">Edit</a>
            </td>
        </tr>
        <%} %>
    </table>
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>Delete</a><a href="<%=URL("P_UserAddressEdit","0","")%>" class="btn btn-7"><s></s>Add</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript">
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int bsrV_index=1;
List<Lebi_FriendLink> bsrVs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink bsrV in bsrVs){%>
            
                <% if (bsrV.Logo != "" && bsrV.IsPic == 1){ %><a href="<%=bsrV.Url%>" target="_blank"><img src="<%=Image(bsrV.Logo) %>" alt="<%=bsrV.Name%>" /></a><%}else{%><a href="<%=bsrV.Url%>" target="_blank"><%=bsrV.Name%></a><%} %>     
