...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserMessageDetails"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> fjEbs=Languages();RecordCount=fjEbs.Count;int fjEb_index=1;
foreach (Shop.Model.Lebi_Language fjEb in fjEbs){%>
            <dd <%if (fjEb_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=fjEb.id%>,'<%=fjEb.Code%>','<%=fjEb.Path%>');"><%if (fjEb.ImageUrl!=""){%><img src="<%=Image(fjEb.ImageUrl) %>" /><%}%><%=fjEb.Name%></a></dd>
            <%fjEb_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int ILbR_index=1;
List<Lebi_Currency> ILbRs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency ILbR in ILbRs){%>
            <dd <%if (ILbR_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=ILbR.id%>,'<%=ILbR.Code%>',<%=ILbR.ExchangeRate%>,'<%=ILbR.Msige%>','<%=ILbR.DecimalLength%>');"><%=ILbR.Code%></a></dd>
            <%ILbR_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MBOv_index=1;
List<Lebi_Searchkey> MBOvs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MBOv in MBOvs){%><%=Lang(MBOv.Name)%><%MBOv_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int dsfk_index=1;
List<Lebi_Searchkey> dsfks = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey dsfk in dsfks){%>
<%if (dsfk.Type==1){ %><a href="<%=URL("P_Search",""+Lang(dsfk.Name)+"") %>"><%}else{ %><a href="<%=dsfk.URL%>" target="_blank"><%} %><span><%=Lang(dsfk.Name)%></span></a>
<%dsfk_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int QglY_index=1;
List<Lebi_Page> QglYs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page QglY in QglYs){%>
<a class="menu" href="<%=URL("","",QglY.url)%>"><span><%=QglY.Name%></span></a>
<%QglY_index++;}%>
...
<div class="user">
    <ul class="tablist">
        <li <%if (message.User_id_From == 0){Response.Write("class=\"selected\"");} %>><a href="<%=URL("P_UserMessage","0","")%>"><span>Inbox<em>(<%=Count_Message(0) %>)</em></span></a></li>
        <li <%if (message.User_id_To == 0){Response.Write("class=\"selected\"");} %>><a href="<%=URL("P_UserMessage","1","")%>"><span>Outbox</span></a></li>
        <li><a href="<%=URL("P_UserMessageWrite","","")%>"><span>Send Message</span></a></li>
    </ul>
    <div class="mt">
        <h2><%=message.Title %></h2>
    ...
    <dl class="dl-btn">
        <dt></dt>
        <dd><a href="javascript:void(0)" onclick="submit();" class="btn btn-11"><s></s>Reply</a></dd>
    </dl>
    </div>
    </div>
    <%} %>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int ALsL_index=1;
List<Lebi_FriendLink> ALsLs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink ALsL in ALsLs){%>
            
                <% if (ALsL.Logo != "" && ALsL.IsPic == 1){ %><a href="<%=ALsL.Url%>" target="_blank"><img src="<%=Image(ALsL.Logo) %>" alt="<%=ALsL.Name%>" /></a><%}else{%><a href="<%=ALsL.Url%>" target="_blank"><%=ALsL.Name%></a><%} %>     
