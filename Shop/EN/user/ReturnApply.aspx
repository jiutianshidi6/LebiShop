...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserReturnApply"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> HYaks=Languages();RecordCount=HYaks.Count;int HYak_index=1;
foreach (Shop.Model.Lebi_Language HYak in HYaks){%>
            <dd <%if (HYak_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=HYak.id%>,'<%=HYak.Code%>','<%=HYak.Path%>');"><%if (HYak.ImageUrl!=""){%><img src="<%=Image(HYak.ImageUrl) %>" /><%}%><%=HYak.Name%></a></dd>
            <%HYak_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int NvGs_index=1;
List<Lebi_Currency> NvGss = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency NvGs in NvGss){%>
            <dd <%if (NvGs_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=NvGs.id%>,'<%=NvGs.Code%>',<%=NvGs.ExchangeRate%>,'<%=NvGs.Msige%>','<%=NvGs.DecimalLength%>');"><%=NvGs.Code%></a></dd>
            <%NvGs_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int yxpt_index=1;
List<Lebi_Searchkey> yxpts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey yxpt in yxpts){%><%=Lang(yxpt.Name)%><%yxpt_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int YuXC_index=1;
List<Lebi_Searchkey> YuXCs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey YuXC in YuXCs){%>
<%if (YuXC.Type==1){ %><a href="<%=URL("P_Search",""+Lang(YuXC.Name)+"") %>"><%}else{ %><a href="<%=YuXC.URL%>" target="_blank"><%} %><span><%=Lang(YuXC.Name)%></span></a>
<%YuXC_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int dRAX_index=1;
List<Lebi_Page> dRAXs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page dRAX in dRAXs){%>
<a class="menu" href="<%=URL("","",dRAX.url)%>"><span><%=dRAX.Name%></span></a>
<%dRAX_index++;}%>
...
<div class="user">
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        <tr>
            ...
            <th>Order item</th>
            <th style="width: 80px;">Received Quantiy</th>
            <th style="width: 80px;">Has retired number</th>
            <th style="width: 80px;">Return Quantity</th>
        </tr>
        <%
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int xyfj_index=1;
List<Lebi_FriendLink> xyfjs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink xyfj in xyfjs){%>
            
                <% if (xyfj.Logo != "" && xyfj.IsPic == 1){ %><a href="<%=xyfj.Url%>" target="_blank"><img src="<%=Image(xyfj.Logo) %>" alt="<%=xyfj.Name%>" /></a><%}else{%><a href="<%=xyfj.Url%>" target="_blank"><%=xyfj.Name%></a><%} %>     
