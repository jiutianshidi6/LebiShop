...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserPoint"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> JnJDs=Languages();RecordCount=JnJDs.Count;int JnJD_index=1;
foreach (Shop.Model.Lebi_Language JnJD in JnJDs){%>
            <dd <%if (JnJD_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=JnJD.id%>,'<%=JnJD.Code%>','<%=JnJD.Path%>');"><%if (JnJD.ImageUrl!=""){%><img src="<%=Image(JnJD.ImageUrl) %>" /><%}%><%=JnJD.Name%></a></dd>
            <%JnJD_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int OWuY_index=1;
List<Lebi_Currency> OWuYs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency OWuY in OWuYs){%>
            <dd <%if (OWuY_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=OWuY.id%>,'<%=OWuY.Code%>',<%=OWuY.ExchangeRate%>,'<%=OWuY.Msige%>','<%=OWuY.DecimalLength%>');"><%=OWuY.Code%></a></dd>
            <%OWuY_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int wncP_index=1;
List<Lebi_Searchkey> wncPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey wncP in wncPs){%><%=Lang(wncP.Name)%><%wncP_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int TXgr_index=1;
List<Lebi_Searchkey> TXgrs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey TXgr in TXgrs){%>
<%if (TXgr.Type==1){ %><a href="<%=URL("P_Search",""+Lang(TXgr.Name)+"") %>"><%}else{ %><a href="<%=TXgr.URL%>" target="_blank"><%} %><span><%=Lang(TXgr.Name)%></span></a>
<%TXgr_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int morn_index=1;
List<Lebi_Page> morns = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page morn in morns){%>
<a class="menu" href="<%=URL("","",morn.url)%>"><span><%=morn.Name%></span></a>
<%morn_index++;}%>
...
<div class="user">
    <div class="searchbox clearfix">
        ...
        <a href="<%=URL("P_UserPointToMoney","") %>" class="btn btn-12"><s></s>Change Points</a>
        <%} %>
    </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int jLzE_index=1;
List<Lebi_FriendLink> jLzEs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink jLzE in jLzEs){%>
            
                <% if (jLzE.Logo != "" && jLzE.IsPic == 1){ %><a href="<%=jLzE.Url%>" target="_blank"><img src="<%=Image(jLzE.Logo) %>" alt="<%=jLzE.Name%>" /></a><%}else{%><a href="<%=jLzE.Url%>" target="_blank"><%=jLzE.Name%></a><%} %>     
