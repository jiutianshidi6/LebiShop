...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserLike"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> eLsNs=Languages();RecordCount=eLsNs.Count;int eLsN_index=1;
foreach (Shop.Model.Lebi_Language eLsN in eLsNs){%>
            <dd <%if (eLsN_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=eLsN.id%>,'<%=eLsN.Code%>','<%=eLsN.Path%>');"><%if (eLsN.ImageUrl!=""){%><img src="<%=Image(eLsN.ImageUrl) %>" /><%}%><%=eLsN.Name%></a></dd>
            <%eLsN_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int LgFj_index=1;
List<Lebi_Currency> LgFjs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency LgFj in LgFjs){%>
            <dd <%if (LgFj_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=LgFj.id%>,'<%=LgFj.Code%>',<%=LgFj.ExchangeRate%>,'<%=LgFj.Msige%>','<%=LgFj.DecimalLength%>');"><%=LgFj.Code%></a></dd>
            <%LgFj_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int OoJe_index=1;
List<Lebi_Searchkey> OoJes = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey OoJe in OoJes){%><%=Lang(OoJe.Name)%><%OoJe_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int nLDa_index=1;
List<Lebi_Searchkey> nLDas = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey nLDa in nLDas){%>
<%if (nLDa.Type==1){ %><a href="<%=URL("P_Search",""+Lang(nLDa.Name)+"") %>"><%}else{ %><a href="<%=nLDa.URL%>" target="_blank"><%} %><span><%=Lang(nLDa.Name)%></span></a>
<%nLDa_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int eCGc_index=1;
List<Lebi_Page> eCGcs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page eCGc in eCGcs){%>
<a class="menu" href="<%=URL("","",eCGc.url)%>"><span><%=eCGc.Name%></span></a>
<%eCGc_index++;}%>
...
<div class="user">
    ...
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>Delete</a><a href="javascript:void(0)" onclick="LikeToBasket();" class="btn btn-7"><s></s>Add to Cart</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript" >
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int PeNO_index=1;
List<Lebi_FriendLink> PeNOs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink PeNO in PeNOs){%>
            
                <% if (PeNO.Logo != "" && PeNO.IsPic == 1){ %><a href="<%=PeNO.Url%>" target="_blank"><img src="<%=Image(PeNO.Logo) %>" alt="<%=PeNO.Name%>" /></a><%}else{%><a href="<%=PeNO.Url%>" target="_blank"><%=PeNO.Name%></a><%} %>     
