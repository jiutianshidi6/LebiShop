...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserOftenBuyDialog"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> SvKqs=Languages();RecordCount=SvKqs.Count;int SvKq_index=1;
foreach (Shop.Model.Lebi_Language SvKq in SvKqs){%>
            <dd <%if (SvKq_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=SvKq.id%>,'<%=SvKq.Code%>','<%=SvKq.Path%>');"><%if (SvKq.ImageUrl!=""){%><img src="<%=Image(SvKq.ImageUrl) %>" /><%}%><%=SvKq.Name%></a></dd>
            <%SvKq_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int rpEd_index=1;
List<Lebi_Currency> rpEds = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency rpEd in rpEds){%>
            <dd <%if (rpEd_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=rpEd.id%>,'<%=rpEd.Code%>',<%=rpEd.ExchangeRate%>,'<%=rpEd.Msige%>','<%=rpEd.DecimalLength%>');"><%=rpEd.Code%></a></dd>
            <%rpEd_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int OsEo_index=1;
List<Lebi_Searchkey> OsEos = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey OsEo in OsEos){%><%=Lang(OsEo.Name)%><%OsEo_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hYbt_index=1;
List<Lebi_Searchkey> hYbts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hYbt in hYbts){%>
<%if (hYbt.Type==1){ %><a href="<%=URL("P_Search",""+Lang(hYbt.Name)+"") %>"><%}else{ %><a href="<%=hYbt.URL%>" target="_blank"><%} %><span><%=Lang(hYbt.Name)%></span></a>
<%hYbt_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int YfiJ_index=1;
List<Lebi_Page> YfiJs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page YfiJ in YfiJs){%>
<a class="menu" href="<%=URL("","",YfiJ.url)%>"><span><%=YfiJ.Name%></span></a>
<%YfiJ_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int tOXY_index=1;
List<Lebi_FriendLink> tOXYs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink tOXY in tOXYs){%>
            
                <% if (tOXY.Logo != "" && tOXY.IsPic == 1){ %><a href="<%=tOXY.Url%>" target="_blank"><img src="<%=Image(tOXY.Logo) %>" alt="<%=tOXY.Name%>" /></a><%}else{%><a href="<%=tOXY.Url%>" target="_blank"><%=tOXY.Name%></a><%} %>     
