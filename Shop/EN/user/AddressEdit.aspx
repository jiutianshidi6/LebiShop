...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserAddressEdit"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> IsOPs=Languages();RecordCount=IsOPs.Count;int IsOP_index=1;
foreach (Shop.Model.Lebi_Language IsOP in IsOPs){%>
            <dd <%if (IsOP_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=IsOP.id%>,'<%=IsOP.Code%>','<%=IsOP.Path%>');"><%if (IsOP.ImageUrl!=""){%><img src="<%=Image(IsOP.ImageUrl) %>" /><%}%><%=IsOP.Name%></a></dd>
            <%IsOP_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int zgvt_index=1;
List<Lebi_Currency> zgvts = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency zgvt in zgvts){%>
            <dd <%if (zgvt_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=zgvt.id%>,'<%=zgvt.Code%>',<%=zgvt.ExchangeRate%>,'<%=zgvt.Msige%>','<%=zgvt.DecimalLength%>');"><%=zgvt.Code%></a></dd>
            <%zgvt_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int NVkM_index=1;
List<Lebi_Searchkey> NVkMs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey NVkM in NVkMs){%><%=Lang(NVkM.Name)%><%NVkM_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int HSdP_index=1;
List<Lebi_Searchkey> HSdPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey HSdP in HSdPs){%>
<%if (HSdP.Type==1){ %><a href="<%=URL("P_Search",""+Lang(HSdP.Name)+"") %>"><%}else{ %><a href="<%=HSdP.URL%>" target="_blank"><%} %><span><%=Lang(HSdP.Name)%></span></a>
<%HSdP_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int llbi_index=1;
List<Lebi_Page> llbis = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page llbi in llbis){%>
<a class="menu" href="<%=URL("","",llbi.url)%>"><span><%=llbi.Name%></span></a>
<%llbi_index++;}%>
...
<script type="text/javascript">
    ...
    function submit() {
        ...
        RequestAjax(url, postData, function () { MsgBox(1, "Operation Successful", "<%=URL("P_UserAddress","") %>") });
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int yREE_index=1;
List<Lebi_FriendLink> yREEs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink yREE in yREEs){%>
            
                <% if (yREE.Logo != "" && yREE.IsPic == 1){ %><a href="<%=yREE.Url%>" target="_blank"><img src="<%=Image(yREE.Logo) %>" alt="<%=yREE.Name%>" /></a><%}else{%><a href="<%=yREE.Url%>" target="_blank"><%=yREE.Name%></a><%} %>     
