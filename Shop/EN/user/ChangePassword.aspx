...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserChangePassword"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> Xfsas=Languages();RecordCount=Xfsas.Count;int Xfsa_index=1;
foreach (Shop.Model.Lebi_Language Xfsa in Xfsas){%>
            <dd <%if (Xfsa_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Xfsa.id%>,'<%=Xfsa.Code%>','<%=Xfsa.Path%>');"><%if (Xfsa.ImageUrl!=""){%><img src="<%=Image(Xfsa.ImageUrl) %>" /><%}%><%=Xfsa.Name%></a></dd>
            <%Xfsa_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int hpyu_index=1;
List<Lebi_Currency> hpyus = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency hpyu in hpyus){%>
            <dd <%if (hpyu_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=hpyu.id%>,'<%=hpyu.Code%>',<%=hpyu.ExchangeRate%>,'<%=hpyu.Msige%>','<%=hpyu.DecimalLength%>');"><%=hpyu.Code%></a></dd>
            <%hpyu_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int iRxI_index=1;
List<Lebi_Searchkey> iRxIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey iRxI in iRxIs){%><%=Lang(iRxI.Name)%><%iRxI_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int FvhX_index=1;
List<Lebi_Searchkey> FvhXs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey FvhX in FvhXs){%>
<%if (FvhX.Type==1){ %><a href="<%=URL("P_Search",""+Lang(FvhX.Name)+"") %>"><%}else{ %><a href="<%=FvhX.URL%>" target="_blank"><%} %><span><%=Lang(FvhX.Name)%></span></a>
<%FvhX_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int JDKn_index=1;
List<Lebi_Page> JDKns = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page JDKn in JDKns){%>
<a class="menu" href="<%=URL("","",JDKn.url)%>"><span><%=JDKn.Name%></span></a>
<%JDKn_index++;}%>
...
<div class="userbox">
    ...
    <dl class="dl-btn">
        <dt></dt>
        <dd><a href="javascript:void(0)" onclick="setpassword();" class="btn btn-11"><s></s>Submit</a></dd>
    </dl>
    </div>
    </div>
</div>
...
<div class="userbox">
    ...
    <dl>
        <dt>Current Password：</dt>
        <dd><input name="Pay_Password" id="Pay_Password" size="30" type="password" paypass="true" min="notnull" class="input" /> <a href="<%=URL("P_UserQuestion","1")%>">Forget Password</a></dd>
    </dl>
    <%} %>
    <dl>
        <dt>New Password：</dt>
    ...
    <dl class="dl-btn">
        <dt></dt>
        <dd><a href="<%=URL("P_UserQuestion","")%>" class="click">Please set the security issue</a></dd>
    </dl>
    <%}else{ %>
    <dl class="dl-btn">
        <dt></dt>
        <dd><a href="javascript:void(0)" onclick="setpaypassword();" class="btn btn-11"><s></s>Submit</a></dd>
    </dl>
    <%} %>
    </div>
    </div>
...
<script type="text/javascript">
    function setpassword() {
        ...
        if (Password == "") {
            CheckNO("Password", "Please enter the original password");
            return false;
        }
        if (Password1 == "") {
            CheckNO("Password1", "Please enter a new password");
            return false;
        }
    ...
    function setpaypassword() {
        ...
        <%if(CurrentUser.Pay_Password!=""){ %>
        if (Pay_Password == "") {
            CheckNO("Pay_Password", "Please enter the original password");
            return false;
        }
        <%} %>
        if (Pay_Password1 == "") {
        ...
        if (Pay_Password1 != Pay_Password2) {
            CheckNO("Pay_Password1", "Passwords do not match");
            return false;
        }
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Qlvu_index=1;
List<Lebi_FriendLink> Qlvus = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Qlvu in Qlvus){%>
            
                <% if (Qlvu.Logo != "" && Qlvu.IsPic == 1){ %><a href="<%=Qlvu.Url%>" target="_blank"><img src="<%=Image(Qlvu.Logo) %>" alt="<%=Qlvu.Name%>" /></a><%}else{%><a href="<%=Qlvu.Url%>" target="_blank"><%=Qlvu.Name%></a><%} %>     
