...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserQuestion"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> Nubys=Languages();RecordCount=Nubys.Count;int Nuby_index=1;
foreach (Shop.Model.Lebi_Language Nuby in Nubys){%>
            <dd <%if (Nuby_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Nuby.id%>,'<%=Nuby.Code%>','<%=Nuby.Path%>');"><%if (Nuby.ImageUrl!=""){%><img src="<%=Image(Nuby.ImageUrl) %>" /><%}%><%=Nuby.Name%></a></dd>
            <%Nuby_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int YtSX_index=1;
List<Lebi_Currency> YtSXs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency YtSX in YtSXs){%>
            <dd <%if (YtSX_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=YtSX.id%>,'<%=YtSX.Code%>',<%=YtSX.ExchangeRate%>,'<%=YtSX.Msige%>','<%=YtSX.DecimalLength%>');"><%=YtSX.Code%></a></dd>
            <%YtSX_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int bsaB_index=1;
List<Lebi_Searchkey> bsaBs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey bsaB in bsaBs){%><%=Lang(bsaB.Name)%><%bsaB_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int bktP_index=1;
List<Lebi_Searchkey> bktPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey bktP in bktPs){%>
<%if (bktP.Type==1){ %><a href="<%=URL("P_Search",""+Lang(bktP.Name)+"") %>"><%}else{ %><a href="<%=bktP.URL%>" target="_blank"><%} %><span><%=Lang(bktP.Name)%></span></a>
<%bktP_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int BIxY_index=1;
List<Lebi_Page> BIxYs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page BIxY in BIxYs){%>
<a class="menu" href="<%=URL("","",BIxY.url)%>"><span><%=BIxY.Name%></span></a>
<%BIxY_index++;}%>
...
<div class="userbox">
    ...
    <dl class="dl-btn">
        <dt></dt>
        <dd><a href="javascript:void(0)" onclick="submit();" class="btn btn-11"><s></s><%if (type == 0){ %>Reset<%}else{ %>Submit<%} %></a></dd>
    </dl>
    <%} %>
    </div>
    </div>
...
<script type="text/javascript">
    function submit() {
        ...
        if ($("#Question_id1").val() == $("#Question_id2").val()){
            MsgBox(2, "Please select two different questions", "");
            return false;
        }
        <%} %>
        ...
        if (res.type == 0){
            MsgBox(1, "Success", "<%=URL("P_UserChangePassword","")%>"); 
        }else{
            MsgBox(1, "Payment password has been cleared", res.url); 
        }
        });
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int iPGC_index=1;
List<Lebi_FriendLink> iPGCs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink iPGC in iPGCs){%>
            
                <% if (iPGC.Logo != "" && iPGC.IsPic == 1){ %><a href="<%=iPGC.Url%>" target="_blank"><img src="<%=Image(iPGC.Logo) %>" alt="<%=iPGC.Name%>" /></a><%}else{%><a href="<%=iPGC.Url%>" target="_blank"><%=iPGC.Name%></a><%} %>     
