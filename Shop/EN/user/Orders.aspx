...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserOrders"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> KDpJs=Languages();RecordCount=KDpJs.Count;int KDpJ_index=1;
foreach (Shop.Model.Lebi_Language KDpJ in KDpJs){%>
            <dd <%if (KDpJ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=KDpJ.id%>,'<%=KDpJ.Code%>','<%=KDpJ.Path%>');"><%if (KDpJ.ImageUrl!=""){%><img src="<%=Image(KDpJ.ImageUrl) %>" /><%}%><%=KDpJ.Name%></a></dd>
            <%KDpJ_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int SLkd_index=1;
List<Lebi_Currency> SLkds = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency SLkd in SLkds){%>
            <dd <%if (SLkd_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=SLkd.id%>,'<%=SLkd.Code%>',<%=SLkd.ExchangeRate%>,'<%=SLkd.Msige%>','<%=SLkd.DecimalLength%>');"><%=SLkd.Code%></a></dd>
            <%SLkd_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int HvsW_index=1;
List<Lebi_Searchkey> HvsWs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey HvsW in HvsWs){%><%=Lang(HvsW.Name)%><%HvsW_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int QPpU_index=1;
List<Lebi_Searchkey> QPpUs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey QPpU in QPpUs){%>
<%if (QPpU.Type==1){ %><a href="<%=URL("P_Search",""+Lang(QPpU.Name)+"") %>"><%}else{ %><a href="<%=QPpU.URL%>" target="_blank"><%} %><span><%=Lang(QPpU.Name)%></span></a>
<%QPpU_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int RyrU_index=1;
List<Lebi_Page> RyrUs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page RyrU in RyrUs){%>
<a class="menu" href="<%=URL("","",RyrU.url)%>"><span><%=RyrU.Name%></span></a>
<%RyrU_index++;}%>
...
<div class="user">
    <ul class="tablist">
        <li <%if (status == ""){Response.Write("class=\"selected\"");} %>><a href="?status="><span>All Orders</span></a></li>
        <li <%if (status == "1"){Response.Write("class=\"selected\"");} %>><a href="?status=1"><span>Awaiting Payment</span></a></li>
        <li <%if (status == "2"){Response.Write("class=\"selected\"");} %>><a href="?status=2"><span>Pending Goods Receiving</span></a></li>
        <li <%if (status == "3"){Response.Write("class=\"selected\"");} %>><a href="?status=3"><span>Received</span></a></li>
        <li <%if (status == "4"){Response.Write("class=\"selected\"");} %>><a href="?status=4"><span>Completed</span></a></li>
        <li <%if (status == "5"){Response.Write("class=\"selected\"");} %>><a href="?status=5"><span>Cancelled</span></a></li>
    </ul>
    <div class="searchbox clearfix">
        <select id="status">
            <option value="">┌ Status</option>
            <option value="1" <%if (status == "1"){Response.Write("class=\"selected\"");} %>>Awaiting Payment</option>
            <option value="2" <%if (status == "2"){Response.Write("class=\"selected\"");} %>>Pending Goods Receiving</option>
            <option value="3" <%if (status == "3"){Response.Write("class=\"selected\"");} %>>Received</option>
            <option value="4" <%if (status == "4"){Response.Write("class=\"selected\"");} %>>Completed</option>
            <option value="5" <%if (status == "5"){Response.Write("class=\"selected\"");} %>>Cancelled</option>
        </select>
        ...
        <a href="javascript:void(0)" onclick="search_();" class="btn btn-12"><s></s>Search</a>
    </div>
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        <tr>
            <th style="width:120px;">Order ID</th>
            <th>Order item</th>
            <th style="width:80px;">Order Amount</th>
        ...
        <tr>
            ...
            <td>
                <a href="<%=URL("P_UserOrderDetails",o.id)%>" target="_blank">View</a><%if(Shop.Bussiness.Order.CancelOrder(o)){ %>
                <br /><a href="javascript:void(0);" onclick="OrderCancel(<%=o.id %>);">Cancel</a><%} %>
                <%if(o.IsInvalid == 0 && o.IsReceived==1 && SYS.IsClosetuihuo=="0"){ %>
                <br /><a href="<%=URL("P_UserReturnApply",o.id)%>" target="_blank">Refund</a>
                <%} %>
            </td>
        </tr>
        <%} %>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int cegi_index=1;
List<Lebi_FriendLink> cegis = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink cegi in cegis){%>
            
                <% if (cegi.Logo != "" && cegi.IsPic == 1){ %><a href="<%=cegi.Url%>" target="_blank"><img src="<%=Image(cegi.Logo) %>" alt="<%=cegi.Name%>" /></a><%}else{%><a href="<%=cegi.Url%>" target="_blank"><%=cegi.Name%></a><%} %>     
