...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserAsk"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> JQSHs=Languages();RecordCount=JQSHs.Count;int JQSH_index=1;
foreach (Shop.Model.Lebi_Language JQSH in JQSHs){%>
            <dd <%if (JQSH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=JQSH.id%>,'<%=JQSH.Code%>','<%=JQSH.Path%>');"><%if (JQSH.ImageUrl!=""){%><img src="<%=Image(JQSH.ImageUrl) %>" /><%}%><%=JQSH.Name%></a></dd>
            <%JQSH_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int GhxR_index=1;
List<Lebi_Currency> GhxRs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency GhxR in GhxRs){%>
            <dd <%if (GhxR_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=GhxR.id%>,'<%=GhxR.Code%>',<%=GhxR.ExchangeRate%>,'<%=GhxR.Msige%>','<%=GhxR.DecimalLength%>');"><%=GhxR.Code%></a></dd>
            <%GhxR_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int GBwF_index=1;
List<Lebi_Searchkey> GBwFs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey GBwF in GBwFs){%><%=Lang(GBwF.Name)%><%GBwF_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int RxrS_index=1;
List<Lebi_Searchkey> RxrSs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey RxrS in RxrSs){%>
<%if (RxrS.Type==1){ %><a href="<%=URL("P_Search",""+Lang(RxrS.Name)+"") %>"><%}else{ %><a href="<%=RxrS.URL%>" target="_blank"><%} %><span><%=Lang(RxrS.Name)%></span></a>
<%RxrS_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int pEdS_index=1;
List<Lebi_Page> pEdSs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page pEdS in pEdSs){%>
<a class="menu" href="<%=URL("","",pEdS.url)%>"><span><%=pEdS.Name%></span></a>
<%pEdS_index++;}%>
...
<div class="user">
    ...
    <tr><td colspan="5">Content：<%=c.Content.Replace("\r\n", "<br/>")%></td></tr>
    <%
    ...
    {
    %>
    <tr><td colspan="5">Seller Reply：<%=creply.Content.Replace("\r\n", "<br/>")%><em><%=FormatTime(creply.Time_Add)%></em></td></tr>
    <%} %>
    </table>
    <%} %>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int VVGL_index=1;
List<Lebi_FriendLink> VVGLs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink VVGL in VVGLs){%>
            
                <% if (VVGL.Logo != "" && VVGL.IsPic == 1){ %><a href="<%=VVGL.Url%>" target="_blank"><img src="<%=Image(VVGL.Logo) %>" alt="<%=VVGL.Name%>" /></a><%}else{%><a href="<%=VVGL.Url%>" target="_blank"><%=VVGL.Name%></a><%} %>     
