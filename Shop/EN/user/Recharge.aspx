...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_Recharge"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> vUmus=Languages();RecordCount=vUmus.Count;int vUmu_index=1;
foreach (Shop.Model.Lebi_Language vUmu in vUmus){%>
            <dd <%if (vUmu_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=vUmu.id%>,'<%=vUmu.Code%>','<%=vUmu.Path%>');"><%if (vUmu.ImageUrl!=""){%><img src="<%=Image(vUmu.ImageUrl) %>" /><%}%><%=vUmu.Name%></a></dd>
            <%vUmu_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int JcAp_index=1;
List<Lebi_Currency> JcAps = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency JcAp in JcAps){%>
            <dd <%if (JcAp_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=JcAp.id%>,'<%=JcAp.Code%>',<%=JcAp.ExchangeRate%>,'<%=JcAp.Msige%>','<%=JcAp.DecimalLength%>');"><%=JcAp.Code%></a></dd>
            <%JcAp_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int GLJi_index=1;
List<Lebi_Searchkey> GLJis = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey GLJi in GLJis){%><%=Lang(GLJi.Name)%><%GLJi_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int EpbB_index=1;
List<Lebi_Searchkey> EpbBs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey EpbB in EpbBs){%>
<%if (EpbB.Type==1){ %><a href="<%=URL("P_Search",""+Lang(EpbB.Name)+"") %>"><%}else{ %><a href="<%=EpbB.URL%>" target="_blank"><%} %><span><%=Lang(EpbB.Name)%></span></a>
<%EpbB_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int aRFf_index=1;
List<Lebi_Page> aRFfs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page aRFf in aRFfs){%>
<a class="menu" href="<%=URL("","",aRFf.url)%>"><span><%=aRFf.Name%></span></a>
<%aRFf_index++;}%>
...
<div class="userbox">
    ...
    <dl>
        <dt>Recharge Amount(<%=DefaultCurrency.Msige %>)：</dt>
   ...
   <dl id="pt0">
    ...
    <dl class="dl-btn">
        <dt></dt>
        <dd id="message"><a href="javascript:void(0)" onclick="submit();" class="btn btn-11"><s></s>Submit</a></dd>
    </dl>
    </div>
    </div>
    </div>
...
<script type="text/javascript">
    function submit() {
        ...
        if (RMoney == "" || RMoney<=0) {
            CheckNO("RMoney", "Recharge amount must be greater than 0");
            return false;
        }
        ...
        if (paytype == 1) {
            RequestAjax(url, postData, function (res) {
                ...
                $('#message').html('<span style="color:red;">Operation Successful!</span>');
            });
        }
        else {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Khjo_index=1;
List<Lebi_FriendLink> Khjos = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Khjo in Khjos){%>
            
                <% if (Khjo.Logo != "" && Khjo.IsPic == 1){ %><a href="<%=Khjo.Url%>" target="_blank"><img src="<%=Image(Khjo.Logo) %>" alt="<%=Khjo.Name%>" /></a><%}else{%><a href="<%=Khjo.Url%>" target="_blank"><%=Khjo.Name%></a><%} %>     
