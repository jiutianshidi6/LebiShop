...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserComment"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> eXAos=Languages();RecordCount=eXAos.Count;int eXAo_index=1;
foreach (Shop.Model.Lebi_Language eXAo in eXAos){%>
            <dd <%if (eXAo_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=eXAo.id%>,'<%=eXAo.Code%>','<%=eXAo.Path%>');"><%if (eXAo.ImageUrl!=""){%><img src="<%=Image(eXAo.ImageUrl) %>" /><%}%><%=eXAo.Name%></a></dd>
            <%eXAo_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int GYvg_index=1;
List<Lebi_Currency> GYvgs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency GYvg in GYvgs){%>
            <dd <%if (GYvg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=GYvg.id%>,'<%=GYvg.Code%>',<%=GYvg.ExchangeRate%>,'<%=GYvg.Msige%>','<%=GYvg.DecimalLength%>');"><%=GYvg.Code%></a></dd>
            <%GYvg_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int VCbt_index=1;
List<Lebi_Searchkey> VCbts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey VCbt in VCbts){%><%=Lang(VCbt.Name)%><%VCbt_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int eVFa_index=1;
List<Lebi_Searchkey> eVFas = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey eVFa in eVFas){%>
<%if (eVFa.Type==1){ %><a href="<%=URL("P_Search",""+Lang(eVFa.Name)+"") %>"><%}else{ %><a href="<%=eVFa.URL%>" target="_blank"><%} %><span><%=Lang(eVFa.Name)%></span></a>
<%eVFa_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int LCkD_index=1;
List<Lebi_Page> LCkDs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page LCkD in LCkDs){%>
<a class="menu" href="<%=URL("","",LCkD.url)%>"><span><%=LCkD.Name%></span></a>
<%LCkD_index++;}%>
...
<div class="user">
    <ul class="tablist">
        <li <%if (type == 0){Response.Write("class=\"selected\"");} %>><a href="<%=URL("P_UserComment","0","")%>"><span>Product had been commented</span></a></li>
        <li <%if (type == 1){Response.Write("class=\"selected\"");} %>><a href="<%=URL("P_UserComment","1","")%>"><span>Awaiting comment<em>(<%=Count_Comment(0) %>)</em></span></a></li>
    </ul>
    <div class="searchbox clearfix">
        <select id="status">
            <option value="">┌ Status</option>
            <option value="0" <%if (status == 0){%>selected<%} %>>Total</option>
    ...
    {
    %>
    <tr><td colspan="5">Seller Reply：<%=creply.Content%><em><%=creply.Time_Add%></em></td></tr>
    <%} %>
    </table>
    <%} %>
    <%}else{ %>
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        ...
        <tr>
            ...
            <td style="width:100px;"><a href="<%=URL("P_UserCommentWrite",op.id) %>">Write a Review</a></td>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int TcyU_index=1;
List<Lebi_FriendLink> TcyUs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink TcyU in TcyUs){%>
            
                <% if (TcyU.Logo != "" && TcyU.IsPic == 1){ %><a href="<%=TcyU.Url%>" target="_blank"><img src="<%=Image(TcyU.Logo) %>" alt="<%=TcyU.Name%>" /></a><%}else{%><a href="<%=TcyU.Url%>" target="_blank"><%=TcyU.Name%></a><%} %>     
