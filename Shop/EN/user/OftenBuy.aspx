...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserOftenBuy"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> QFTas=Languages();RecordCount=QFTas.Count;int QFTa_index=1;
foreach (Shop.Model.Lebi_Language QFTa in QFTas){%>
            <dd <%if (QFTa_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=QFTa.id%>,'<%=QFTa.Code%>','<%=QFTa.Path%>');"><%if (QFTa.ImageUrl!=""){%><img src="<%=Image(QFTa.ImageUrl) %>" /><%}%><%=QFTa.Name%></a></dd>
            <%QFTa_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int BtVx_index=1;
List<Lebi_Currency> BtVxs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency BtVx in BtVxs){%>
            <dd <%if (BtVx_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=BtVx.id%>,'<%=BtVx.Code%>',<%=BtVx.ExchangeRate%>,'<%=BtVx.Msige%>','<%=BtVx.DecimalLength%>');"><%=BtVx.Code%></a></dd>
            <%BtVx_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int sQdi_index=1;
List<Lebi_Searchkey> sQdis = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey sQdi in sQdis){%><%=Lang(sQdi.Name)%><%sQdi_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int OxgI_index=1;
List<Lebi_Searchkey> OxgIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey OxgI in OxgIs){%>
<%if (OxgI.Type==1){ %><a href="<%=URL("P_Search",""+Lang(OxgI.Name)+"") %>"><%}else{ %><a href="<%=OxgI.URL%>" target="_blank"><%} %><span><%=Lang(OxgI.Name)%></span></a>
<%OxgI_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int FhFr_index=1;
List<Lebi_Page> FhFrs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page FhFr in FhFrs){%>
<a class="menu" href="<%=URL("","",FhFr.url)%>"><span><%=FhFr.Name%></span></a>
<%FhFr_index++;}%>
...
<div class="user">
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        <tr>
            ...
            <th style="width:80px;">Buying cycle</th>
            <th style="width:120px;">Pre-time</th>
        </tr>
        ...
        <tr>
            ...
            <td><input name="WarnDays<%=u_p.id %>" id="WarnDays<%=u_p.id %>" type="text" value="<%=u_p.WarnDays %>" onkeyup="value=value.replace(/[^\d]/g,'');" shop="true" size="3" maxlength="5" class="input" /> Day</td>
            <td><%=FormatDate(u_p.Time_addemail)%></td>
        </tr>
        <%} %>
    </table>
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>Delete</a><a href="javascript:void(0)" onclick="Update();" class="btn btn-11"><s></s>Saved</a><a href="javascript:void(0)" onclick="LikeToBasket();" class="btn btn-7"><s></s>Add to Cart</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript" >
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int qSoO_index=1;
List<Lebi_FriendLink> qSoOs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink qSoO in qSoOs){%>
            
                <% if (qSoO.Logo != "" && qSoO.IsPic == 1){ %><a href="<%=qSoO.Url%>" target="_blank"><img src="<%=Image(qSoO.Logo) %>" alt="<%=qSoO.Name%>" /></a><%}else{%><a href="<%=qSoO.Url%>" target="_blank"><%=qSoO.Name%></a><%} %>     
