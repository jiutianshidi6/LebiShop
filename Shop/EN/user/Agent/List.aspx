...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserAgentList"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> afqhs=Languages();RecordCount=afqhs.Count;int afqh_index=1;
foreach (Shop.Model.Lebi_Language afqh in afqhs){%>
            <dd <%if (afqh_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=afqh.id%>,'<%=afqh.Code%>','<%=afqh.Path%>');"><%if (afqh.ImageUrl!=""){%><img src="<%=Image(afqh.ImageUrl) %>" /><%}%><%=afqh.Name%></a></dd>
            <%afqh_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int kfXV_index=1;
List<Lebi_Currency> kfXVs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency kfXV in kfXVs){%>
            <dd <%if (kfXV_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=kfXV.id%>,'<%=kfXV.Code%>',<%=kfXV.ExchangeRate%>,'<%=kfXV.Msige%>','<%=kfXV.DecimalLength%>');"><%=kfXV.Code%></a></dd>
            <%kfXV_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int emJP_index=1;
List<Lebi_Searchkey> emJPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey emJP in emJPs){%><%=Lang(emJP.Name)%><%emJP_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zkmj_index=1;
List<Lebi_Searchkey> zkmjs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zkmj in zkmjs){%>
<%if (zkmj.Type==1){ %><a href="<%=URL("P_Search",""+Lang(zkmj.Name)+"") %>"><%}else{ %><a href="<%=zkmj.URL%>" target="_blank"><%} %><span><%=Lang(zkmj.Name)%></span></a>
<%zkmj_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int KSAY_index=1;
List<Lebi_Page> KSAYs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page KSAY in KSAYs){%>
<a class="menu" href="<%=URL("","",KSAY.url)%>"><span><%=KSAY.Name%></span></a>
<%KSAY_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int QkYN_index=1;
List<Lebi_FriendLink> QkYNs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink QkYN in QkYNs){%>
            
                <% if (QkYN.Logo != "" && QkYN.IsPic == 1){ %><a href="<%=QkYN.Url%>" target="_blank"><img src="<%=Image(QkYN.Logo) %>" alt="<%=QkYN.Name%>" /></a><%}else{%><a href="<%=QkYN.Url%>" target="_blank"><%=QkYN.Name%></a><%} %>     
