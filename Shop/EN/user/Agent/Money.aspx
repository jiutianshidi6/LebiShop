...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserAgentMoney"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> qMiys=Languages();RecordCount=qMiys.Count;int qMiy_index=1;
foreach (Shop.Model.Lebi_Language qMiy in qMiys){%>
            <dd <%if (qMiy_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=qMiy.id%>,'<%=qMiy.Code%>','<%=qMiy.Path%>');"><%if (qMiy.ImageUrl!=""){%><img src="<%=Image(qMiy.ImageUrl) %>" /><%}%><%=qMiy.Name%></a></dd>
            <%qMiy_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int DUPv_index=1;
List<Lebi_Currency> DUPvs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency DUPv in DUPvs){%>
            <dd <%if (DUPv_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=DUPv.id%>,'<%=DUPv.Code%>',<%=DUPv.ExchangeRate%>,'<%=DUPv.Msige%>','<%=DUPv.DecimalLength%>');"><%=DUPv.Code%></a></dd>
            <%DUPv_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int uuOd_index=1;
List<Lebi_Searchkey> uuOds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey uuOd in uuOds){%><%=Lang(uuOd.Name)%><%uuOd_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Gwoe_index=1;
List<Lebi_Searchkey> Gwoes = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Gwoe in Gwoes){%>
<%if (Gwoe.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Gwoe.Name)+"") %>"><%}else{ %><a href="<%=Gwoe.URL%>" target="_blank"><%} %><span><%=Lang(Gwoe.Name)%></span></a>
<%Gwoe_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int lArD_index=1;
List<Lebi_Page> lArDs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page lArD in lArDs){%>
<a class="menu" href="<%=URL("","",lArD.url)%>"><span><%=lArD.Name%></span></a>
<%lArD_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int oPHg_index=1;
List<Lebi_FriendLink> oPHgs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink oPHg in oPHgs){%>
            
                <% if (oPHg.Logo != "" && oPHg.IsPic == 1){ %><a href="<%=oPHg.Url%>" target="_blank"><img src="<%=Image(oPHg.Logo) %>" alt="<%=oPHg.Name%>" /></a><%}else{%><a href="<%=oPHg.Url%>" target="_blank"><%=oPHg.Name%></a><%} %>     
