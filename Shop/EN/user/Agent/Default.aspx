...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_UserAgent"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> FxnRs=Languages();RecordCount=FxnRs.Count;int FxnR_index=1;
foreach (Shop.Model.Lebi_Language FxnR in FxnRs){%>
            <dd <%if (FxnR_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=FxnR.id%>,'<%=FxnR.Code%>','<%=FxnR.Path%>');"><%if (FxnR.ImageUrl!=""){%><img src="<%=Image(FxnR.ImageUrl) %>" /><%}%><%=FxnR.Name%></a></dd>
            <%FxnR_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int BTxx_index=1;
List<Lebi_Currency> BTxxs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency BTxx in BTxxs){%>
            <dd <%if (BTxx_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=BTxx.id%>,'<%=BTxx.Code%>',<%=BTxx.ExchangeRate%>,'<%=BTxx.Msige%>','<%=BTxx.DecimalLength%>');"><%=BTxx.Code%></a></dd>
            <%BTxx_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Rtbn_index=1;
List<Lebi_Searchkey> Rtbns = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Rtbn in Rtbns){%><%=Lang(Rtbn.Name)%><%Rtbn_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int RPVn_index=1;
List<Lebi_Searchkey> RPVns = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey RPVn in RPVns){%>
<%if (RPVn.Type==1){ %><a href="<%=URL("P_Search",""+Lang(RPVn.Name)+"") %>"><%}else{ %><a href="<%=RPVn.URL%>" target="_blank"><%} %><span><%=Lang(RPVn.Name)%></span></a>
<%RPVn_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Gsii_index=1;
List<Lebi_Page> Gsiis = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Gsii in Gsiis){%>
<a class="menu" href="<%=URL("","",Gsii.url)%>"><span><%=Gsii.Name%></span></a>
<%Gsii_index++;}%>
...
<div class="userbox">
    ...
    <dl>
        <dt><%=Tag("我的推广链接")%>：</dt>
        <dd><span  id="myurl">http://<%=SYS.Domain==""?ShopCache.GetMainSite().Domain:SYS.Domain%>/EN/?v=<%=CurrentUser.id %></span></dd>
    </dl>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Bdxv_index=1;
List<Lebi_FriendLink> Bdxvs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Bdxv in Bdxvs){%>
            
                <% if (Bdxv.Logo != "" && Bdxv.IsPic == 1){ %><a href="<%=Bdxv.Url%>" target="_blank"><img src="<%=Image(Bdxv.Logo) %>" alt="<%=Bdxv.Name%>" /></a><%}else{%><a href="<%=Bdxv.Url%>" target="_blank"><%=Bdxv.Name%></a><%} %>     
