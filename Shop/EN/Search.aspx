...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_Search"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> MVESs=Languages();RecordCount=MVESs.Count;int MVES_index=1;
foreach (Shop.Model.Lebi_Language MVES in MVESs){%>
            <dd <%if (MVES_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=MVES.id%>,'<%=MVES.Code%>','<%=MVES.Path%>');"><%if (MVES.ImageUrl!=""){%><img src="<%=Image(MVES.ImageUrl) %>" /><%}%><%=MVES.Name%></a></dd>
            <%MVES_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int aJLP_index=1;
List<Lebi_Currency> aJLPs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency aJLP in aJLPs){%>
            <dd <%if (aJLP_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=aJLP.id%>,'<%=aJLP.Code%>',<%=aJLP.ExchangeRate%>,'<%=aJLP.Msige%>','<%=aJLP.DecimalLength%>');"><%=aJLP.Code%></a></dd>
            <%aJLP_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int GKMQ_index=1;
List<Lebi_Searchkey> GKMQs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey GKMQ in GKMQs){%><%=Lang(GKMQ.Name)%><%GKMQ_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int byhe_index=1;
List<Lebi_Searchkey> byhes = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey byhe in byhes){%>
<%if (byhe.Type==1){ %><a href="<%=URL("P_Search",""+Lang(byhe.Name)+"") %>"><%}else{ %><a href="<%=byhe.URL%>" target="_blank"><%} %><span><%=Lang(byhe.Name)%></span></a>
<%byhe_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int CGbF_index=1;
List<Lebi_Page> CGbFs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page CGbF in CGbFs){%>
<a class="menu" href="<%=URL("","",CGbF.url)%>"><span><%=CGbF.Name%></span></a>
<%CGbF_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int kDcu_index=1;
List<Lebi_Page> kDcus = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page kDcu in kDcus){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",kDcu.id,kDcu.url)%>" target="<%=kDcu.target%>" title="<%=kDcu.Name%>"><span style="color:<%=kDcu.NameColor%>"><%=kDcu.Name%></span></a></li>
        <%kDcu_index++;}%>
    </ul>
</div>
</div>
...
<div id="filter">
	<div class="fore1">
		<dl class="order">
			<dt>Sort：</dt>
			<dd <%if(sort == "1" || sort == "1a"){ %>class="curr <%if(sort == "1"){ %>up<%}else if(sort == "1a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Search",""+keyword+","+list+",1"+ordertmp+"") %>">Sales</a><b></b></dd>
			<dd <%if(sort == "2" || sort == "2a"){ %>class="curr <%if(sort == "2"){ %>up<%}else if(sort == "2a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Search",keyword+","+list+",2"+ordertmp+"") %>">Price</a><b></b></dd>
			<dd <%if(sort == "3" || sort == "3a"){ %>class="curr <%if(sort == "3"){ %>up<%}else if(sort == "3a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Search",keyword+","+list+",3"+ordertmp+"") %>">Reviews</a><b></b></dd>
			<dd <%if(sort == "4" || sort == "4a"){ %>class="curr <%if(sort == "4"){ %>up<%}else if(sort == "4a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Search",keyword+","+list+",4"+ordertmp+"") %>">Shelf Time</a><b></b></dd>
		</dl>
        <div class="list-cutover">
			<a id="list-unselected"  href="<%=URL("P_Search",keyword+",2,"+ sort) %>" class="modfiy-url list-unselected <%=list=="2"?"list-curr":"" %>"><b></b>List</a>
			<a id="grid-unselected"  href="<%=URL("P_Search",keyword+",1,"+ sort) %>" class="modfiy-url grid-unselected <%=(list=="1" || list=="")?"grid-curr":"" %>"><b></b>Grid</a>
		</div>
...
<div class="mc clearfix">
    <div class="productlist loadlist"><ul>
        ...
        <li class="grid loadli">
            <div class="proimg">
                <p><a href="<%=URL("P_Product",pro.id) %>"><img <%if (CurrentLanguage.IsLazyLoad==1){ %>class="listlazy" src="/Theme/system/images/lazy.gif" data-original<%}else{ %>src<%} %>="<%=Image(pro.ImageOriginal,"medium")%>" alt="<%=Lang(pro.Name) %>" title="<%=Lang(pro.Name) %>" /></a></p></div>
            <div class="proname">
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int PoGP_index=1;
List<Lebi_FriendLink> PoGPs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink PoGP in PoGPs){%>
            
                <% if (PoGP.Logo != "" && PoGP.IsPic == 1){ %><a href="<%=PoGP.Url%>" target="_blank"><img src="<%=Image(PoGP.Logo) %>" alt="<%=PoGP.Name%>" /></a><%}else{%><a href="<%=PoGP.Url%>" target="_blank"><%=PoGP.Name%></a><%} %>     
