...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> SkmKs=Languages();RecordCount=SkmKs.Count;int SkmK_index=1;
foreach (Shop.Model.Lebi_Language SkmK in SkmKs){%>
            <dd <%if (SkmK_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=SkmK.id%>,'<%=SkmK.Code%>','<%=SkmK.Path%>');"><%if (SkmK.ImageUrl!=""){%><img src="<%=Image(SkmK.ImageUrl) %>" /><%}%><%=SkmK.Name%></a></dd>
            <%SkmK_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int bJaA_index=1;
List<Lebi_Currency> bJaAs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency bJaA in bJaAs){%>
            <dd <%if (bJaA_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=bJaA.id%>,'<%=bJaA.Code%>',<%=bJaA.ExchangeRate%>,'<%=bJaA.Msige%>','<%=bJaA.DecimalLength%>');"><%=bJaA.Code%></a></dd>
            <%bJaA_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int EUgw_index=1;
List<Lebi_Searchkey> EUgws = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey EUgw in EUgws){%><%=Lang(EUgw.Name)%><%EUgw_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vtyz_index=1;
List<Lebi_Searchkey> vtyzs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vtyz in vtyzs){%>
<%if (vtyz.Type==1){ %><a href="<%=URL("P_Search",""+Lang(vtyz.Name)+"") %>"><%}else{ %><a href="<%=vtyz.URL%>" target="_blank"><%} %><span><%=Lang(vtyz.Name)%></span></a>
<%vtyz_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int QMep_index=1;
List<Lebi_Page> QMeps = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page QMep in QMeps){%>
<a class="menu" href="<%=URL("","",QMep.url)%>"><span><%=QMep.Name%></span></a>
<%QMep_index++;}%>
...
<ul class="big">
    <%List<Shop.Model.Lebi_Pro_Type> pDqvs=Cateporys(id);RecordCount=pDqvs.Count;int pDqv_index=1;
foreach (Shop.Model.Lebi_Pro_Type pDqv in pDqvs){%>
    <%if (pDqv.IsIndexShow == 1){ %>
    <li><a href="<%=URL("P_ProductCategory",pDqv.id,Lang(pDqv.Url)) %>" title="<%=Lang(pDqv.Name) %>"><%=Lang(pDqv.Name) %></a> <em>(<%=Shop.Bussiness.EX_Product.OnSaleTypeProductCount(pDqv.id)%>)</em>
    </li>
    <%} %>
    <%pDqv_index++;}%>
</ul>
...
List<Lebi_Product> producttops = B_Lebi_Product.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Product producttop in producttops){%>
	...
	<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=15;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ErHk_index=1;
List<Lebi_Searchkey> ErHks = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ErHk in ErHks){%>
	<%if (ErHk.Type==1){ %><a href="<%=URL("P_Search",""+Lang(ErHk.Name)+"") %>"><%}else{ %><a href="<%=ErHk.URL%>" target="_blank"><%} %><span><%=Lang(ErHk.Name)%></span></a>,
	<%ErHk_index++;}%> 
	</div>
    ...
    <div class="searchselect">
        ...
        <div class="mc clearfix">
            <ul class="selectvalue">
                <li><%Table="Lebi_Brand";Where="','+Pro_Type_id+',' like '%,"+pro_type.id+",%'";Order="Sort desc";PageSize=999;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int dqRp_index=1;
List<Lebi_Brand> dqRps = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand dqRp in dqRps){%>
                    <% 
            if(dqRp_index==1 && RecordCount>0){
                    %>
                    <span>Product Brand：</span> <a href="<%=URL("P_ProductCategory",""+id+",0,"+cid+",$,$,"+tid+"") %>"
                        <%=pid==0?"class=\"selectall\"":"" %>>Total</a>
                    <%} %>
                    <a href="<%=URL("P_ProductCategory",""+id+","+dqRp.id+","+cid+",$,$,"+tid+"") %>" <%=pid==dqRp.id?"class=\"selectone\"":"" %>>
                        <%=Lang(dqRp.Name) %></a> <%dqRp_index++;}%> </li>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int tQUT_index=1;
List<Lebi_FriendLink> tQUTs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink tQUT in tQUTs){%>
            
                <% if (tQUT.Logo != "" && tQUT.IsPic == 1){ %><a href="<%=tQUT.Url%>" target="_blank"><img src="<%=Image(tQUT.Logo) %>" alt="<%=tQUT.Name%>" /></a><%}else{%><a href="<%=tQUT.Url%>" target="_blank"><%=tQUT.Name%></a><%} %>     
