...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_qianhaiPay"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> YgVJs=Languages();RecordCount=YgVJs.Count;int YgVJ_index=1;
foreach (Shop.Model.Lebi_Language YgVJ in YgVJs){%>
            <dd <%if (YgVJ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=YgVJ.id%>,'<%=YgVJ.Code%>','<%=YgVJ.Path%>');"><%if (YgVJ.ImageUrl!=""){%><img src="<%=Image(YgVJ.ImageUrl) %>" /><%}%><%=YgVJ.Name%></a></dd>
            <%YgVJ_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int HfmA_index=1;
List<Lebi_Currency> HfmAs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency HfmA in HfmAs){%>
            <dd <%if (HfmA_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=HfmA.id%>,'<%=HfmA.Code%>',<%=HfmA.ExchangeRate%>,'<%=HfmA.Msige%>','<%=HfmA.DecimalLength%>');"><%=HfmA.Code%></a></dd>
            <%HfmA_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int sIGw_index=1;
List<Lebi_Searchkey> sIGws = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey sIGw in sIGws){%><%=Lang(sIGw.Name)%><%sIGw_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hIyf_index=1;
List<Lebi_Searchkey> hIyfs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hIyf in hIyfs){%>
<%if (hIyf.Type==1){ %><a href="<%=URL("P_Search",""+Lang(hIyf.Name)+"") %>"><%}else{ %><a href="<%=hIyf.URL%>" target="_blank"><%} %><span><%=Lang(hIyf.Name)%></span></a>
<%hIyf_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int XCna_index=1;
List<Lebi_Page> XCnas = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page XCna in XCnas){%>
<a class="menu" href="<%=URL("","",XCna.url)%>"><span><%=XCna.Name%></span></a>
<%XCna_index++;}%>
    ...
    <div class="user">
        <div class="reg clearfix">
            <div class="mt clearfix">
                <h2>Order Number：<%=order.Code %></h2>
            </div>
            <div class="mc clearfix">
            <div class="dl-table clearfix">
            <dl>
                <dt><font color="red">*</font>Sum：</dt>
                <dd>
                    <%=currency %><%=Money %>
                </dd>
            </dl>
            <dl>
                <dt><font color="red">*</font>Cardholder’s name：</dt>
            ...
            <dl>
                <dt><font color="red">*</font>Cardholder’s mailbox：</dt>
            ...
            <dl>
                <dt><font color="red">*</font>Cardholder’s phone：</dt>
            ...
            <dl>
                <dt><font color="red">*</font>Cardholder billing country：</dt>
            ...
            <dl>
                <dt><font color="red">*</font>Cardholder city：</dt>
             ...
             <dl>
                <dt><font color="red">*</font>Cardholder’s address details：</dt>
            ...
            <dl>
                <dt><font color="red">*</font>Cardholder’s zip code：</dt>
            ...
            <dl class="dl-btn">
                <dt></dt>
                <dd><a href="javascript:void(0)" onclick="document.forms[0].submit();" class="btn btn-6"><s></s>Submit Now</a></dd>
            </dl>
            </div>
            </div>
        </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int cbXW_index=1;
List<Lebi_FriendLink> cbXWs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink cbXW in cbXWs){%>
            
                <% if (cbXW.Logo != "" && cbXW.IsPic == 1){ %><a href="<%=cbXW.Url%>" target="_blank"><img src="<%=Image(cbXW.Logo) %>" alt="<%=cbXW.Name%>" /></a><%}else{%><a href="<%=cbXW.Url%>" target="_blank"><%=cbXW.Name%></a><%} %>     
