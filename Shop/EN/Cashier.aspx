...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_Cashier"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> fIbjs=Languages();RecordCount=fIbjs.Count;int fIbj_index=1;
foreach (Shop.Model.Lebi_Language fIbj in fIbjs){%>
            <dd <%if (fIbj_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=fIbj.id%>,'<%=fIbj.Code%>','<%=fIbj.Path%>');"><%if (fIbj.ImageUrl!=""){%><img src="<%=Image(fIbj.ImageUrl) %>" /><%}%><%=fIbj.Name%></a></dd>
            <%fIbj_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int xYzn_index=1;
List<Lebi_Currency> xYzns = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency xYzn in xYzns){%>
            <dd <%if (xYzn_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=xYzn.id%>,'<%=xYzn.Code%>',<%=xYzn.ExchangeRate%>,'<%=xYzn.Msige%>','<%=xYzn.DecimalLength%>');"><%=xYzn.Code%></a></dd>
            <%xYzn_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int rOFw_index=1;
List<Lebi_Searchkey> rOFws = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey rOFw in rOFws){%><%=Lang(rOFw.Name)%><%rOFw_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int sIrR_index=1;
List<Lebi_Searchkey> sIrRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey sIrR in sIrRs){%>
<%if (sIrR.Type==1){ %><a href="<%=URL("P_Search",""+Lang(sIrR.Name)+"") %>"><%}else{ %><a href="<%=sIrR.URL%>" target="_blank"><%} %><span><%=Lang(sIrR.Name)%></span></a>
<%sIrR_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Bgpz_index=1;
List<Lebi_Page> Bgpzs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Bgpz in Bgpzs){%>
<a class="menu" href="<%=URL("","",Bgpz.url)%>"><span><%=Bgpz.Name%></span></a>
<%Bgpz_index++;}%>
...
<ul>
	...
	<li class="current"><span class="con">Order Successfully Submitted</span></li>
...
<div class="userbox">
    <div class="mt clearfix">
        <h2>Your order have been submit.</h2>
    </div>
    <div class="mc clearfix">
    <div class="cont">
        <%if(CurrentUser.IsAnonymous!=1){
            if(order.Type_id_OrderType==215){ %>
        <a href="<%=URL("P_UserOrders","")%>" class="btn btn-11"><s></s>Check Order</a>
        <%}else{ %>
        <a href="<%=URL("P_UserOrderDetails",order.id)%>" class="btn btn-11"><s></s>Check Order</a>
        <%}} %>
        <%
        if((onlinepay!=null && order.IsPaid==0 && pay.Code=="OnlinePay") || order.Type_id_OrderType==215){
        %>
        <a href="<%=URL("P_Pay",order.id+","+order.Code+",cashier")%>" target="_blank" class="btn btn-7"><s></s>Pay Now</a>
        <%} %>
    </div>
    </div>
    </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int ezlq_index=1;
List<Lebi_FriendLink> ezlqs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink ezlq in ezlqs){%>
            
                <% if (ezlq.Logo != "" && ezlq.IsPic == 1){ %><a href="<%=ezlq.Url%>" target="_blank"><img src="<%=Image(ezlq.Logo) %>" alt="<%=ezlq.Name%>" /></a><%}else{%><a href="<%=ezlq.Url%>" target="_blank"><%=ezlq.Name%></a><%} %>     
