...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_Register"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> XiJds=Languages();RecordCount=XiJds.Count;int XiJd_index=1;
foreach (Shop.Model.Lebi_Language XiJd in XiJds){%>
            <dd <%if (XiJd_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=XiJd.id%>,'<%=XiJd.Code%>','<%=XiJd.Path%>');"><%if (XiJd.ImageUrl!=""){%><img src="<%=Image(XiJd.ImageUrl) %>" /><%}%><%=XiJd.Name%></a></dd>
            <%XiJd_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int WHRm_index=1;
List<Lebi_Currency> WHRms = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency WHRm in WHRms){%>
            <dd <%if (WHRm_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=WHRm.id%>,'<%=WHRm.Code%>',<%=WHRm.ExchangeRate%>,'<%=WHRm.Msige%>','<%=WHRm.DecimalLength%>');"><%=WHRm.Code%></a></dd>
            <%WHRm_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int kiGy_index=1;
List<Lebi_Searchkey> kiGys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey kiGy in kiGys){%><%=Lang(kiGy.Name)%><%kiGy_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int TCLl_index=1;
List<Lebi_Searchkey> TCLls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey TCLl in TCLls){%>
<%if (TCLl.Type==1){ %><a href="<%=URL("P_Search",""+Lang(TCLl.Name)+"") %>"><%}else{ %><a href="<%=TCLl.URL%>" target="_blank"><%} %><span><%=Lang(TCLl.Name)%></span></a>
<%TCLl_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int HRKo_index=1;
List<Lebi_Page> HRKos = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page HRKo in HRKos){%>
<a class="menu" href="<%=URL("","",HRKo.url)%>"><span><%=HRKo.Name%></span></a>
<%HRKo_index++;}%>
...
<div class="nbbox clearfix">
    <div class="user">
        <div class="reg clearfix">
            ...
            <dl>
                <dt><font color="red">*</font>Username：</dt>
                ...
                <span id="FormUserName" class="FormALT">4- 20 character. Can be numbers or alphabets</span>
                </dd>
            </dl>
            <dl>
                <dt><font color="red">*</font>Password：</dt>
                <dd><input type="password" shop="true" name="Password" id="Password" style="width: 200px;" maxlength="16" class="input" min="6" />
                <span id="FormPassword" class="FormALT">Passwords are case sensitive</span>
                </dd>
            </dl>
            <dl>
                <dt><font color="red">*</font>RE-Password：</dt>
                <dd><input type="password" name="Password1" shop="true" id="Password1" style="width: 200px;" maxlength="16" class="input" />
                <span id="FormPassword1" class="FormALT">Please re-enter password</span>
                </dd>
            </dl>
            <dl>
                <dt><%if(Checkmobilephone){ %><font color="red">*</font><%} %>Mobile Number：</dt>
                <dd><input type="text" shop="true" name="MobilePhone" id="MobilePhone" <%if(Checkmobilephone){ %>min="notnull"<%} %> style="width: 200px;" class="input" />
                    <%if(Checkmobilephone){ %>
                    <%if (CurrentSite.IsMobile==0){ %>&nbsp;<%}else{%><br /><%}%>Security Code：
            ...
            <dl>
                <dt><%if(Checkemail){ %><font color="red">*</font> <%} %>E-mail：</dt>
...
<%}}%>
            ...
            <dl>
                <dt><font color="red">*</font>Security Code：</dt>
                <dd><input name="verifycode" type="text" shop="true" id="verifycode" shop="true" size="6"
 class="input" /><img class="verifycode" id="img1" src="<%=WebPath%>/code.aspx" /><img src="/Theme/system/images/reload.gif" class="refresh" title="Click to refresh Security Code" onclick="refresh('img1')" /></dd>
            </dl>
            <%} %>
            <%} %>
            <dl>
                <dt></dt>
                <dd><label><input type="checkbox" name="agree" id="agree" />Read and Agree<a href="<%=URL("P_Help","$,agreement")%>" target="_blank">《Agreement》</a></label>
                <label><input type="checkbox" name="IsOpen" id="IsOpen" onclick="OpenMore();" />Enter information</label></dd>
            </dl>
            ...
            <dl>
                <dt>Gender：</dt>
                <dd><label><input type="radio" name="Sex" value="男" checked />Male</label>
                    <label><input type="radio" name="Sex" value="女" />Female</label></dd>
            </dl>
            <dl>
                <dt>Birthday：</dt>
            ...
            <dl class="dl-btn">
                <dt></dt>
                <dd><a href="javascript:void(0)" onclick="submit();" class="btn btn-6"><s></s>Submit</a></dd>
            </dl>
            </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    function submit() {
        ...
        if (Password != Password1) {
            CheckNO("Password1", "Passwords do not match");
            return false;
        }
        var flag = $("#agree").is(":checked");
        if (!flag) {
            MsgBox(2, "Read and Agree License", "");
            return false;
        }
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.msg == "OK") {
                    MsgBox(1, "Registration Successful!", res.url);
                }
                else {
    ...
    function Check_username() {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == "OK") { CheckOK("UserName", "Username can be registered"); } else { CheckNO("UserName", "Username has been registered"); return false; }
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int VKQs_index=1;
List<Lebi_FriendLink> VKQss = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink VKQs in VKQss){%>
            
                <% if (VKQs.Logo != "" && VKQs.IsPic == 1){ %><a href="<%=VKQs.Url%>" target="_blank"><img src="<%=Image(VKQs.Logo) %>" alt="<%=VKQs.Name%>" /></a><%}else{%><a href="<%=VKQs.Url%>" target="_blank"><%=VKQs.Name%></a><%} %>     
