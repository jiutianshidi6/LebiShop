...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_Pay"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> fBaYs=Languages();RecordCount=fBaYs.Count;int fBaY_index=1;
foreach (Shop.Model.Lebi_Language fBaY in fBaYs){%>
            <dd <%if (fBaY_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=fBaY.id%>,'<%=fBaY.Code%>','<%=fBaY.Path%>');"><%if (fBaY.ImageUrl!=""){%><img src="<%=Image(fBaY.ImageUrl) %>" /><%}%><%=fBaY.Name%></a></dd>
            <%fBaY_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int vGgx_index=1;
List<Lebi_Currency> vGgxs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency vGgx in vGgxs){%>
            <dd <%if (vGgx_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=vGgx.id%>,'<%=vGgx.Code%>',<%=vGgx.ExchangeRate%>,'<%=vGgx.Msige%>','<%=vGgx.DecimalLength%>');"><%=vGgx.Code%></a></dd>
            <%vGgx_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int WdAw_index=1;
List<Lebi_Searchkey> WdAws = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey WdAw in WdAws){%><%=Lang(WdAw.Name)%><%WdAw_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Cxai_index=1;
List<Lebi_Searchkey> Cxais = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Cxai in Cxais){%>
<%if (Cxai.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Cxai.Name)+"") %>"><%}else{ %><a href="<%=Cxai.URL%>" target="_blank"><%} %><span><%=Lang(Cxai.Name)%></span></a>
<%Cxai_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int vpwD_index=1;
List<Lebi_Page> vpwDs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page vpwD in vpwDs){%>
<a class="menu" href="<%=URL("","",vpwD.url)%>"><span><%=vpwD.Name%></span></a>
<%vpwD_index++;}%>
                ...
                <div class="dl-table clearfix">
                    ...
                    <dl class="dl-btn">
                        ...
                        <dd>
                            ...
                            if (mes == ""){
                            %>
                            <a id="nowpay" href="javascript:void(0)" onclick="pay();" class="btn btn-7"><s></s>Pay Now</a>
                            <%}else{%>
                            <%=mes %>
                            <%} %>
                        </dd>
...
<script type="text/javascript">
    ...
    if (Isweixin && weixinqrcode){
        ...
        if (pagefrom == "cashier"){
            $("#pay").hide();MsgBox(1, "Submitting……", pay());
        }
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int kDbx_index=1;
List<Lebi_FriendLink> kDbxs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink kDbx in kDbxs){%>
            
                <% if (kDbx.Logo != "" && kDbx.IsPic == 1){ %><a href="<%=kDbx.Url%>" target="_blank"><img src="<%=Image(kDbx.Logo) %>" alt="<%=kDbx.Name%>" /></a><%}else{%><a href="<%=kDbx.Url%>" target="_blank"><%=kDbx.Name%></a><%} %>     
