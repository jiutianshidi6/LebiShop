...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_GroupPurchase"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> Ggpfs=Languages();RecordCount=Ggpfs.Count;int Ggpf_index=1;
foreach (Shop.Model.Lebi_Language Ggpf in Ggpfs){%>
            <dd <%if (Ggpf_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Ggpf.id%>,'<%=Ggpf.Code%>','<%=Ggpf.Path%>');"><%if (Ggpf.ImageUrl!=""){%><img src="<%=Image(Ggpf.ImageUrl) %>" /><%}%><%=Ggpf.Name%></a></dd>
            <%Ggpf_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int vyVG_index=1;
List<Lebi_Currency> vyVGs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency vyVG in vyVGs){%>
            <dd <%if (vyVG_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=vyVG.id%>,'<%=vyVG.Code%>',<%=vyVG.ExchangeRate%>,'<%=vyVG.Msige%>','<%=vyVG.DecimalLength%>');"><%=vyVG.Code%></a></dd>
            <%vyVG_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int EyXP_index=1;
List<Lebi_Searchkey> EyXPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey EyXP in EyXPs){%><%=Lang(EyXP.Name)%><%EyXP_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int YFHn_index=1;
List<Lebi_Searchkey> YFHns = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey YFHn in YFHns){%>
<%if (YFHn.Type==1){ %><a href="<%=URL("P_Search",""+Lang(YFHn.Name)+"") %>"><%}else{ %><a href="<%=YFHn.URL%>" target="_blank"><%} %><span><%=Lang(YFHn.Name)%></span></a>
<%YFHn_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int tniu_index=1;
List<Lebi_Page> tnius = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page tniu in tnius){%>
<a class="menu" href="<%=URL("","",tniu.url)%>"><span><%=tniu.Name%></span></a>
<%tniu_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Txwj_index=1;
List<Lebi_Page> Txwjs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Txwj in Txwjs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",Txwj.id,Txwj.url)%>" target="<%=Txwj.target%>" title="<%=Txwj.Name%>"><span style="color:<%=Txwj.NameColor%>"><%=Txwj.Name%></span></a></li>
        <%Txwj_index++;}%>
    </ul>
</div>
</div>
...
<div id="filter">
	<div class="fore1">
		<dl class="order">
			<dt>Sort：</dt>
			<dd <%if(sort == "" || sort == "1" || sort == "1a"){ %>class="curr <%if(sort == "" || sort == "1"){ %>up<%}else if(sort == "1a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_GroupPurchase",""+list+",1"+ordertmp+"") %>">Sales</a><b></b></dd>
			<dd <%if(sort == "2" || sort == "2a"){ %>class="curr <%if(sort == "2"){ %>up<%}else if(sort == "2a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_GroupPurchase",""+list+",2"+ordertmp+"") %>">Price</a><b></b></dd>
			<dd <%if(sort == "3" || sort == "3a"){ %>class="curr <%if(sort == "3"){ %>up<%}else if(sort == "3a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_GroupPurchase",""+list+",3"+ordertmp+"") %>">Reviews</a><b></b></dd>
			<dd <%if(sort == "4" || sort == "4a"){ %>class="curr <%if(sort == "4"){ %>up<%}else if(sort == "4a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_GroupPurchase",""+list+",4"+ordertmp+"") %>">Shelf Time</a><b></b></dd>
		</dl>
        <div class="list-cutover">
			<a id="list-unselected"  href="<%=URL("P_GroupPurchase","2,"+ sort) %>" class="modfiy-url list-unselected <%=list=="2"?"list-curr":"" %>"><b></b>List</a>
			<a id="grid-unselected"  href="<%=URL("P_GroupPurchase","1,"+ sort) %>" class="modfiy-url grid-unselected <%=(list=="1" || list=="")?"grid-curr":"" %>"><b></b>Grid</a>
		</div>
...
<div class="mc clearfix">
    <div class="productlist limitbuylist loadlist"><ul>
        ...
        <li class="grid loadli">
            ...
            <div class="proinfo">
            <div class="buyprice"><i>Group Price：</i><strong><%=FormatMoney(pro.Price_Sale) %></strong><em><%=Discount(pro.Price_Market,pro.Price_Sale) %>% OFF</em></div>
            <div class="marketprice"><i>Sales Price：</i><strong><%=FormatMoney(pro.Price) %></strong></div>
            <div class="count"><i>Sales：</i><strong><%=pro.Count_Sales_Show%></strong></div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int dFdQ_index=1;
List<Lebi_FriendLink> dFdQs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink dFdQ in dFdQs){%>
            
                <% if (dFdQ.Logo != "" && dFdQ.IsPic == 1){ %><a href="<%=dFdQ.Url%>" target="_blank"><img src="<%=Image(dFdQ.Logo) %>" alt="<%=dFdQ.Name%>" /></a><%}else{%><a href="<%=dFdQ.Url%>" target="_blank"><%=dFdQ.Name%></a><%} %>     
