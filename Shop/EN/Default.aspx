...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_Index"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> Ahtqs=Languages();RecordCount=Ahtqs.Count;int Ahtq_index=1;
foreach (Shop.Model.Lebi_Language Ahtq in Ahtqs){%>
            <dd <%if (Ahtq_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Ahtq.id%>,'<%=Ahtq.Code%>','<%=Ahtq.Path%>');"><%if (Ahtq.ImageUrl!=""){%><img src="<%=Image(Ahtq.ImageUrl) %>" /><%}%><%=Ahtq.Name%></a></dd>
            <%Ahtq_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int AmaW_index=1;
List<Lebi_Currency> AmaWs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency AmaW in AmaWs){%>
            <dd <%if (AmaW_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=AmaW.id%>,'<%=AmaW.Code%>',<%=AmaW.ExchangeRate%>,'<%=AmaW.Msige%>','<%=AmaW.DecimalLength%>');"><%=AmaW.Code%></a></dd>
            <%AmaW_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int pjkY_index=1;
List<Lebi_Searchkey> pjkYs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey pjkY in pjkYs){%><%=Lang(pjkY.Name)%><%pjkY_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int HTGK_index=1;
List<Lebi_Searchkey> HTGKs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey HTGK in HTGKs){%>
<%if (HTGK.Type==1){ %><a href="<%=URL("P_Search",""+Lang(HTGK.Name)+"") %>"><%}else{ %><a href="<%=HTGK.URL%>" target="_blank"><%} %><span><%=Lang(HTGK.Name)%></span></a>
<%HTGK_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int mYrd_index=1;
List<Lebi_Page> mYrds = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page mYrd in mYrds){%>
<a class="menu" href="<%=URL("","",mYrd.url)%>"><span><%=mYrd.Name%></span></a>
<%mYrd_index++;}%>
...
<%producttop_index++;}%>
        ...
        <h2 class="title">Sales charts</h2>
        <div class="tuijian">
...
<div class="ind-tabs">
	<div class="hd">
    	<ul>
        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int rAyj_index=1;
List<Lebi_Brand> rAyjs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand rAyj in rAyjs){%>
        	<li><%=Lang(rAyj.Name) %></li>
        <%rAyj_index++;}%>
        </ul>
    </div>
    <div class="bd">
      <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int Ruqj_index=1;
List<Lebi_Brand> Ruqjs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand Ruqj in Ruqjs){%>
   	  <div class="lh">
        	<div class="pic"><a href="<%=URL("P_Brand",Ruqj.id)%>"><img src="<%=Image(Ruqj.ImageUrl) %>" alt="<%=Lang(Ruqj.Name) %>" title="<%=Lang(Ruqj.Name) %>" width="230" height="230" /></a></div>
            <div class="info">
            	<div class="desc"><%=Lang(Ruqj.Description)%></div>
                <a href="<%=URL("P_Brand",Ruqj.id)%>" class="read-more"><%=Lang(Ruqj.Name) %> Product List >></a>
            </div>
      </div>
      <%Ruqj_index++;}%>
    </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int wPEt_index=1;
List<Lebi_FriendLink> wPEts = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink wPEt in wPEts){%>
            
                <% if (wPEt.Logo != "" && wPEt.IsPic == 1){ %><a href="<%=wPEt.Url%>" target="_blank"><img src="<%=Image(wPEt.Logo) %>" alt="<%=wPEt.Name%>" /></a><%}else{%><a href="<%=wPEt.Url%>" target="_blank"><%=wPEt.Name%></a><%} %>     
