...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_ForgetPassword"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> RfvOs=Languages();RecordCount=RfvOs.Count;int RfvO_index=1;
foreach (Shop.Model.Lebi_Language RfvO in RfvOs){%>
            <dd <%if (RfvO_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=RfvO.id%>,'<%=RfvO.Code%>','<%=RfvO.Path%>');"><%if (RfvO.ImageUrl!=""){%><img src="<%=Image(RfvO.ImageUrl) %>" /><%}%><%=RfvO.Name%></a></dd>
            <%RfvO_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int JIKN_index=1;
List<Lebi_Currency> JIKNs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency JIKN in JIKNs){%>
            <dd <%if (JIKN_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=JIKN.id%>,'<%=JIKN.Code%>',<%=JIKN.ExchangeRate%>,'<%=JIKN.Msige%>','<%=JIKN.DecimalLength%>');"><%=JIKN.Code%></a></dd>
            <%JIKN_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int WRPD_index=1;
List<Lebi_Searchkey> WRPDs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey WRPD in WRPDs){%><%=Lang(WRPD.Name)%><%WRPD_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int uGgI_index=1;
List<Lebi_Searchkey> uGgIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey uGgI in uGgIs){%>
<%if (uGgI.Type==1){ %><a href="<%=URL("P_Search",""+Lang(uGgI.Name)+"") %>"><%}else{ %><a href="<%=uGgI.URL%>" target="_blank"><%} %><span><%=Lang(uGgI.Name)%></span></a>
<%uGgI_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int KwgL_index=1;
List<Lebi_Page> KwgLs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page KwgL in KwgLs){%>
<a class="menu" href="<%=URL("","",KwgL.url)%>"><span><%=KwgL.Name%></span></a>
<%KwgL_index++;}%>
...
<div class="userbox">
    ...
    <dl>
        <dt>Security Code：</dt>
        <dd><input name="verifycode" type="text" shop="true" id="verifycode" shop="true" size="6" class="input" /><img class="verifycode" id="img1" src="<%=WebPath%>/code.aspx" /><img src="/Theme/system/images/reload.gif" class="refresh" title="Click to refresh Security Code" onclick="refresh('img1')" /></dd>
    </dl>
    <%} %>
    <dl class="dl-btn">
        <dt></dt>
...
<script type="text/javascript">
    function submit() {
        ...
        if (type == 0) {
            if (Email == "") {
                CheckNO("Email", "Please enter your email");
                return false;
            }
            var postData = GetFormJsonData("shop");
            var url = path + "/ajax/ajax_user.aspx?__Action=User_forgetpwd&type=0";
            RequestAjax(url, postData, function () { $("#msg").html("Forgot your password email has been sent to your email"); MsgBoxClose(); });
        } else {
            if (UserName == "") {
                CheckNO("UserName", "Please enter your login account");
                return false;
            }
            var postData = GetFormJsonData("shop");
            var url = path + "/ajax/ajax_user.aspx?__Action=User_forgetpwd&type=" + type + "";
            RequestAjax(url, postData, function (res) { if (type == 1) { location.href = res.url; } else { MsgBox(1, "New password has been sent to your phone, check your", res.url); } });
        }
    }
    function GetType() {
        var type = $("input[name='type']:checked").val();
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int gTmE_index=1;
List<Lebi_FriendLink> gTmEs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink gTmE in gTmEs){%>
            
                <% if (gTmE.Logo != "" && gTmE.IsPic == 1){ %><a href="<%=gTmE.Url%>" target="_blank"><img src="<%=Image(gTmE.Logo) %>" alt="<%=gTmE.Name%>" /></a><%}else{%><a href="<%=gTmE.Url%>" target="_blank"><%=gTmE.Name%></a><%} %>     
