...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_ArticleDetails"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> sytss=Languages();RecordCount=sytss.Count;int syts_index=1;
foreach (Shop.Model.Lebi_Language syts in sytss){%>
            <dd <%if (syts_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=syts.id%>,'<%=syts.Code%>','<%=syts.Path%>');"><%if (syts.ImageUrl!=""){%><img src="<%=Image(syts.ImageUrl) %>" /><%}%><%=syts.Name%></a></dd>
            <%syts_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int kbSQ_index=1;
List<Lebi_Currency> kbSQs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency kbSQ in kbSQs){%>
            <dd <%if (kbSQ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=kbSQ.id%>,'<%=kbSQ.Code%>',<%=kbSQ.ExchangeRate%>,'<%=kbSQ.Msige%>','<%=kbSQ.DecimalLength%>');"><%=kbSQ.Code%></a></dd>
            <%kbSQ_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int kxnX_index=1;
List<Lebi_Searchkey> kxnXs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey kxnX in kxnXs){%><%=Lang(kxnX.Name)%><%kxnX_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int yGVa_index=1;
List<Lebi_Searchkey> yGVas = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey yGVa in yGVas){%>
<%if (yGVa.Type==1){ %><a href="<%=URL("P_Search",""+Lang(yGVa.Name)+"") %>"><%}else{ %><a href="<%=yGVa.URL%>" target="_blank"><%} %><span><%=Lang(yGVa.Name)%></span></a>
<%yGVa_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int gbEB_index=1;
List<Lebi_Page> gbEBs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page gbEB in gbEBs){%>
<a class="menu" href="<%=URL("","",gbEB.url)%>"><span><%=gbEB.Name%></span></a>
<%gbEB_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id = "+ node.id +" and Language like '%" + CurrentLanguage.Code +"%' and id <> "+ id +"";Order="id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int JJVq_index=1;
List<Lebi_Page> JJVqs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page JJVq in JJVqs){%>
        <%if (JJVq.ImageSmall !=""){ %>
        <li class="image"><div class="image"><p><a href="<%=URL("P_ArticleDetails",JJVq.id,JJVq.url)%>" target="_blank"><img <%if (CurrentLanguage.IsLazyLoad==1){ %>class="lazy" src="/Theme/system/images/lazy.gif" data-original<%}else{ %>src<%} %>="<%=Image(JJVq.ImageSmall) %>" title="<%=Lang(JJVq.Name) %>" /></a></p></div><div class="title"><a href="<%=URL("P_ArticleDetails",JJVq.id,JJVq.url)%>" title="<%=JJVq.Name%>" target="_blank"><span style="color:<%=JJVq.NameColor%>"><%=JJVq.Name%></span></a></div></li>
        <%} else{%>
        <li class="text"><div class="title"><a href="<%=URL("P_ArticleDetails",JJVq.id,JJVq.url)%>" title="<%=JJVq.Name%>" target="_blank"><span style="color:<%=JJVq.NameColor%>"><%=JJVq.Name%></span></a></div></li>
        <%} %>
        <%JJVq_index++;}%>
    </ul>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int xGze_index=1;
List<Lebi_FriendLink> xGzes = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink xGze in xGzes){%>
            
                <% if (xGze.Logo != "" && xGze.IsPic == 1){ %><a href="<%=xGze.Url%>" target="_blank"><img src="<%=Image(xGze.Logo) %>" alt="<%=xGze.Name%>" /></a><%}else{%><a href="<%=xGze.Url%>" target="_blank"><%=xGze.Name%></a><%} %>     
