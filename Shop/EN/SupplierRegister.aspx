...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_SupplierRegister"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> wptms=Languages();RecordCount=wptms.Count;int wptm_index=1;
foreach (Shop.Model.Lebi_Language wptm in wptms){%>
            <dd <%if (wptm_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=wptm.id%>,'<%=wptm.Code%>','<%=wptm.Path%>');"><%if (wptm.ImageUrl!=""){%><img src="<%=Image(wptm.ImageUrl) %>" /><%}%><%=wptm.Name%></a></dd>
            <%wptm_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int KMSz_index=1;
List<Lebi_Currency> KMSzs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency KMSz in KMSzs){%>
            <dd <%if (KMSz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=KMSz.id%>,'<%=KMSz.Code%>',<%=KMSz.ExchangeRate%>,'<%=KMSz.Msige%>','<%=KMSz.DecimalLength%>');"><%=KMSz.Code%></a></dd>
            <%KMSz_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int yQJB_index=1;
List<Lebi_Searchkey> yQJBs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey yQJB in yQJBs){%><%=Lang(yQJB.Name)%><%yQJB_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int WWWK_index=1;
List<Lebi_Searchkey> WWWKs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey WWWK in WWWKs){%>
<%if (WWWK.Type==1){ %><a href="<%=URL("P_Search",""+Lang(WWWK.Name)+"") %>"><%}else{ %><a href="<%=WWWK.URL%>" target="_blank"><%} %><span><%=Lang(WWWK.Name)%></span></a>
<%WWWK_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int pCcd_index=1;
List<Lebi_Page> pCcds = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page pCcd in pCcds){%>
<a class="menu" href="<%=URL("","",pCcd.url)%>"><span><%=pCcd.Name%></span></a>
<%pCcd_index++;}%>
    ...
    <div class="user">
        <div class="reg clearfix">
            ...
            <%if(supplier.Type_id_SupplierStatus==443){ %>
             <div class="mc">
                Your business account has been disabled
             </div>
            <%}else{ %>
            <div class="mc">
                <table cellpadding="0" cellspacing="0">
                    <tr>
                        <th>
                            <font color="red">*</font>Application Type：
                        </th>
                        <td>
                    ...
                    <tr>
                        <th>
                            <font color="red">*</font>Shop Acronym：
                        </th>
                        <td>
                            <input type="text" name="SubName" id="SubName" style="width: 200px;" value="<%=supplier.SubName %>" min="notnull" maxlength="6"
                                class="input" shop="true" /><span id="FormSubName" class="FormALT">Approval will not be modified</span>
                        </td>
                    </tr>
                     <tr>
                        <th>
                    ...
                    <tr>
                        <th>
                            <font color="red">*</font>Security Code：
                        </th>
                        <td>
                            <input name="verifycode" type="text" shop="true" id="verifycode" min="notnull" shop="true" size="6"
                                class="input" />
                            <img class="img_c" align="absmiddle" style="width: 80px; height: 25px;" title="Click to refresh Security Code"
            ...
            <table cellpadding="0" cellspacing="0" align="center">
                <tbody>
                    <tr>
                        ...
                        <td height="50">
                            <a href="javascript:void(0)" onclick="submit();" class="btn btn-6"><s></s>Submit</a>
                        </td>
                    </tr>
                </tbody>
            </table>
...
<script type="text/javascript">
    ...
    function submit() {
        ...
        RequestAjax(url, postData, function (res) { MsgBox(1, "Registration Successful!", '<%=backurl %>') });
    }
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int GtsM_index=1;
List<Lebi_FriendLink> GtsMs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink GtsM in GtsMs){%>
            
                <% if (GtsM.Logo != "" && GtsM.IsPic == 1){ %><a href="<%=GtsM.Url%>" target="_blank"><img src="<%=Image(GtsM.Logo) %>" alt="<%=GtsM.Name%>" /></a><%}else{%><a href="<%=GtsM.Url%>" target="_blank"><%=GtsM.Name%></a><%} %>     
