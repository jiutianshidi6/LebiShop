...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_Article"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> uvtgs=Languages();RecordCount=uvtgs.Count;int uvtg_index=1;
foreach (Shop.Model.Lebi_Language uvtg in uvtgs){%>
            <dd <%if (uvtg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=uvtg.id%>,'<%=uvtg.Code%>','<%=uvtg.Path%>');"><%if (uvtg.ImageUrl!=""){%><img src="<%=Image(uvtg.ImageUrl) %>" /><%}%><%=uvtg.Name%></a></dd>
            <%uvtg_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int OqPY_index=1;
List<Lebi_Currency> OqPYs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency OqPY in OqPYs){%>
            <dd <%if (OqPY_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=OqPY.id%>,'<%=OqPY.Code%>',<%=OqPY.ExchangeRate%>,'<%=OqPY.Msige%>','<%=OqPY.DecimalLength%>');"><%=OqPY.Code%></a></dd>
            <%OqPY_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int IPTW_index=1;
List<Lebi_Searchkey> IPTWs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey IPTW in IPTWs){%><%=Lang(IPTW.Name)%><%IPTW_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int TIQC_index=1;
List<Lebi_Searchkey> TIQCs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey TIQC in TIQCs){%>
<%if (TIQC.Type==1){ %><a href="<%=URL("P_Search",""+Lang(TIQC.Name)+"") %>"><%}else{ %><a href="<%=TIQC.URL%>" target="_blank"><%} %><span><%=Lang(TIQC.Name)%></span></a>
<%TIQC_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int vmfY_index=1;
List<Lebi_Page> vmfYs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page vmfY in vmfYs){%>
<a class="menu" href="<%=URL("","",vmfY.url)%>"><span><%=vmfY.Name%></span></a>
<%vmfY_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int nVKU_index=1;
List<Lebi_FriendLink> nVKUs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink nVKU in nVKUs){%>
            
                <% if (nVKU.Logo != "" && nVKU.IsPic == 1){ %><a href="<%=nVKU.Url%>" target="_blank"><img src="<%=Image(nVKU.Logo) %>" alt="<%=nVKU.Name%>" /></a><%}else{%><a href="<%=nVKU.Url%>" target="_blank"><%=nVKU.Name%></a><%} %>     
