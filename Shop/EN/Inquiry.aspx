...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_Inquiry"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> EMJXs=Languages();RecordCount=EMJXs.Count;int EMJX_index=1;
foreach (Shop.Model.Lebi_Language EMJX in EMJXs){%>
            <dd <%if (EMJX_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=EMJX.id%>,'<%=EMJX.Code%>','<%=EMJX.Path%>');"><%if (EMJX.ImageUrl!=""){%><img src="<%=Image(EMJX.ImageUrl) %>" /><%}%><%=EMJX.Name%></a></dd>
            <%EMJX_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int YWra_index=1;
List<Lebi_Currency> YWras = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency YWra in YWras){%>
            <dd <%if (YWra_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=YWra.id%>,'<%=YWra.Code%>',<%=YWra.ExchangeRate%>,'<%=YWra.Msige%>','<%=YWra.DecimalLength%>');"><%=YWra.Code%></a></dd>
            <%YWra_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Abnx_index=1;
List<Lebi_Searchkey> Abnxs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Abnx in Abnxs){%><%=Lang(Abnx.Name)%><%Abnx_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Dvld_index=1;
List<Lebi_Searchkey> Dvlds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Dvld in Dvlds){%>
<%if (Dvld.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Dvld.Name)+"") %>"><%}else{ %><a href="<%=Dvld.URL%>" target="_blank"><%} %><span><%=Lang(Dvld.Name)%></span></a>
<%Dvld_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int ifJa_index=1;
List<Lebi_Page> ifJas = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page ifJa in ifJas){%>
<a class="menu" href="<%=URL("","",ifJa.url)%>"><span><%=ifJa.Name%></span></a>
<%ifJa_index++;}%>
...
<script type="text/javascript">
    function submit() {
        ...
        RequestAjax(url, postData, function () { MsgBox(1, "Success", "?");$("#UserName").val("");$("#Email").val("");$("#Subject").val("");$("#Content").val(""); });
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int KSuc_index=1;
List<Lebi_FriendLink> KSucs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink KSuc in KSucs){%>
            
                <% if (KSuc.Logo != "" && KSuc.IsPic == 1){ %><a href="<%=KSuc.Url%>" target="_blank"><img src="<%=Image(KSuc.Logo) %>" alt="<%=KSuc.Name%>" /></a><%}else{%><a href="<%=KSuc.Url%>" target="_blank"><%=KSuc.Name%></a><%} %>     
