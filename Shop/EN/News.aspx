...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_News"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> cITFs=Languages();RecordCount=cITFs.Count;int cITF_index=1;
foreach (Shop.Model.Lebi_Language cITF in cITFs){%>
            <dd <%if (cITF_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=cITF.id%>,'<%=cITF.Code%>','<%=cITF.Path%>');"><%if (cITF.ImageUrl!=""){%><img src="<%=Image(cITF.ImageUrl) %>" /><%}%><%=cITF.Name%></a></dd>
            <%cITF_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int yyfm_index=1;
List<Lebi_Currency> yyfms = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency yyfm in yyfms){%>
            <dd <%if (yyfm_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=yyfm.id%>,'<%=yyfm.Code%>',<%=yyfm.ExchangeRate%>,'<%=yyfm.Msige%>','<%=yyfm.DecimalLength%>');"><%=yyfm.Code%></a></dd>
            <%yyfm_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int aLnD_index=1;
List<Lebi_Searchkey> aLnDs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey aLnD in aLnDs){%><%=Lang(aLnD.Name)%><%aLnD_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int WUYy_index=1;
List<Lebi_Searchkey> WUYys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey WUYy in WUYys){%>
<%if (WUYy.Type==1){ %><a href="<%=URL("P_Search",""+Lang(WUYy.Name)+"") %>"><%}else{ %><a href="<%=WUYy.URL%>" target="_blank"><%} %><span><%=Lang(WUYy.Name)%></span></a>
<%WUYy_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int uAzE_index=1;
List<Lebi_Page> uAzEs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page uAzE in uAzEs){%>
<a class="menu" href="<%=URL("","",uAzE.url)%>"><span><%=uAzE.Name%></span></a>
<%uAzE_index++;}%>
...
<ul class="text">
    <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int msgo_index=1;
List<Lebi_Brand> msgos = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand msgo in msgos){%>
    <li><a href="<%=URL("P_Brand",msgo.id)%>" title="<%=Lang(msgo.Name) %>"><%=Lang(msgo.Name) %></a></li>
    <%msgo_index++;}%>
</ul>
</div>
</div>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Qxhj_index=1;
List<Lebi_Page> Qxhjs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Qxhj in Qxhjs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",Qxhj.id,Qxhj.url)%>" target="<%=Qxhj.target%>" title="<%=Qxhj.Name%>"><span style="color:<%=Qxhj.NameColor%>"><%=Qxhj.Name%></span></a></li>
        <%Qxhj_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int SjAL_index=1;
List<Lebi_FriendLink> SjALs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink SjAL in SjALs){%>
            
                <% if (SjAL.Logo != "" && SjAL.IsPic == 1){ %><a href="<%=SjAL.Url%>" target="_blank"><img src="<%=Image(SjAL.Logo) %>" alt="<%=SjAL.Name%>" /></a><%}else{%><a href="<%=SjAL.Url%>" target="_blank"><%=SjAL.Name%></a><%} %>     
