<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.P_SendFriend" validateRequest="false"%>


<%@ Import Namespace="Shop.Bussiness" %>
<%@ Import Namespace="Shop.Model" %>
<%@ Import Namespace="System.Collections.Generic" %>
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_SendFriend"); %>
...
<title><%=ThemePageMeta("P_SendFriend","title")%></title>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Language" content="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0;" />
<meta name="keywords" content="<%=ThemePageMeta("P_SendFriend","keywords")%>" />
<meta name="description" content="<%=ThemePageMeta("P_SendFriend","description")%>" />
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> xNiNs=Languages();RecordCount=xNiNs.Count;int xNiN_index=1;
foreach (Shop.Model.Lebi_Language xNiN in xNiNs){%>
            <dd <%if (xNiN_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=xNiN.id%>,'<%=xNiN.Code%>','<%=xNiN.Path%>');"><%if (xNiN.ImageUrl!=""){%><img src="<%=Image(xNiN.ImageUrl) %>" /><%}%><%=xNiN.Name%></a></dd>
            <%xNiN_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int JJOD_index=1;
List<Lebi_Currency> JJODs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency JJOD in JJODs){%>
            <dd <%if (JJOD_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=JJOD.id%>,'<%=JJOD.Code%>',<%=JJOD.ExchangeRate%>,'<%=JJOD.Msige%>','<%=JJOD.DecimalLength%>');"><%=JJOD.Code%></a></dd>
            <%JJOD_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int VNTK_index=1;
List<Lebi_Searchkey> VNTKs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey VNTK in VNTKs){%><%=Lang(VNTK.Name)%><%VNTK_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vrdY_index=1;
List<Lebi_Searchkey> vrdYs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vrdY in vrdYs){%>
<%if (vrdY.Type==1){ %><a href="<%=URL("P_Search",""+Lang(vrdY.Name)+"") %>"><%}else{ %><a href="<%=vrdY.URL%>" target="_blank"><%} %><span><%=Lang(vrdY.Name)%></span></a>
<%vrdY_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int uJPC_index=1;
List<Lebi_Page> uJPCs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page uJPC in uJPCs){%>
<a class="menu" href="<%=URL("","",uJPC.url)%>"><span><%=uJPC.Name%></span></a>
<%uJPC_index++;}%>
...
<div class="userbox">
    ...
    <dl>
        <dt>Name：</dt>
        <dd><input type="text" name="UserName" id="UserName" value="<%=CurrentUser.UserName %>" size="30" class="input" shop="true" min="notnull"/></dd>
    </dl>
    <dl>
        <dt>Email：</dt>
        <dd><input type="text" name="Email" id="Email" value="<%=CurrentUser.Email %>" size="30" class="input" shop="true" min="email"/></dd>
    </dl>
    <dl>
        <dt>Content：</dt>
    ...
    <dl>
        <dt>Name：</dt>
        <dd><input type="text" name="ToUserName" id="ToUserName" value="" size="30" class="input" shop="true" min="notnull"/></dd>
    </dl>
    <dl>
        <dt>Email：</dt>
        <dd><input type="text" name="ToEmail" id="ToEmail" value="" size="30" class="input" shop="true" min="email"/></dd>
    </dl>
    <dl class="dl-btn">
        <dt></dt>
...
<script type="text/javascript">
    function submit() {
        ...
        if (!CheckForm("shop", "span"))
            return false;
        var url = path+"/Ajax/Ajax_user.aspx?__Action=SendFriend&id=<%=id%>";
        RequestAjax(url, postData, function () { MsgBox(1, "Send successful", "<%=URL("P_Product",id) %>");});
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int lCde_index=1;
List<Lebi_FriendLink> lCdes = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink lCde in lCdes){%>
            
                <% if (lCde.Logo != "" && lCde.IsPic == 1){ %><a href="<%=lCde.Url%>" target="_blank"><img src="<%=Image(lCde.Logo) %>" alt="<%=lCde.Name%>" /></a><%}else{%><a href="<%=lCde.Url%>" target="_blank"><%=lCde.Name%></a><%} %>     
