...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_Help"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> lbqws=Languages();RecordCount=lbqws.Count;int lbqw_index=1;
foreach (Shop.Model.Lebi_Language lbqw in lbqws){%>
            <dd <%if (lbqw_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=lbqw.id%>,'<%=lbqw.Code%>','<%=lbqw.Path%>');"><%if (lbqw.ImageUrl!=""){%><img src="<%=Image(lbqw.ImageUrl) %>" /><%}%><%=lbqw.Name%></a></dd>
            <%lbqw_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int HeOm_index=1;
List<Lebi_Currency> HeOms = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency HeOm in HeOms){%>
            <dd <%if (HeOm_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=HeOm.id%>,'<%=HeOm.Code%>',<%=HeOm.ExchangeRate%>,'<%=HeOm.Msige%>','<%=HeOm.DecimalLength%>');"><%=HeOm.Code%></a></dd>
            <%HeOm_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hxfR_index=1;
List<Lebi_Searchkey> hxfRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hxfR in hxfRs){%><%=Lang(hxfR.Name)%><%hxfR_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int QYOW_index=1;
List<Lebi_Searchkey> QYOWs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey QYOW in QYOWs){%>
<%if (QYOW.Type==1){ %><a href="<%=URL("P_Search",""+Lang(QYOW.Name)+"") %>"><%}else{ %><a href="<%=QYOW.URL%>" target="_blank"><%} %><span><%=Lang(QYOW.Name)%></span></a>
<%QYOW_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int xjnj_index=1;
List<Lebi_Page> xjnjs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page xjnj in xjnjs){%>
<a class="menu" href="<%=URL("","",xjnj.url)%>"><span><%=xjnj.Name%></span></a>
<%xjnj_index++;}%>
...
<div class="mc">
    ...
    if (type !=""){
        ...
        %>
    <%Table="Lebi_Page";Where=""+strwhere+"";PageSize=999;pageindex=Rint("page");RecordCount=B_Lebi_Page.Counts(Where);int frQz_index=1;
List<Lebi_Page> frQzs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page frQz in frQzs){%>
    <div class="nodeitem clearfix" id="help<%=frQz.id%>">
    <div class="mt">
        <h2><font style="color:<%=frQz.NameColor%>"><%=frQz.Name%></font></h2>
    </div>
    <%if (frQz.Description != ""){ %><div class="tips"><%=frQz.Description%></div><%} %>
    <div class="content">
    <%=ImgSrc(frQz.Content,CurrentLanguage.IsLazyLoad) %>
    </div>
    </div>
    <%frQz_index++;}%>
    <%} %>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int eDTe_index=1;
List<Lebi_FriendLink> eDTes = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink eDTe in eDTes){%>
            
                <% if (eDTe.Logo != "" && eDTe.IsPic == 1){ %><a href="<%=eDTe.Url%>" target="_blank"><img src="<%=Image(eDTe.Logo) %>" alt="<%=eDTe.Name%>" /></a><%}else{%><a href="<%=eDTe.Url%>" target="_blank"><%=eDTe.Name%></a><%} %>     
