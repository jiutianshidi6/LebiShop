...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_AllProductCategories"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> jMLvs=Languages();RecordCount=jMLvs.Count;int jMLv_index=1;
foreach (Shop.Model.Lebi_Language jMLv in jMLvs){%>
            <dd <%if (jMLv_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=jMLv.id%>,'<%=jMLv.Code%>','<%=jMLv.Path%>');"><%if (jMLv.ImageUrl!=""){%><img src="<%=Image(jMLv.ImageUrl) %>" /><%}%><%=jMLv.Name%></a></dd>
            <%jMLv_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int LjSV_index=1;
List<Lebi_Currency> LjSVs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency LjSV in LjSVs){%>
            <dd <%if (LjSV_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=LjSV.id%>,'<%=LjSV.Code%>',<%=LjSV.ExchangeRate%>,'<%=LjSV.Msige%>','<%=LjSV.DecimalLength%>');"><%=LjSV.Code%></a></dd>
            <%LjSV_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Uykr_index=1;
List<Lebi_Searchkey> Uykrs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Uykr in Uykrs){%><%=Lang(Uykr.Name)%><%Uykr_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int mdMk_index=1;
List<Lebi_Searchkey> mdMks = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey mdMk in mdMks){%>
<%if (mdMk.Type==1){ %><a href="<%=URL("P_Search",""+Lang(mdMk.Name)+"") %>"><%}else{ %><a href="<%=mdMk.URL%>" target="_blank"><%} %><span><%=Lang(mdMk.Name)%></span></a>
<%mdMk_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int krNP_index=1;
List<Lebi_Page> krNPs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page krNP in krNPs){%>
<a class="menu" href="<%=URL("","",krNP.url)%>"><span><%=krNP.Name%></span></a>
<%krNP_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int JaHK_index=1;
List<Lebi_FriendLink> JaHKs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink JaHK in JaHKs){%>
            
                <% if (JaHK.Logo != "" && JaHK.IsPic == 1){ %><a href="<%=JaHK.Url%>" target="_blank"><img src="<%=Image(JaHK.Logo) %>" alt="<%=JaHK.Name%>" /></a><%}else{%><a href="<%=JaHK.Url%>" target="_blank"><%=JaHK.Name%></a><%} %>     
