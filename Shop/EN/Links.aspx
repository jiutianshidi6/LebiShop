...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_FriendLink"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> dFUWs=Languages();RecordCount=dFUWs.Count;int dFUW_index=1;
foreach (Shop.Model.Lebi_Language dFUW in dFUWs){%>
            <dd <%if (dFUW_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=dFUW.id%>,'<%=dFUW.Code%>','<%=dFUW.Path%>');"><%if (dFUW.ImageUrl!=""){%><img src="<%=Image(dFUW.ImageUrl) %>" /><%}%><%=dFUW.Name%></a></dd>
            <%dFUW_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int pppP_index=1;
List<Lebi_Currency> pppPs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency pppP in pppPs){%>
            <dd <%if (pppP_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=pppP.id%>,'<%=pppP.Code%>',<%=pppP.ExchangeRate%>,'<%=pppP.Msige%>','<%=pppP.DecimalLength%>');"><%=pppP.Code%></a></dd>
            <%pppP_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Yfkl_index=1;
List<Lebi_Searchkey> Yfkls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Yfkl in Yfkls){%><%=Lang(Yfkl.Name)%><%Yfkl_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int fySD_index=1;
List<Lebi_Searchkey> fySDs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey fySD in fySDs){%>
<%if (fySD.Type==1){ %><a href="<%=URL("P_Search",""+Lang(fySD.Name)+"") %>"><%}else{ %><a href="<%=fySD.URL%>" target="_blank"><%} %><span><%=Lang(fySD.Name)%></span></a>
<%fySD_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int rwiO_index=1;
List<Lebi_Page> rwiOs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page rwiO in rwiOs){%>
<a class="menu" href="<%=URL("","",rwiO.url)%>"><span><%=rwiO.Name%></span></a>
<%rwiO_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int rSYn_index=1;
List<Lebi_Page> rSYns = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page rSYn in rSYns){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",rSYn.id,rSYn.url)%>" target="<%=rSYn.target%>" title="<%=rSYn.Name%>"><span style="color:<%=rSYn.NameColor%>"><%=rSYn.Name%></span></a></li>
        <%rSYn_index++;}%>
    </ul>
</div>
</div>
...
<div class="friendlinklist">
    ...
    <div class="mc clearfix">
        <ul class="image">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and IsPic=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_FriendLink.Counts(Where);int Qdio_index=1;
List<Lebi_FriendLink> Qdios = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Qdio in Qdios){%>
            <li>
                <p><a href="<%=Qdio.Url%>" target="_blank"><img src="<%=Qdio.Logo%>" alt="<%=Qdio.Name%>" /></a></p>     
            </li>
            <%Qdio_index++;}%>
        </ul>
        <div class="clear"></div>
    ...
    <div class="mc clearfix">
        <ul class="text">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and IsPic=0 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_FriendLink.Counts(Where);int RRwm_index=1;
List<Lebi_FriendLink> RRwms = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink RRwm in RRwms){%>
            <li>
                <a href="<%=RRwm.Url%>" target="_blank"><%=RRwm.Name%></a>     
            </li>
            <%RRwm_index++;}%>
        </ul>
        <div class="clear"></div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int PkUm_index=1;
List<Lebi_FriendLink> PkUms = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink PkUm in PkUms){%>
            
                <% if (PkUm.Logo != "" && PkUm.IsPic == 1){ %><a href="<%=PkUm.Url%>" target="_blank"><img src="<%=Image(PkUm.Logo) %>" alt="<%=PkUm.Name%>" /></a><%}else{%><a href="<%=PkUm.Url%>" target="_blank"><%=PkUm.Name%></a><%} %>     
