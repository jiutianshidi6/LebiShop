...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_AddToBasket"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> WuBqs=Languages();RecordCount=WuBqs.Count;int WuBq_index=1;
foreach (Shop.Model.Lebi_Language WuBq in WuBqs){%>
            <dd <%if (WuBq_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=WuBq.id%>,'<%=WuBq.Code%>','<%=WuBq.Path%>');"><%if (WuBq.ImageUrl!=""){%><img src="<%=Image(WuBq.ImageUrl) %>" /><%}%><%=WuBq.Name%></a></dd>
            <%WuBq_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int WuAW_index=1;
List<Lebi_Currency> WuAWs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency WuAW in WuAWs){%>
            <dd <%if (WuAW_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=WuAW.id%>,'<%=WuAW.Code%>',<%=WuAW.ExchangeRate%>,'<%=WuAW.Msige%>','<%=WuAW.DecimalLength%>');"><%=WuAW.Code%></a></dd>
            <%WuAW_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int QWhx_index=1;
List<Lebi_Searchkey> QWhxs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey QWhx in QWhxs){%><%=Lang(QWhx.Name)%><%QWhx_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int jhtp_index=1;
List<Lebi_Searchkey> jhtps = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey jhtp in jhtps){%>
<%if (jhtp.Type==1){ %><a href="<%=URL("P_Search",""+Lang(jhtp.Name)+"") %>"><%}else{ %><a href="<%=jhtp.URL%>" target="_blank"><%} %><span><%=Lang(jhtp.Name)%></span></a>
<%jhtp_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int HuyV_index=1;
List<Lebi_Page> HuyVs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page HuyV in HuyVs){%>
<a class="menu" href="<%=URL("","",HuyV.url)%>"><span><%=HuyV.Name%></span></a>
<%HuyV_index++;}%>
...
<div class="nbbox clearfix">
    <div class="success">
        <div class="mt">
            <h2>
                Product has been added to Cart</h2>
        </div>
        <div class="mc clearfix">
            <a href="javascript:history.go(-1);" class="btn btn-11"><s></s>
                Continue Shopping</a><a href="<%=URL("P_Basket","") %>" class="btn btn-7"><s></s>View Cart And Checkout</a>
        </div>
    </div>
</div>
...
<div class="tag-tab">
    <ul class="tablist">
    <%Table="Lebi_Pro_Tag";Where="1=1";Order="Sort desc";PageSize=4;pageindex=1;RecordCount=B_Lebi_Pro_Tag.Counts(Where);int CuhS_index=1;
List<Lebi_Pro_Tag> CuhSs = B_Lebi_Pro_Tag.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Pro_Tag CuhS in CuhSs){%>
        <li <%=CuhS_index==1?" class=\"selected\"":"" %>><a href="<%=URL("P_ProductCategory","$,$,$,$,$,"+CuhS.id+"")%>" target="_blank"><span><%=Lang(CuhS.Name) %></span></a></li>
    <%CuhS_index++;}%>
    </ul>
</div>
<div class="tag-cont clearfix">
    <%Table="Lebi_Pro_Tag";Where="1=1";Order="Sort desc";PageSize=4;pageindex=1;RecordCount=B_Lebi_Pro_Tag.Counts(Where);int tab_tag_index=1;
List<Lebi_Pro_Tag> tab_tags = B_Lebi_Pro_Tag.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Pro_Tag tab_tag in tab_tags){%>
    <div style="display: <%=tab_tag_index==1?"block":"none" %>" class="cont">
        <div class="productshow"><ul class="image">
        <%Table="Lebi_Product";Where=""+ProductWhere+" and Type_id_ProductType = 320 and "+ TagWhere(tab_tag.id) +"";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Product.Counts(Where);int tab_tag_pro_index=1;
List<Lebi_Product> tab_tag_pros = B_Lebi_Product.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Product tab_tag_pro in tab_tag_pros){%>
        <li>
            <div class="proimg"><p><a href="<%=URL("P_Product",tab_tag_pro.id) %>"><img <%if (CurrentLanguage.IsLazyLoad==1){ %>class="lazy<%=tab_tag_index%>" src="/Theme/system/images/lazy.gif" data-original<%}else{ %>src<%} %>="<%=Image(tab_tag_pro.ImageOriginal,"medium") %>" alt="<%=Lang(tab_tag_pro.Name) %>" title="<%=Lang(tab_tag_pro.Name) %>" /></a></p></div>
            <div class="proname"><a href="<%=URL("P_Product",tab_tag_pro.id) %>" title="<%=Lang(tab_tag_pro.Name) %>"><%=Lang(tab_tag_pro.Name) %></a></div>
            <div class="proprice"><%if (tab_tag_pro.Price_Market>0){ %><div class="marketprice"><i>Market Price：</i><strong><%=FormatMoney(tab_tag_pro.Price_Market) %></strong></div><%} %><div class="buyprice"><i><%=Lang(CurrentUserLevel.PriceName) %>：</i><strong><%=FormatMoney(ProductPrice(tab_tag_pro)) %></strong></div></div>                    
        </li>
        <%tab_tag_pro_index++;}%>
        </ul></div>
    </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Niht_index=1;
List<Lebi_FriendLink> Nihts = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Niht in Nihts){%>
            
                <% if (Niht.Logo != "" && Niht.IsPic == 1){ %><a href="<%=Niht.Url%>" target="_blank"><img src="<%=Image(Niht.Logo) %>" alt="<%=Niht.Name%>" /></a><%}else{%><a href="<%=Niht.Url%>" target="_blank"><%=Niht.Name%></a><%} %>     
