...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_Exchange"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> LhFks=Languages();RecordCount=LhFks.Count;int LhFk_index=1;
foreach (Shop.Model.Lebi_Language LhFk in LhFks){%>
            <dd <%if (LhFk_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=LhFk.id%>,'<%=LhFk.Code%>','<%=LhFk.Path%>');"><%if (LhFk.ImageUrl!=""){%><img src="<%=Image(LhFk.ImageUrl) %>" /><%}%><%=LhFk.Name%></a></dd>
            <%LhFk_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int OdXf_index=1;
List<Lebi_Currency> OdXfs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency OdXf in OdXfs){%>
            <dd <%if (OdXf_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=OdXf.id%>,'<%=OdXf.Code%>',<%=OdXf.ExchangeRate%>,'<%=OdXf.Msige%>','<%=OdXf.DecimalLength%>');"><%=OdXf.Code%></a></dd>
            <%OdXf_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int pNsJ_index=1;
List<Lebi_Searchkey> pNsJs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey pNsJ in pNsJs){%><%=Lang(pNsJ.Name)%><%pNsJ_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int TINF_index=1;
List<Lebi_Searchkey> TINFs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey TINF in TINFs){%>
<%if (TINF.Type==1){ %><a href="<%=URL("P_Search",""+Lang(TINF.Name)+"") %>"><%}else{ %><a href="<%=TINF.URL%>" target="_blank"><%} %><span><%=Lang(TINF.Name)%></span></a>
<%TINF_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int kPSk_index=1;
List<Lebi_Page> kPSks = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page kPSk in kPSks){%>
<a class="menu" href="<%=URL("","",kPSk.url)%>"><span><%=kPSk.Name%></span></a>
<%kPSk_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int UYKE_index=1;
List<Lebi_Page> UYKEs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page UYKE in UYKEs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",UYKE.id,UYKE.url)%>" target="<%=UYKE.target%>" title="<%=UYKE.Name%>"><span style="color:<%=UYKE.NameColor%>"><%=UYKE.Name%></span></a></li>
        <%UYKE_index++;}%>
    </ul>
</div>
</div>
...
<div id="filter">
	<div class="fore1">
		<dl class="order">
			<dt>Sort：</dt>
			<dd <%if(sort == "" || sort == "1" || sort == "1a"){ %>class="curr <%if(sort == "" || sort == "1"){ %>up<%}else if(sort == "1a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Exchange",""+list+",1"+ordertmp+"") %>">Sales</a><b></b></dd>
			<dd <%if(sort == "2" || sort == "2a"){ %>class="curr <%if(sort == "2"){ %>up<%}else if(sort == "2a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Exchange",""+list+",2"+ordertmp+"") %>">Point</a><b></b></dd>
			<dd <%if(sort == "3" || sort == "3a"){ %>class="curr <%if(sort == "3"){ %>up<%}else if(sort == "3a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Exchange",""+list+",3"+ordertmp+"") %>">Reviews</a><b></b></dd>
			<dd <%if(sort == "4" || sort == "4a"){ %>class="curr <%if(sort == "4"){ %>up<%}else if(sort == "4a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Exchange",""+list+",4"+ordertmp+"") %>">Shelf Time</a><b></b></dd>
		</dl>
        <div class="list-cutover">
			<a id="list-unselected"  href="<%=URL("P_Exchange","2,"+ sort) %>" class="modfiy-url list-unselected <%=list=="2"?"list-curr":"" %>"><b></b>List</a>
			<a id="grid-unselected"  href="<%=URL("P_Exchange","1,"+ sort) %>" class="modfiy-url grid-unselected <%=(list=="1" || list=="")?"grid-curr":"" %>"><b></b>Grid</a>
		</div>
...
<div class="mc clearfix">
    <div class="productlist limitbuylist loadlist"><ul>
        ...
        <li class="grid loadli">
            ...
            <div class="proinfo">
            <div class="buyprice"><i>Point：</i><strong><%=pro.Price_Sale %></strong></div>
            <div class="probtn">
            <%if(ProductStock(pro)<1){ %>
            <strong>Change the light</strong>
            <%}else{ %>
            <a href="javascript:void(0)" onclick="UserProduct_Edit(<%=pro.id%>,142,$('#pro_num<%=pro.id%>').val());" class="buy btn btn-12"><s></s>Exchange</a>
            <%} %>
            </div>
            </div>
        </li>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int DDRN_index=1;
List<Lebi_FriendLink> DDRNs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink DDRN in DDRNs){%>
            
                <% if (DDRN.Logo != "" && DDRN.IsPic == 1){ %><a href="<%=DDRN.Url%>" target="_blank"><img src="<%=Image(DDRN.Logo) %>" alt="<%=DDRN.Name%>" /></a><%}else{%><a href="<%=DDRN.Url%>" target="_blank"><%=DDRN.Name%></a><%} %>     
