...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> eKPps=Languages();RecordCount=eKPps.Count;int eKPp_index=1;
foreach (Shop.Model.Lebi_Language eKPp in eKPps){%>
            <dd <%if (eKPp_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=eKPp.id%>,'<%=eKPp.Code%>','<%=eKPp.Path%>');"><%if (eKPp.ImageUrl!=""){%><img src="<%=Image(eKPp.ImageUrl) %>" /><%}%><%=eKPp.Name%></a></dd>
            <%eKPp_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int lhPw_index=1;
List<Lebi_Currency> lhPws = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency lhPw in lhPws){%>
            <dd <%if (lhPw_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=lhPw.id%>,'<%=lhPw.Code%>',<%=lhPw.ExchangeRate%>,'<%=lhPw.Msige%>','<%=lhPw.DecimalLength%>');"><%=lhPw.Code%></a></dd>
            <%lhPw_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int gAzs_index=1;
List<Lebi_Searchkey> gAzss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey gAzs in gAzss){%><%=Lang(gAzs.Name)%><%gAzs_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int bckR_index=1;
List<Lebi_Searchkey> bckRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey bckR in bckRs){%>
<%if (bckR.Type==1){ %><a href="<%=URL("P_Search",""+Lang(bckR.Name)+"") %>"><%}else{ %><a href="<%=bckR.URL%>" target="_blank"><%} %><span><%=Lang(bckR.Name)%></span></a>
<%bckR_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int jRGS_index=1;
List<Lebi_Page> jRGSs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page jRGS in jRGSs){%>
<a class="menu" href="<%=URL("","",jRGS.url)%>"><span><%=jRGS.Name%></span></a>
<%jRGS_index++;}%>
...
<ul class="text">
    <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int XmGc_index=1;
List<Lebi_Brand> XmGcs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand XmGc in XmGcs){%>
    <li><a href="<%=URL("P_Brand",XmGc.id)%>" title="<%=Lang(XmGc.Name) %>"><%=Lang(XmGc.Name) %></a></li>
    <%XmGc_index++;}%>
</ul>
</div>
</div>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int zLhp_index=1;
List<Lebi_Page> zLhps = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page zLhp in zLhps){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",zLhp.id,zLhp.url)%>" target="<%=zLhp.target%>" title="<%=zLhp.Name%>"><span style="color:<%=zLhp.NameColor%>"><%=zLhp.Name%></span></a></li>
        <%zLhp_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int jPOa_index=1;
List<Lebi_FriendLink> jPOas = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink jPOa in jPOas){%>
            
                <% if (jPOa.Logo != "" && jPOa.IsPic == 1){ %><a href="<%=jPOa.Url%>" target="_blank"><img src="<%=Image(jPOa.Logo) %>" alt="<%=jPOa.Name%>" /></a><%}else{%><a href="<%=jPOa.Url%>" target="_blank"><%=jPOa.Name%></a><%} %>     
