...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_FindPassword"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> fBhgs=Languages();RecordCount=fBhgs.Count;int fBhg_index=1;
foreach (Shop.Model.Lebi_Language fBhg in fBhgs){%>
            <dd <%if (fBhg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=fBhg.id%>,'<%=fBhg.Code%>','<%=fBhg.Path%>');"><%if (fBhg.ImageUrl!=""){%><img src="<%=Image(fBhg.ImageUrl) %>" /><%}%><%=fBhg.Name%></a></dd>
            <%fBhg_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int XkRd_index=1;
List<Lebi_Currency> XkRds = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency XkRd in XkRds){%>
            <dd <%if (XkRd_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=XkRd.id%>,'<%=XkRd.Code%>',<%=XkRd.ExchangeRate%>,'<%=XkRd.Msige%>','<%=XkRd.DecimalLength%>');"><%=XkRd.Code%></a></dd>
            <%XkRd_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int okhk_index=1;
List<Lebi_Searchkey> okhks = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey okhk in okhks){%><%=Lang(okhk.Name)%><%okhk_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vqVM_index=1;
List<Lebi_Searchkey> vqVMs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vqVM in vqVMs){%>
<%if (vqVM.Type==1){ %><a href="<%=URL("P_Search",""+Lang(vqVM.Name)+"") %>"><%}else{ %><a href="<%=vqVM.URL%>" target="_blank"><%} %><span><%=Lang(vqVM.Name)%></span></a>
<%vqVM_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int tHUW_index=1;
List<Lebi_Page> tHUWs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page tHUW in tHUWs){%>
<a class="menu" href="<%=URL("","",tHUW.url)%>"><span><%=tHUW.Name%></span></a>
<%tHUW_index++;}%>
...
<div class="userbox">
    ...
    <dl class="dl-btn">
        <dt></dt>
        <dd><a href="javascript:void(0)" onclick="submit();" class="btn btn-11"><s></s>Submit</a>
...
<script type="text/javascript">
    function submit() {
        ...
        if (Answer1 == "") {
            CheckNO("Answer1", "Please answer answer");
            return false;
        }
        if (Answer2 == "") {
            CheckNO("Answer2", "Please answer answer");
            return false;
        }
        <%} %>
        if (PWD == "") {
            CheckNO("PWD", "password should be more than 6 characters.");
            return false;
        }
        if (PWD != RPWD) {
            CheckNO("RPWD", "Passwords do not match");
            return false;
        }
        var postData = GetFormJsonData("shop");
        var url = path+"/ajax/ajax_user.aspx?__Action=User_resetpwd&v=<%=v%>&type=<%=type %>";
        RequestAjax(url, postData, function () { MsgBox(1, "password changed", "<%=URL("P_Login","")%>") });
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int HrjW_index=1;
List<Lebi_FriendLink> HrjWs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink HrjW in HrjWs){%>
            
                <% if (HrjW.Logo != "" && HrjW.IsPic == 1){ %><a href="<%=HrjW.Url%>" target="_blank"><img src="<%=Image(HrjW.Logo) %>" alt="<%=HrjW.Name%>" /></a><%}else{%><a href="<%=HrjW.Url%>" target="_blank"><%=HrjW.Name%></a><%} %>     
