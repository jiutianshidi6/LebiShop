...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_Login"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> NoKNs=Languages();RecordCount=NoKNs.Count;int NoKN_index=1;
foreach (Shop.Model.Lebi_Language NoKN in NoKNs){%>
            <dd <%if (NoKN_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=NoKN.id%>,'<%=NoKN.Code%>','<%=NoKN.Path%>');"><%if (NoKN.ImageUrl!=""){%><img src="<%=Image(NoKN.ImageUrl) %>" /><%}%><%=NoKN.Name%></a></dd>
            <%NoKN_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int qCtF_index=1;
List<Lebi_Currency> qCtFs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency qCtF in qCtFs){%>
            <dd <%if (qCtF_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=qCtF.id%>,'<%=qCtF.Code%>',<%=qCtF.ExchangeRate%>,'<%=qCtF.Msige%>','<%=qCtF.DecimalLength%>');"><%=qCtF.Code%></a></dd>
            <%qCtF_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int WsqR_index=1;
List<Lebi_Searchkey> WsqRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey WsqR in WsqRs){%><%=Lang(WsqR.Name)%><%WsqR_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int GAMI_index=1;
List<Lebi_Searchkey> GAMIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey GAMI in GAMIs){%>
<%if (GAMI.Type==1){ %><a href="<%=URL("P_Search",""+Lang(GAMI.Name)+"") %>"><%}else{ %><a href="<%=GAMI.URL%>" target="_blank"><%} %><span><%=Lang(GAMI.Name)%></span></a>
<%GAMI_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Tfdz_index=1;
List<Lebi_Page> Tfdzs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Tfdz in Tfdzs){%>
<a class="menu" href="<%=URL("","",Tfdz.url)%>"><span><%=Tfdz.Name%></span></a>
<%Tfdz_index++;}%>
            ...
            <div class="right">
                <div class="userloginbox">
                    ...
                    <dl id="code" class="last" <%if (LoginError != true){ %>style="display:none"<%}else{ %>style="display:" <%} %>>
                        <dt>Security Code：</dt>
                        <dd><input name="verifycode" type="text" id="verifycode" shop="true" size="6" class="input" /><img class="verifycode" id="img1" src="<%=WebPath%>/code.aspx" /><img src="/Theme/system/images/reload.gif" class="refresh" title="Click to refresh Security Code" onclick="refresh('img1')" /></dd>
                    </dl>
                    <%} %>
                    <dl>
                        <dt></dt>
                        <dd><a href="javascript:void(0)" onclick="submitlogin();" class="btn btn-7"><s></s>Sign In</a>&nbsp;&nbsp;<a href="<%=URL("P_ForgetPassword","")%>">Forget Password</a></dd>
                    </dl>
                    </div>
                    ...
                    <div class="mes">
                        <h2><span>Not yet member？</span></h2>
                        <div class="cont">
                            Welcome to our member<a href="<%=URL("P_Register", ""+HttpUtility.UrlEncode(backurl)+"," + GetUrlToken(backurl)+ "") %>">Free Register</a></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    function submitlogin() {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.msg == "OK") {
                    MsgBox(1, "Login Successful", res.url);
                }
                else {
                    $("#code").show();
                    MsgError(res.msg);
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Xupi_index=1;
List<Lebi_FriendLink> Xupis = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Xupi in Xupis){%>
            
                <% if (Xupi.Logo != "" && Xupi.IsPic == 1){ %><a href="<%=Xupi.Url%>" target="_blank"><img src="<%=Image(Xupi.Logo) %>" alt="<%=Xupi.Name%>" /></a><%}else{%><a href="<%=Xupi.Url%>" target="_blank"><%=Xupi.Name%></a><%} %>     
