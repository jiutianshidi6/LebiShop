...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_CheckOut"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> AxYis=Languages();RecordCount=AxYis.Count;int AxYi_index=1;
foreach (Shop.Model.Lebi_Language AxYi in AxYis){%>
            <dd <%if (AxYi_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=AxYi.id%>,'<%=AxYi.Code%>','<%=AxYi.Path%>');"><%if (AxYi.ImageUrl!=""){%><img src="<%=Image(AxYi.ImageUrl) %>" /><%}%><%=AxYi.Name%></a></dd>
            <%AxYi_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int GeFD_index=1;
List<Lebi_Currency> GeFDs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency GeFD in GeFDs){%>
            <dd <%if (GeFD_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=GeFD.id%>,'<%=GeFD.Code%>',<%=GeFD.ExchangeRate%>,'<%=GeFD.Msige%>','<%=GeFD.DecimalLength%>');"><%=GeFD.Code%></a></dd>
            <%GeFD_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int gPWh_index=1;
List<Lebi_Searchkey> gPWhs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey gPWh in gPWhs){%><%=Lang(gPWh.Name)%><%gPWh_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MAHH_index=1;
List<Lebi_Searchkey> MAHHs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MAHH in MAHHs){%>
<%if (MAHH.Type==1){ %><a href="<%=URL("P_Search",""+Lang(MAHH.Name)+"") %>"><%}else{ %><a href="<%=MAHH.URL%>" target="_blank"><%} %><span><%=Lang(MAHH.Name)%></span></a>
<%MAHH_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int KCda_index=1;
List<Lebi_Page> KCdas = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page KCda in KCdas){%>
<a class="menu" href="<%=URL("","",KCda.url)%>"><span><%=KCda.Name%></span></a>
<%KCda_index++;}%>
...
<ul>
	<li class="over"><span>Add to Cart</span></li>
	<li class="current"><span class="con">Enter order information</span></li>
...
<script type="text/javascript">
    ...
    MsgBox(2, "Shopping cart is empty，please add to cart first", "<%=LanguagePath%>");
    <%
    } %>
</script>
    ...
    <div id="orderinfo">
        ...
        <div class="ordertips">
            <div class="ordertotal">
                <%if (basket.Money_Market >0){ %>Market Price：<font color="#ff3333"><%=FormatMoney(basket.Money_Market)%></font><%} %>
                <%if(basket.Point_Buy>0){ %>
                &nbsp;&nbsp;&nbsp;&nbsp;Points Required：<font color="#ff3333"><%=basket.Point_Buy%></font>
                <%} %>
                <%if(basket.Point>0){ %>
                &nbsp;&nbsp;&nbsp;&nbsp;Earn points：<font color="#ff3333"><%=basket.Point%></font>
                <%} %>
                <%if(basket.Weight>0){ %>
                &nbsp;&nbsp;&nbsp;&nbsp;Total Weight：<font color="#ff3333"><%=basket.Weight%></font> KG
                <%} %>
                <%if(basket.Volume>0){ %>
                &nbsp;&nbsp;&nbsp;&nbsp;Volume：<font color="#ff3333"><%=(basket.Volume/1000000).ToString("0.00")%></font> m&sup3;
                <%} %>
            </div>
...
<div class="otherpay">
    ...
    <div class="item">
        ...
        <div class="title"><input type="checkbox" name="usermoneytype" value="1" order="true"/>Use Vouchers</div>
        <div class="content">
            <div class="tablebox">
    ...
    <div class="item">
        ...
        <div class="title"><input type="checkbox" name="usermoneytype" value="2" order="true"/>Use gift cards</div>
        <div class="content">
            Number：<input type="text" id="moneycardcode" name="moneycardcode" order="true" value="" maxlength="20" size="15" class="input" /> 
            Password：<input type="text" id="moneycardpwd" name="moneycardpwd" order="true" value="" maxlength="20" size="10" class="input" />
            <a href="javascript:void(0)" onclick="checkmoneycard();" class="btn btn-11"><s></s>Search</a>
    ...
    <div class="item">
        <div class="title"><input type="checkbox" value="3" id="payusermoney" name="usermoneytype" onclick="selectusermongy();" order="true"/>Used Balance</div>
        <div class="content">
            <table class="cardlist" cellspacing="0" cellpadding="0" id="usermoneyitem">
                <tr>
                    <td class="usermoney">
                    Balance：<%=FormatMoney(CurrentUser.Money) %>&nbsp;&nbsp;Used：<input type="hidden" id="Money_canused" name="Money_canused" value="<%=CurrentUser.Money%>" /><input type="hidden" id="CurrentExchangeRate" name="CurrentExchangeRate" value="<%=CurrentCurrency.ExchangeRate %>" /><span class="show_moneyuse"><%=FormatMoney(0) %></span><input type="hidden" name="Money_UserCut" order="true" value="0" />&nbsp;&nbsp;&nbsp;&nbsp;Pay-Password：<%if (CurrentUser.Pay_Password!=""){ %><input type="password" id="Pay_Password" name="Pay_Password" size="20" value="" order="true" class="input" /> <a href="<%=URL("P_UserQuestion","1")%>" target="_blank">Forget Password</a><%}else{%><a href="<%=URL("P_UserChangePassword","")%>" target="_blank">set payment password</a><%}%>
                    </td>
                </tr>
            </table>
        </div>
    ...
    <div class="ordersubmit">
        <a href="<%=URL("P_Basket","") %>" class="btn btn-10"><s></s>
            Return to modify</a><a href="javascript:void(0);" onclick="OrderSubmit();" class="btn btn-6"><s></s>Submit Now</a>
    </div>
...
<script type="text/javascript">
    ...
    function OrderSubmit() {
        ...
        RequestAjax(url, postData, function (res) { MsgBox(1, "Submitting……", "<%=URL("P_Cashier","")%>?order_id=" + res.id +"") });
    }
    $(function () {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int qoNP_index=1;
List<Lebi_FriendLink> qoNPs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink qoNP in qoNPs){%>
            
                <% if (qoNP.Logo != "" && qoNP.IsPic == 1){ %><a href="<%=qoNP.Url%>" target="_blank"><img src="<%=Image(qoNP.Logo) %>" alt="<%=qoNP.Name%>" /></a><%}else{%><a href="<%=qoNP.Url%>" target="_blank"><%=qoNP.Name%></a><%} %>     
