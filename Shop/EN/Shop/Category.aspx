...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_ShopProductCategory"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> dPGzs=Languages();RecordCount=dPGzs.Count;int dPGz_index=1;
foreach (Shop.Model.Lebi_Language dPGz in dPGzs){%>
            <dd <%if (dPGz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=dPGz.id%>,'<%=dPGz.Code%>','<%=dPGz.Path%>');"><%if (dPGz.ImageUrl!=""){%><img src="<%=Image(dPGz.ImageUrl) %>" /><%}%><%=dPGz.Name%></a></dd>
            <%dPGz_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int BCEn_index=1;
List<Lebi_Currency> BCEns = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency BCEn in BCEns){%>
            <dd <%if (BCEn_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=BCEn.id%>,'<%=BCEn.Code%>',<%=BCEn.ExchangeRate%>,'<%=BCEn.Msige%>','<%=BCEn.DecimalLength%>');"><%=BCEn.Code%></a></dd>
            <%BCEn_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int bISk_index=1;
List<Lebi_Searchkey> bISks = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey bISk in bISks){%><%=Lang(bISk.Name)%><%bISk_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int fHWv_index=1;
List<Lebi_Searchkey> fHWvs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey fHWv in fHWvs){%>
<%if (fHWv.Type==1){ %><a href="<%=URL("P_Search",""+Lang(fHWv.Name)+"") %>"><%}else{ %><a href="<%=fHWv.URL%>" target="_blank"><%} %><span><%=Lang(fHWv.Name)%></span></a>
<%fHWv_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int REWa_index=1;
List<Lebi_Page> REWas = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page REWa in REWas){%>
<a class="menu" href="<%=URL("","",REWa.url)%>"><span><%=REWa.Name%></span></a>
<%REWa_index++;}%>
...
<div class="categories">
    <%Table="Lebi_Supplier_ProductType";Where="parentid = 0 and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int zBrf_index=1;
List<Lebi_Supplier_ProductType> zBrfs = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType zBrf in zBrfs){%>
    <dl <%if ((parentcid == 0 && zBrf_index == 1) || parentcid == zBrf.id){ %>class="current" <%} %>>
    <dt class="double">
        <label class="pic"></label>
        <a href="<%=URL("P_ShopProductCategory",""+id+","+zBrf.id+"") %>"><%=Lang(zBrf.Name) %></a>
    </dt>
    <dd>
        <ul>
            <%Table="Lebi_Supplier_ProductType";Where="parentid = "+zBrf.id+" and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int lfJE_index=1;
List<Lebi_Supplier_ProductType> lfJEs = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType lfJE in lfJEs){%>
            <li class="<%=cid==lfJE.id?"currentlink":"" %>"><a href="<%=URL("P_ShopProductCategory",""+id+","+lfJE.id+"") %>">
                <%=Lang(lfJE.Name) %></a></li>
            <%lfJE_index++;}%>
        </ul>
    </dd>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int xshz_index=1;
List<Lebi_FriendLink> xshzs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink xshz in xshzs){%>
            
                <% if (xshz.Logo != "" && xshz.IsPic == 1){ %><a href="<%=xshz.Url%>" target="_blank"><img src="<%=Image(xshz.Logo) %>" alt="<%=xshz.Name%>" /></a><%}else{%><a href="<%=xshz.Url%>" target="_blank"><%=xshz.Name%></a><%} %>     
