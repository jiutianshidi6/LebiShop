...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_ShopIndex"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> FlcGs=Languages();RecordCount=FlcGs.Count;int FlcG_index=1;
foreach (Shop.Model.Lebi_Language FlcG in FlcGs){%>
            <dd <%if (FlcG_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=FlcG.id%>,'<%=FlcG.Code%>','<%=FlcG.Path%>');"><%if (FlcG.ImageUrl!=""){%><img src="<%=Image(FlcG.ImageUrl) %>" /><%}%><%=FlcG.Name%></a></dd>
            <%FlcG_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int dcjL_index=1;
List<Lebi_Currency> dcjLs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency dcjL in dcjLs){%>
            <dd <%if (dcjL_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=dcjL.id%>,'<%=dcjL.Code%>',<%=dcjL.ExchangeRate%>,'<%=dcjL.Msige%>','<%=dcjL.DecimalLength%>');"><%=dcjL.Code%></a></dd>
            <%dcjL_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hwTy_index=1;
List<Lebi_Searchkey> hwTys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hwTy in hwTys){%><%=Lang(hwTy.Name)%><%hwTy_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int EnVk_index=1;
List<Lebi_Searchkey> EnVks = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey EnVk in EnVks){%>
<%if (EnVk.Type==1){ %><a href="<%=URL("P_Search",""+Lang(EnVk.Name)+"") %>"><%}else{ %><a href="<%=EnVk.URL%>" target="_blank"><%} %><span><%=Lang(EnVk.Name)%></span></a>
<%EnVk_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int fRaO_index=1;
List<Lebi_Page> fRaOs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page fRaO in fRaOs){%>
<a class="menu" href="<%=URL("","",fRaO.url)%>"><span><%=fRaO.Name%></span></a>
<%fRaO_index++;}%>
...
<div class="mc">
    <input id="shop_keyword" value="" type="text" name="shop_keyword" onkeydown="if(event.keyCode==13){shopsearch();}" /><input type="button" value="Search" class="button" onclick="shopsearch();" />
</div>
</div>
</div>
<script type="text/javascript" >
...
<div class="categories">
    <%Table="Lebi_Supplier_ProductType";Where="parentid = 0 and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int BAts_index=1;
List<Lebi_Supplier_ProductType> BAtss = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType BAts in BAtss){%>
    <dl <%if ((parentcid == 0 && BAts_index == 1) || parentcid == BAts.id){ %>class="current" <%} %>>
    <dt class="double">
        <label class="pic"></label>
        <a href="<%=URL("P_ShopProductCategory",""+id+","+BAts.id+"") %>"><%=Lang(BAts.Name) %></a>
    </dt>
    <dd>
        <ul>
            <%Table="Lebi_Supplier_ProductType";Where="parentid = "+BAts.id+" and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int dcki_index=1;
List<Lebi_Supplier_ProductType> dckis = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType dcki in dckis){%>
            <li class="<%=cid==dcki.id?"currentlink":"" %>"><a href="<%=URL("P_ShopProductCategory",""+id+","+dcki.id+"") %>">
                <%=Lang(dcki.Name) %></a></li>
            <%dcki_index++;}%>
        </ul>
    </dd>
...
<div class="mc">
    <ul>
    <li>Shop Acronym：<%=Lang(supplier.Name) %></li>
    <li>Company Name：<%=supplier.Company %></li>
    <li>Contact：<%=supplier.RealName %></li>
    <li>Mobile Number：<%=supplier.MobilePhone %></li>
    <li>Phone：<%=supplier.Phone %></li>
    <li>Email：<%=supplier.Email %></li>
    <li>QQ：<%=supplier.QQ %></li>
    <li>Postal Code：<%=supplier.Postalcode %></li>
    <li>Area：<%=Shop.Bussiness.EX_Area.GetAreaName(supplier.Area_id)%></li>
...
<%Table="Lebi_Supplier_ProductType";Where="parentid = 0 and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int YIBJ_index=1;
List<Lebi_Supplier_ProductType> YIBJs = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType YIBJ in YIBJs){%>
<div class="nbbox clearfix">
<div class="categoryproducttop clearfix">
    <div class="mt clearfix">
        <div class="left">
            <h2><%=Lang(YIBJ.Name) %></h2>
        </div>
        <div class="right"><a href="<%=URL("P_ShopProductCategory",""+id+","+YIBJ.id+"") %>">More</a></div>
    </div>
    <div class="mc clearfix">
        <div class="productshow">
            <ul class="image">
                <%Table="Lebi_Product";Where=""+ProductWhere+" and Type_id_ProductType = 320 and "+ ShopCategoryWhere(YIBJ.id) +"";Order="Sort desc,id desc";PageSize=8;pageindex=1;RecordCount=B_Lebi_Product.Counts(Where);int pro_index=1;
List<Lebi_Product> pros = B_Lebi_Product.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Product pro in pros){%>
                <li>
                    ...
                    <div class="proprice"><%if (pro.Price_Market > 0){ %><div class="marketprice"><i>Market Price：</i><strong><%=FormatMoney(pro.Price_Market) %></strong></div><%} %><div class="buyprice"><i><%=Lang(CurrentUserLevel.PriceName) %>：</i><strong><%=FormatMoney(ProductPrice(pro)) %></strong></div></div>                    
                </li>
                <%pro_index++;}%>
            </ul>
            <div class="clear"></div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int rDpS_index=1;
List<Lebi_FriendLink> rDpSs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink rDpS in rDpSs){%>
            
                <% if (rDpS.Logo != "" && rDpS.IsPic == 1){ %><a href="<%=rDpS.Url%>" target="_blank"><img src="<%=Image(rDpS.Logo) %>" alt="<%=rDpS.Name%>" /></a><%}else{%><a href="<%=rDpS.Url%>" target="_blank"><%=rDpS.Name%></a><%} %>     
