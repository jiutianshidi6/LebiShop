<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.P_ProductCommentindex" validateRequest="false"%>


<%@ Import Namespace="Shop.Bussiness" %>
<%@ Import Namespace="Shop.Model" %>
<%@ Import Namespace="System.Collections.Generic" %>
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_ProductCommentIndex"); %>
...
<title><%=ThemePageMeta("P_ProductCommentIndex","title")%></title>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Language" content="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0;" />
<meta name="keywords" content="<%=ThemePageMeta("P_ProductCommentIndex","keywords")%>" />
<meta name="description" content="<%=ThemePageMeta("P_ProductCommentIndex","description")%>" />
...
<script src="/Theme/system/js/jquery.masonry.min.js" type="text/javascript"></script>
<script type="text/javascript">
    $(function () {
        $('#ordershare').masonry({
            itemSelector: '.item',
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> aUOls=Languages();RecordCount=aUOls.Count;int aUOl_index=1;
foreach (Shop.Model.Lebi_Language aUOl in aUOls){%>
            <dd <%if (aUOl_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=aUOl.id%>,'<%=aUOl.Code%>','<%=aUOl.Path%>');"><%if (aUOl.ImageUrl!=""){%><img src="<%=Image(aUOl.ImageUrl) %>" /><%}%><%=aUOl.Name%></a></dd>
            <%aUOl_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int HDwU_index=1;
List<Lebi_Currency> HDwUs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency HDwU in HDwUs){%>
            <dd <%if (HDwU_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=HDwU.id%>,'<%=HDwU.Code%>',<%=HDwU.ExchangeRate%>,'<%=HDwU.Msige%>','<%=HDwU.DecimalLength%>');"><%=HDwU.Code%></a></dd>
            <%HDwU_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Jxxl_index=1;
List<Lebi_Searchkey> Jxxls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Jxxl in Jxxls){%><%=Lang(Jxxl.Name)%><%Jxxl_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int LYTy_index=1;
List<Lebi_Searchkey> LYTys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey LYTy in LYTys){%>
<%if (LYTy.Type==1){ %><a href="<%=URL("P_Search",""+Lang(LYTy.Name)+"") %>"><%}else{ %><a href="<%=LYTy.URL%>" target="_blank"><%} %><span><%=Lang(LYTy.Name)%></span></a>
<%LYTy_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int RsQx_index=1;
List<Lebi_Page> RsQxs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page RsQx in RsQxs){%>
<a class="menu" href="<%=URL("","",RsQx.url)%>"><span><%=RsQx.Name%></span></a>
<%RsQx_index++;}%>
...
<div class="body clearfix">
  <div class="location"><div class="path"><%=path%></div></div>
    ...
    <div id="load">
        <div id="status"></div>
        <a href="javascript:void(0)" onclick="LoadMore();" class="btn btn-11"><s></s>Show more</a>
    </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int UGBm_index=1;
List<Lebi_FriendLink> UGBms = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink UGBm in UGBms){%>
            
                <% if (UGBm.Logo != "" && UGBm.IsPic == 1){ %><a href="<%=UGBm.Url%>" target="_blank"><img src="<%=Image(UGBm.Logo) %>" alt="<%=UGBm.Name%>" /></a><%}else{%><a href="<%=UGBm.Url%>" target="_blank"><%=UGBm.Name%></a><%} %>     
