...
<% LoadPage("wangdayu523@163.com_37",1,"EN","P_About"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> shvFs=Languages();RecordCount=shvFs.Count;int shvF_index=1;
foreach (Shop.Model.Lebi_Language shvF in shvFs){%>
            <dd <%if (shvF_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=shvF.id%>,'<%=shvF.Code%>','<%=shvF.Path%>');"><%if (shvF.ImageUrl!=""){%><img src="<%=Image(shvF.ImageUrl) %>" /><%}%><%=shvF.Name%></a></dd>
            <%shvF_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int klaO_index=1;
List<Lebi_Currency> klaOs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency klaO in klaOs){%>
            <dd <%if (klaO_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=klaO.id%>,'<%=klaO.Code%>',<%=klaO.ExchangeRate%>,'<%=klaO.Msige%>','<%=klaO.DecimalLength%>');"><%=klaO.Code%></a></dd>
            <%klaO_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Snza_index=1;
List<Lebi_Searchkey> Snzas = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Snza in Snzas){%><%=Lang(Snza.Name)%><%Snza_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int prJD_index=1;
List<Lebi_Searchkey> prJDs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey prJD in prJDs){%>
<%if (prJD.Type==1){ %><a href="<%=URL("P_Search",""+Lang(prJD.Name)+"") %>"><%}else{ %><a href="<%=prJD.URL%>" target="_blank"><%} %><span><%=Lang(prJD.Name)%></span></a>
<%prJD_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int hpDm_index=1;
List<Lebi_Page> hpDms = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page hpDm in hpDms){%>
<a class="menu" href="<%=URL("","",hpDm.url)%>"><span><%=hpDm.Name%></span></a>
<%hpDm_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int YAwm_index=1;
List<Lebi_FriendLink> YAwms = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink YAwm in YAwms){%>
            
                <% if (YAwm.Logo != "" && YAwm.IsPic == 1){ %><a href="<%=YAwm.Url%>" target="_blank"><img src="<%=Image(YAwm.Logo) %>" alt="<%=YAwm.Name%>" /></a><%}else{%><a href="<%=YAwm.Url%>" target="_blank"><%=YAwm.Name%></a><%} %>     
