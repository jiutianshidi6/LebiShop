...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_Brand"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> JjPCs=Languages();RecordCount=JjPCs.Count;int JjPC_index=1;
foreach (Shop.Model.Lebi_Language JjPC in JjPCs){%>
            <dd <%if (JjPC_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=JjPC.id%>,'<%=JjPC.Code%>','<%=JjPC.Path%>');"><%if (JjPC.ImageUrl!=""){%><img src="<%=Image(JjPC.ImageUrl) %>" /><%}%><%=JjPC.Name%></a></dd>
            <%JjPC_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int BzmE_index=1;
List<Lebi_Currency> BzmEs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency BzmE in BzmEs){%>
            <dd <%if (BzmE_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=BzmE.id%>,'<%=BzmE.Code%>',<%=BzmE.ExchangeRate%>,'<%=BzmE.Msige%>','<%=BzmE.DecimalLength%>');"><%=BzmE.Code%></a></dd>
            <%BzmE_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int aKcu_index=1;
List<Lebi_Searchkey> aKcus = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey aKcu in aKcus){%><%=Lang(aKcu.Name)%><%aKcu_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int GSLS_index=1;
List<Lebi_Searchkey> GSLSs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey GSLS in GSLSs){%>
<%if (GSLS.Type==1){ %><a href="<%=URL("P_Search",""+Lang(GSLS.Name)+"") %>"><%}else{ %><a href="<%=GSLS.URL%>" target="_blank"><%} %><span><%=Lang(GSLS.Name)%></span></a>
<%GSLS_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int oudL_index=1;
List<Lebi_Page> oudLs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page oudL in oudLs){%>
<a class="menu" href="<%=URL("","",oudL.url)%>"><span><%=oudL.Name%></span></a>
<%oudL_index++;}%>
...
<ul class="text">
    <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int UEcj_index=1;
List<Lebi_Brand> UEcjs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand UEcj in UEcjs){%>
    <li><a href="<%=URL("P_Brand",UEcj.id)%>" title="<%=Lang(UEcj.Name) %>"><%=Lang(UEcj.Name) %></a></li>
    <%UEcj_index++;}%>
</ul>
</div>
</div>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int eraA_index=1;
List<Lebi_Page> eraAs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page eraA in eraAs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",eraA.id,eraA.url)%>" target="<%=eraA.target%>" title="<%=eraA.Name%>"><span style="color:<%=eraA.NameColor%>"><%=eraA.Name%></span></a></li>
        <%eraA_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int gjqg_index=1;
List<Lebi_FriendLink> gjqgs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink gjqg in gjqgs){%>
            
                <% if (gjqg.Logo != "" && gjqg.IsPic == 1){ %><a href="<%=gjqg.Url%>" target="_blank"><img src="<%=Image(gjqg.Logo) %>" alt="<%=gjqg.Name%>" /></a><%}else{%><a href="<%=gjqg.Url%>" target="_blank"><%=gjqg.Name%></a><%} %>     
