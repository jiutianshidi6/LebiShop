...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_LimitBuy"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> Lxtzs=Languages();RecordCount=Lxtzs.Count;int Lxtz_index=1;
foreach (Shop.Model.Lebi_Language Lxtz in Lxtzs){%>
            <dd <%if (Lxtz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Lxtz.id%>,'<%=Lxtz.Code%>','<%=Lxtz.Path%>');"><%if (Lxtz.ImageUrl!=""){%><img src="<%=Image(Lxtz.ImageUrl) %>" /><%}%><%=Lxtz.Name%></a></dd>
            <%Lxtz_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int lUVo_index=1;
List<Lebi_Currency> lUVos = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency lUVo in lUVos){%>
            <dd <%if (lUVo_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=lUVo.id%>,'<%=lUVo.Code%>',<%=lUVo.ExchangeRate%>,'<%=lUVo.Msige%>','<%=lUVo.DecimalLength%>');"><%=lUVo.Code%></a></dd>
            <%lUVo_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int eMaD_index=1;
List<Lebi_Searchkey> eMaDs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey eMaD in eMaDs){%><%=Lang(eMaD.Name)%><%eMaD_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int aBor_index=1;
List<Lebi_Searchkey> aBors = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey aBor in aBors){%>
<%if (aBor.Type==1){ %><a href="<%=URL("P_Search",""+Lang(aBor.Name)+"") %>"><%}else{ %><a href="<%=aBor.URL%>" target="_blank"><%} %><span><%=Lang(aBor.Name)%></span></a>
<%aBor_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int wguK_index=1;
List<Lebi_Page> wguKs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page wguK in wguKs){%>
<a class="menu" href="<%=URL("","",wguK.url)%>"><span><%=wguK.Name%></span></a>
<%wguK_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int IWnL_index=1;
List<Lebi_Page> IWnLs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page IWnL in IWnLs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",IWnL.id,IWnL.url)%>" target="<%=IWnL.target%>" title="<%=IWnL.Name%>"><span style="color:<%=IWnL.NameColor%>"><%=IWnL.Name%></span></a></li>
        <%IWnL_index++;}%>
    </ul>
</div>
</div>
...
<div id="filter">
	<div class="fore1">
		<dl class="order">
			<dt>排序：</dt>
			<dd <%if(sort == "" || sort == "1" || sort == "1a"){ %>class="curr <%if(sort == "" || sort == "1"){ %>up<%}else if(sort == "1a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_LimitBuy",""+list+",1"+ordertmp+"") %>">销量</a><b></b></dd>
			<dd <%if(sort == "2" || sort == "2a"){ %>class="curr <%if(sort == "2"){ %>up<%}else if(sort == "2a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_LimitBuy",""+list+",2"+ordertmp+"") %>">价格</a><b></b></dd>
			<dd <%if(sort == "3" || sort == "3a"){ %>class="curr <%if(sort == "3"){ %>up<%}else if(sort == "3a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_LimitBuy",""+list+",3"+ordertmp+"") %>">评价数</a><b></b></dd>
			<dd <%if(sort == "4" || sort == "4a"){ %>class="curr <%if(sort == "4"){ %>up<%}else if(sort == "4a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_LimitBuy",""+list+",4"+ordertmp+"") %>">上架时间</a><b></b></dd>
		</dl>
        <div class="list-cutover">
			...
			<a id="grid-unselected"  href="<%=URL("P_LimitBuy","1,"+ sort) %>" class="modfiy-url grid-unselected <%=(list=="1" || list=="")?"grid-curr":"" %>"><b></b>网格</a>
		</div>
...
<div class="mc clearfix">
    <div class="productlist limitbuylist loadlist"><ul>
        ...
        <li class="grid loadli">
            ...
            <%if (pro.Price > 0){ %><div class="marketprice"><i>销售价：</i><strong><%=FormatMoney(pro.Price) %></strong></div><%} %>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int NhlL_index=1;
List<Lebi_FriendLink> NhlLs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink NhlL in NhlLs){%>
            
                <% if (NhlL.Logo != "" && NhlL.IsPic == 1){ %><a href="<%=NhlL.Url%>" target="_blank"><img src="<%=Image(NhlL.Logo) %>" alt="<%=NhlL.Name%>" /></a><%}else{%><a href="<%=NhlL.Url%>" target="_blank"><%=NhlL.Name%></a><%} %>     
