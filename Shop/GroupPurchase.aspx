...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> wpvFs=Languages();RecordCount=wpvFs.Count;int wpvF_index=1;
foreach (Shop.Model.Lebi_Language wpvF in wpvFs){%>
            <dd <%if (wpvF_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=wpvF.id%>,'<%=wpvF.Code%>','<%=wpvF.Path%>');"><%if (wpvF.ImageUrl!=""){%><img src="<%=Image(wpvF.ImageUrl) %>" /><%}%><%=wpvF.Name%></a></dd>
            <%wpvF_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int bDDk_index=1;
List<Lebi_Currency> bDDks = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency bDDk in bDDks){%>
            <dd <%if (bDDk_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=bDDk.id%>,'<%=bDDk.Code%>',<%=bDDk.ExchangeRate%>,'<%=bDDk.Msige%>','<%=bDDk.DecimalLength%>');"><%=bDDk.Code%></a></dd>
            <%bDDk_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int UnJl_index=1;
List<Lebi_Searchkey> UnJls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey UnJl in UnJls){%><%=Lang(UnJl.Name)%><%UnJl_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int wpiy_index=1;
List<Lebi_Searchkey> wpiys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey wpiy in wpiys){%>
<%if (wpiy.Type==1){ %><a href="<%=URL("P_Search",""+Lang(wpiy.Name)+"") %>"><%}else{ %><a href="<%=wpiy.URL%>" target="_blank"><%} %><span><%=Lang(wpiy.Name)%></span></a>
<%wpiy_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int mOVV_index=1;
List<Lebi_Page> mOVVs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page mOVV in mOVVs){%>
<a class="menu" href="<%=URL("","",mOVV.url)%>"><span><%=mOVV.Name%></span></a>
<%mOVV_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int QEFE_index=1;
List<Lebi_Page> QEFEs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page QEFE in QEFEs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",QEFE.id,QEFE.url)%>" target="<%=QEFE.target%>" title="<%=QEFE.Name%>"><span style="color:<%=QEFE.NameColor%>"><%=QEFE.Name%></span></a></li>
        <%QEFE_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Vewi_index=1;
List<Lebi_FriendLink> Vewis = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Vewi in Vewis){%>
            
                <% if (Vewi.Logo != "" && Vewi.IsPic == 1){ %><a href="<%=Vewi.Url%>" target="_blank"><img src="<%=Image(Vewi.Logo) %>" alt="<%=Vewi.Name%>" /></a><%}else{%><a href="<%=Vewi.Url%>" target="_blank"><%=Vewi.Name%></a><%} %>     
