...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_ShopProducts"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> WMKrs=Languages();RecordCount=WMKrs.Count;int WMKr_index=1;
foreach (Shop.Model.Lebi_Language WMKr in WMKrs){%>
            <dd <%if (WMKr_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=WMKr.id%>,'<%=WMKr.Code%>','<%=WMKr.Path%>');"><%if (WMKr.ImageUrl!=""){%><img src="<%=Image(WMKr.ImageUrl) %>" /><%}%><%=WMKr.Name%></a></dd>
            <%WMKr_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int hMAj_index=1;
List<Lebi_Currency> hMAjs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency hMAj in hMAjs){%>
            <dd <%if (hMAj_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=hMAj.id%>,'<%=hMAj.Code%>',<%=hMAj.ExchangeRate%>,'<%=hMAj.Msige%>','<%=hMAj.DecimalLength%>');"><%=hMAj.Code%></a></dd>
            <%hMAj_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int uCIj_index=1;
List<Lebi_Searchkey> uCIjs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey uCIj in uCIjs){%><%=Lang(uCIj.Name)%><%uCIj_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int gJKy_index=1;
List<Lebi_Searchkey> gJKys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey gJKy in gJKys){%>
<%if (gJKy.Type==1){ %><a href="<%=URL("P_Search",""+Lang(gJKy.Name)+"") %>"><%}else{ %><a href="<%=gJKy.URL%>" target="_blank"><%} %><span><%=Lang(gJKy.Name)%></span></a>
<%gJKy_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Hpfv_index=1;
List<Lebi_Page> Hpfvs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Hpfv in Hpfvs){%>
<a class="menu" href="<%=URL("","",Hpfv.url)%>"><span><%=Hpfv.Name%></span></a>
<%Hpfv_index++;}%>
...
<div id="shopcategory">
    ...
    <div class="categories">
        <%Table="Lebi_Supplier_ProductType";Where="parentid = 0 and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int tSGr_index=1;
List<Lebi_Supplier_ProductType> tSGrs = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType tSGr in tSGrs){%>
        <dl <%if ((parentcid == 0 && tSGr_index == 1) || parentcid == tSGr.id){ %>class="current" <%} %>>
            <dt class="double">
                <label class="pic">
                </label>
                <a href="<%=URL("P_ShopProductCategory",""+id+","+tSGr.id+"") %>">
                    <%=Lang(tSGr.Name) %></a></dt>
            <dd>
                <ul>
                    <%Table="Lebi_Supplier_ProductType";Where="parentid = "+tSGr.id+" and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int xrsR_index=1;
List<Lebi_Supplier_ProductType> xrsRs = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType xrsR in xrsRs){%>
                    <li class="<%=cid==xrsR.id?"currentlink":"" %>"><a href="<%=URL("P_ShopProductCategory",""+id+","+xrsR.id+"") %>">
                        <%=Lang(xrsR.Name) %></a></li>
                    <%xrsR_index++;}%>
                </ul>
            </dd>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int aImQ_index=1;
List<Lebi_FriendLink> aImQs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink aImQ in aImQs){%>
            
                <% if (aImQ.Logo != "" && aImQ.IsPic == 1){ %><a href="<%=aImQ.Url%>" target="_blank"><img src="<%=Image(aImQ.Logo) %>" alt="<%=aImQ.Name%>" /></a><%}else{%><a href="<%=aImQ.Url%>" target="_blank"><%=aImQ.Name%></a><%} %>     
