...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_ShopProductCategory"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> djwss=Languages();RecordCount=djwss.Count;int djws_index=1;
foreach (Shop.Model.Lebi_Language djws in djwss){%>
            <dd <%if (djws_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=djws.id%>,'<%=djws.Code%>','<%=djws.Path%>');"><%if (djws.ImageUrl!=""){%><img src="<%=Image(djws.ImageUrl) %>" /><%}%><%=djws.Name%></a></dd>
            <%djws_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int toto_index=1;
List<Lebi_Searchkey> totos = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey toto in totos){%><%=Lang(toto.Name)%><%toto_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int iqAc_index=1;
List<Lebi_Searchkey> iqAcs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey iqAc in iqAcs){%>
<%if (iqAc.Type==1){ %><a href="<%=URL("P_Search",""+Lang(iqAc.Name)+"") %>"><%}else{ %><a href="<%=iqAc.URL%>" target="_blank"><%} %><span><%=Lang(iqAc.Name)%></span></a>
<%iqAc_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int HyoQ_index=1;
List<Lebi_Brand> HyoQs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand HyoQ in HyoQs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",HyoQ.id)%>"><img alt="<%=Lang(HyoQ.Name) %>" src="<%=Image(HyoQ.ImageUrl) %>"><span><%=Lang(HyoQ.Name) %></span></a></li>
                                        <%HyoQ_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int lnrH_index=1;
List<Lebi_Page> lnrHs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page lnrH in lnrHs){%>
<a class="menu" href="<%=URL("","",lnrH.url)%>"><span><%=lnrH.Name%></span></a>
<%lnrH_index++;}%>
...
<div class="categories">
    <%Table="Lebi_Supplier_ProductType";Where="parentid = 0 and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int Sdic_index=1;
List<Lebi_Supplier_ProductType> Sdics = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType Sdic in Sdics){%>
    <dl <%if ((parentcid == 0 && Sdic_index == 1) || parentcid == Sdic.id){ %>class="current" <%} %>>
    <dt class="double">
        <label class="pic"></label>
        <a href="<%=URL("P_ShopProductCategory",""+id+","+Sdic.id+"") %>"><%=Lang(Sdic.Name) %></a>
    </dt>
    <dd>
        <ul>
            <%Table="Lebi_Supplier_ProductType";Where="parentid = "+Sdic.id+" and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int vkyu_index=1;
List<Lebi_Supplier_ProductType> vkyus = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType vkyu in vkyus){%>
            <li class="<%=cid==vkyu.id?"currentlink":"" %>"><a href="<%=URL("P_ShopProductCategory",""+id+","+vkyu.id+"") %>">
                <%=Lang(vkyu.Name) %></a></li>
            <%vkyu_index++;}%>
        </ul>
    </dd>
