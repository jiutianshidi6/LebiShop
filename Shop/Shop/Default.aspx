...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_ShopIndex"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> QLmDs=Languages();RecordCount=QLmDs.Count;int QLmD_index=1;
foreach (Shop.Model.Lebi_Language QLmD in QLmDs){%>
            <dd <%if (QLmD_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=QLmD.id%>,'<%=QLmD.Code%>','<%=QLmD.Path%>');"><%if (QLmD.ImageUrl!=""){%><img src="<%=Image(QLmD.ImageUrl) %>" /><%}%><%=QLmD.Name%></a></dd>
            <%QLmD_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int qmbN_index=1;
List<Lebi_Currency> qmbNs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency qmbN in qmbNs){%>
            <dd <%if (qmbN_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=qmbN.id%>,'<%=qmbN.Code%>',<%=qmbN.ExchangeRate%>,'<%=qmbN.Msige%>','<%=qmbN.DecimalLength%>');"><%=qmbN.Code%></a></dd>
            <%qmbN_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int lUEG_index=1;
List<Lebi_Searchkey> lUEGs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey lUEG in lUEGs){%><%=Lang(lUEG.Name)%><%lUEG_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ecKH_index=1;
List<Lebi_Searchkey> ecKHs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ecKH in ecKHs){%>
<%if (ecKH.Type==1){ %><a href="<%=URL("P_Search",""+Lang(ecKH.Name)+"") %>"><%}else{ %><a href="<%=ecKH.URL%>" target="_blank"><%} %><span><%=Lang(ecKH.Name)%></span></a>
<%ecKH_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int jmqJ_index=1;
List<Lebi_Page> jmqJs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page jmqJ in jmqJs){%>
<a class="menu" href="<%=URL("","",jmqJ.url)%>"><span><%=jmqJ.Name%></span></a>
<%jmqJ_index++;}%>
...
<div class="categories">
    <%Table="Lebi_Supplier_ProductType";Where="parentid = 0 and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int rxIe_index=1;
List<Lebi_Supplier_ProductType> rxIes = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType rxIe in rxIes){%>
    <dl <%if ((parentcid == 0 && rxIe_index == 1) || parentcid == rxIe.id){ %>class="current" <%} %>>
    <dt class="double">
        <label class="pic"></label>
        <a href="<%=URL("P_ShopProductCategory",""+id+","+rxIe.id+"") %>"><%=Lang(rxIe.Name) %></a>
    </dt>
    <dd>
        <ul>
            <%Table="Lebi_Supplier_ProductType";Where="parentid = "+rxIe.id+" and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int RlHS_index=1;
List<Lebi_Supplier_ProductType> RlHSs = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType RlHS in RlHSs){%>
            <li class="<%=cid==RlHS.id?"currentlink":"" %>"><a href="<%=URL("P_ShopProductCategory",""+id+","+RlHS.id+"") %>">
                <%=Lang(RlHS.Name) %></a></li>
            <%RlHS_index++;}%>
        </ul>
    </dd>
...
<%Table="Lebi_Supplier_ProductType";Where="parentid = 0 and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int RCPO_index=1;
List<Lebi_Supplier_ProductType> RCPOs = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType RCPO in RCPOs){%>
<div class="nbbox clearfix">
<div class="categoryproducttop clearfix">
    <div class="mt clearfix">
        <div class="left">
            <h2><%=Lang(RCPO.Name) %></h2>
        </div>
        <div class="right"><a href="<%=URL("P_ShopProductCategory",""+id+","+RCPO.id+"") %>">更多</a></div>
    </div>
    <div class="mc clearfix">
        <div class="productshow">
            <ul class="image">
                <%Table="Lebi_Product";Where=""+ProductWhere+" and Type_id_ProductType = 320 and "+ ShopCategoryWhere(RCPO.id) +"";Order="Sort desc,id desc";PageSize=8;pageindex=1;RecordCount=B_Lebi_Product.Counts(Where);int pro_index=1;
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int NXfO_index=1;
List<Lebi_FriendLink> NXfOs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink NXfO in NXfOs){%>
            
                <% if (NXfO.Logo != "" && NXfO.IsPic == 1){ %><a href="<%=NXfO.Url%>" target="_blank"><img src="<%=Image(NXfO.Logo) %>" alt="<%=NXfO.Name%>" /></a><%}else{%><a href="<%=NXfO.Url%>" target="_blank"><%=NXfO.Name%></a><%} %>     
