...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_ShopSearch"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> tvLBs=Languages();RecordCount=tvLBs.Count;int tvLB_index=1;
foreach (Shop.Model.Lebi_Language tvLB in tvLBs){%>
            <dd <%if (tvLB_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=tvLB.id%>,'<%=tvLB.Code%>','<%=tvLB.Path%>');"><%if (tvLB.ImageUrl!=""){%><img src="<%=Image(tvLB.ImageUrl) %>" /><%}%><%=tvLB.Name%></a></dd>
            <%tvLB_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int eReE_index=1;
List<Lebi_Searchkey> eReEs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey eReE in eReEs){%><%=Lang(eReE.Name)%><%eReE_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int GINi_index=1;
List<Lebi_Searchkey> GINis = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey GINi in GINis){%>
<%if (GINi.Type==1){ %><a href="<%=URL("P_Search",""+Lang(GINi.Name)+"") %>"><%}else{ %><a href="<%=GINi.URL%>" target="_blank"><%} %><span><%=Lang(GINi.Name)%></span></a>
<%GINi_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int hJmQ_index=1;
List<Lebi_Brand> hJmQs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand hJmQ in hJmQs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",hJmQ.id)%>"><img alt="<%=Lang(hJmQ.Name) %>" src="<%=Image(hJmQ.ImageUrl) %>"><span><%=Lang(hJmQ.Name) %></span></a></li>
                                        <%hJmQ_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int GCEx_index=1;
List<Lebi_Page> GCExs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page GCEx in GCExs){%>
<a class="menu" href="<%=URL("","",GCEx.url)%>"><span><%=GCEx.Name%></span></a>
<%GCEx_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int CVsp_index=1;
List<Lebi_Page> CVsps = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page CVsp in CVsps){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",CVsp.id,CVsp.url)%>" target="<%=CVsp.target%>" title="<%=CVsp.Name%>"><span style="color:<%=CVsp.NameColor%>"><%=CVsp.Name%></span></a></li>
        <%CVsp_index++;}%>
    </ul>
</div>
</div>
