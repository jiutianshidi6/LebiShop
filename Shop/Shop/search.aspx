...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_ShopSearch"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> EzPps=Languages();RecordCount=EzPps.Count;int EzPp_index=1;
foreach (Shop.Model.Lebi_Language EzPp in EzPps){%>
            <dd <%if (EzPp_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=EzPp.id%>,'<%=EzPp.Code%>','<%=EzPp.Path%>');"><%if (EzPp.ImageUrl!=""){%><img src="<%=Image(EzPp.ImageUrl) %>" /><%}%><%=EzPp.Name%></a></dd>
            <%EzPp_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int sDcB_index=1;
List<Lebi_Currency> sDcBs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency sDcB in sDcBs){%>
            <dd <%if (sDcB_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=sDcB.id%>,'<%=sDcB.Code%>',<%=sDcB.ExchangeRate%>,'<%=sDcB.Msige%>','<%=sDcB.DecimalLength%>');"><%=sDcB.Code%></a></dd>
            <%sDcB_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int RSLp_index=1;
List<Lebi_Searchkey> RSLps = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey RSLp in RSLps){%><%=Lang(RSLp.Name)%><%RSLp_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int voyk_index=1;
List<Lebi_Searchkey> voyks = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey voyk in voyks){%>
<%if (voyk.Type==1){ %><a href="<%=URL("P_Search",""+Lang(voyk.Name)+"") %>"><%}else{ %><a href="<%=voyk.URL%>" target="_blank"><%} %><span><%=Lang(voyk.Name)%></span></a>
<%voyk_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int CqgW_index=1;
List<Lebi_Page> CqgWs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page CqgW in CqgWs){%>
<a class="menu" href="<%=URL("","",CqgW.url)%>"><span><%=CqgW.Name%></span></a>
<%CqgW_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int wCcw_index=1;
List<Lebi_Page> wCcws = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page wCcw in wCcws){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",wCcw.id,wCcw.url)%>" target="<%=wCcw.target%>" title="<%=wCcw.Name%>"><span style="color:<%=wCcw.NameColor%>"><%=wCcw.Name%></span></a></li>
        <%wCcw_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int YFTp_index=1;
List<Lebi_FriendLink> YFTps = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink YFTp in YFTps){%>
            
                <% if (YFTp.Logo != "" && YFTp.IsPic == 1){ %><a href="<%=YFTp.Url%>" target="_blank"><img src="<%=Image(YFTp.Logo) %>" alt="<%=YFTp.Name%>" /></a><%}else{%><a href="<%=YFTp.Url%>" target="_blank"><%=YFTp.Name%></a><%} %>     
