...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_Index"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> deVXs=Languages();RecordCount=deVXs.Count;int deVX_index=1;
foreach (Shop.Model.Lebi_Language deVX in deVXs){%>
            <dd <%if (deVX_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=deVX.id%>,'<%=deVX.Code%>','<%=deVX.Path%>');"><%if (deVX.ImageUrl!=""){%><img src="<%=Image(deVX.ImageUrl) %>" /><%}%><%=deVX.Name%></a></dd>
            <%deVX_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int ABOn_index=1;
List<Lebi_Currency> ABOns = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency ABOn in ABOns){%>
            <dd <%if (ABOn_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=ABOn.id%>,'<%=ABOn.Code%>',<%=ABOn.ExchangeRate%>,'<%=ABOn.Msige%>','<%=ABOn.DecimalLength%>');"><%=ABOn.Code%></a></dd>
            <%ABOn_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int XzRN_index=1;
List<Lebi_Searchkey> XzRNs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey XzRN in XzRNs){%><%=Lang(XzRN.Name)%><%XzRN_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int WiJv_index=1;
List<Lebi_Searchkey> WiJvs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey WiJv in WiJvs){%>
<%if (WiJv.Type==1){ %><a href="<%=URL("P_Search",""+Lang(WiJv.Name)+"") %>"><%}else{ %><a href="<%=WiJv.URL%>" target="_blank"><%} %><span><%=Lang(WiJv.Name)%></span></a>
<%WiJv_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int iEIU_index=1;
List<Lebi_Page> iEIUs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page iEIU in iEIUs){%>
<a class="menu" href="<%=URL("","",iEIU.url)%>"><span><%=iEIU.Name%></span></a>
<%iEIU_index++;}%>
...
<div class="ind-tabs">
	<div class="hd">
    	<ul>
        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int EjdO_index=1;
List<Lebi_Brand> EjdOs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand EjdO in EjdOs){%>
        	<li><%=Lang(EjdO.Name) %></li>
        <%EjdO_index++;}%>
        </ul>
    </div>
    <div class="bd">
      <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int sqzz_index=1;
List<Lebi_Brand> sqzzs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand sqzz in sqzzs){%>
   	  <div class="lh">
        	<div class="pic"><a href="<%=URL("P_Brand",sqzz.id)%>"><img src="<%=Image(sqzz.ImageUrl) %>" alt="<%=Lang(sqzz.Name) %>" title="<%=Lang(sqzz.Name) %>" width="230" height="230" /></a></div>
            <div class="info">
            	<div class="desc"><%=Lang(sqzz.Description)%></div>
                <a href="<%=URL("P_Brand",sqzz.id)%>" class="read-more"><%=Lang(sqzz.Name) %> 商品列表 >></a>
            </div>
      </div>
      <%sqzz_index++;}%>
    </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int GGol_index=1;
List<Lebi_FriendLink> GGols = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink GGol in GGols){%>
            
                <% if (GGol.Logo != "" && GGol.IsPic == 1){ %><a href="<%=GGol.Url%>" target="_blank"><img src="<%=Image(GGol.Logo) %>" alt="<%=GGol.Name%>" /></a><%}else{%><a href="<%=GGol.Url%>" target="_blank"><%=GGol.Name%></a><%} %>     
