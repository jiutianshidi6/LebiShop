...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> kLBes=Languages();RecordCount=kLBes.Count;int kLBe_index=1;
foreach (Shop.Model.Lebi_Language kLBe in kLBes){%>
            <dd <%if (kLBe_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=kLBe.id%>,'<%=kLBe.Code%>','<%=kLBe.Path%>');"><%if (kLBe.ImageUrl!=""){%><img src="<%=Image(kLBe.ImageUrl) %>" /><%}%><%=kLBe.Name%></a></dd>
            <%kLBe_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int taWk_index=1;
List<Lebi_Currency> taWks = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency taWk in taWks){%>
            <dd <%if (taWk_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=taWk.id%>,'<%=taWk.Code%>',<%=taWk.ExchangeRate%>,'<%=taWk.Msige%>','<%=taWk.DecimalLength%>');"><%=taWk.Code%></a></dd>
            <%taWk_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int BJLl_index=1;
List<Lebi_Searchkey> BJLls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey BJLl in BJLls){%><%=Lang(BJLl.Name)%><%BJLl_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int mVzJ_index=1;
List<Lebi_Searchkey> mVzJs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey mVzJ in mVzJs){%>
<%if (mVzJ.Type==1){ %><a href="<%=URL("P_Search",""+Lang(mVzJ.Name)+"") %>"><%}else{ %><a href="<%=mVzJ.URL%>" target="_blank"><%} %><span><%=Lang(mVzJ.Name)%></span></a>
<%mVzJ_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int MPSt_index=1;
List<Lebi_Page> MPSts = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page MPSt in MPSts){%>
<a class="menu" href="<%=URL("","",MPSt.url)%>"><span><%=MPSt.Name%></span></a>
<%MPSt_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int rhnY_index=1;
List<Lebi_Page> rhnYs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page rhnY in rhnYs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",rhnY.id,rhnY.url)%>" target="<%=rhnY.target%>" title="<%=rhnY.Name%>"><span style="color:<%=rhnY.NameColor%>"><%=rhnY.Name%></span></a></li>
        <%rhnY_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int mAGm_index=1;
List<Lebi_FriendLink> mAGms = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink mAGm in mAGms){%>
            
                <% if (mAGm.Logo != "" && mAGm.IsPic == 1){ %><a href="<%=mAGm.Url%>" target="_blank"><img src="<%=Image(mAGm.Logo) %>" alt="<%=mAGm.Name%>" /></a><%}else{%><a href="<%=mAGm.Url%>" target="_blank"><%=mAGm.Name%></a><%} %>     
