<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.P_ShopSearch" validateRequest="false"%>


<%@ Import Namespace="Shop.Bussiness" %>
<%@ Import Namespace="Shop.Model" %>
<%@ Import Namespace="System.Collections.Generic" %>
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_ShopSearch"); %>
...
<title><%=ThemePageMeta("P_ShopSearch","title")%></title>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Language" content="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0;" />
<meta name="keywords" content="<%=ThemePageMeta("P_ShopSearch","keywords")%>" />
<meta name="description" content="<%=ThemePageMeta("P_ShopSearch","description")%>" />
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> MFiFs=Languages();RecordCount=MFiFs.Count;int MFiF_index=1;
foreach (Shop.Model.Lebi_Language MFiF in MFiFs){%>
            <dd <%if (MFiF_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=MFiF.id%>,'<%=MFiF.Code%>','<%=MFiF.Path%>');"><%if (MFiF.ImageUrl!=""){%><img src="<%=Image(MFiF.ImageUrl) %>" /><%}%><%=MFiF.Name%></a></dd>
            <%MFiF_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int TtSH_index=1;
List<Lebi_Currency> TtSHs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency TtSH in TtSHs){%>
            <dd <%if (TtSH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=TtSH.id%>,'<%=TtSH.Code%>',<%=TtSH.ExchangeRate%>,'<%=TtSH.Msige%>','<%=TtSH.DecimalLength%>');"><%=TtSH.Code%></a></dd>
            <%TtSH_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int SDTL_index=1;
List<Lebi_Searchkey> SDTLs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey SDTL in SDTLs){%><%=Lang(SDTL.Name)%><%SDTL_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int JLlW_index=1;
List<Lebi_Searchkey> JLlWs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey JLlW in JLlWs){%>
<%if (JLlW.Type==1){ %><a href="<%=URL("P_Search",""+Lang(JLlW.Name)+"") %>"><%}else{ %><a href="<%=JLlW.URL%>" target="_blank"><%} %><span><%=Lang(JLlW.Name)%></span></a>
<%JLlW_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int uFfg_index=1;
List<Lebi_Page> uFfgs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page uFfg in uFfgs){%>
<a class="menu" href="<%=URL("","",uFfg.url)%>"><span><%=uFfg.Name%></span></a>
<%uFfg_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int AjHb_index=1;
List<Lebi_Page> AjHbs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page AjHb in AjHbs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",AjHb.id,AjHb.url)%>" target="<%=AjHb.target%>" title="<%=AjHb.Name%>"><span style="color:<%=AjHb.NameColor%>"><%=AjHb.Name%></span></a></li>
        <%AjHb_index++;}%>
    </ul>
</div>
</div>
...
<div class="nbbox clearfix">
    ...
    <div class="mc clearfix">
        <div class="productlist loadlist">
            <ul>
                <%foreach (Shop.Model.Lebi_Supplier model in shops)
        {
                %>
                <li class="grid loadli">
                    <div class="proimg">
                        <p>
                            <a href="<%=URL("P_ShopIndex",model.id) %>"><img <%if (CurrentLanguage.IsLazyLoad==1){ %>class="lazy" src="/Theme/system/images/lazy.gif" data-original<%}else{ %>src<%} %>="<%=Image(model.Logo)%>" alt="<%=Lang(model.Name) %>" title="<%=Lang(model.Name) %>" /></a></p>
                    </div>
                    <div class="proname">
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int hzUh_index=1;
List<Lebi_FriendLink> hzUhs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink hzUh in hzUhs){%>
            
                <% if (hzUh.Logo != "" && hzUh.IsPic == 1){ %><a href="<%=hzUh.Url%>" target="_blank"><img src="<%=Image(hzUh.Logo) %>" alt="<%=hzUh.Name%>" /></a><%}else{%><a href="<%=hzUh.Url%>" target="_blank"><%=hzUh.Name%></a><%} %>     
