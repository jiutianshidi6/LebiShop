...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_ShopProducts"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> MFtrs=Languages();RecordCount=MFtrs.Count;int MFtr_index=1;
foreach (Shop.Model.Lebi_Language MFtr in MFtrs){%>
            <dd <%if (MFtr_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=MFtr.id%>,'<%=MFtr.Code%>','<%=MFtr.Path%>');"><%if (MFtr.ImageUrl!=""){%><img src="<%=Image(MFtr.ImageUrl) %>" /><%}%><%=MFtr.Name%></a></dd>
            <%MFtr_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int WFPN_index=1;
List<Lebi_Currency> WFPNs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency WFPN in WFPNs){%>
            <dd <%if (WFPN_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=WFPN.id%>,'<%=WFPN.Code%>',<%=WFPN.ExchangeRate%>,'<%=WFPN.Msige%>','<%=WFPN.DecimalLength%>');"><%=WFPN.Code%></a></dd>
            <%WFPN_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int LSHn_index=1;
List<Lebi_Searchkey> LSHns = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey LSHn in LSHns){%><%=Lang(LSHn.Name)%><%LSHn_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int FkEy_index=1;
List<Lebi_Searchkey> FkEys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey FkEy in FkEys){%>
<%if (FkEy.Type==1){ %><a href="<%=URL("P_Search",""+Lang(FkEy.Name)+"") %>"><%}else{ %><a href="<%=FkEy.URL%>" target="_blank"><%} %><span><%=Lang(FkEy.Name)%></span></a>
<%FkEy_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int rWXt_index=1;
List<Lebi_Page> rWXts = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page rWXt in rWXts){%>
<a class="menu" href="<%=URL("","",rWXt.url)%>"><span><%=rWXt.Name%></span></a>
<%rWXt_index++;}%>
...
<div id="shopcategory">
    ...
    <div class="categories">
        <%Table="Lebi_Supplier_ProductType";Where="parentid = 0 and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int uvAw_index=1;
List<Lebi_Supplier_ProductType> uvAws = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType uvAw in uvAws){%>
        <dl <%if ((parentcid == 0 && uvAw_index == 1) || parentcid == uvAw.id){ %>class="current" <%} %>>
            <dt class="double">
                <label class="pic">
                </label>
                <a href="<%=URL("P_ShopProductCategory",""+id+","+uvAw.id+"") %>">
                    <%=Lang(uvAw.Name) %></a></dt>
            <dd>
                <ul>
                    <%Table="Lebi_Supplier_ProductType";Where="parentid = "+uvAw.id+" and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int eVXs_index=1;
List<Lebi_Supplier_ProductType> eVXss = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType eVXs in eVXss){%>
                    <li class="<%=cid==eVXs.id?"currentlink":"" %>"><a href="<%=URL("P_ShopProductCategory",""+id+","+eVXs.id+"") %>">
                        <%=Lang(eVXs.Name) %></a></li>
                    <%eVXs_index++;}%>
                </ul>
            </dd>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int yUjy_index=1;
List<Lebi_FriendLink> yUjys = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink yUjy in yUjys){%>
            
                <% if (yUjy.Logo != "" && yUjy.IsPic == 1){ %><a href="<%=yUjy.Url%>" target="_blank"><img src="<%=Image(yUjy.Logo) %>" alt="<%=yUjy.Name%>" /></a><%}else{%><a href="<%=yUjy.Url%>" target="_blank"><%=yUjy.Name%></a><%} %>     
