...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_ShopProductCategory"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> ntKes=Languages();RecordCount=ntKes.Count;int ntKe_index=1;
foreach (Shop.Model.Lebi_Language ntKe in ntKes){%>
            <dd <%if (ntKe_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=ntKe.id%>,'<%=ntKe.Code%>','<%=ntKe.Path%>');"><%if (ntKe.ImageUrl!=""){%><img src="<%=Image(ntKe.ImageUrl) %>" /><%}%><%=ntKe.Name%></a></dd>
            <%ntKe_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int iPPx_index=1;
List<Lebi_Currency> iPPxs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency iPPx in iPPxs){%>
            <dd <%if (iPPx_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=iPPx.id%>,'<%=iPPx.Code%>',<%=iPPx.ExchangeRate%>,'<%=iPPx.Msige%>','<%=iPPx.DecimalLength%>');"><%=iPPx.Code%></a></dd>
            <%iPPx_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int NJLm_index=1;
List<Lebi_Searchkey> NJLms = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey NJLm in NJLms){%><%=Lang(NJLm.Name)%><%NJLm_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int CcdC_index=1;
List<Lebi_Searchkey> CcdCs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey CcdC in CcdCs){%>
<%if (CcdC.Type==1){ %><a href="<%=URL("P_Search",""+Lang(CcdC.Name)+"") %>"><%}else{ %><a href="<%=CcdC.URL%>" target="_blank"><%} %><span><%=Lang(CcdC.Name)%></span></a>
<%CcdC_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int tWrh_index=1;
List<Lebi_Page> tWrhs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page tWrh in tWrhs){%>
<a class="menu" href="<%=URL("","",tWrh.url)%>"><span><%=tWrh.Name%></span></a>
<%tWrh_index++;}%>
...
<div class="categories">
    <%Table="Lebi_Supplier_ProductType";Where="parentid = 0 and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int kIxH_index=1;
List<Lebi_Supplier_ProductType> kIxHs = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType kIxH in kIxHs){%>
    <dl <%if ((parentcid == 0 && kIxH_index == 1) || parentcid == kIxH.id){ %>class="current" <%} %>>
    <dt class="double">
        <label class="pic"></label>
        <a href="<%=URL("P_ShopProductCategory",""+id+","+kIxH.id+"") %>"><%=Lang(kIxH.Name) %></a>
    </dt>
    <dd>
        <ul>
            <%Table="Lebi_Supplier_ProductType";Where="parentid = "+kIxH.id+" and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int kwHQ_index=1;
List<Lebi_Supplier_ProductType> kwHQs = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType kwHQ in kwHQs){%>
            <li class="<%=cid==kwHQ.id?"currentlink":"" %>"><a href="<%=URL("P_ShopProductCategory",""+id+","+kwHQ.id+"") %>">
                <%=Lang(kwHQ.Name) %></a></li>
            <%kwHQ_index++;}%>
        </ul>
    </dd>
...
<div id="filter">
	<div class="fore1">
		<dl class="order">
			<dt>排序：</dt>
			<dd <%if(sort == "" || sort == "1" || sort == "1a"){ %>class="curr <%if(sort == "" || sort == "1"){ %>up<%}else if(sort == "1a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_ShopProductCategory",""+id+","+cid+","+list+",1"+ordertmp+"") %>">銷量</a><b></b></dd>
			<dd <%if(sort == "2" || sort == "2a"){ %>class="curr <%if(sort == "2"){ %>up<%}else if(sort == "2a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_ShopProductCategory",id+","+cid+","+list+",2"+ordertmp+"") %>">價格</a><b></b></dd>
			<dd <%if(sort == "3" || sort == "3a"){ %>class="curr <%if(sort == "3"){ %>up<%}else if(sort == "3a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_ShopProductCategory",id+","+cid+","+list+",3"+ordertmp+"") %>">評價數</a><b></b></dd>
			<dd <%if(sort == "4" || sort == "4a"){ %>class="curr <%if(sort == "4"){ %>up<%}else if(sort == "4a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_ShopProductCategory",id+","+cid+","+list+",4"+ordertmp+"") %>">上架時間</a><b></b></dd>
		</dl>
        <div class="list-cutover">
			...
			<a id="grid-unselected"  href="<%=URL("P_ShopProductCategory",""+id+","+cid+",1,"+ sort +"") %>" class="modfiy-url grid-unselected <%=(list=="1" || list=="")?"grid-curr":"" %>"><b></b>網格</a>
		</div>
    ...
    <div class="productlist loadlist"><ul>
        ...
        <li class="grid loadli">
            ...
            <div class="proinfo">
              ...
              <div class="comment"><a href="<%=URL("P_Product",pro.id) %>">已有<%=pro.Count_Comment %>人评价</a><span>|</span><%if(ProductStock(pro)<1){ %>已售罄<%}else { %>有貨<%} %></div>
            </div>
        </li>
        <%}else{ %>
        <li class="list loadli">
            ...
            <div class="proinfo">
            <%if (pro.Price_Market > 0){ %><div class="marketprice"><i>市場價：</i><strong><%=FormatMoney(pro.Price_Market) %></strong></div><%} %>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int EnRG_index=1;
List<Lebi_FriendLink> EnRGs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink EnRG in EnRGs){%>
            
                <% if (EnRG.Logo != "" && EnRG.IsPic == 1){ %><a href="<%=EnRG.Url%>" target="_blank"><img src="<%=Image(EnRG.Logo) %>" alt="<%=EnRG.Name%>" /></a><%}else{%><a href="<%=EnRG.Url%>" target="_blank"><%=EnRG.Name%></a><%} %>     
