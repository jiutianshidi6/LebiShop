...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_Register"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> QakAs=Languages();RecordCount=QakAs.Count;int QakA_index=1;
foreach (Shop.Model.Lebi_Language QakA in QakAs){%>
            <dd <%if (QakA_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=QakA.id%>,'<%=QakA.Code%>','<%=QakA.Path%>');"><%if (QakA.ImageUrl!=""){%><img src="<%=Image(QakA.ImageUrl) %>" /><%}%><%=QakA.Name%></a></dd>
            <%QakA_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int tPMl_index=1;
List<Lebi_Currency> tPMls = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency tPMl in tPMls){%>
            <dd <%if (tPMl_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=tPMl.id%>,'<%=tPMl.Code%>',<%=tPMl.ExchangeRate%>,'<%=tPMl.Msige%>','<%=tPMl.DecimalLength%>');"><%=tPMl.Code%></a></dd>
            <%tPMl_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int rbOq_index=1;
List<Lebi_Searchkey> rbOqs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey rbOq in rbOqs){%><%=Lang(rbOq.Name)%><%rbOq_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int kEQo_index=1;
List<Lebi_Searchkey> kEQos = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey kEQo in kEQos){%>
<%if (kEQo.Type==1){ %><a href="<%=URL("P_Search",""+Lang(kEQo.Name)+"") %>"><%}else{ %><a href="<%=kEQo.URL%>" target="_blank"><%} %><span><%=Lang(kEQo.Name)%></span></a>
<%kEQo_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Kfdf_index=1;
List<Lebi_Page> Kfdfs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Kfdf in Kfdfs){%>
<a class="menu" href="<%=URL("","",Kfdf.url)%>"><span><%=Kfdf.Name%></span></a>
<%Kfdf_index++;}%>
...
<div class="nbbox clearfix">
    <div class="user">
        <div class="reg clearfix">
            ...
            <dl>
                <dt><font color="red">*</font>登錄帳號：</dt>
                ...
                <span id="FormUserName" class="FormALT">4-20個字符，可以是字母、數字或中文</span>
                </dd>
            </dl>
            <dl>
                <dt><font color="red">*</font>帳號密碼：</dt>
                <dd><input type="password" shop="true" name="Password" id="Password" style="width: 200px;" maxlength="16" class="input" min="6" />
                <span id="FormPassword" class="FormALT">密碼區分大小寫</span>
                </dd>
            </dl>
            <dl>
                <dt><font color="red">*</font>確認密碼：</dt>
                <dd><input type="password" name="Password1" shop="true" id="Password1" style="width: 200px;" maxlength="16" class="input" />
                <span id="FormPassword1" class="FormALT">請再次填寫密碼</span>
                </dd>
            </dl>
            <dl>
                <dt><%if(Checkmobilephone){ %><font color="red">*</font><%} %>手機號碼：</dt>
                <dd><input type="text" shop="true" name="MobilePhone" id="MobilePhone" <%if(Checkmobilephone){ %>min="notnull"<%} %> style="width: 200px;" class="input" />
                    <%if(Checkmobilephone){ %>
                    <%if (CurrentSite.IsMobile==0){ %>&nbsp;<%}else{%><br /><%}%>驗證碼：
...
<%}}%>
            ...
            <dl>
                <dt><font color="red">*</font>驗證碼：</dt>
                <dd><input name="verifycode" type="text" shop="true" id="verifycode" shop="true" size="6"
 class="input" /><img class="verifycode" id="img1" src="<%=WebPath%>/code.aspx" /><img src="/Theme/system/images/reload.gif" class="refresh" title="點擊刷新驗證碼" onclick="refresh('img1')" /></dd>
            </dl>
            <%} %>
            <%} %>
            <dl>
                <dt></dt>
                <dd><label><input type="checkbox" name="agree" id="agree" />我已閱讀並同意<a href="<%=URL("P_Help","$,agreement")%>" target="_blank">《註冊協議》</a></label>
                <label><input type="checkbox" name="IsOpen" id="IsOpen" onclick="OpenMore();" />填寫詳細資料</label></dd>
            </dl>
            ...
            <dl class="dl-btn">
                <dt></dt>
                <dd><a href="javascript:void(0)" onclick="submit();" class="btn btn-6"><s></s>提交註冊</a></dd>
            </dl>
            </div>
            </div>
        </div>
    ...
    function Check_username() {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == "OK") { CheckOK("UserName", "用戶名可以註冊"); } else { CheckNO("UserName", "用戶名已被註冊"); return false; }
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int yxUY_index=1;
List<Lebi_FriendLink> yxUYs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink yxUY in yxUYs){%>
            
                <% if (yxUY.Logo != "" && yxUY.IsPic == 1){ %><a href="<%=yxUY.Url%>" target="_blank"><img src="<%=Image(yxUY.Logo) %>" alt="<%=yxUY.Name%>" /></a><%}else{%><a href="<%=yxUY.Url%>" target="_blank"><%=yxUY.Name%></a><%} %>     
