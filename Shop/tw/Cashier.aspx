...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_Cashier"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> JOlns=Languages();RecordCount=JOlns.Count;int JOln_index=1;
foreach (Shop.Model.Lebi_Language JOln in JOlns){%>
            <dd <%if (JOln_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=JOln.id%>,'<%=JOln.Code%>','<%=JOln.Path%>');"><%if (JOln.ImageUrl!=""){%><img src="<%=Image(JOln.ImageUrl) %>" /><%}%><%=JOln.Name%></a></dd>
            <%JOln_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int VBoy_index=1;
List<Lebi_Currency> VBoys = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency VBoy in VBoys){%>
            <dd <%if (VBoy_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=VBoy.id%>,'<%=VBoy.Code%>',<%=VBoy.ExchangeRate%>,'<%=VBoy.Msige%>','<%=VBoy.DecimalLength%>');"><%=VBoy.Code%></a></dd>
            <%VBoy_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int NGOV_index=1;
List<Lebi_Searchkey> NGOVs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey NGOV in NGOVs){%><%=Lang(NGOV.Name)%><%NGOV_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int wmFv_index=1;
List<Lebi_Searchkey> wmFvs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey wmFv in wmFvs){%>
<%if (wmFv.Type==1){ %><a href="<%=URL("P_Search",""+Lang(wmFv.Name)+"") %>"><%}else{ %><a href="<%=wmFv.URL%>" target="_blank"><%} %><span><%=Lang(wmFv.Name)%></span></a>
<%wmFv_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int MEAY_index=1;
List<Lebi_Page> MEAYs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page MEAY in MEAYs){%>
<a class="menu" href="<%=URL("","",MEAY.url)%>"><span><%=MEAY.Name%></span></a>
<%MEAY_index++;}%>
...
<ul>
	...
	<li class="current"><span class="con">成功提交訂單</span></li>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int UBsx_index=1;
List<Lebi_FriendLink> UBsxs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink UBsx in UBsxs){%>
            
                <% if (UBsx.Logo != "" && UBsx.IsPic == 1){ %><a href="<%=UBsx.Url%>" target="_blank"><img src="<%=Image(UBsx.Logo) %>" alt="<%=UBsx.Name%>" /></a><%}else{%><a href="<%=UBsx.Url%>" target="_blank"><%=UBsx.Name%></a><%} %>     
