...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_Help"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> bXMKs=Languages();RecordCount=bXMKs.Count;int bXMK_index=1;
foreach (Shop.Model.Lebi_Language bXMK in bXMKs){%>
            <dd <%if (bXMK_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=bXMK.id%>,'<%=bXMK.Code%>','<%=bXMK.Path%>');"><%if (bXMK.ImageUrl!=""){%><img src="<%=Image(bXMK.ImageUrl) %>" /><%}%><%=bXMK.Name%></a></dd>
            <%bXMK_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int CEBR_index=1;
List<Lebi_Currency> CEBRs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency CEBR in CEBRs){%>
            <dd <%if (CEBR_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=CEBR.id%>,'<%=CEBR.Code%>',<%=CEBR.ExchangeRate%>,'<%=CEBR.Msige%>','<%=CEBR.DecimalLength%>');"><%=CEBR.Code%></a></dd>
            <%CEBR_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int tIhd_index=1;
List<Lebi_Searchkey> tIhds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey tIhd in tIhds){%><%=Lang(tIhd.Name)%><%tIhd_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int iCdM_index=1;
List<Lebi_Searchkey> iCdMs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey iCdM in iCdMs){%>
<%if (iCdM.Type==1){ %><a href="<%=URL("P_Search",""+Lang(iCdM.Name)+"") %>"><%}else{ %><a href="<%=iCdM.URL%>" target="_blank"><%} %><span><%=Lang(iCdM.Name)%></span></a>
<%iCdM_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int uuWa_index=1;
List<Lebi_Page> uuWas = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page uuWa in uuWas){%>
<a class="menu" href="<%=URL("","",uuWa.url)%>"><span><%=uuWa.Name%></span></a>
<%uuWa_index++;}%>
...
<div class="mc">
    ...
    if (type !=""){
        ...
        %>
    <%Table="Lebi_Page";Where=""+strwhere+"";PageSize=999;pageindex=Rint("page");RecordCount=B_Lebi_Page.Counts(Where);int KSAf_index=1;
List<Lebi_Page> KSAfs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page KSAf in KSAfs){%>
    <div class="nodeitem clearfix" id="help<%=KSAf.id%>">
    <div class="mt">
        <h2><font style="color:<%=KSAf.NameColor%>"><%=KSAf.Name%></font></h2>
    </div>
    <%if (KSAf.Description != ""){ %><div class="tips"><%=KSAf.Description%></div><%} %>
    <div class="content">
    <%=ImgSrc(KSAf.Content,CurrentLanguage.IsLazyLoad) %>
    </div>
    </div>
    <%KSAf_index++;}%>
    <%} %>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int KiMn_index=1;
List<Lebi_FriendLink> KiMns = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink KiMn in KiMns){%>
            
                <% if (KiMn.Logo != "" && KiMn.IsPic == 1){ %><a href="<%=KiMn.Url%>" target="_blank"><img src="<%=Image(KiMn.Logo) %>" alt="<%=KiMn.Name%>" /></a><%}else{%><a href="<%=KiMn.Url%>" target="_blank"><%=KiMn.Name%></a><%} %>     
