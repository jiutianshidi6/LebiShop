...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_Search"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> zwmDs=Languages();RecordCount=zwmDs.Count;int zwmD_index=1;
foreach (Shop.Model.Lebi_Language zwmD in zwmDs){%>
            <dd <%if (zwmD_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=zwmD.id%>,'<%=zwmD.Code%>','<%=zwmD.Path%>');"><%if (zwmD.ImageUrl!=""){%><img src="<%=Image(zwmD.ImageUrl) %>" /><%}%><%=zwmD.Name%></a></dd>
            <%zwmD_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int gEec_index=1;
List<Lebi_Currency> gEecs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency gEec in gEecs){%>
            <dd <%if (gEec_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=gEec.id%>,'<%=gEec.Code%>',<%=gEec.ExchangeRate%>,'<%=gEec.Msige%>','<%=gEec.DecimalLength%>');"><%=gEec.Code%></a></dd>
            <%gEec_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int JyjE_index=1;
List<Lebi_Searchkey> JyjEs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey JyjE in JyjEs){%><%=Lang(JyjE.Name)%><%JyjE_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int AeJF_index=1;
List<Lebi_Searchkey> AeJFs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey AeJF in AeJFs){%>
<%if (AeJF.Type==1){ %><a href="<%=URL("P_Search",""+Lang(AeJF.Name)+"") %>"><%}else{ %><a href="<%=AeJF.URL%>" target="_blank"><%} %><span><%=Lang(AeJF.Name)%></span></a>
<%AeJF_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int BxNE_index=1;
List<Lebi_Page> BxNEs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page BxNE in BxNEs){%>
<a class="menu" href="<%=URL("","",BxNE.url)%>"><span><%=BxNE.Name%></span></a>
<%BxNE_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int qqvP_index=1;
List<Lebi_Page> qqvPs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page qqvP in qqvPs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",qqvP.id,qqvP.url)%>" target="<%=qqvP.target%>" title="<%=qqvP.Name%>"><span style="color:<%=qqvP.NameColor%>"><%=qqvP.Name%></span></a></li>
        <%qqvP_index++;}%>
    </ul>
</div>
</div>
...
<div id="filter">
	<div class="fore1">
		<dl class="order">
			<dt>排序：</dt>
			<dd <%if(sort == "1" || sort == "1a"){ %>class="curr <%if(sort == "1"){ %>up<%}else if(sort == "1a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Search",""+keyword+","+list+",1"+ordertmp+"") %>">銷量</a><b></b></dd>
			<dd <%if(sort == "2" || sort == "2a"){ %>class="curr <%if(sort == "2"){ %>up<%}else if(sort == "2a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Search",keyword+","+list+",2"+ordertmp+"") %>">價格</a><b></b></dd>
			<dd <%if(sort == "3" || sort == "3a"){ %>class="curr <%if(sort == "3"){ %>up<%}else if(sort == "3a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Search",keyword+","+list+",3"+ordertmp+"") %>">評價數</a><b></b></dd>
			<dd <%if(sort == "4" || sort == "4a"){ %>class="curr <%if(sort == "4"){ %>up<%}else if(sort == "4a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Search",keyword+","+list+",4"+ordertmp+"") %>">上架時間</a><b></b></dd>
		</dl>
        <div class="list-cutover">
			...
			<a id="grid-unselected"  href="<%=URL("P_Search",keyword+",1,"+ sort) %>" class="modfiy-url grid-unselected <%=(list=="1" || list=="")?"grid-curr":"" %>"><b></b>網格</a>
		</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int oxLc_index=1;
List<Lebi_FriendLink> oxLcs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink oxLc in oxLcs){%>
            
                <% if (oxLc.Logo != "" && oxLc.IsPic == 1){ %><a href="<%=oxLc.Url%>" target="_blank"><img src="<%=Image(oxLc.Logo) %>" alt="<%=oxLc.Name%>" /></a><%}else{%><a href="<%=oxLc.Url%>" target="_blank"><%=oxLc.Name%></a><%} %>     
