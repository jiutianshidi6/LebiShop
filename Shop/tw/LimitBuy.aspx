...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_LimitBuy"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> qqNds=Languages();RecordCount=qqNds.Count;int qqNd_index=1;
foreach (Shop.Model.Lebi_Language qqNd in qqNds){%>
            <dd <%if (qqNd_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=qqNd.id%>,'<%=qqNd.Code%>','<%=qqNd.Path%>');"><%if (qqNd.ImageUrl!=""){%><img src="<%=Image(qqNd.ImageUrl) %>" /><%}%><%=qqNd.Name%></a></dd>
            <%qqNd_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int uKXA_index=1;
List<Lebi_Currency> uKXAs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency uKXA in uKXAs){%>
            <dd <%if (uKXA_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=uKXA.id%>,'<%=uKXA.Code%>',<%=uKXA.ExchangeRate%>,'<%=uKXA.Msige%>','<%=uKXA.DecimalLength%>');"><%=uKXA.Code%></a></dd>
            <%uKXA_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int mkxc_index=1;
List<Lebi_Searchkey> mkxcs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey mkxc in mkxcs){%><%=Lang(mkxc.Name)%><%mkxc_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int onEn_index=1;
List<Lebi_Searchkey> onEns = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey onEn in onEns){%>
<%if (onEn.Type==1){ %><a href="<%=URL("P_Search",""+Lang(onEn.Name)+"") %>"><%}else{ %><a href="<%=onEn.URL%>" target="_blank"><%} %><span><%=Lang(onEn.Name)%></span></a>
<%onEn_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int oJQm_index=1;
List<Lebi_Page> oJQms = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page oJQm in oJQms){%>
<a class="menu" href="<%=URL("","",oJQm.url)%>"><span><%=oJQm.Name%></span></a>
<%oJQm_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Pjmd_index=1;
List<Lebi_Page> Pjmds = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Pjmd in Pjmds){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",Pjmd.id,Pjmd.url)%>" target="<%=Pjmd.target%>" title="<%=Pjmd.Name%>"><span style="color:<%=Pjmd.NameColor%>"><%=Pjmd.Name%></span></a></li>
        <%Pjmd_index++;}%>
    </ul>
</div>
</div>
...
<div id="filter">
	<div class="fore1">
		<dl class="order">
			<dt>排序：</dt>
			<dd <%if(sort == "" || sort == "1" || sort == "1a"){ %>class="curr <%if(sort == "" || sort == "1"){ %>up<%}else if(sort == "1a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_LimitBuy",""+list+",1"+ordertmp+"") %>">銷量</a><b></b></dd>
			<dd <%if(sort == "2" || sort == "2a"){ %>class="curr <%if(sort == "2"){ %>up<%}else if(sort == "2a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_LimitBuy",""+list+",2"+ordertmp+"") %>">價格</a><b></b></dd>
			<dd <%if(sort == "3" || sort == "3a"){ %>class="curr <%if(sort == "3"){ %>up<%}else if(sort == "3a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_LimitBuy",""+list+",3"+ordertmp+"") %>">評價數</a><b></b></dd>
			<dd <%if(sort == "4" || sort == "4a"){ %>class="curr <%if(sort == "4"){ %>up<%}else if(sort == "4a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_LimitBuy",""+list+",4"+ordertmp+"") %>">上架時間</a><b></b></dd>
		</dl>
        <div class="list-cutover">
			<a id="list-unselected"  href="<%=URL("P_LimitBuy","2,"+ sort) %>" class="modfiy-url list-unselected <%=list=="2"?"list-curr":"" %>"><b></b>列表</a>
			<a id="grid-unselected"  href="<%=URL("P_LimitBuy","1,"+ sort) %>" class="modfiy-url grid-unselected <%=(list=="1" || list=="")?"grid-curr":"" %>"><b></b>網格</a>
		</div>
...
<div class="mc clearfix">
    <div class="productlist limitbuylist loadlist"><ul>
        ...
        <li class="grid loadli">
            ...
            <%if (pro.Price > 0){ %><div class="marketprice"><i>銷售價：</i><strong><%=FormatMoney(pro.Price) %></strong></div><%} %>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int xqFa_index=1;
List<Lebi_FriendLink> xqFas = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink xqFa in xqFas){%>
            
                <% if (xqFa.Logo != "" && xqFa.IsPic == 1){ %><a href="<%=xqFa.Url%>" target="_blank"><img src="<%=Image(xqFa.Logo) %>" alt="<%=xqFa.Name%>" /></a><%}else{%><a href="<%=xqFa.Url%>" target="_blank"><%=xqFa.Name%></a><%} %>     
