...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserAgentList"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> wIGBs=Languages();RecordCount=wIGBs.Count;int wIGB_index=1;
foreach (Shop.Model.Lebi_Language wIGB in wIGBs){%>
            <dd <%if (wIGB_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=wIGB.id%>,'<%=wIGB.Code%>','<%=wIGB.Path%>');"><%if (wIGB.ImageUrl!=""){%><img src="<%=Image(wIGB.ImageUrl) %>" /><%}%><%=wIGB.Name%></a></dd>
            <%wIGB_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int sbVR_index=1;
List<Lebi_Currency> sbVRs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency sbVR in sbVRs){%>
            <dd <%if (sbVR_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=sbVR.id%>,'<%=sbVR.Code%>',<%=sbVR.ExchangeRate%>,'<%=sbVR.Msige%>','<%=sbVR.DecimalLength%>');"><%=sbVR.Code%></a></dd>
            <%sbVR_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int BHmy_index=1;
List<Lebi_Searchkey> BHmys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey BHmy in BHmys){%><%=Lang(BHmy.Name)%><%BHmy_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Hvbj_index=1;
List<Lebi_Searchkey> Hvbjs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Hvbj in Hvbjs){%>
<%if (Hvbj.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Hvbj.Name)+"") %>"><%}else{ %><a href="<%=Hvbj.URL%>" target="_blank"><%} %><span><%=Lang(Hvbj.Name)%></span></a>
<%Hvbj_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int yLqS_index=1;
List<Lebi_Page> yLqSs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page yLqS in yLqSs){%>
<a class="menu" href="<%=URL("","",yLqS.url)%>"><span><%=yLqS.Name%></span></a>
<%yLqS_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int KjqQ_index=1;
List<Lebi_FriendLink> KjqQs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink KjqQ in KjqQs){%>
            
                <% if (KjqQ.Logo != "" && KjqQ.IsPic == 1){ %><a href="<%=KjqQ.Url%>" target="_blank"><img src="<%=Image(KjqQ.Logo) %>" alt="<%=KjqQ.Name%>" /></a><%}else{%><a href="<%=KjqQ.Url%>" target="_blank"><%=KjqQ.Name%></a><%} %>     
