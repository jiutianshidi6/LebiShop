...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserAgent"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> GAIHs=Languages();RecordCount=GAIHs.Count;int GAIH_index=1;
foreach (Shop.Model.Lebi_Language GAIH in GAIHs){%>
            <dd <%if (GAIH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=GAIH.id%>,'<%=GAIH.Code%>','<%=GAIH.Path%>');"><%if (GAIH.ImageUrl!=""){%><img src="<%=Image(GAIH.ImageUrl) %>" /><%}%><%=GAIH.Name%></a></dd>
            <%GAIH_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int iJYW_index=1;
List<Lebi_Currency> iJYWs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency iJYW in iJYWs){%>
            <dd <%if (iJYW_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=iJYW.id%>,'<%=iJYW.Code%>',<%=iJYW.ExchangeRate%>,'<%=iJYW.Msige%>','<%=iJYW.DecimalLength%>');"><%=iJYW.Code%></a></dd>
            <%iJYW_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hiYU_index=1;
List<Lebi_Searchkey> hiYUs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hiYU in hiYUs){%><%=Lang(hiYU.Name)%><%hiYU_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ftwI_index=1;
List<Lebi_Searchkey> ftwIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ftwI in ftwIs){%>
<%if (ftwI.Type==1){ %><a href="<%=URL("P_Search",""+Lang(ftwI.Name)+"") %>"><%}else{ %><a href="<%=ftwI.URL%>" target="_blank"><%} %><span><%=Lang(ftwI.Name)%></span></a>
<%ftwI_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int fbHU_index=1;
List<Lebi_Page> fbHUs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page fbHU in fbHUs){%>
<a class="menu" href="<%=URL("","",fbHU.url)%>"><span><%=fbHU.Name%></span></a>
<%fbHU_index++;}%>
...
<div class="userbox">
    ...
    <dl>
        <dt><%=Tag("我的推广链接")%>：</dt>
        <dd><span  id="myurl">http://<%=SYS.Domain==""?ShopCache.GetMainSite().Domain:SYS.Domain%>/tw/?v=<%=CurrentUser.id %></span></dd>
    </dl>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int zBdi_index=1;
List<Lebi_FriendLink> zBdis = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink zBdi in zBdis){%>
            
                <% if (zBdi.Logo != "" && zBdi.IsPic == 1){ %><a href="<%=zBdi.Url%>" target="_blank"><img src="<%=Image(zBdi.Logo) %>" alt="<%=zBdi.Name%>" /></a><%}else{%><a href="<%=zBdi.Url%>" target="_blank"><%=zBdi.Name%></a><%} %>     
