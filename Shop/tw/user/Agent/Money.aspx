...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserAgentMoney"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> xflls=Languages();RecordCount=xflls.Count;int xfll_index=1;
foreach (Shop.Model.Lebi_Language xfll in xflls){%>
            <dd <%if (xfll_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=xfll.id%>,'<%=xfll.Code%>','<%=xfll.Path%>');"><%if (xfll.ImageUrl!=""){%><img src="<%=Image(xfll.ImageUrl) %>" /><%}%><%=xfll.Name%></a></dd>
            <%xfll_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int qWxz_index=1;
List<Lebi_Currency> qWxzs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency qWxz in qWxzs){%>
            <dd <%if (qWxz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=qWxz.id%>,'<%=qWxz.Code%>',<%=qWxz.ExchangeRate%>,'<%=qWxz.Msige%>','<%=qWxz.DecimalLength%>');"><%=qWxz.Code%></a></dd>
            <%qWxz_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hprF_index=1;
List<Lebi_Searchkey> hprFs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hprF in hprFs){%><%=Lang(hprF.Name)%><%hprF_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int kwuC_index=1;
List<Lebi_Searchkey> kwuCs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey kwuC in kwuCs){%>
<%if (kwuC.Type==1){ %><a href="<%=URL("P_Search",""+Lang(kwuC.Name)+"") %>"><%}else{ %><a href="<%=kwuC.URL%>" target="_blank"><%} %><span><%=Lang(kwuC.Name)%></span></a>
<%kwuC_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int cQtW_index=1;
List<Lebi_Page> cQtWs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page cQtW in cQtWs){%>
<a class="menu" href="<%=URL("","",cQtW.url)%>"><span><%=cQtW.Name%></span></a>
<%cQtW_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Bykj_index=1;
List<Lebi_FriendLink> Bykjs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Bykj in Bykjs){%>
            
                <% if (Bykj.Logo != "" && Bykj.IsPic == 1){ %><a href="<%=Bykj.Url%>" target="_blank"><img src="<%=Image(Bykj.Logo) %>" alt="<%=Bykj.Name%>" /></a><%}else{%><a href="<%=Bykj.Url%>" target="_blank"><%=Bykj.Name%></a><%} %>     
