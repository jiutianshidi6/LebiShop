...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserAccount"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> GIcws=Languages();RecordCount=GIcws.Count;int GIcw_index=1;
foreach (Shop.Model.Lebi_Language GIcw in GIcws){%>
            <dd <%if (GIcw_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=GIcw.id%>,'<%=GIcw.Code%>','<%=GIcw.Path%>');"><%if (GIcw.ImageUrl!=""){%><img src="<%=Image(GIcw.ImageUrl) %>" /><%}%><%=GIcw.Name%></a></dd>
            <%GIcw_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int NaKP_index=1;
List<Lebi_Currency> NaKPs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency NaKP in NaKPs){%>
            <dd <%if (NaKP_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=NaKP.id%>,'<%=NaKP.Code%>',<%=NaKP.ExchangeRate%>,'<%=NaKP.Msige%>','<%=NaKP.DecimalLength%>');"><%=NaKP.Code%></a></dd>
            <%NaKP_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ElIo_index=1;
List<Lebi_Searchkey> ElIos = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ElIo in ElIos){%><%=Lang(ElIo.Name)%><%ElIo_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ivmh_index=1;
List<Lebi_Searchkey> ivmhs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ivmh in ivmhs){%>
<%if (ivmh.Type==1){ %><a href="<%=URL("P_Search",""+Lang(ivmh.Name)+"") %>"><%}else{ %><a href="<%=ivmh.URL%>" target="_blank"><%} %><span><%=Lang(ivmh.Name)%></span></a>
<%ivmh_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int GNvY_index=1;
List<Lebi_Page> GNvYs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page GNvY in GNvYs){%>
<a class="menu" href="<%=URL("","",GNvY.url)%>"><span><%=GNvY.Name%></span></a>
<%GNvY_index++;}%>
...
<div class="userbox">
    ...
    <dl>
        ...
        <dd><%if(CurrentUser.bind_qq_id=="") {%>
            <a href="<%=Shop.Platform.QQ.Instance.LoginURL(backurl) %>" >綁定</a>
            <%}else{ %>
            <a href="javascript:void(0);" onclick="cancalbind('qq');" >取消綁定</a>
            <%} %></dd>
    </dl>
    <%} %>
    ...
    <dl>
        ...
        <dd><%if(CurrentUser.bind_weibo_id=="") {%>
            <a href="<%=Shop.Platform.Weibo.Instance.LoginURL(backurl) %>" >綁定</a>
            <%}else{ %>
            <a href="javascript:void(0);" onclick="cancalbind('weibo');" >取消綁定</a>
            <%} %></dd>
    </dl>
    <%} %>
    ...
    <dl>
        ...
        <dd><%if(CurrentUser.bind_taobao_id=="") {%>
            <a href="<%=Shop.Platform.Taobao.Instance.LoginURL(backurl) %>" >綁定</a>
            <%}else{ %>
            <a href="javascript:void(0);" onclick="cancalbind('taobao');" >取消綁定</a>
            <%} %></dd>
    </dl>
    <%} %>
    ...
    <dl>
        ...
        <dd><%if(CurrentUser.bind_facebook_id=="") {%>
            <a href="<%=Shop.Platform.Facebook.Instance.LoginURL(backurl) %>" >綁定</a>
            <%}else{ %>
            <a href="javascript:void(0);" onclick="cancalbind('facebook');" >取消綁定</a>
            <%} %></dd>
    </dl>
    <%} %>
    ...
    <dl>
        ...
        <dd><%if(CurrentUser.bind_weixin_id=="") {%>
            ...
            <%}else{ %>
            <a href="javascript:void(0);" onclick="cancalbind('weixin');" >取消綁定</a>
            <%} %></dd>
    </dl>
    <%} %>
    </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int pxfS_index=1;
List<Lebi_FriendLink> pxfSs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink pxfS in pxfSs){%>
            
                <% if (pxfS.Logo != "" && pxfS.IsPic == 1){ %><a href="<%=pxfS.Url%>" target="_blank"><img src="<%=Image(pxfS.Logo) %>" alt="<%=pxfS.Name%>" /></a><%}else{%><a href="<%=pxfS.Url%>" target="_blank"><%=pxfS.Name%></a><%} %>     
