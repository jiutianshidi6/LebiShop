<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.P_UserPoint" validateRequest="false"%>


<%@ Import Namespace="Shop.Bussiness" %>
<%@ Import Namespace="Shop.Model" %>
<%@ Import Namespace="System.Collections.Generic" %>
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserPoint"); %>
...
<title><%=ThemePageMeta("P_UserPoint","title")%></title>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Language" content="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0;" />
<meta name="keywords" content="<%=ThemePageMeta("P_UserPoint","keywords")%>" />
<meta name="description" content="<%=ThemePageMeta("P_UserPoint","description")%>" />
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> aSwLs=Languages();RecordCount=aSwLs.Count;int aSwL_index=1;
foreach (Shop.Model.Lebi_Language aSwL in aSwLs){%>
            <dd <%if (aSwL_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=aSwL.id%>,'<%=aSwL.Code%>','<%=aSwL.Path%>');"><%if (aSwL.ImageUrl!=""){%><img src="<%=Image(aSwL.ImageUrl) %>" /><%}%><%=aSwL.Name%></a></dd>
            <%aSwL_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int zBAv_index=1;
List<Lebi_Currency> zBAvs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency zBAv in zBAvs){%>
            <dd <%if (zBAv_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=zBAv.id%>,'<%=zBAv.Code%>',<%=zBAv.ExchangeRate%>,'<%=zBAv.Msige%>','<%=zBAv.DecimalLength%>');"><%=zBAv.Code%></a></dd>
            <%zBAv_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MplF_index=1;
List<Lebi_Searchkey> MplFs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MplF in MplFs){%><%=Lang(MplF.Name)%><%MplF_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ICMQ_index=1;
List<Lebi_Searchkey> ICMQs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ICMQ in ICMQs){%>
<%if (ICMQ.Type==1){ %><a href="<%=URL("P_Search",""+Lang(ICMQ.Name)+"") %>"><%}else{ %><a href="<%=ICMQ.URL%>" target="_blank"><%} %><span><%=Lang(ICMQ.Name)%></span></a>
<%ICMQ_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int xwKe_index=1;
List<Lebi_Page> xwKes = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page xwKe in xwKes){%>
<a class="menu" href="<%=URL("","",xwKe.url)%>"><span><%=xwKe.Name%></span></a>
<%xwKe_index++;}%>
...
<div class="user">
    <div class="searchbox clearfix">
        ...
        <a href="<%=URL("P_UserPointToMoney","") %>" class="btn btn-12"><s></s>積分轉換</a>
        <%} %>
    </div>
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        <tr>
            <th style="width:120px;">時間</th>
            <th style="width:80px;">積分</th>
            <th style="width:90px;">狀態</th>
            <th>備註</th>
        </tr>
        <%foreach (Shop.Model.Lebi_User_Point u_p in user_points)
        {%>
        <tr>
            <td><%=FormatTime(u_p.Time_Add)%></td>
            <td><%=u_p.Point%></td>
            <td><%=TypeName(u_p.Type_id_PointStatus)%></td>
            <td><%=u_p.Remark %></td>
        </tr>
        <%} %>
    </table>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int MAuq_index=1;
List<Lebi_FriendLink> MAuqs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink MAuq in MAuqs){%>
            
                <% if (MAuq.Logo != "" && MAuq.IsPic == 1){ %><a href="<%=MAuq.Url%>" target="_blank"><img src="<%=Image(MAuq.Logo) %>" alt="<%=MAuq.Name%>" /></a><%}else{%><a href="<%=MAuq.Url%>" target="_blank"><%=MAuq.Name%></a><%} %>     
