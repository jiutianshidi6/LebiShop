...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserOftenBuyDialog"); %>
...
<title><%=ThemePageMeta("P_UserOftenBuyDialog","title")%></title>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Language" content="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0;" />
<meta name="keywords" content="<%=ThemePageMeta("P_UserOftenBuyDialog","keywords")%>" />
<meta name="description" content="<%=ThemePageMeta("P_UserOftenBuyDialog","description")%>" />
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> IGUes=Languages();RecordCount=IGUes.Count;int IGUe_index=1;
foreach (Shop.Model.Lebi_Language IGUe in IGUes){%>
            <dd <%if (IGUe_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=IGUe.id%>,'<%=IGUe.Code%>','<%=IGUe.Path%>');"><%if (IGUe.ImageUrl!=""){%><img src="<%=Image(IGUe.ImageUrl) %>" /><%}%><%=IGUe.Name%></a></dd>
            <%IGUe_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int zOYr_index=1;
List<Lebi_Currency> zOYrs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency zOYr in zOYrs){%>
            <dd <%if (zOYr_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=zOYr.id%>,'<%=zOYr.Code%>',<%=zOYr.ExchangeRate%>,'<%=zOYr.Msige%>','<%=zOYr.DecimalLength%>');"><%=zOYr.Code%></a></dd>
            <%zOYr_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int USVt_index=1;
List<Lebi_Searchkey> USVts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey USVt in USVts){%><%=Lang(USVt.Name)%><%USVt_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int LIRe_index=1;
List<Lebi_Searchkey> LIRes = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey LIRe in LIRes){%>
<%if (LIRe.Type==1){ %><a href="<%=URL("P_Search",""+Lang(LIRe.Name)+"") %>"><%}else{ %><a href="<%=LIRe.URL%>" target="_blank"><%} %><span><%=Lang(LIRe.Name)%></span></a>
<%LIRe_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int qvSw_index=1;
List<Lebi_Page> qvSws = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page qvSw in qvSws){%>
<a class="menu" href="<%=URL("","",qvSw.url)%>"><span><%=qvSw.Name%></span></a>
<%qvSw_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int LOfK_index=1;
List<Lebi_FriendLink> LOfKs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink LOfK in LOfKs){%>
            
                <% if (LOfK.Logo != "" && LOfK.IsPic == 1){ %><a href="<%=LOfK.Url%>" target="_blank"><img src="<%=Image(LOfK.Logo) %>" alt="<%=LOfK.Name%>" /></a><%}else{%><a href="<%=LOfK.Url%>" target="_blank"><%=LOfK.Name%></a><%} %>     
