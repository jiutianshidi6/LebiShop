...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_Recharge"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> rNOos=Languages();RecordCount=rNOos.Count;int rNOo_index=1;
foreach (Shop.Model.Lebi_Language rNOo in rNOos){%>
            <dd <%if (rNOo_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=rNOo.id%>,'<%=rNOo.Code%>','<%=rNOo.Path%>');"><%if (rNOo.ImageUrl!=""){%><img src="<%=Image(rNOo.ImageUrl) %>" /><%}%><%=rNOo.Name%></a></dd>
            <%rNOo_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int aJDh_index=1;
List<Lebi_Currency> aJDhs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency aJDh in aJDhs){%>
            <dd <%if (aJDh_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=aJDh.id%>,'<%=aJDh.Code%>',<%=aJDh.ExchangeRate%>,'<%=aJDh.Msige%>','<%=aJDh.DecimalLength%>');"><%=aJDh.Code%></a></dd>
            <%aJDh_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int UYlF_index=1;
List<Lebi_Searchkey> UYlFs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey UYlF in UYlFs){%><%=Lang(UYlF.Name)%><%UYlF_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int qDzE_index=1;
List<Lebi_Searchkey> qDzEs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey qDzE in qDzEs){%>
<%if (qDzE.Type==1){ %><a href="<%=URL("P_Search",""+Lang(qDzE.Name)+"") %>"><%}else{ %><a href="<%=qDzE.URL%>" target="_blank"><%} %><span><%=Lang(qDzE.Name)%></span></a>
<%qDzE_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int dupn_index=1;
List<Lebi_Page> dupns = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page dupn in dupns){%>
<a class="menu" href="<%=URL("","",dupn.url)%>"><span><%=dupn.Name%></span></a>
<%dupn_index++;}%>
...
<div class="userbox">
    ...
    <dl>
        <dt>充值金額(<%=DefaultCurrency.Msige %>)：</dt>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int QjNc_index=1;
List<Lebi_FriendLink> QjNcs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink QjNc in QjNcs){%>
            
                <% if (QjNc.Logo != "" && QjNc.IsPic == 1){ %><a href="<%=QjNc.Url%>" target="_blank"><img src="<%=Image(QjNc.Logo) %>" alt="<%=QjNc.Name%>" /></a><%}else{%><a href="<%=QjNc.Url%>" target="_blank"><%=QjNc.Name%></a><%} %>     
