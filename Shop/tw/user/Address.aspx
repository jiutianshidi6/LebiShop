<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.P_UserAddress" validateRequest="false"%>


<%@ Import Namespace="Shop.Bussiness" %>
<%@ Import Namespace="Shop.Model" %>
<%@ Import Namespace="System.Collections.Generic" %>
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserAddress"); %>
...
<title><%=ThemePageMeta("P_UserAddress","title")%></title>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Language" content="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0;" />
<meta name="keywords" content="<%=ThemePageMeta("P_UserAddress","keywords")%>" />
<meta name="description" content="<%=ThemePageMeta("P_UserAddress","description")%>" />
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> Pyivs=Languages();RecordCount=Pyivs.Count;int Pyiv_index=1;
foreach (Shop.Model.Lebi_Language Pyiv in Pyivs){%>
            <dd <%if (Pyiv_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Pyiv.id%>,'<%=Pyiv.Code%>','<%=Pyiv.Path%>');"><%if (Pyiv.ImageUrl!=""){%><img src="<%=Image(Pyiv.ImageUrl) %>" /><%}%><%=Pyiv.Name%></a></dd>
            <%Pyiv_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int prBz_index=1;
List<Lebi_Currency> prBzs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency prBz in prBzs){%>
            <dd <%if (prBz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=prBz.id%>,'<%=prBz.Code%>',<%=prBz.ExchangeRate%>,'<%=prBz.Msige%>','<%=prBz.DecimalLength%>');"><%=prBz.Code%></a></dd>
            <%prBz_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int odVk_index=1;
List<Lebi_Searchkey> odVks = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey odVk in odVks){%><%=Lang(odVk.Name)%><%odVk_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Sdwm_index=1;
List<Lebi_Searchkey> Sdwms = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Sdwm in Sdwms){%>
<%if (Sdwm.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Sdwm.Name)+"") %>"><%}else{ %><a href="<%=Sdwm.URL%>" target="_blank"><%} %><span><%=Lang(Sdwm.Name)%></span></a>
<%Sdwm_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int zLpp_index=1;
List<Lebi_Page> zLpps = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page zLpp in zLpps){%>
<a class="menu" href="<%=URL("","",zLpp.url)%>"><span><%=zLpp.Name%></span></a>
<%zLpp_index++;}%>
...
<div class="user">
    <div class="searchbox clearfix">
        ...
        <a href="javascript:void(0)" onclick="search_();" class="btn btn-12"><s></s>查詢</a>
    </div>
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        <tr>
            <th style="width:30px;" class="selectAll">
                <a href="javascript:void(0);" onclick="$('input[name=\'id\']').attr('checked',!$(this).attr('checked'));$(this).attr('checked',!$(this).attr('checked'));">全選</a>
            </th>
            <th style="width:100px;">姓名</th>
            <th style="width:100px;">手機號碼</th>
            <th style="width:100px;">電話號碼</th>
            <th>詳細地址</th>
            <th style="width:60px;">狀態</th>
            <th style="width:60px;">操作</th>
        </tr>
        <%foreach (Shop.Model.Lebi_User_Address ua in user_addresss)
        {%>
        <tr>
            ...
            <td><%if (CurrentUser.User_Address_id == ua.id){ %>默認<%}else{ %>&nbsp;<%} %></td>
            <td><a href="<%=URL("P_UserAddressEdit",ua.id,"")%>">編輯</a>
            </td>
        </tr>
        <%} %>
    </table>
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>刪除</a><a href="<%=URL("P_UserAddressEdit","0","")%>" class="btn btn-7"><s></s>添加</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript">
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int xvAM_index=1;
List<Lebi_FriendLink> xvAMs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink xvAM in xvAMs){%>
            
                <% if (xvAM.Logo != "" && xvAM.IsPic == 1){ %><a href="<%=xvAM.Url%>" target="_blank"><img src="<%=Image(xvAM.Logo) %>" alt="<%=xvAM.Name%>" /></a><%}else{%><a href="<%=xvAM.Url%>" target="_blank"><%=xvAM.Name%></a><%} %>     
