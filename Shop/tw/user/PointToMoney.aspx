...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserPointToMoney"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> koIps=Languages();RecordCount=koIps.Count;int koIp_index=1;
foreach (Shop.Model.Lebi_Language koIp in koIps){%>
            <dd <%if (koIp_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=koIp.id%>,'<%=koIp.Code%>','<%=koIp.Path%>');"><%if (koIp.ImageUrl!=""){%><img src="<%=Image(koIp.ImageUrl) %>" /><%}%><%=koIp.Name%></a></dd>
            <%koIp_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int SJfg_index=1;
List<Lebi_Currency> SJfgs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency SJfg in SJfgs){%>
            <dd <%if (SJfg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=SJfg.id%>,'<%=SJfg.Code%>',<%=SJfg.ExchangeRate%>,'<%=SJfg.Msige%>','<%=SJfg.DecimalLength%>');"><%=SJfg.Code%></a></dd>
            <%SJfg_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int TbjG_index=1;
List<Lebi_Searchkey> TbjGs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey TbjG in TbjGs){%><%=Lang(TbjG.Name)%><%TbjG_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int GFeN_index=1;
List<Lebi_Searchkey> GFeNs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey GFeN in GFeNs){%>
<%if (GFeN.Type==1){ %><a href="<%=URL("P_Search",""+Lang(GFeN.Name)+"") %>"><%}else{ %><a href="<%=GFeN.URL%>" target="_blank"><%} %><span><%=Lang(GFeN.Name)%></span></a>
<%GFeN_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int AWun_index=1;
List<Lebi_Page> AWuns = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page AWun in AWuns){%>
<a class="menu" href="<%=URL("","",AWun.url)%>"><span><%=AWun.Name%></span></a>
<%AWun_index++;}%>
...
<div class="userbox">
    ...
    %>
    <dl>
        <dt><input type="radio" name="ptom" value="<%=v %>" checked /><%=val[0] %>積分：</dt>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int NAkf_index=1;
List<Lebi_FriendLink> NAkfs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink NAkf in NAkfs){%>
            
                <% if (NAkf.Logo != "" && NAkf.IsPic == 1){ %><a href="<%=NAkf.Url%>" target="_blank"><img src="<%=Image(NAkf.Logo) %>" alt="<%=NAkf.Name%>" /></a><%}else{%><a href="<%=NAkf.Url%>" target="_blank"><%=NAkf.Name%></a><%} %>     
