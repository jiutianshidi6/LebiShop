...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserCommentWrite"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> EnWAs=Languages();RecordCount=EnWAs.Count;int EnWA_index=1;
foreach (Shop.Model.Lebi_Language EnWA in EnWAs){%>
            <dd <%if (EnWA_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=EnWA.id%>,'<%=EnWA.Code%>','<%=EnWA.Path%>');"><%if (EnWA.ImageUrl!=""){%><img src="<%=Image(EnWA.ImageUrl) %>" /><%}%><%=EnWA.Name%></a></dd>
            <%EnWA_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int UDsG_index=1;
List<Lebi_Currency> UDsGs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency UDsG in UDsGs){%>
            <dd <%if (UDsG_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=UDsG.id%>,'<%=UDsG.Code%>',<%=UDsG.ExchangeRate%>,'<%=UDsG.Msige%>','<%=UDsG.DecimalLength%>');"><%=UDsG.Code%></a></dd>
            <%UDsG_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zCER_index=1;
List<Lebi_Searchkey> zCERs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zCER in zCERs){%><%=Lang(zCER.Name)%><%zCER_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int NBBp_index=1;
List<Lebi_Searchkey> NBBps = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey NBBp in NBBps){%>
<%if (NBBp.Type==1){ %><a href="<%=URL("P_Search",""+Lang(NBBp.Name)+"") %>"><%}else{ %><a href="<%=NBBp.URL%>" target="_blank"><%} %><span><%=Lang(NBBp.Name)%></span></a>
<%NBBp_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Emhd_index=1;
List<Lebi_Page> Emhds = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Emhd in Emhds){%>
<a class="menu" href="<%=URL("","",Emhd.url)%>"><span><%=Emhd.Name%></span></a>
<%Emhd_index++;}%>
...
<div class="nbbox clearfix">
    <div class="user">
        <ul class="tablist">
            <li><a href="<%=URL("P_UserComment","0","")%>"><span>已評價商品</span></a></li>
            <li class="selected"><a href="<%=URL("P_UserComment","1","")%>"><span>待評價商品<em>(<%=Count_Comment(0) %>)</em></span></a></li>
        </ul>
        ...
        <dl class="dl-btn">
            <dt></dt>
            <dd><a href="javascript:void(0)" onclick="submit();" class="btn btn-11"><s></s>發表評價</a><input type="hidden" id="Images" name="Images" value="" shop="true"/><input type="hidden" id="ImagesSmall" name="ImagesSmall" value="" shop="true"/></dd>
        </dl>
        </div>
    </div>
</div>
<script type="text/javascript">
    ...
    function insertimage(image,small)
    {
        ...
        str+='<span class="image-manage" ><a onclick="deleteimage(\''+image+'\',\''+small+'\');" href="javascript:void(0)">刪除</a></span>';
        str+='</li>'
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int eATH_index=1;
List<Lebi_FriendLink> eATHs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink eATH in eATHs){%>
            
                <% if (eATH.Logo != "" && eATH.IsPic == 1){ %><a href="<%=eATH.Url%>" target="_blank"><img src="<%=Image(eATH.Logo) %>" alt="<%=eATH.Name%>" /></a><%}else{%><a href="<%=eATH.Url%>" target="_blank"><%=eATH.Name%></a><%} %>     
