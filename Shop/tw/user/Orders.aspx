...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserOrders"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> UbYrs=Languages();RecordCount=UbYrs.Count;int UbYr_index=1;
foreach (Shop.Model.Lebi_Language UbYr in UbYrs){%>
            <dd <%if (UbYr_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=UbYr.id%>,'<%=UbYr.Code%>','<%=UbYr.Path%>');"><%if (UbYr.ImageUrl!=""){%><img src="<%=Image(UbYr.ImageUrl) %>" /><%}%><%=UbYr.Name%></a></dd>
            <%UbYr_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int ihdS_index=1;
List<Lebi_Currency> ihdSs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency ihdS in ihdSs){%>
            <dd <%if (ihdS_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=ihdS.id%>,'<%=ihdS.Code%>',<%=ihdS.ExchangeRate%>,'<%=ihdS.Msige%>','<%=ihdS.DecimalLength%>');"><%=ihdS.Code%></a></dd>
            <%ihdS_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int tmqy_index=1;
List<Lebi_Searchkey> tmqys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey tmqy in tmqys){%><%=Lang(tmqy.Name)%><%tmqy_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int YPsG_index=1;
List<Lebi_Searchkey> YPsGs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey YPsG in YPsGs){%>
<%if (YPsG.Type==1){ %><a href="<%=URL("P_Search",""+Lang(YPsG.Name)+"") %>"><%}else{ %><a href="<%=YPsG.URL%>" target="_blank"><%} %><span><%=Lang(YPsG.Name)%></span></a>
<%YPsG_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int cFes_index=1;
List<Lebi_Page> cFess = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page cFes in cFess){%>
<a class="menu" href="<%=URL("","",cFes.url)%>"><span><%=cFes.Name%></span></a>
<%cFes_index++;}%>
...
<div class="user">
    <ul class="tablist">
        <li <%if (status == ""){Response.Write("class=\"selected\"");} %>><a href="?status="><span>全部訂單</span></a></li>
        <li <%if (status == "1"){Response.Write("class=\"selected\"");} %>><a href="?status=1"><span>等待付款</span></a></li>
        <li <%if (status == "2"){Response.Write("class=\"selected\"");} %>><a href="?status=2"><span>等待收貨</span></a></li>
        <li <%if (status == "3"){Response.Write("class=\"selected\"");} %>><a href="?status=3"><span>已收貨</span></a></li>
    ...
    <div class="searchbox clearfix">
        <select id="status">
            ...
            <option value="2" <%if (status == "2"){Response.Write("class=\"selected\"");} %>>等待收貨</option>
            <option value="3" <%if (status == "3"){Response.Write("class=\"selected\"");} %>>已收貨</option>
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        ...
        <tr>
            ...
            <td>
                ...
                <br /><a href="<%=URL("P_UserReturnApply",o.id)%>" target="_blank">退貨</a>
                <%} %>
            </td>
        </tr>
        <%} %>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int DRev_index=1;
List<Lebi_FriendLink> DRevs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink DRev in DRevs){%>
            
                <% if (DRev.Logo != "" && DRev.IsPic == 1){ %><a href="<%=DRev.Url%>" target="_blank"><img src="<%=Image(DRev.Logo) %>" alt="<%=DRev.Name%>" /></a><%}else{%><a href="<%=DRev.Url%>" target="_blank"><%=DRev.Name%></a><%} %>     
