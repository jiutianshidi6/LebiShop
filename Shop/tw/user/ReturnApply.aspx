...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserReturnApply"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> dbXos=Languages();RecordCount=dbXos.Count;int dbXo_index=1;
foreach (Shop.Model.Lebi_Language dbXo in dbXos){%>
            <dd <%if (dbXo_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=dbXo.id%>,'<%=dbXo.Code%>','<%=dbXo.Path%>');"><%if (dbXo.ImageUrl!=""){%><img src="<%=Image(dbXo.ImageUrl) %>" /><%}%><%=dbXo.Name%></a></dd>
            <%dbXo_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int WRXf_index=1;
List<Lebi_Currency> WRXfs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency WRXf in WRXfs){%>
            <dd <%if (WRXf_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=WRXf.id%>,'<%=WRXf.Code%>',<%=WRXf.ExchangeRate%>,'<%=WRXf.Msige%>','<%=WRXf.DecimalLength%>');"><%=WRXf.Code%></a></dd>
            <%WRXf_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int fBTB_index=1;
List<Lebi_Searchkey> fBTBs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey fBTB in fBTBs){%><%=Lang(fBTB.Name)%><%fBTB_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int mknU_index=1;
List<Lebi_Searchkey> mknUs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey mknU in mknUs){%>
<%if (mknU.Type==1){ %><a href="<%=URL("P_Search",""+Lang(mknU.Name)+"") %>"><%}else{ %><a href="<%=mknU.URL%>" target="_blank"><%} %><span><%=Lang(mknU.Name)%></span></a>
<%mknU_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int aYoL_index=1;
List<Lebi_Page> aYoLs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page aYoL in aYoLs){%>
<a class="menu" href="<%=URL("","",aYoL.url)%>"><span><%=aYoL.Name%></span></a>
<%aYoL_index++;}%>
...
<div class="user">
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        <tr>
            ...
            <th style="width: 80px;">已退數量</th>
            <th style="width: 80px;">退貨數量</th>
        </tr>
        <%
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int hDkW_index=1;
List<Lebi_FriendLink> hDkWs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink hDkW in hDkWs){%>
            
                <% if (hDkW.Logo != "" && hDkW.IsPic == 1){ %><a href="<%=hDkW.Url%>" target="_blank"><img src="<%=Image(hDkW.Logo) %>" alt="<%=hDkW.Name%>" /></a><%}else{%><a href="<%=hDkW.Url%>" target="_blank"><%=hDkW.Name%></a><%} %>     
