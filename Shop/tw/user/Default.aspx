...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserCenter"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> JjTns=Languages();RecordCount=JjTns.Count;int JjTn_index=1;
foreach (Shop.Model.Lebi_Language JjTn in JjTns){%>
            <dd <%if (JjTn_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=JjTn.id%>,'<%=JjTn.Code%>','<%=JjTn.Path%>');"><%if (JjTn.ImageUrl!=""){%><img src="<%=Image(JjTn.ImageUrl) %>" /><%}%><%=JjTn.Name%></a></dd>
            <%JjTn_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int YTgh_index=1;
List<Lebi_Currency> YTghs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency YTgh in YTghs){%>
            <dd <%if (YTgh_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=YTgh.id%>,'<%=YTgh.Code%>',<%=YTgh.ExchangeRate%>,'<%=YTgh.Msige%>','<%=YTgh.DecimalLength%>');"><%=YTgh.Code%></a></dd>
            <%YTgh_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int XXDD_index=1;
List<Lebi_Searchkey> XXDDs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey XXDD in XXDDs){%><%=Lang(XXDD.Name)%><%XXDD_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int lMov_index=1;
List<Lebi_Searchkey> lMovs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey lMov in lMovs){%>
<%if (lMov.Type==1){ %><a href="<%=URL("P_Search",""+Lang(lMov.Name)+"") %>"><%}else{ %><a href="<%=lMov.URL%>" target="_blank"><%} %><span><%=Lang(lMov.Name)%></span></a>
<%lMov_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int IKmF_index=1;
List<Lebi_Page> IKmFs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page IKmF in IKmFs){%>
<a class="menu" href="<%=URL("","",IKmF.url)%>"><span><%=IKmF.Name%></span></a>
<%IKmF_index++;}%>
...
<div class="userbox">
    <div class="mt">
        <h2><%=CurrentUser.NickName %>&nbsp;歡迎您回來</h2>
    </div>
...
<div class="userbox">
    ...
    <div class="mc default-static">
        ...
        <dl>
            <dt>我的訂單：</dt>
            <dd>總計：<a href="<%=URL("P_UserOrders","")%>"><strong><%=Shop.Bussiness.Order.GetCount_Order("","","User_id = "+ CurrentUser.id +" and Type_id_OrderType = 211") %></strong></a>&nbsp;&nbsp;等待付款：<a href="<%=URL("P_UserOrders","")%>?status=1"><strong><%=Shop.Bussiness.Order.GetCount_Order("","","User_id = "+ CurrentUser.id +" and Type_id_OrderType = 211 and IsPaid = 0") %></strong></a>&nbsp;&nbsp;等待收貨：<a href="<%=URL("P_UserOrders","")%>?status=2"><strong><%=Shop.Bussiness.Order.GetCount_Order("","","User_id = "+ CurrentUser.id +" and Type_id_OrderType = 211 and IsShipped = 1 and IsReceived = 0") %></strong></a>&nbsp;&nbsp;已收貨：<a href="<%=URL("P_UserOrders","")%>?status=3"><strong><%=Shop.Bussiness.Order.GetCount_Order("","","User_id = "+ CurrentUser.id +" and Type_id_OrderType = 211 and IsReceived = 1") %></strong></a>&nbsp;&nbsp;已完成：<a href="<%=URL("P_UserOrders","")%>?status=2"><strong><%=Shop.Bussiness.Order.GetCount_Order("","","User_id = "+ CurrentUser.id +" and Type_id_OrderType = 211 and IsCompleted = 1") %></strong></a></dd>
        </dl>
        <dl>
            <dt>我的收藏：</dt>
            <dd>總計：<a href="<%=URL("P_UserLike","")%>"><strong><%=Like_Product_Count() %></strong></a></dd>
        </dl>
        <dl>
            <dt>商品評價：</dt>
            <dd>總計：<a href="<%=URL("P_UserLike","")%>"><strong><%=Count_Comment(0)+Count_Comment(1) %></strong></a>&nbsp;&nbsp;已評價：<a href="<%=URL("P_UserComment","0","")%>"><strong><%=Count_Comment(1) %></strong></a>&nbsp;&nbsp;待評價：<%if(Count_Comment(0)>0){%><a href="<%=URL("P_UserComment","1","")%>"><strong style="color:#ff0000"><%=Count_Comment(0) %></strong></a><%}else{ %>0<%} %></dd>
        </dl>
        <dl>
            <dt>商品諮詢：</dt>
            <dd>總計：<a href="<%=URL("P_UserAsk","")%>"><strong><%=Count_ProductAsk(0)+Count_ProductAsk(1) %></strong></a>&nbsp;&nbsp;已回复：<a href="<%=URL("P_UserAsk","","")%>?status=283"><strong><%=Count_ProductAsk(1) %></strong></a>&nbsp;&nbsp;
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int tpjq_index=1;
List<Lebi_FriendLink> tpjqs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink tpjq in tpjqs){%>
            
                <% if (tpjq.Logo != "" && tpjq.IsPic == 1){ %><a href="<%=tpjq.Url%>" target="_blank"><img src="<%=Image(tpjq.Logo) %>" alt="<%=tpjq.Name%>" /></a><%}else{%><a href="<%=tpjq.Url%>" target="_blank"><%=tpjq.Name%></a><%} %>     
