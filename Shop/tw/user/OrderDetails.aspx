...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserOrderDetails"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> JRCjs=Languages();RecordCount=JRCjs.Count;int JRCj_index=1;
foreach (Shop.Model.Lebi_Language JRCj in JRCjs){%>
            <dd <%if (JRCj_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=JRCj.id%>,'<%=JRCj.Code%>','<%=JRCj.Path%>');"><%if (JRCj.ImageUrl!=""){%><img src="<%=Image(JRCj.ImageUrl) %>" /><%}%><%=JRCj.Name%></a></dd>
            <%JRCj_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int LTzL_index=1;
List<Lebi_Currency> LTzLs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency LTzL in LTzLs){%>
            <dd <%if (LTzL_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=LTzL.id%>,'<%=LTzL.Code%>',<%=LTzL.ExchangeRate%>,'<%=LTzL.Msige%>','<%=LTzL.DecimalLength%>');"><%=LTzL.Code%></a></dd>
            <%LTzL_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ducn_index=1;
List<Lebi_Searchkey> ducns = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ducn in ducns){%><%=Lang(ducn.Name)%><%ducn_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int CFEy_index=1;
List<Lebi_Searchkey> CFEys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey CFEy in CFEys){%>
<%if (CFEy.Type==1){ %><a href="<%=URL("P_Search",""+Lang(CFEy.Name)+"") %>"><%}else{ %><a href="<%=CFEy.URL%>" target="_blank"><%} %><span><%=Lang(CFEy.Name)%></span></a>
<%CFEy_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int TTOs_index=1;
List<Lebi_Page> TTOss = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page TTOs in TTOss){%>
<a class="menu" href="<%=URL("","",TTOs.url)%>"><span><%=TTOs.Name%></span></a>
<%TTOs_index++;}%>
...
<div id="orderstate">
    <div class="mt">
        <div class="left">
            <strong>訂單編號：<%=order.Code %>&nbsp;&nbsp;&nbsp;&nbsp;狀態：<span class="ftx14"><%=Shop.Bussiness.Order.OrderStatus(order, CurrentLanguage.Code) %></span></strong>
        </div>
        <div class="right">
            <div class="toolbar">
...
<div id="ordertrack" class="clearfix">
    <div class="mt"><strong>訂??單追踪</strong></div>
    <div class="mc tabcon">
        <table width="100%" cellspacing="0" cellpadding="0">
            <tbody id="tbody_track">
                <tr>
                    <th colspan="2" style="text-align: left; height: 30px;">
                        <%=t.Transport_Name %> 運單號：<%=t.Code %> 狀態：<%=TypeName(t.Type_id_TransportOrderStatus) %>
                        <%if(t.Type_id_TransportOrderStatus==220){ %>
                        <a href="javascript:void(0)" onclick="Received(<%=order.id%>,<%=t.id %>);" class="btn btn-11"><s></s>確認收貨</a>
                        <%} %>
                    </th>
                </tr>
                <tr>
...
<div id="orderinfo" class="clearfix">
    ...
    <div class="mc">
        <dl class="fore clearfix">
            <dd>
                <ul>
                    <li>訂購時間：<%=FormatTime(order.Time_Add) %></li>
                </ul>
            </dd>
        </dl>
        <dl>
        ...
        <dl>
            ...
            <dd>
                <ul>
                    ...
                    <li>自提點：<%=order.PickUp_Name %></li>
                    <li>自提時間：<span id="pickupdateshow"><%=order.PickUp_Date.ToString("yyyy-MM-dd") %></span><input type="hidden" id="pickupdate_<%=order.PickUp_id %>" name="pickupdate_<%=order.PickUp_id %>" value="<%=order.PickUp_Date.ToString("yyyy-MM-dd") %>" order="true" size="12" class="input-calendar" readonly />
        ...
        <dl>
            ...
            <dd>
                <ul>
                    <li>發票類型：<%=Lang(order.BillType_Name) %></li>
                    <%foreach (Shop.Model.Lebi_Bill b in bills){%>
                    <%if(b.Type_id_BillType==151){ %>
                    <li>發票內容：<%=b.Content%></li>
                    <li>發票抬頭：<%=b.Title%></li>
                    <%}else{ %>
                    <li>單位名稱：<%=b.Company_Name%></li>
                    <li>納稅人識別號：<%=b.Company_Code%></li>
                    <li>註冊地址：<%=b.Company_Address%></li>
                    <li>註冊電話：<%=b.Company_Phone%></li>
                    <li>開戶銀行：<%=b.Company_Bank%></li>
                    <li>銀行帳戶：<%=b.Company_Bank_User%></li>
                    <%} %>
                    <%} %>
                </ul>
            </dd>
       ...
       <%} %>
        ...
        <dl>
            ...
            <dd>
                <ul>
                    <li>所含稅額：<%=FormatMoney(order.Refund_VAT)%></li>
                    <li>退稅手續費：<%=FormatMoney(order.Refund_Fee)%></li>
                    <li>退稅總額：<%=FormatMoney(order.Refund_VAT - order.Refund_Fee)%></li>
                </ul>
            </dd>
        </dl>
       <%} %>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <a target="_blank" href="<%=URL("P_UserReturnApply",o_p.Order_id)%>">退貨</a><br />
                                <%} %>
                                <a href="javascript:void(0);" onclick="AddOftenBuy(<%=o_p.Product_id %>,<%=o_p.Count %>,'<%=o_p.ProPerty134 %>')">加入常購清單</a>
                            </td>
                        </tr>
                        <%} %>
                    </tbody>
    ...
    <div class="total">
        <ul>
            <li><strong>商品總額：</strong><%=FormatMoney(order.Money_Product) %></li>
            <%if(order.Money_Transport!=0){ %><li><strong>+ 運費：</strong><%=FormatMoney(order.Money_Transport) %></li><%} %>
            <%if(order.Money_Bill>0){ %><li><strong>+ ：</strong><%=FormatMoney(order.Money_Bill) %></li><%} %>
            <%if(order.Money_Tax>0){ %><li><strong>+ 稅金：</strong><%=FormatMoney(order.Money_Tax) %></li><%} %>
            <%if(order.Money_Property>0){ %><li><strong>+ 其它費用：</strong><%=FormatMoney(order.Money_Property) %></li><%} %>
            ...
            <%} %>
            <%if(order.Money_Cut>0){ %><li><strong>- 優惠：</strong><%=FormatMoney(order.Money_Cut) %></li><%} %>
            <%if (order.Money_UserCut > 0){ %><li><strong>- 使用餘額：</strong><%=FormatMoney(order.Money_UserCut)%></li><%} %>
        </ul>
        <span class="clear"></span>
        <div class="extra">
            應付金額：<span class="red"><b><%=FormatMoney(order.Money_Pay)%></b></span></div>
    </div>
</div>
<script type="text/javascript">
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int ltmh_index=1;
List<Lebi_FriendLink> ltmhs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink ltmh in ltmhs){%>
            
                <% if (ltmh.Logo != "" && ltmh.IsPic == 1){ %><a href="<%=ltmh.Url%>" target="_blank"><img src="<%=Image(ltmh.Logo) %>" alt="<%=ltmh.Name%>" /></a><%}else{%><a href="<%=ltmh.Url%>" target="_blank"><%=ltmh.Name%></a><%} %>     
