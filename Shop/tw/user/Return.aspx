...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserReturn"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> LBwds=Languages();RecordCount=LBwds.Count;int LBwd_index=1;
foreach (Shop.Model.Lebi_Language LBwd in LBwds){%>
            <dd <%if (LBwd_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=LBwd.id%>,'<%=LBwd.Code%>','<%=LBwd.Path%>');"><%if (LBwd.ImageUrl!=""){%><img src="<%=Image(LBwd.ImageUrl) %>" /><%}%><%=LBwd.Name%></a></dd>
            <%LBwd_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int oetk_index=1;
List<Lebi_Currency> oetks = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency oetk in oetks){%>
            <dd <%if (oetk_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=oetk.id%>,'<%=oetk.Code%>',<%=oetk.ExchangeRate%>,'<%=oetk.Msige%>','<%=oetk.DecimalLength%>');"><%=oetk.Code%></a></dd>
            <%oetk_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int tJWu_index=1;
List<Lebi_Searchkey> tJWus = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey tJWu in tJWus){%><%=Lang(tJWu.Name)%><%tJWu_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hgid_index=1;
List<Lebi_Searchkey> hgids = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hgid in hgids){%>
<%if (hgid.Type==1){ %><a href="<%=URL("P_Search",""+Lang(hgid.Name)+"") %>"><%}else{ %><a href="<%=hgid.URL%>" target="_blank"><%} %><span><%=Lang(hgid.Name)%></span></a>
<%hgid_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int sSdR_index=1;
List<Lebi_Page> sSdRs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page sSdR in sSdRs){%>
<a class="menu" href="<%=URL("","",sSdR.url)%>"><span><%=sSdR.Name%></span></a>
<%sSdR_index++;}%>
...
<div class="user">
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        ...
        <tr>
            ...
            <td><%if(o.IsShipped==1){ %><a href="<%=URL("P_UserReturnDetails",o.id)%>" target="_blank">查看</a><%}else{ %><a href="<%=URL("P_UserReturnShip",o.id)%>" target="_blank">查看</a><%} %>
            </td>
        </tr>
        <%} %>
    </table>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int ulcj_index=1;
List<Lebi_FriendLink> ulcjs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink ulcj in ulcjs){%>
            
                <% if (ulcj.Logo != "" && ulcj.IsPic == 1){ %><a href="<%=ulcj.Url%>" target="_blank"><img src="<%=Image(ulcj.Logo) %>" alt="<%=ulcj.Name%>" /></a><%}else{%><a href="<%=ulcj.Url%>" target="_blank"><%=ulcj.Name%></a><%} %>     
