...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserCardGet"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> gUXVs=Languages();RecordCount=gUXVs.Count;int gUXV_index=1;
foreach (Shop.Model.Lebi_Language gUXV in gUXVs){%>
            <dd <%if (gUXV_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=gUXV.id%>,'<%=gUXV.Code%>','<%=gUXV.Path%>');"><%if (gUXV.ImageUrl!=""){%><img src="<%=Image(gUXV.ImageUrl) %>" /><%}%><%=gUXV.Name%></a></dd>
            <%gUXV_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int FzNY_index=1;
List<Lebi_Currency> FzNYs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency FzNY in FzNYs){%>
            <dd <%if (FzNY_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=FzNY.id%>,'<%=FzNY.Code%>',<%=FzNY.ExchangeRate%>,'<%=FzNY.Msige%>','<%=FzNY.DecimalLength%>');"><%=FzNY.Code%></a></dd>
            <%FzNY_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int TIKb_index=1;
List<Lebi_Searchkey> TIKbs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey TIKb in TIKbs){%><%=Lang(TIKb.Name)%><%TIKb_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int dBls_index=1;
List<Lebi_Searchkey> dBlss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey dBls in dBlss){%>
<%if (dBls.Type==1){ %><a href="<%=URL("P_Search",""+Lang(dBls.Name)+"") %>"><%}else{ %><a href="<%=dBls.URL%>" target="_blank"><%} %><span><%=Lang(dBls.Name)%></span></a>
<%dBls_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int yIkp_index=1;
List<Lebi_Page> yIkps = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page yIkp in yIkps){%>
<a class="menu" href="<%=URL("","",yIkp.url)%>"><span><%=yIkp.Name%></span></a>
<%yIkp_index++;}%>
...
class="input" /> <img class="img_c" align="absmiddle" style="width: 80px; height: 25px;" title="點擊重新獲取驗證碼" id="img1" src="<%=WebPath%>/code.aspx" language="javascript" onclick="this.src=this.src+'?'"></dd>
        </dl>
        <dl class="dl-btn">
            <dt></dt>
            <dd><a href="javascript:void(0)" onclick="submit();" class="btn btn-11"><s></s>領取</a></dd>
        </dl>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    function submit() {
        ...
        RequestAjax(url, postData, function () { MsgBox(1, "操作成功", "<%=URL("P_UserCard","","")%>") });
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int jmKz_index=1;
List<Lebi_FriendLink> jmKzs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink jmKz in jmKzs){%>
            
                <% if (jmKz.Logo != "" && jmKz.IsPic == 1){ %><a href="<%=jmKz.Url%>" target="_blank"><img src="<%=Image(jmKz.Logo) %>" alt="<%=jmKz.Name%>" /></a><%}else{%><a href="<%=jmKz.Url%>" target="_blank"><%=jmKz.Name%></a><%} %>     
