...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserLike"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> FWXBs=Languages();RecordCount=FWXBs.Count;int FWXB_index=1;
foreach (Shop.Model.Lebi_Language FWXB in FWXBs){%>
            <dd <%if (FWXB_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=FWXB.id%>,'<%=FWXB.Code%>','<%=FWXB.Path%>');"><%if (FWXB.ImageUrl!=""){%><img src="<%=Image(FWXB.ImageUrl) %>" /><%}%><%=FWXB.Name%></a></dd>
            <%FWXB_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int JtoC_index=1;
List<Lebi_Currency> JtoCs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency JtoC in JtoCs){%>
            <dd <%if (JtoC_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=JtoC.id%>,'<%=JtoC.Code%>',<%=JtoC.ExchangeRate%>,'<%=JtoC.Msige%>','<%=JtoC.DecimalLength%>');"><%=JtoC.Code%></a></dd>
            <%JtoC_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int YYVG_index=1;
List<Lebi_Searchkey> YYVGs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey YYVG in YYVGs){%><%=Lang(YYVG.Name)%><%YYVG_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hLDv_index=1;
List<Lebi_Searchkey> hLDvs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hLDv in hLDvs){%>
<%if (hLDv.Type==1){ %><a href="<%=URL("P_Search",""+Lang(hLDv.Name)+"") %>"><%}else{ %><a href="<%=hLDv.URL%>" target="_blank"><%} %><span><%=Lang(hLDv.Name)%></span></a>
<%hLDv_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int aEkV_index=1;
List<Lebi_Page> aEkVs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page aEkV in aEkVs){%>
<a class="menu" href="<%=URL("","",aEkV.url)%>"><span><%=aEkV.Name%></span></a>
<%aEkV_index++;}%>
...
<div class="user">
    ...
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>刪除</a><a href="javascript:void(0)" onclick="LikeToBasket();" class="btn btn-7"><s></s>加入購物車</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript" >
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int mpMi_index=1;
List<Lebi_FriendLink> mpMis = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink mpMi in mpMis){%>
            
                <% if (mpMi.Logo != "" && mpMi.IsPic == 1){ %><a href="<%=mpMi.Url%>" target="_blank"><img src="<%=Image(mpMi.Logo) %>" alt="<%=mpMi.Name%>" /></a><%}else{%><a href="<%=mpMi.Url%>" target="_blank"><%=mpMi.Name%></a><%} %>     
