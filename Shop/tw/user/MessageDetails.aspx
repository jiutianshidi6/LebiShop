...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserMessageDetails"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> dITDs=Languages();RecordCount=dITDs.Count;int dITD_index=1;
foreach (Shop.Model.Lebi_Language dITD in dITDs){%>
            <dd <%if (dITD_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=dITD.id%>,'<%=dITD.Code%>','<%=dITD.Path%>');"><%if (dITD.ImageUrl!=""){%><img src="<%=Image(dITD.ImageUrl) %>" /><%}%><%=dITD.Name%></a></dd>
            <%dITD_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int HLGp_index=1;
List<Lebi_Currency> HLGps = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency HLGp in HLGps){%>
            <dd <%if (HLGp_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=HLGp.id%>,'<%=HLGp.Code%>',<%=HLGp.ExchangeRate%>,'<%=HLGp.Msige%>','<%=HLGp.DecimalLength%>');"><%=HLGp.Code%></a></dd>
            <%HLGp_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int FFKK_index=1;
List<Lebi_Searchkey> FFKKs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey FFKK in FFKKs){%><%=Lang(FFKK.Name)%><%FFKK_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int HUjN_index=1;
List<Lebi_Searchkey> HUjNs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey HUjN in HUjNs){%>
<%if (HUjN.Type==1){ %><a href="<%=URL("P_Search",""+Lang(HUjN.Name)+"") %>"><%}else{ %><a href="<%=HUjN.URL%>" target="_blank"><%} %><span><%=Lang(HUjN.Name)%></span></a>
<%HUjN_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int coYy_index=1;
List<Lebi_Page> coYys = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page coYy in coYys){%>
<a class="menu" href="<%=URL("","",coYy.url)%>"><span><%=coYy.Name%></span></a>
<%coYy_index++;}%>
...
<div class="user">
    <ul class="tablist">
        ...
        <li <%if (message.User_id_To == 0){Response.Write("class=\"selected\"");} %>><a href="<%=URL("P_UserMessage","1","")%>"><span>發件箱</span></a></li>
        <li><a href="<%=URL("P_UserMessageWrite","","")%>"><span>發信息</span></a></li>
    </ul>
    <div class="mt">
        <h2><%=message.Title %></h2>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int YinV_index=1;
List<Lebi_FriendLink> YinVs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink YinV in YinVs){%>
            
                <% if (YinV.Logo != "" && YinV.IsPic == 1){ %><a href="<%=YinV.Url%>" target="_blank"><img src="<%=Image(YinV.Logo) %>" alt="<%=YinV.Name%>" /></a><%}else{%><a href="<%=YinV.Url%>" target="_blank"><%=YinV.Name%></a><%} %>     
