...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserBankEdit"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> NXJKs=Languages();RecordCount=NXJKs.Count;int NXJK_index=1;
foreach (Shop.Model.Lebi_Language NXJK in NXJKs){%>
            <dd <%if (NXJK_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=NXJK.id%>,'<%=NXJK.Code%>','<%=NXJK.Path%>');"><%if (NXJK.ImageUrl!=""){%><img src="<%=Image(NXJK.ImageUrl) %>" /><%}%><%=NXJK.Name%></a></dd>
            <%NXJK_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int lWdO_index=1;
List<Lebi_Currency> lWdOs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency lWdO in lWdOs){%>
            <dd <%if (lWdO_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=lWdO.id%>,'<%=lWdO.Code%>',<%=lWdO.ExchangeRate%>,'<%=lWdO.Msige%>','<%=lWdO.DecimalLength%>');"><%=lWdO.Code%></a></dd>
            <%lWdO_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int cWsE_index=1;
List<Lebi_Searchkey> cWsEs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey cWsE in cWsEs){%><%=Lang(cWsE.Name)%><%cWsE_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int gvTq_index=1;
List<Lebi_Searchkey> gvTqs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey gvTq in gvTqs){%>
<%if (gvTq.Type==1){ %><a href="<%=URL("P_Search",""+Lang(gvTq.Name)+"") %>"><%}else{ %><a href="<%=gvTq.URL%>" target="_blank"><%} %><span><%=Lang(gvTq.Name)%></span></a>
<%gvTq_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int YBfk_index=1;
List<Lebi_Page> YBfks = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page YBfk in YBfks){%>
<a class="menu" href="<%=URL("","",YBfk.url)%>"><span><%=YBfk.Name%></span></a>
<%YBfk_index++;}%>
...
<div class="userbox">
    ...
    <dl>
        <dt>收款方式：</dt>
        <dd>
        <label><input type="radio" name="PayType" value="bank" shop="true" onclick="SetType();" <%=model.PayType == "bank"?"":"checked"%> />銀行轉賬</label>
        <label><input type="radio" name="PayType" value="alipay" shop="true" onclick="SetType();" <%=model.PayType == "alipay"?"checked":""%> />支付寶</label>
        </dd>
    </dl>
    <dl id="DIVUserName">
        <dt>姓名：</dt>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int DSIS_index=1;
List<Lebi_FriendLink> DSISs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink DSIS in DSISs){%>
            
                <% if (DSIS.Logo != "" && DSIS.IsPic == 1){ %><a href="<%=DSIS.Url%>" target="_blank"><img src="<%=Image(DSIS.Logo) %>" alt="<%=DSIS.Name%>" /></a><%}else{%><a href="<%=DSIS.Url%>" target="_blank"><%=DSIS.Name%></a><%} %>     
