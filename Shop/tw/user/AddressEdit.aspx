...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserAddressEdit"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> IEhbs=Languages();RecordCount=IEhbs.Count;int IEhb_index=1;
foreach (Shop.Model.Lebi_Language IEhb in IEhbs){%>
            <dd <%if (IEhb_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=IEhb.id%>,'<%=IEhb.Code%>','<%=IEhb.Path%>');"><%if (IEhb.ImageUrl!=""){%><img src="<%=Image(IEhb.ImageUrl) %>" /><%}%><%=IEhb.Name%></a></dd>
            <%IEhb_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int uwRD_index=1;
List<Lebi_Currency> uwRDs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency uwRD in uwRDs){%>
            <dd <%if (uwRD_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=uwRD.id%>,'<%=uwRD.Code%>',<%=uwRD.ExchangeRate%>,'<%=uwRD.Msige%>','<%=uwRD.DecimalLength%>');"><%=uwRD.Code%></a></dd>
            <%uwRD_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ywDP_index=1;
List<Lebi_Searchkey> ywDPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ywDP in ywDPs){%><%=Lang(ywDP.Name)%><%ywDP_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zIlq_index=1;
List<Lebi_Searchkey> zIlqs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zIlq in zIlqs){%>
<%if (zIlq.Type==1){ %><a href="<%=URL("P_Search",""+Lang(zIlq.Name)+"") %>"><%}else{ %><a href="<%=zIlq.URL%>" target="_blank"><%} %><span><%=Lang(zIlq.Name)%></span></a>
<%zIlq_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int XedE_index=1;
List<Lebi_Page> XedEs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page XedE in XedEs){%>
<a class="menu" href="<%=URL("","",XedE.url)%>"><span><%=XedE.Name%></span></a>
<%XedE_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int NDFS_index=1;
List<Lebi_FriendLink> NDFSs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink NDFS in NDFSs){%>
            
                <% if (NDFS.Logo != "" && NDFS.IsPic == 1){ %><a href="<%=NDFS.Url%>" target="_blank"><img src="<%=Image(NDFS.Logo) %>" alt="<%=NDFS.Name%>" /></a><%}else{%><a href="<%=NDFS.Url%>" target="_blank"><%=NDFS.Name%></a><%} %>     
