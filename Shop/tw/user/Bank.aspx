<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.P_UserBank" validateRequest="false"%>


<%@ Import Namespace="Shop.Bussiness" %>
<%@ Import Namespace="Shop.Model" %>
<%@ Import Namespace="System.Collections.Generic" %>
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserBank"); %>
...
<title><%=ThemePageMeta("P_UserBank","title")%></title>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Language" content="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0;" />
<meta name="keywords" content="<%=ThemePageMeta("P_UserBank","keywords")%>" />
<meta name="description" content="<%=ThemePageMeta("P_UserBank","description")%>" />
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> aDCTs=Languages();RecordCount=aDCTs.Count;int aDCT_index=1;
foreach (Shop.Model.Lebi_Language aDCT in aDCTs){%>
            <dd <%if (aDCT_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=aDCT.id%>,'<%=aDCT.Code%>','<%=aDCT.Path%>');"><%if (aDCT.ImageUrl!=""){%><img src="<%=Image(aDCT.ImageUrl) %>" /><%}%><%=aDCT.Name%></a></dd>
            <%aDCT_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int FhPU_index=1;
List<Lebi_Currency> FhPUs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency FhPU in FhPUs){%>
            <dd <%if (FhPU_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=FhPU.id%>,'<%=FhPU.Code%>',<%=FhPU.ExchangeRate%>,'<%=FhPU.Msige%>','<%=FhPU.DecimalLength%>');"><%=FhPU.Code%></a></dd>
            <%FhPU_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int uoWm_index=1;
List<Lebi_Searchkey> uoWms = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey uoWm in uoWms){%><%=Lang(uoWm.Name)%><%uoWm_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ACXv_index=1;
List<Lebi_Searchkey> ACXvs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ACXv in ACXvs){%>
<%if (ACXv.Type==1){ %><a href="<%=URL("P_Search",""+Lang(ACXv.Name)+"") %>"><%}else{ %><a href="<%=ACXv.URL%>" target="_blank"><%} %><span><%=Lang(ACXv.Name)%></span></a>
<%ACXv_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int DHfP_index=1;
List<Lebi_Page> DHfPs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page DHfP in DHfPs){%>
<a class="menu" href="<%=URL("","",DHfP.url)%>"><span><%=DHfP.Name%></span></a>
<%DHfP_index++;}%>
...
<div class="user">
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        ...
        <tr>
            ...
            <td><%=ub.Code%></td>
            <td><%=ub.Name%></td>
            <td><a href="<%=URL("P_UserBankEdit",ub.id,"")%>">編輯</a></td>
        </tr>
        <%} %>
    </table>
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>刪除</a><a href="<%=URL("P_UserBankEdit","0","")%>" class="btn btn-7"><s></s>添加</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript">
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int ORoH_index=1;
List<Lebi_FriendLink> ORoHs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink ORoH in ORoHs){%>
            
                <% if (ORoH.Logo != "" && ORoH.IsPic == 1){ %><a href="<%=ORoH.Url%>" target="_blank"><img src="<%=Image(ORoH.Logo) %>" alt="<%=ORoH.Name%>" /></a><%}else{%><a href="<%=ORoH.Url%>" target="_blank"><%=ORoH.Name%></a><%} %>     
