...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserCash"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> UFEas=Languages();RecordCount=UFEas.Count;int UFEa_index=1;
foreach (Shop.Model.Lebi_Language UFEa in UFEas){%>
            <dd <%if (UFEa_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=UFEa.id%>,'<%=UFEa.Code%>','<%=UFEa.Path%>');"><%if (UFEa.ImageUrl!=""){%><img src="<%=Image(UFEa.ImageUrl) %>" /><%}%><%=UFEa.Name%></a></dd>
            <%UFEa_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Nzcu_index=1;
List<Lebi_Currency> Nzcus = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Nzcu in Nzcus){%>
            <dd <%if (Nzcu_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Nzcu.id%>,'<%=Nzcu.Code%>',<%=Nzcu.ExchangeRate%>,'<%=Nzcu.Msige%>','<%=Nzcu.DecimalLength%>');"><%=Nzcu.Code%></a></dd>
            <%Nzcu_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int IRBl_index=1;
List<Lebi_Searchkey> IRBls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey IRBl in IRBls){%><%=Lang(IRBl.Name)%><%IRBl_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int RLwr_index=1;
List<Lebi_Searchkey> RLwrs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey RLwr in RLwrs){%>
<%if (RLwr.Type==1){ %><a href="<%=URL("P_Search",""+Lang(RLwr.Name)+"") %>"><%}else{ %><a href="<%=RLwr.URL%>" target="_blank"><%} %><span><%=Lang(RLwr.Name)%></span></a>
<%RLwr_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int GWMC_index=1;
List<Lebi_Page> GWMCs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page GWMC in GWMCs){%>
<a class="menu" href="<%=URL("","",GWMC.url)%>"><span><%=GWMC.Name%></span></a>
<%GWMC_index++;}%>
...
<div class="user">
    ...
    <dl>
        <dt>收款方式：</dt>
        <dd>
        <label><input type="radio" name="PayType" value="bank" shop="true" onclick="SetType();" checked />銀行轉賬</label>
        <label><input type="radio" name="PayType" value="alipay" shop="true" onclick="SetType();" />支付寶</label>
        </dd>
    </dl>
    <dl>
        <dt>姓名：</dt>
    ...
    <dl>
        <dt>提現金額(<%=DefaultCurrency.Msige %>)：</dt>
        ...
        <em>帳戶餘額(<%=DefaultCurrency.Msige %>):<%=CurrentUser.Money-CurrentUser.Money_fanxian %> &nbsp;&nbsp;最低提現金額(<%=DefaultCurrency.Msige %>):<%=SYS.TakeMoneyLimit %></em></dd>
    </dl>
    <dl>
        <dt>支付密碼：</dt>
        <dd><input name="Pay_Password" id="Pay_Password" size="30" type="password" shop="true" min="notnull" class="input" /> <a href="<%=URL("P_UserQuestion","1")%>" target="_blank">忘記密碼</a></dd>
    </dl>
    <%if(SubmitShow){ %>
    <dl class="dl-btn">
        <dt></dt>
    ...
    <dl class="dl-btn">
        <dt></dt>
        <dd>最低提現金額(<%=DefaultCurrency.Msige %>):<%=SYS.TakeMoneyLimit %></dd>
    </dl>
    <%} %>
    </div>
    </div>
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        ...
        <tr>
            ...
            <td><%=ub.Code%></td>
            <td><%=ub.Name%></td>
            <td><a href="javascript:void(0)" onclick="SetBank('<%=ub.PayType %>','<%=ub.UserName %>','<%=ub.Code %>','<%=ub.Name %>');">選擇</a></td>
        </tr>
        <%} %>
    </table>
    <%
    ...
    <div>
        <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
            <tr>
                ...
                <th style="width:120px;">金額/手續費</th>
                <th style="width:150px;">賬號</th>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int lGwI_index=1;
List<Lebi_FriendLink> lGwIs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink lGwI in lGwIs){%>
            
                <% if (lGwI.Logo != "" && lGwI.IsPic == 1){ %><a href="<%=lGwI.Url%>" target="_blank"><img src="<%=Image(lGwI.Logo) %>" alt="<%=lGwI.Name%>" /></a><%}else{%><a href="<%=lGwI.Url%>" target="_blank"><%=lGwI.Name%></a><%} %>     
