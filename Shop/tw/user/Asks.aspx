...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserAsk"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> eSLas=Languages();RecordCount=eSLas.Count;int eSLa_index=1;
foreach (Shop.Model.Lebi_Language eSLa in eSLas){%>
            <dd <%if (eSLa_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=eSLa.id%>,'<%=eSLa.Code%>','<%=eSLa.Path%>');"><%if (eSLa.ImageUrl!=""){%><img src="<%=Image(eSLa.ImageUrl) %>" /><%}%><%=eSLa.Name%></a></dd>
            <%eSLa_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int jnCI_index=1;
List<Lebi_Currency> jnCIs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency jnCI in jnCIs){%>
            <dd <%if (jnCI_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=jnCI.id%>,'<%=jnCI.Code%>',<%=jnCI.ExchangeRate%>,'<%=jnCI.Msige%>','<%=jnCI.DecimalLength%>');"><%=jnCI.Code%></a></dd>
            <%jnCI_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int bEMd_index=1;
List<Lebi_Searchkey> bEMds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey bEMd in bEMds){%><%=Lang(bEMd.Name)%><%bEMd_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zANR_index=1;
List<Lebi_Searchkey> zANRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zANR in zANRs){%>
<%if (zANR.Type==1){ %><a href="<%=URL("P_Search",""+Lang(zANR.Name)+"") %>"><%}else{ %><a href="<%=zANR.URL%>" target="_blank"><%} %><span><%=Lang(zANR.Name)%></span></a>
<%zANR_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int zhwK_index=1;
List<Lebi_Page> zhwKs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page zhwK in zhwKs){%>
<a class="menu" href="<%=URL("","",zhwK.url)%>"><span><%=zhwK.Name%></span></a>
<%zhwK_index++;}%>
...
<div class="user">
    ...
    <tr><td colspan="5">內容：<%=c.Content.Replace("\r\n", "<br/>")%></td></tr>
    <%
    ...
    {
    %>
    <tr><td colspan="5">客服回复：<%=creply.Content.Replace("\r\n", "<br/>")%><em><%=FormatTime(creply.Time_Add)%></em></td></tr>
    <%} %>
    </table>
    <%} %>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int nHCn_index=1;
List<Lebi_FriendLink> nHCns = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink nHCn in nHCns){%>
            
                <% if (nHCn.Logo != "" && nHCn.IsPic == 1){ %><a href="<%=nHCn.Url%>" target="_blank"><img src="<%=Image(nHCn.Logo) %>" alt="<%=nHCn.Name%>" /></a><%}else{%><a href="<%=nHCn.Url%>" target="_blank"><%=nHCn.Name%></a><%} %>     
