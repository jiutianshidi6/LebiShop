...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserQuestion"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> luGos=Languages();RecordCount=luGos.Count;int luGo_index=1;
foreach (Shop.Model.Lebi_Language luGo in luGos){%>
            <dd <%if (luGo_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=luGo.id%>,'<%=luGo.Code%>','<%=luGo.Path%>');"><%if (luGo.ImageUrl!=""){%><img src="<%=Image(luGo.ImageUrl) %>" /><%}%><%=luGo.Name%></a></dd>
            <%luGo_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int xWUO_index=1;
List<Lebi_Currency> xWUOs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency xWUO in xWUOs){%>
            <dd <%if (xWUO_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=xWUO.id%>,'<%=xWUO.Code%>',<%=xWUO.ExchangeRate%>,'<%=xWUO.Msige%>','<%=xWUO.DecimalLength%>');"><%=xWUO.Code%></a></dd>
            <%xWUO_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vsLI_index=1;
List<Lebi_Searchkey> vsLIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vsLI in vsLIs){%><%=Lang(vsLI.Name)%><%vsLI_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int uFNd_index=1;
List<Lebi_Searchkey> uFNds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey uFNd in uFNds){%>
<%if (uFNd.Type==1){ %><a href="<%=URL("P_Search",""+Lang(uFNd.Name)+"") %>"><%}else{ %><a href="<%=uFNd.URL%>" target="_blank"><%} %><span><%=Lang(uFNd.Name)%></span></a>
<%uFNd_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int qwkK_index=1;
List<Lebi_Page> qwkKs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page qwkK in qwkKs){%>
<a class="menu" href="<%=URL("","",qwkK.url)%>"><span><%=qwkK.Name%></span></a>
<%qwkK_index++;}%>
...
<script type="text/javascript">
    function submit() {
        ...
        if (res.type == 0){
            ...
            MsgBox(1, "原始支付密碼已清空，請重新設置支付密碼", res.url); 
        }
        });
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int PwAy_index=1;
List<Lebi_FriendLink> PwAys = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink PwAy in PwAys){%>
            
                <% if (PwAy.Logo != "" && PwAy.IsPic == 1){ %><a href="<%=PwAy.Url%>" target="_blank"><img src="<%=Image(PwAy.Logo) %>" alt="<%=PwAy.Name%>" /></a><%}else{%><a href="<%=PwAy.Url%>" target="_blank"><%=PwAy.Name%></a><%} %>     
