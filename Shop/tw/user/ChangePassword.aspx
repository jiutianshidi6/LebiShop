...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserChangePassword"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> qwyPs=Languages();RecordCount=qwyPs.Count;int qwyP_index=1;
foreach (Shop.Model.Lebi_Language qwyP in qwyPs){%>
            <dd <%if (qwyP_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=qwyP.id%>,'<%=qwyP.Code%>','<%=qwyP.Path%>');"><%if (qwyP.ImageUrl!=""){%><img src="<%=Image(qwyP.ImageUrl) %>" /><%}%><%=qwyP.Name%></a></dd>
            <%qwyP_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int YHMQ_index=1;
List<Lebi_Currency> YHMQs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency YHMQ in YHMQs){%>
            <dd <%if (YHMQ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=YHMQ.id%>,'<%=YHMQ.Code%>',<%=YHMQ.ExchangeRate%>,'<%=YHMQ.Msige%>','<%=YHMQ.DecimalLength%>');"><%=YHMQ.Code%></a></dd>
            <%YHMQ_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int GJqI_index=1;
List<Lebi_Searchkey> GJqIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey GJqI in GJqIs){%><%=Lang(GJqI.Name)%><%GJqI_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int UqPm_index=1;
List<Lebi_Searchkey> UqPms = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey UqPm in UqPms){%>
<%if (UqPm.Type==1){ %><a href="<%=URL("P_Search",""+Lang(UqPm.Name)+"") %>"><%}else{ %><a href="<%=UqPm.URL%>" target="_blank"><%} %><span><%=Lang(UqPm.Name)%></span></a>
<%UqPm_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int VizS_index=1;
List<Lebi_Page> VizSs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page VizS in VizSs){%>
<a class="menu" href="<%=URL("","",VizS.url)%>"><span><%=VizS.Name%></span></a>
<%VizS_index++;}%>
...
<div class="userbox">
    ...
    <dl>
        <dt>原始密碼：</dt>
        <dd><input name="Pay_Password" id="Pay_Password" size="30" type="password" paypass="true" min="notnull" class="input" /> <a href="<%=URL("P_UserQuestion","1")%>">忘記密碼</a></dd>
    </dl>
    <%} %>
    <dl>
        <dt>新密碼：</dt>
    ...
    <dl class="dl-btn">
        <dt></dt>
        <dd><a href="<%=URL("P_UserQuestion","")%>" class="click">請先設置安全問題</a></dd>
    </dl>
    <%}else{ %>
    <dl class="dl-btn">
        <dt></dt>
...
<script type="text/javascript">
    function setpassword() {
        ...
        if (Password == "") {
            CheckNO("Password", "請輸入原始密碼");
            return false;
        }
        if (Password1 == "") {
            CheckNO("Password1", "請輸入新密碼");
            return false;
        }
    ...
    function setpaypassword() {
        ...
        if (Pay_Password1 != Pay_Password2) {
            CheckNO("Pay_Password1", "兩次輸入的密碼不一致");
            return false;
        }
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int rGMc_index=1;
List<Lebi_FriendLink> rGMcs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink rGMc in rGMcs){%>
            
                <% if (rGMc.Logo != "" && rGMc.IsPic == 1){ %><a href="<%=rGMc.Url%>" target="_blank"><img src="<%=Image(rGMc.Logo) %>" alt="<%=rGMc.Name%>" /></a><%}else{%><a href="<%=rGMc.Url%>" target="_blank"><%=rGMc.Name%></a><%} %>     
