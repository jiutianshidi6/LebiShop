...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserReturnShip"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> OuETs=Languages();RecordCount=OuETs.Count;int OuET_index=1;
foreach (Shop.Model.Lebi_Language OuET in OuETs){%>
            <dd <%if (OuET_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=OuET.id%>,'<%=OuET.Code%>','<%=OuET.Path%>');"><%if (OuET.ImageUrl!=""){%><img src="<%=Image(OuET.ImageUrl) %>" /><%}%><%=OuET.Name%></a></dd>
            <%OuET_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int MNco_index=1;
List<Lebi_Currency> MNcos = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency MNco in MNcos){%>
            <dd <%if (MNco_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=MNco.id%>,'<%=MNco.Code%>',<%=MNco.ExchangeRate%>,'<%=MNco.Msige%>','<%=MNco.DecimalLength%>');"><%=MNco.Code%></a></dd>
            <%MNco_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vQTK_index=1;
List<Lebi_Searchkey> vQTKs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vQTK in vQTKs){%><%=Lang(vQTK.Name)%><%vQTK_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int gjon_index=1;
List<Lebi_Searchkey> gjons = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey gjon in gjons){%>
<%if (gjon.Type==1){ %><a href="<%=URL("P_Search",""+Lang(gjon.Name)+"") %>"><%}else{ %><a href="<%=gjon.URL%>" target="_blank"><%} %><span><%=Lang(gjon.Name)%></span></a>
<%gjon_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int rzVg_index=1;
List<Lebi_Page> rzVgs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page rzVg in rzVgs){%>
<a class="menu" href="<%=URL("","",rzVg.url)%>"><span><%=rzVg.Name%></span></a>
<%rzVg_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int gxav_index=1;
List<Lebi_FriendLink> gxavs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink gxav in gxavs){%>
            
                <% if (gxav.Logo != "" && gxav.IsPic == 1){ %><a href="<%=gxav.Url%>" target="_blank"><img src="<%=Image(gxav.Logo) %>" alt="<%=gxav.Name%>" /></a><%}else{%><a href="<%=gxav.Url%>" target="_blank"><%=gxav.Name%></a><%} %>     
