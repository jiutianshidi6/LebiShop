...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserMessageWrite"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> ivpXs=Languages();RecordCount=ivpXs.Count;int ivpX_index=1;
foreach (Shop.Model.Lebi_Language ivpX in ivpXs){%>
            <dd <%if (ivpX_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=ivpX.id%>,'<%=ivpX.Code%>','<%=ivpX.Path%>');"><%if (ivpX.ImageUrl!=""){%><img src="<%=Image(ivpX.ImageUrl) %>" /><%}%><%=ivpX.Name%></a></dd>
            <%ivpX_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int OUNB_index=1;
List<Lebi_Currency> OUNBs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency OUNB in OUNBs){%>
            <dd <%if (OUNB_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=OUNB.id%>,'<%=OUNB.Code%>',<%=OUNB.ExchangeRate%>,'<%=OUNB.Msige%>','<%=OUNB.DecimalLength%>');"><%=OUNB.Code%></a></dd>
            <%OUNB_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int PHOe_index=1;
List<Lebi_Searchkey> PHOes = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey PHOe in PHOes){%><%=Lang(PHOe.Name)%><%PHOe_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int kOSI_index=1;
List<Lebi_Searchkey> kOSIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey kOSI in kOSIs){%>
<%if (kOSI.Type==1){ %><a href="<%=URL("P_Search",""+Lang(kOSI.Name)+"") %>"><%}else{ %><a href="<%=kOSI.URL%>" target="_blank"><%} %><span><%=Lang(kOSI.Name)%></span></a>
<%kOSI_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int pjwn_index=1;
List<Lebi_Page> pjwns = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page pjwn in pjwns){%>
<a class="menu" href="<%=URL("","",pjwn.url)%>"><span><%=pjwn.Name%></span></a>
<%pjwn_index++;}%>
...
<div class="user">
    <ul class="tablist">
        <li><a href="<%=URL("P_UserMessage","0","")%>"><span>收件箱<em>(<%=Count_Message(0) %>)</em></span></a></li>
        <li><a href="<%=URL("P_UserMessage","1","")%>"><span>發件箱</span></a></li>
        <li class="selected"><a href="<%=URL("P_UserMessageWrite","","")%>"><span>發信息</span></a></li>
    </ul>
    ...
    <dl class="dl-btn">
        <dt></dt>
        <dd><a href="javascript:void(0)" onclick="submit();" class="btn btn-11"><s></s>發送</a></dd>
    </dl>
    </div>
    </div>
</div>
...
<script type="text/javascript">
    function submit() {
        ...
        RequestAjax(url, postData, function () { MsgBox(1, "發送成功", "<%=URL("P_UserMessage","1","")%>") });
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int qoiK_index=1;
List<Lebi_FriendLink> qoiKs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink qoiK in qoiKs){%>
            
                <% if (qoiK.Logo != "" && qoiK.IsPic == 1){ %><a href="<%=qoiK.Url%>" target="_blank"><img src="<%=Image(qoiK.Logo) %>" alt="<%=qoiK.Name%>" /></a><%}else{%><a href="<%=qoiK.Url%>" target="_blank"><%=qoiK.Name%></a><%} %>     
