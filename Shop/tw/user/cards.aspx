...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserCard"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> giyrs=Languages();RecordCount=giyrs.Count;int giyr_index=1;
foreach (Shop.Model.Lebi_Language giyr in giyrs){%>
            <dd <%if (giyr_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=giyr.id%>,'<%=giyr.Code%>','<%=giyr.Path%>');"><%if (giyr.ImageUrl!=""){%><img src="<%=Image(giyr.ImageUrl) %>" /><%}%><%=giyr.Name%></a></dd>
            <%giyr_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int whNk_index=1;
List<Lebi_Currency> whNks = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency whNk in whNks){%>
            <dd <%if (whNk_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=whNk.id%>,'<%=whNk.Code%>',<%=whNk.ExchangeRate%>,'<%=whNk.Msige%>','<%=whNk.DecimalLength%>');"><%=whNk.Code%></a></dd>
            <%whNk_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int BRBU_index=1;
List<Lebi_Searchkey> BRBUs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey BRBU in BRBUs){%><%=Lang(BRBU.Name)%><%BRBU_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int jtva_index=1;
List<Lebi_Searchkey> jtvas = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey jtva in jtvas){%>
<%if (jtva.Type==1){ %><a href="<%=URL("P_Search",""+Lang(jtva.Name)+"") %>"><%}else{ %><a href="<%=jtva.URL%>" target="_blank"><%} %><span><%=Lang(jtva.Name)%></span></a>
<%jtva_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int fOWX_index=1;
List<Lebi_Page> fOWXs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page fOWX in fOWXs){%>
<a class="menu" href="<%=URL("","",fOWX.url)%>"><span><%=fOWX.Name%></span></a>
<%fOWX_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int QgIk_index=1;
List<Lebi_FriendLink> QgIks = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink QgIk in QgIks){%>
            
                <% if (QgIk.Logo != "" && QgIk.IsPic == 1){ %><a href="<%=QgIk.Url%>" target="_blank"><img src="<%=Image(QgIk.Logo) %>" alt="<%=QgIk.Name%>" /></a><%}else{%><a href="<%=QgIk.Url%>" target="_blank"><%=QgIk.Name%></a><%} %>     
