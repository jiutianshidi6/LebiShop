...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserOftenBuy"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> xlLUs=Languages();RecordCount=xlLUs.Count;int xlLU_index=1;
foreach (Shop.Model.Lebi_Language xlLU in xlLUs){%>
            <dd <%if (xlLU_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=xlLU.id%>,'<%=xlLU.Code%>','<%=xlLU.Path%>');"><%if (xlLU.ImageUrl!=""){%><img src="<%=Image(xlLU.ImageUrl) %>" /><%}%><%=xlLU.Name%></a></dd>
            <%xlLU_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int yWGv_index=1;
List<Lebi_Currency> yWGvs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency yWGv in yWGvs){%>
            <dd <%if (yWGv_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=yWGv.id%>,'<%=yWGv.Code%>',<%=yWGv.ExchangeRate%>,'<%=yWGv.Msige%>','<%=yWGv.DecimalLength%>');"><%=yWGv.Code%></a></dd>
            <%yWGv_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int DHnF_index=1;
List<Lebi_Searchkey> DHnFs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey DHnF in DHnFs){%><%=Lang(DHnF.Name)%><%DHnF_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int XDDK_index=1;
List<Lebi_Searchkey> XDDKs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey XDDK in XDDKs){%>
<%if (XDDK.Type==1){ %><a href="<%=URL("P_Search",""+Lang(XDDK.Name)+"") %>"><%}else{ %><a href="<%=XDDK.URL%>" target="_blank"><%} %><span><%=Lang(XDDK.Name)%></span></a>
<%XDDK_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int MneI_index=1;
List<Lebi_Page> MneIs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page MneI in MneIs){%>
<a class="menu" href="<%=URL("","",MneI.url)%>"><span><%=MneI.Name%></span></a>
<%MneI_index++;}%>
...
<div class="user">
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        <tr>
            ...
            <th style="width:80px;">購買週期</th>
            <th style="width:120px;">預購時間</th>
        </tr>
        ...
        <tr>
            ...
            <td><input name="WarnDays<%=u_p.id %>" id="WarnDays<%=u_p.id %>" type="text" value="<%=u_p.WarnDays %>" onkeyup="value=value.replace(/[^\d]/g,'');" shop="true" size="3" maxlength="5" class="input" /> 天</td>
            <td><%=FormatDate(u_p.Time_addemail)%></td>
        </tr>
        <%} %>
    </table>
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>刪除</a><a href="javascript:void(0)" onclick="Update();" class="btn btn-11"><s></s>保存</a><a href="javascript:void(0)" onclick="LikeToBasket();" class="btn btn-7"><s></s>加入購物車</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript" >
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int BUyy_index=1;
List<Lebi_FriendLink> BUyys = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink BUyy in BUyys){%>
            
                <% if (BUyy.Logo != "" && BUyy.IsPic == 1){ %><a href="<%=BUyy.Url%>" target="_blank"><img src="<%=Image(BUyy.Logo) %>" alt="<%=BUyy.Name%>" /></a><%}else{%><a href="<%=BUyy.Url%>" target="_blank"><%=BUyy.Name%></a><%} %>     
