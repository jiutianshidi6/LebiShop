...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> vaDgs=Languages();RecordCount=vaDgs.Count;int vaDg_index=1;
foreach (Shop.Model.Lebi_Language vaDg in vaDgs){%>
            <dd <%if (vaDg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=vaDg.id%>,'<%=vaDg.Code%>','<%=vaDg.Path%>');"><%if (vaDg.ImageUrl!=""){%><img src="<%=Image(vaDg.ImageUrl) %>" /><%}%><%=vaDg.Name%></a></dd>
            <%vaDg_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int UKPY_index=1;
List<Lebi_Currency> UKPYs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency UKPY in UKPYs){%>
            <dd <%if (UKPY_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=UKPY.id%>,'<%=UKPY.Code%>',<%=UKPY.ExchangeRate%>,'<%=UKPY.Msige%>','<%=UKPY.DecimalLength%>');"><%=UKPY.Code%></a></dd>
            <%UKPY_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MKCn_index=1;
List<Lebi_Searchkey> MKCns = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MKCn in MKCns){%><%=Lang(MKCn.Name)%><%MKCn_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int DdIj_index=1;
List<Lebi_Searchkey> DdIjs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey DdIj in DdIjs){%>
<%if (DdIj.Type==1){ %><a href="<%=URL("P_Search",""+Lang(DdIj.Name)+"") %>"><%}else{ %><a href="<%=DdIj.URL%>" target="_blank"><%} %><span><%=Lang(DdIj.Name)%></span></a>
<%DdIj_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int TPpJ_index=1;
List<Lebi_Page> TPpJs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page TPpJ in TPpJs){%>
<a class="menu" href="<%=URL("","",TPpJ.url)%>"><span><%=TPpJ.Name%></span></a>
<%TPpJ_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int EDgG_index=1;
List<Lebi_FriendLink> EDgGs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink EDgG in EDgGs){%>
            
                <% if (EDgG.Logo != "" && EDgG.IsPic == 1){ %><a href="<%=EDgG.Url%>" target="_blank"><img src="<%=Image(EDgG.Logo) %>" alt="<%=EDgG.Name%>" /></a><%}else{%><a href="<%=EDgG.Url%>" target="_blank"><%=EDgG.Name%></a><%} %>     
