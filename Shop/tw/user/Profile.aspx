...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> lSWys=Languages();RecordCount=lSWys.Count;int lSWy_index=1;
foreach (Shop.Model.Lebi_Language lSWy in lSWys){%>
            <dd <%if (lSWy_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=lSWy.id%>,'<%=lSWy.Code%>','<%=lSWy.Path%>');"><%if (lSWy.ImageUrl!=""){%><img src="<%=Image(lSWy.ImageUrl) %>" /><%}%><%=lSWy.Name%></a></dd>
            <%lSWy_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int AWNH_index=1;
List<Lebi_Currency> AWNHs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency AWNH in AWNHs){%>
            <dd <%if (AWNH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=AWNH.id%>,'<%=AWNH.Code%>',<%=AWNH.ExchangeRate%>,'<%=AWNH.Msige%>','<%=AWNH.DecimalLength%>');"><%=AWNH.Code%></a></dd>
            <%AWNH_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int HYyM_index=1;
List<Lebi_Searchkey> HYyMs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey HYyM in HYyMs){%><%=Lang(HYyM.Name)%><%HYyM_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int aPeR_index=1;
List<Lebi_Searchkey> aPeRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey aPeR in aPeRs){%>
<%if (aPeR.Type==1){ %><a href="<%=URL("P_Search",""+Lang(aPeR.Name)+"") %>"><%}else{ %><a href="<%=aPeR.URL%>" target="_blank"><%} %><span><%=Lang(aPeR.Name)%></span></a>
<%aPeR_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int tGqk_index=1;
List<Lebi_Page> tGqks = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page tGqk in tGqks){%>
<a class="menu" href="<%=URL("","",tGqk.url)%>"><span><%=tGqk.Name%></span></a>
<%tGqk_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int kXCd_index=1;
List<Lebi_FriendLink> kXCds = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink kXCd in kXCds){%>
            
                <% if (kXCd.Logo != "" && kXCd.IsPic == 1){ %><a href="<%=kXCd.Url%>" target="_blank"><img src="<%=Image(kXCd.Logo) %>" alt="<%=kXCd.Name%>" /></a><%}else{%><a href="<%=kXCd.Url%>" target="_blank"><%=kXCd.Name%></a><%} %>     
