...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserMoney"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> TlDls=Languages();RecordCount=TlDls.Count;int TlDl_index=1;
foreach (Shop.Model.Lebi_Language TlDl in TlDls){%>
            <dd <%if (TlDl_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=TlDl.id%>,'<%=TlDl.Code%>','<%=TlDl.Path%>');"><%if (TlDl.ImageUrl!=""){%><img src="<%=Image(TlDl.ImageUrl) %>" /><%}%><%=TlDl.Name%></a></dd>
            <%TlDl_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int rLfF_index=1;
List<Lebi_Currency> rLfFs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency rLfF in rLfFs){%>
            <dd <%if (rLfF_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=rLfF.id%>,'<%=rLfF.Code%>',<%=rLfF.ExchangeRate%>,'<%=rLfF.Msige%>','<%=rLfF.DecimalLength%>');"><%=rLfF.Code%></a></dd>
            <%rLfF_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int BvQd_index=1;
List<Lebi_Searchkey> BvQds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey BvQd in BvQds){%><%=Lang(BvQd.Name)%><%BvQd_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int JSxh_index=1;
List<Lebi_Searchkey> JSxhs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey JSxh in JSxhs){%>
<%if (JSxh.Type==1){ %><a href="<%=URL("P_Search",""+Lang(JSxh.Name)+"") %>"><%}else{ %><a href="<%=JSxh.URL%>" target="_blank"><%} %><span><%=Lang(JSxh.Name)%></span></a>
<%JSxh_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int xMdE_index=1;
List<Lebi_Page> xMdEs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page xMdE in xMdEs){%>
<a class="menu" href="<%=URL("","",xMdE.url)%>"><span><%=xMdE.Name%></span></a>
<%xMdE_index++;}%>
...
<div class="user">
    <div class="searchbox clearfix">
        ...
        <a href="<%=URL("P_Recharge","") %>"  class="btn btn-12"><s></s>充值</a>
        <a href="<%=URL("P_UserCash","") %>"  class="btn btn-12"><s></s>提現</a>
        <div style="padding:10px 0 0 0;">
         帳戶餘額：<%=FormatMoney(CurrentUser.Money) %>
        </div>
    </div>
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        ...
        <tr>
            ...
            <td><%=TypeName(model.Type_id_MoneyType)%></td>
            <td><%=FormatMoney(model.Money) %></td>
            <td><%=model.Remark %></td>
        </tr>
        <%} %>
    </table>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int kXGW_index=1;
List<Lebi_FriendLink> kXGWs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink kXGW in kXGWs){%>
            
                <% if (kXGW.Logo != "" && kXGW.IsPic == 1){ %><a href="<%=kXGW.Url%>" target="_blank"><img src="<%=Image(kXGW.Logo) %>" alt="<%=kXGW.Name%>" /></a><%}else{%><a href="<%=kXGW.Url%>" target="_blank"><%=kXGW.Name%></a><%} %>     
