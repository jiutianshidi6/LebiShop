...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_UserMessage"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> FWYqs=Languages();RecordCount=FWYqs.Count;int FWYq_index=1;
foreach (Shop.Model.Lebi_Language FWYq in FWYqs){%>
            <dd <%if (FWYq_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=FWYq.id%>,'<%=FWYq.Code%>','<%=FWYq.Path%>');"><%if (FWYq.ImageUrl!=""){%><img src="<%=Image(FWYq.ImageUrl) %>" /><%}%><%=FWYq.Name%></a></dd>
            <%FWYq_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int CvAL_index=1;
List<Lebi_Currency> CvALs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency CvAL in CvALs){%>
            <dd <%if (CvAL_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=CvAL.id%>,'<%=CvAL.Code%>',<%=CvAL.ExchangeRate%>,'<%=CvAL.Msige%>','<%=CvAL.DecimalLength%>');"><%=CvAL.Code%></a></dd>
            <%CvAL_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int AusW_index=1;
List<Lebi_Searchkey> AusWs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey AusW in AusWs){%><%=Lang(AusW.Name)%><%AusW_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int FddW_index=1;
List<Lebi_Searchkey> FddWs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey FddW in FddWs){%>
<%if (FddW.Type==1){ %><a href="<%=URL("P_Search",""+Lang(FddW.Name)+"") %>"><%}else{ %><a href="<%=FddW.URL%>" target="_blank"><%} %><span><%=Lang(FddW.Name)%></span></a>
<%FddW_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int KMwr_index=1;
List<Lebi_Page> KMwrs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page KMwr in KMwrs){%>
<a class="menu" href="<%=URL("","",KMwr.url)%>"><span><%=KMwr.Name%></span></a>
<%KMwr_index++;}%>
...
<div class="user">
    <ul class="tablist">
        ...
        <li <%if (type == 1){Response.Write("class=\"selected\"");} %>><a href="<%=URL("P_UserMessage","1","")%>"><span>發件箱</span></a></li>
        <li><a href="<%=URL("P_UserMessageWrite","","")%>"><span>發信息</span></a></li>
    </ul>
    <div class="searchbox clearfix">
        <select id="type_id">
            ...
            <%} %>
            <option value="0" <%if (type_id == "0"){%>selected<%} %>>系統消息</option>
        </select>
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        <tr>
            <th style="width:30px;" class="selectAll"><a href="javascript:void(0);" onclick="$('input[name=\'id\']').attr('checked',!$(this).attr('checked'));$(this).attr('checked',!$(this).attr('checked'));">全選</a></th>
            <th style="width:30px;">狀態</th>
            <th style="width:120px;">時間</th>
            <th style="width:*;">標題</th>     
        </tr>
        ...
        <tr class="list">
            ...
            <td style="text-align:center"><%if (model.IsRead == 0){%><img src="/Theme/system/images/icon/email.png" title="未讀" /><%}else{ %><img src="/Theme/system/images/icon/email_open.png" title="已讀" /><%} %></td>
            <td><%=FormatTime(model.Time_Add) %></td>
            <td><a href="<%=URL("P_UserMessageDetails",model.id)%>"><%=model.Title %></a></td>
        </tr>
        <%} %>
    </table>
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>刪除</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript">
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int oRct_index=1;
List<Lebi_FriendLink> oRcts = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink oRct in oRcts){%>
            
                <% if (oRct.Logo != "" && oRct.IsPic == 1){ %><a href="<%=oRct.Url%>" target="_blank"><img src="<%=Image(oRct.Logo) %>" alt="<%=oRct.Name%>" /></a><%}else{%><a href="<%=oRct.Url%>" target="_blank"><%=oRct.Name%></a><%} %>     
