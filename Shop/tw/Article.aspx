...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_Article"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> RNsXs=Languages();RecordCount=RNsXs.Count;int RNsX_index=1;
foreach (Shop.Model.Lebi_Language RNsX in RNsXs){%>
            <dd <%if (RNsX_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=RNsX.id%>,'<%=RNsX.Code%>','<%=RNsX.Path%>');"><%if (RNsX.ImageUrl!=""){%><img src="<%=Image(RNsX.ImageUrl) %>" /><%}%><%=RNsX.Name%></a></dd>
            <%RNsX_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int SmeK_index=1;
List<Lebi_Currency> SmeKs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency SmeK in SmeKs){%>
            <dd <%if (SmeK_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=SmeK.id%>,'<%=SmeK.Code%>',<%=SmeK.ExchangeRate%>,'<%=SmeK.Msige%>','<%=SmeK.DecimalLength%>');"><%=SmeK.Code%></a></dd>
            <%SmeK_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int HWKq_index=1;
List<Lebi_Searchkey> HWKqs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey HWKq in HWKqs){%><%=Lang(HWKq.Name)%><%HWKq_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int jTPU_index=1;
List<Lebi_Searchkey> jTPUs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey jTPU in jTPUs){%>
<%if (jTPU.Type==1){ %><a href="<%=URL("P_Search",""+Lang(jTPU.Name)+"") %>"><%}else{ %><a href="<%=jTPU.URL%>" target="_blank"><%} %><span><%=Lang(jTPU.Name)%></span></a>
<%jTPU_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int UsYq_index=1;
List<Lebi_Page> UsYqs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page UsYq in UsYqs){%>
<a class="menu" href="<%=URL("","",UsYq.url)%>"><span><%=UsYq.Name%></span></a>
<%UsYq_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int AwbR_index=1;
List<Lebi_FriendLink> AwbRs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink AwbR in AwbRs){%>
            
                <% if (AwbR.Logo != "" && AwbR.IsPic == 1){ %><a href="<%=AwbR.Url%>" target="_blank"><img src="<%=Image(AwbR.Logo) %>" alt="<%=AwbR.Name%>" /></a><%}else{%><a href="<%=AwbR.Url%>" target="_blank"><%=AwbR.Name%></a><%} %>     
