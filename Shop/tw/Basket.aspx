...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_Basket"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> oXzEs=Languages();RecordCount=oXzEs.Count;int oXzE_index=1;
foreach (Shop.Model.Lebi_Language oXzE in oXzEs){%>
            <dd <%if (oXzE_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=oXzE.id%>,'<%=oXzE.Code%>','<%=oXzE.Path%>');"><%if (oXzE.ImageUrl!=""){%><img src="<%=Image(oXzE.ImageUrl) %>" /><%}%><%=oXzE.Name%></a></dd>
            <%oXzE_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int lcXR_index=1;
List<Lebi_Currency> lcXRs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency lcXR in lcXRs){%>
            <dd <%if (lcXR_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=lcXR.id%>,'<%=lcXR.Code%>',<%=lcXR.ExchangeRate%>,'<%=lcXR.Msige%>','<%=lcXR.DecimalLength%>');"><%=lcXR.Code%></a></dd>
            <%lcXR_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int xqOI_index=1;
List<Lebi_Searchkey> xqOIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey xqOI in xqOIs){%><%=Lang(xqOI.Name)%><%xqOI_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int jBWN_index=1;
List<Lebi_Searchkey> jBWNs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey jBWN in jBWNs){%>
<%if (jBWN.Type==1){ %><a href="<%=URL("P_Search",""+Lang(jBWN.Name)+"") %>"><%}else{ %><a href="<%=jBWN.URL%>" target="_blank"><%} %><span><%=Lang(jBWN.Name)%></span></a>
<%jBWN_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int zOlg_index=1;
List<Lebi_Page> zOlgs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page zOlg in zOlgs){%>
<a class="menu" href="<%=URL("","",zOlg.url)%>"><span><%=zOlg.Name%></span></a>
<%zOlg_index++;}%>
...
<ul>
	<li class="current"><span>加入購物車</span></li>
	<li class="over"><span class="con">填寫訂單信息</span></li>
...
<%if (basket.Products.Count == 0 && basket.FreeProducts.Count == 0){%>
    ...
    <tr><td class="basketempty">購物車為空，請將您要購買的商品放入購物車</td></tr>
    </table>
<%}else{ %>
<%foreach(Shop.Model.BasketShop shop in basket.Shops){ %>
<%if (shop.Shop.id==0 && basket.Shops.Count==1){%><%}else{ %>
<div class="shop"><span><%if (shop.Shop.id==0){%>自營商品<%}else {%><%=Lang(shop.Shop.Name) %><%} %></span></div>
<%} %>
<div class="basketlist">
    <table align="center" cellpadding="0" cellspacing="0">
        ...
        <tr valign="middle">
            ...
            <td class="list" style="text-align: center">
                <a href="javascript:void(0)" onclick="UserProduct_Del(<%=p.Product_id %>,142);" class="btn btn-12"><s></s>移除購物車</a>
            </td>
        </tr>
        <%} %>  
        </table>
    ...
    <%if (shop.Products.Count > 0 || shop.FreeProducts.Count > 0)
      {%>
    <div class="money">
        <%if (shop.Money_Market > 0){ %>市場價：<span><%=FormatMoney(shop.Money_Market)%></span>&nbsp;&nbsp;<%} %>
        <%if (shop.Money_Cut>0){ %>減免：<span><%=FormatMoney(shop.Money_Cut)%></span>&nbsp;&nbsp;<%} %>
        購買價：<span><%=FormatMoney(shop.Money_Product-shop.Money_Cut)%></span>&nbsp;&nbsp;
        <%if(shop.Money_Property>0){ %>
        其它費用：<span><%=FormatMoney(shop.Money_Property)%></span>&nbsp;&nbsp;
        <%} %>
        <%if (shop.Money_Market - shop.Money_Product - shop.Money_Cut >0){ %>節省：<s><span><%=FormatMoney(shop.Money_Market - shop.Money_Product-shop.Money_Cut)%></span></s>&nbsp;&nbsp;<%} %>
        <%if(shop.Point_Buy>0){ %>
        所需積分：<span><%=shop.Point_Buy%></span>&nbsp;&nbsp;
        <%} %>
        <%if(shop.Point>0){ %>
        獲得積分：<span><%=shop.Point%></span>&nbsp;&nbsp;
        <%} %>
        <%if (shop.Weight>0){ %>總重量：<span><%=shop.Weight%> KG</span><%} %>
    </div>
   <%} %>
    <%if(IsMutiCash==true){ %>
    <div class="basketbtn">
        <a href="javascript:void(0)" onclick="location.href='<%=URL("P_CheckOut",shop.Shop.id) %>'" class="btn btn-7"><s></s>去收銀台</a>
    </div>
    <%} %>
<%} %>
<%if(IsMutiCash==false){ %>
<%if (basket.Shops.Count > 1){%>
    <div class="money">
        <%if (basket.Money_Market > 0){ %><p>市場價：<span><%=FormatMoney(basket.Money_Market)%></span></p><%} %>
        <%if (basket.Money_Cut>0){ %><p>減免：<span><%=FormatMoney(basket.Money_Cut)%></span></p><%} %>
        <p>購買價：<span><%=FormatMoney(basket.Money_Product-basket.Money_Cut)%></span></p>
        <%if(basket.Money_Property>0){ %>
        <p>其它費用：<span><%=FormatMoney(basket.Money_Property)%></span></p>
        <%} %>
        <%if (basket.Money_Market - basket.Money_Product - basket.Money_Cut >0){ %><p>節省：<s><span><%=FormatMoney(basket.Money_Market - basket.Money_Product-basket.Money_Cut)%></span></s></p><%} %>
        <%if(basket.Point_Buy>0){ %>
        <p>所需積分：<span><%=basket.Point_Buy%></span></p>
        <%} %>
        <%if(basket.Point>0){ %>
        <p>獲得積分：<span><%=basket.Point%></span></p>
        <%} %>
        <%if (basket.Weight>0){ %><p>總重量：<span><%=basket.Weight%> KG</span></p><%} %>
    </div>
<%} %>
<%} %>
<%if(basket.Money_Refund_VAT>0){ %>
    <div class="money">
        ...
        <p>退稅總額：<span><%=FormatMoney(basket.Money_Refund)%></span></p>
        <%} %>
        <%if(basket.Money_Refund>basket.OtherSite_Money_Refund && basket.OtherSite_Money_Refund>0){ %>
        <p>高街退稅額：<span><%=FormatMoney(basket.OtherSite_Money_Refund)%></span></p>
        <%} %>
        <%if(basket.Money_Refund - basket.OtherSite_Money_Refund>0){ %>
        <p>預計本次購物您多得退稅額：<span><%=FormatMoney(basket.Money_Refund - basket.OtherSite_Money_Refund)%></span></p>
        <%} %>
    </div>
<%} %>
<%if(basket.Money_Tax>0){ %>
    <div class="money">
        <p>稅金：<span><%=FormatMoney(basket.Money_Tax)%></span></p>
    </div>
<%} %>
<div class="basketbtn">
    <a href="javascript:void(0)" onclick="javascript:history.back();" class="btn btn-11"><s></s>繼續購物</a>
    <a href="javascript:void(0)" onclick="UserProduct_Del('all',142);" class="btn btn-11"><s></s>清空購物車</a>
    <%if(IsMutiCash==false){ %>
    <a href="<%=URL("P_CheckOut","") %>" class="btn btn-7"><s></s>去收銀台</a>
    <%} %>
</div>
<%} %>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int DoKW_index=1;
List<Lebi_FriendLink> DoKWs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink DoKW in DoKWs){%>
            
                <% if (DoKW.Logo != "" && DoKW.IsPic == 1){ %><a href="<%=DoKW.Url%>" target="_blank"><img src="<%=Image(DoKW.Logo) %>" alt="<%=DoKW.Name%>" /></a><%}else{%><a href="<%=DoKW.Url%>" target="_blank"><%=DoKW.Name%></a><%} %>     
