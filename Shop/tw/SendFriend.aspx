...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_SendFriend"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> xovHs=Languages();RecordCount=xovHs.Count;int xovH_index=1;
foreach (Shop.Model.Lebi_Language xovH in xovHs){%>
            <dd <%if (xovH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=xovH.id%>,'<%=xovH.Code%>','<%=xovH.Path%>');"><%if (xovH.ImageUrl!=""){%><img src="<%=Image(xovH.ImageUrl) %>" /><%}%><%=xovH.Name%></a></dd>
            <%xovH_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int dBQm_index=1;
List<Lebi_Currency> dBQms = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency dBQm in dBQms){%>
            <dd <%if (dBQm_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=dBQm.id%>,'<%=dBQm.Code%>',<%=dBQm.ExchangeRate%>,'<%=dBQm.Msige%>','<%=dBQm.DecimalLength%>');"><%=dBQm.Code%></a></dd>
            <%dBQm_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int dvmF_index=1;
List<Lebi_Searchkey> dvmFs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey dvmF in dvmFs){%><%=Lang(dvmF.Name)%><%dvmF_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int yQdR_index=1;
List<Lebi_Searchkey> yQdRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey yQdR in yQdRs){%>
<%if (yQdR.Type==1){ %><a href="<%=URL("P_Search",""+Lang(yQdR.Name)+"") %>"><%}else{ %><a href="<%=yQdR.URL%>" target="_blank"><%} %><span><%=Lang(yQdR.Name)%></span></a>
<%yQdR_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int EIhx_index=1;
List<Lebi_Page> EIhxs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page EIhx in EIhxs){%>
<a class="menu" href="<%=URL("","",EIhx.url)%>"><span><%=EIhx.Name%></span></a>
<%EIhx_index++;}%>
...
<script type="text/javascript">
    function submit() {
        ...
        RequestAjax(url, postData, function () { MsgBox(1, "發送成功", "<%=URL("P_Product",id) %>");});
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int AUiy_index=1;
List<Lebi_FriendLink> AUiys = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink AUiy in AUiys){%>
            
                <% if (AUiy.Logo != "" && AUiy.IsPic == 1){ %><a href="<%=AUiy.Url%>" target="_blank"><img src="<%=Image(AUiy.Logo) %>" alt="<%=AUiy.Name%>" /></a><%}else{%><a href="<%=AUiy.Url%>" target="_blank"><%=AUiy.Name%></a><%} %>     
