...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_Pay"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> OVdbs=Languages();RecordCount=OVdbs.Count;int OVdb_index=1;
foreach (Shop.Model.Lebi_Language OVdb in OVdbs){%>
            <dd <%if (OVdb_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=OVdb.id%>,'<%=OVdb.Code%>','<%=OVdb.Path%>');"><%if (OVdb.ImageUrl!=""){%><img src="<%=Image(OVdb.ImageUrl) %>" /><%}%><%=OVdb.Name%></a></dd>
            <%OVdb_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int PxAz_index=1;
List<Lebi_Currency> PxAzs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency PxAz in PxAzs){%>
            <dd <%if (PxAz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=PxAz.id%>,'<%=PxAz.Code%>',<%=PxAz.ExchangeRate%>,'<%=PxAz.Msige%>','<%=PxAz.DecimalLength%>');"><%=PxAz.Code%></a></dd>
            <%PxAz_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hkll_index=1;
List<Lebi_Searchkey> hklls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hkll in hklls){%><%=Lang(hkll.Name)%><%hkll_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int VMIg_index=1;
List<Lebi_Searchkey> VMIgs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey VMIg in VMIgs){%>
<%if (VMIg.Type==1){ %><a href="<%=URL("P_Search",""+Lang(VMIg.Name)+"") %>"><%}else{ %><a href="<%=VMIg.URL%>" target="_blank"><%} %><span><%=Lang(VMIg.Name)%></span></a>
<%VMIg_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int VOMF_index=1;
List<Lebi_Page> VOMFs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page VOMF in VOMFs){%>
<a class="menu" href="<%=URL("","",VOMF.url)%>"><span><%=VOMF.Name%></span></a>
<%VOMF_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int AUku_index=1;
List<Lebi_FriendLink> AUkus = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink AUku in AUkus){%>
            
                <% if (AUku.Logo != "" && AUku.IsPic == 1){ %><a href="<%=AUku.Url%>" target="_blank"><img src="<%=Image(AUku.Logo) %>" alt="<%=AUku.Name%>" /></a><%}else{%><a href="<%=AUku.Url%>" target="_blank"><%=AUku.Name%></a><%} %>     
