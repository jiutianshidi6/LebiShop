...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_ArticleDetails"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> PvmPs=Languages();RecordCount=PvmPs.Count;int PvmP_index=1;
foreach (Shop.Model.Lebi_Language PvmP in PvmPs){%>
            <dd <%if (PvmP_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=PvmP.id%>,'<%=PvmP.Code%>','<%=PvmP.Path%>');"><%if (PvmP.ImageUrl!=""){%><img src="<%=Image(PvmP.ImageUrl) %>" /><%}%><%=PvmP.Name%></a></dd>
            <%PvmP_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Ofuc_index=1;
List<Lebi_Currency> Ofucs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Ofuc in Ofucs){%>
            <dd <%if (Ofuc_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Ofuc.id%>,'<%=Ofuc.Code%>',<%=Ofuc.ExchangeRate%>,'<%=Ofuc.Msige%>','<%=Ofuc.DecimalLength%>');"><%=Ofuc.Code%></a></dd>
            <%Ofuc_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int qCPX_index=1;
List<Lebi_Searchkey> qCPXs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey qCPX in qCPXs){%><%=Lang(qCPX.Name)%><%qCPX_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int cVhE_index=1;
List<Lebi_Searchkey> cVhEs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey cVhE in cVhEs){%>
<%if (cVhE.Type==1){ %><a href="<%=URL("P_Search",""+Lang(cVhE.Name)+"") %>"><%}else{ %><a href="<%=cVhE.URL%>" target="_blank"><%} %><span><%=Lang(cVhE.Name)%></span></a>
<%cVhE_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int CUIP_index=1;
List<Lebi_Page> CUIPs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page CUIP in CUIPs){%>
<a class="menu" href="<%=URL("","",CUIP.url)%>"><span><%=CUIP.Name%></span></a>
<%CUIP_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id = "+ node.id +" and Language like '%" + CurrentLanguage.Code +"%' and id <> "+ id +"";Order="id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int TvBf_index=1;
List<Lebi_Page> TvBfs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page TvBf in TvBfs){%>
        <%if (TvBf.ImageSmall !=""){ %>
        <li class="image"><div class="image"><p><a href="<%=URL("P_ArticleDetails",TvBf.id,TvBf.url)%>" target="_blank"><img <%if (CurrentLanguage.IsLazyLoad==1){ %>class="lazy" src="/Theme/system/images/lazy.gif" data-original<%}else{ %>src<%} %>="<%=Image(TvBf.ImageSmall) %>" title="<%=Lang(TvBf.Name) %>" /></a></p></div><div class="title"><a href="<%=URL("P_ArticleDetails",TvBf.id,TvBf.url)%>" title="<%=TvBf.Name%>" target="_blank"><span style="color:<%=TvBf.NameColor%>"><%=TvBf.Name%></span></a></div></li>
        <%} else{%>
        <li class="text"><div class="title"><a href="<%=URL("P_ArticleDetails",TvBf.id,TvBf.url)%>" title="<%=TvBf.Name%>" target="_blank"><span style="color:<%=TvBf.NameColor%>"><%=TvBf.Name%></span></a></div></li>
        <%} %>
        <%TvBf_index++;}%>
    </ul>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int jOlx_index=1;
List<Lebi_FriendLink> jOlxs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink jOlx in jOlxs){%>
            
                <% if (jOlx.Logo != "" && jOlx.IsPic == 1){ %><a href="<%=jOlx.Url%>" target="_blank"><img src="<%=Image(jOlx.Logo) %>" alt="<%=jOlx.Name%>" /></a><%}else{%><a href="<%=jOlx.Url%>" target="_blank"><%=jOlx.Name%></a><%} %>     
