...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_AddToBasket"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> tdHHs=Languages();RecordCount=tdHHs.Count;int tdHH_index=1;
foreach (Shop.Model.Lebi_Language tdHH in tdHHs){%>
            <dd <%if (tdHH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=tdHH.id%>,'<%=tdHH.Code%>','<%=tdHH.Path%>');"><%if (tdHH.ImageUrl!=""){%><img src="<%=Image(tdHH.ImageUrl) %>" /><%}%><%=tdHH.Name%></a></dd>
            <%tdHH_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int yqeM_index=1;
List<Lebi_Currency> yqeMs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency yqeM in yqeMs){%>
            <dd <%if (yqeM_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=yqeM.id%>,'<%=yqeM.Code%>',<%=yqeM.ExchangeRate%>,'<%=yqeM.Msige%>','<%=yqeM.DecimalLength%>');"><%=yqeM.Code%></a></dd>
            <%yqeM_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int aFRU_index=1;
List<Lebi_Searchkey> aFRUs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey aFRU in aFRUs){%><%=Lang(aFRU.Name)%><%aFRU_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MBtt_index=1;
List<Lebi_Searchkey> MBtts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MBtt in MBtts){%>
<%if (MBtt.Type==1){ %><a href="<%=URL("P_Search",""+Lang(MBtt.Name)+"") %>"><%}else{ %><a href="<%=MBtt.URL%>" target="_blank"><%} %><span><%=Lang(MBtt.Name)%></span></a>
<%MBtt_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int wVhd_index=1;
List<Lebi_Page> wVhds = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page wVhd in wVhds){%>
<a class="menu" href="<%=URL("","",wVhd.url)%>"><span><%=wVhd.Name%></span></a>
<%wVhd_index++;}%>
    ...
    <div class="success">
        ...
        <div class="mc clearfix">
            <a href="javascript:history.go(-1);" class="btn btn-11"><s></s>
                繼續購物</a><a href="<%=URL("P_Basket","") %>" class="btn btn-7"><s></s>查看購物車並結算</a>
        </div>
    </div>
</div>
...
<div class="tag-tab">
    <ul class="tablist">
    <%Table="Lebi_Pro_Tag";Where="1=1";Order="Sort desc";PageSize=4;pageindex=1;RecordCount=B_Lebi_Pro_Tag.Counts(Where);int nKoX_index=1;
List<Lebi_Pro_Tag> nKoXs = B_Lebi_Pro_Tag.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Pro_Tag nKoX in nKoXs){%>
        <li <%=nKoX_index==1?" class=\"selected\"":"" %>><a href="<%=URL("P_ProductCategory","$,$,$,$,$,"+nKoX.id+"")%>" target="_blank"><span><%=Lang(nKoX.Name) %></span></a></li>
    <%nKoX_index++;}%>
    </ul>
</div>
...
List<Lebi_Product> tab_tag_pros = B_Lebi_Product.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Product tab_tag_pro in tab_tag_pros){%>
        <li>
            ...
            <div class="proprice"><%if (tab_tag_pro.Price_Market>0){ %><div class="marketprice"><i>市場價：</i><strong><%=FormatMoney(tab_tag_pro.Price_Market) %></strong></div><%} %><div class="buyprice"><i><%=Lang(CurrentUserLevel.PriceName) %>：</i><strong><%=FormatMoney(ProductPrice(tab_tag_pro)) %></strong></div></div>                    
        </li>
        <%tab_tag_pro_index++;}%>
        </ul></div>
    </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int QyTN_index=1;
List<Lebi_FriendLink> QyTNs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink QyTN in QyTNs){%>
            
                <% if (QyTN.Logo != "" && QyTN.IsPic == 1){ %><a href="<%=QyTN.Url%>" target="_blank"><img src="<%=Image(QyTN.Logo) %>" alt="<%=QyTN.Name%>" /></a><%}else{%><a href="<%=QyTN.Url%>" target="_blank"><%=QyTN.Name%></a><%} %>     
