...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_ForgetPassword"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> WwWbs=Languages();RecordCount=WwWbs.Count;int WwWb_index=1;
foreach (Shop.Model.Lebi_Language WwWb in WwWbs){%>
            <dd <%if (WwWb_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=WwWb.id%>,'<%=WwWb.Code%>','<%=WwWb.Path%>');"><%if (WwWb.ImageUrl!=""){%><img src="<%=Image(WwWb.ImageUrl) %>" /><%}%><%=WwWb.Name%></a></dd>
            <%WwWb_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Tagj_index=1;
List<Lebi_Currency> Tagjs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Tagj in Tagjs){%>
            <dd <%if (Tagj_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Tagj.id%>,'<%=Tagj.Code%>',<%=Tagj.ExchangeRate%>,'<%=Tagj.Msige%>','<%=Tagj.DecimalLength%>');"><%=Tagj.Code%></a></dd>
            <%Tagj_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int RYLB_index=1;
List<Lebi_Searchkey> RYLBs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey RYLB in RYLBs){%><%=Lang(RYLB.Name)%><%RYLB_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int RUle_index=1;
List<Lebi_Searchkey> RUles = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey RUle in RUles){%>
<%if (RUle.Type==1){ %><a href="<%=URL("P_Search",""+Lang(RUle.Name)+"") %>"><%}else{ %><a href="<%=RUle.URL%>" target="_blank"><%} %><span><%=Lang(RUle.Name)%></span></a>
<%RUle_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int gztR_index=1;
List<Lebi_Page> gztRs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page gztR in gztRs){%>
<a class="menu" href="<%=URL("","",gztR.url)%>"><span><%=gztR.Name%></span></a>
<%gztR_index++;}%>
...
<div class="userbox">
    ...
    <dl>
        <dt>驗證碼：</dt>
        <dd><input name="verifycode" type="text" shop="true" id="verifycode" shop="true" size="6" class="input" /><img class="verifycode" id="img1" src="<%=WebPath%>/code.aspx" /><img src="/Theme/system/images/reload.gif" class="refresh" title="點擊刷新驗證碼" onclick="refresh('img1')" /></dd>
    </dl>
    <%} %>
    <dl class="dl-btn">
        <dt></dt>
...
<script type="text/javascript">
    function submit() {
        ...
        if (type == 0) {
            if (Email == "") {
                CheckNO("Email", "請輸入您的郵箱");
                return false;
            }
            var postData = GetFormJsonData("shop");
            var url = path + "/ajax/ajax_user.aspx?__Action=User_forgetpwd&type=0";
            RequestAjax(url, postData, function () { $("#msg").html("找回密碼郵件已發送至您的郵箱，請查收"); MsgBoxClose(); });
        } else {
            if (UserName == "") {
                CheckNO("UserName", "請輸入您的登錄賬號");
                return false;
            }
            var postData = GetFormJsonData("shop");
            var url = path + "/ajax/ajax_user.aspx?__Action=User_forgetpwd&type=" + type + "";
            RequestAjax(url, postData, function (res) { if (type == 1) { location.href = res.url; } else { MsgBox(1, "新密碼已發送至您的手機，請查收", res.url); } });
        }
    }
    function GetType() {
        var type = $("input[name='type']:checked").val();
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int CnAw_index=1;
List<Lebi_FriendLink> CnAws = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink CnAw in CnAws){%>
            
                <% if (CnAw.Logo != "" && CnAw.IsPic == 1){ %><a href="<%=CnAw.Url%>" target="_blank"><img src="<%=Image(CnAw.Logo) %>" alt="<%=CnAw.Name%>" /></a><%}else{%><a href="<%=CnAw.Url%>" target="_blank"><%=CnAw.Name%></a><%} %>     
