<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.P_NewsDetails" validateRequest="false"%>


<%@ Import Namespace="Shop.Bussiness" %>
<%@ Import Namespace="Shop.Model" %>
<%@ Import Namespace="System.Collections.Generic" %>
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_NewsDetails"); %>
...
<title><%=ThemePageMeta("P_NewsDetails","title")%></title>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Language" content="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0;" />
<meta name="keywords" content="<%=ThemePageMeta("P_NewsDetails","keywords")%>" />
<meta name="description" content="<%=ThemePageMeta("P_NewsDetails","description")%>" />
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> Igmrs=Languages();RecordCount=Igmrs.Count;int Igmr_index=1;
foreach (Shop.Model.Lebi_Language Igmr in Igmrs){%>
            <dd <%if (Igmr_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Igmr.id%>,'<%=Igmr.Code%>','<%=Igmr.Path%>');"><%if (Igmr.ImageUrl!=""){%><img src="<%=Image(Igmr.ImageUrl) %>" /><%}%><%=Igmr.Name%></a></dd>
            <%Igmr_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int tCIJ_index=1;
List<Lebi_Currency> tCIJs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency tCIJ in tCIJs){%>
            <dd <%if (tCIJ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=tCIJ.id%>,'<%=tCIJ.Code%>',<%=tCIJ.ExchangeRate%>,'<%=tCIJ.Msige%>','<%=tCIJ.DecimalLength%>');"><%=tCIJ.Code%></a></dd>
            <%tCIJ_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int WaEi_index=1;
List<Lebi_Searchkey> WaEis = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey WaEi in WaEis){%><%=Lang(WaEi.Name)%><%WaEi_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int iAea_index=1;
List<Lebi_Searchkey> iAeas = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey iAea in iAeas){%>
<%if (iAea.Type==1){ %><a href="<%=URL("P_Search",""+Lang(iAea.Name)+"") %>"><%}else{ %><a href="<%=iAea.URL%>" target="_blank"><%} %><span><%=Lang(iAea.Name)%></span></a>
<%iAea_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int XNyx_index=1;
List<Lebi_Page> XNyxs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page XNyx in XNyxs){%>
<a class="menu" href="<%=URL("","",XNyx.url)%>"><span><%=XNyx.Name%></span></a>
<%XNyx_index++;}%>
...
<ul class="text">
    <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int CxVa_index=1;
List<Lebi_Brand> CxVas = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand CxVa in CxVas){%>
    <li><a href="<%=URL("P_Brand",CxVa.id)%>" title="<%=Lang(CxVa.Name) %>"><%=Lang(CxVa.Name) %></a></li>
    <%CxVa_index++;}%>
</ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int hXlR_index=1;
List<Lebi_FriendLink> hXlRs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink hXlR in hXlRs){%>
            
                <% if (hXlR.Logo != "" && hXlR.IsPic == 1){ %><a href="<%=hXlR.Url%>" target="_blank"><img src="<%=Image(hXlR.Logo) %>" alt="<%=hXlR.Name%>" /></a><%}else{%><a href="<%=hXlR.Url%>" target="_blank"><%=hXlR.Name%></a><%} %>     
