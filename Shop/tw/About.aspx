...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_About"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> NYgTs=Languages();RecordCount=NYgTs.Count;int NYgT_index=1;
foreach (Shop.Model.Lebi_Language NYgT in NYgTs){%>
            <dd <%if (NYgT_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=NYgT.id%>,'<%=NYgT.Code%>','<%=NYgT.Path%>');"><%if (NYgT.ImageUrl!=""){%><img src="<%=Image(NYgT.ImageUrl) %>" /><%}%><%=NYgT.Name%></a></dd>
            <%NYgT_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Litt_index=1;
List<Lebi_Currency> Litts = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Litt in Litts){%>
            <dd <%if (Litt_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Litt.id%>,'<%=Litt.Code%>',<%=Litt.ExchangeRate%>,'<%=Litt.Msige%>','<%=Litt.DecimalLength%>');"><%=Litt.Code%></a></dd>
            <%Litt_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vBbP_index=1;
List<Lebi_Searchkey> vBbPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vBbP in vBbPs){%><%=Lang(vBbP.Name)%><%vBbP_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int UqTD_index=1;
List<Lebi_Searchkey> UqTDs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey UqTD in UqTDs){%>
<%if (UqTD.Type==1){ %><a href="<%=URL("P_Search",""+Lang(UqTD.Name)+"") %>"><%}else{ %><a href="<%=UqTD.URL%>" target="_blank"><%} %><span><%=Lang(UqTD.Name)%></span></a>
<%UqTD_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int emlz_index=1;
List<Lebi_Page> emlzs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page emlz in emlzs){%>
<a class="menu" href="<%=URL("","",emlz.url)%>"><span><%=emlz.Name%></span></a>
<%emlz_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int KBsj_index=1;
List<Lebi_FriendLink> KBsjs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink KBsj in KBsjs){%>
            
                <% if (KBsj.Logo != "" && KBsj.IsPic == 1){ %><a href="<%=KBsj.Url%>" target="_blank"><img src="<%=Image(KBsj.Logo) %>" alt="<%=KBsj.Name%>" /></a><%}else{%><a href="<%=KBsj.Url%>" target="_blank"><%=KBsj.Name%></a><%} %>     
