...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_Login"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> psRgs=Languages();RecordCount=psRgs.Count;int psRg_index=1;
foreach (Shop.Model.Lebi_Language psRg in psRgs){%>
            <dd <%if (psRg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=psRg.id%>,'<%=psRg.Code%>','<%=psRg.Path%>');"><%if (psRg.ImageUrl!=""){%><img src="<%=Image(psRg.ImageUrl) %>" /><%}%><%=psRg.Name%></a></dd>
            <%psRg_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int gKTO_index=1;
List<Lebi_Currency> gKTOs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency gKTO in gKTOs){%>
            <dd <%if (gKTO_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=gKTO.id%>,'<%=gKTO.Code%>',<%=gKTO.ExchangeRate%>,'<%=gKTO.Msige%>','<%=gKTO.DecimalLength%>');"><%=gKTO.Code%></a></dd>
            <%gKTO_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vtjf_index=1;
List<Lebi_Searchkey> vtjfs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vtjf in vtjfs){%><%=Lang(vtjf.Name)%><%vtjf_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zObs_index=1;
List<Lebi_Searchkey> zObss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zObs in zObss){%>
<%if (zObs.Type==1){ %><a href="<%=URL("P_Search",""+Lang(zObs.Name)+"") %>"><%}else{ %><a href="<%=zObs.URL%>" target="_blank"><%} %><span><%=Lang(zObs.Name)%></span></a>
<%zObs_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int DOjW_index=1;
List<Lebi_Page> DOjWs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page DOjW in DOjWs){%>
<a class="menu" href="<%=URL("","",DOjW.url)%>"><span><%=DOjW.Name%></span></a>
<%DOjW_index++;}%>
            ...
            <div class="right">
                <div class="userloginbox">
                    ...
                    <dl id="code" class="last" <%if (LoginError != true){ %>style="display:none"<%}else{ %>style="display:" <%} %>>
                        <dt>驗證碼：</dt>
                        <dd><input name="verifycode" type="text" id="verifycode" shop="true" size="6" class="input" /><img class="verifycode" id="img1" src="<%=WebPath%>/code.aspx" /><img src="/Theme/system/images/reload.gif" class="refresh" title="點擊刷新驗證碼" onclick="refresh('img1')" /></dd>
                    </dl>
                    <%} %>
                    <dl>
                        <dt></dt>
                        <dd><a href="javascript:void(0)" onclick="submitlogin();" class="btn btn-7"><s></s>登錄</a>&nbsp;&nbsp;<a href="<%=URL("P_ForgetPassword","")%>">忘記密碼</a></dd>
                    </dl>
                    </div>
                    ...
                    <div class="mes">
                        <h2><span>還不是商城會員？</span></h2>
                        <div class="cont">
                            一分鐘輕鬆註冊，就可以方便購物！<a href="<%=URL("P_Register", ""+HttpUtility.UrlEncode(backurl)+"," + GetUrlToken(backurl)+ "") %>">免費註冊</a></div>
                    </div>
                </div>
            </div>
        </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int GAVh_index=1;
List<Lebi_FriendLink> GAVhs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink GAVh in GAVhs){%>
            
                <% if (GAVh.Logo != "" && GAVh.IsPic == 1){ %><a href="<%=GAVh.Url%>" target="_blank"><img src="<%=Image(GAVh.Logo) %>" alt="<%=GAVh.Name%>" /></a><%}else{%><a href="<%=GAVh.Url%>" target="_blank"><%=GAVh.Name%></a><%} %>     
