...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_GroupPurchase"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> byvos=Languages();RecordCount=byvos.Count;int byvo_index=1;
foreach (Shop.Model.Lebi_Language byvo in byvos){%>
            <dd <%if (byvo_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=byvo.id%>,'<%=byvo.Code%>','<%=byvo.Path%>');"><%if (byvo.ImageUrl!=""){%><img src="<%=Image(byvo.ImageUrl) %>" /><%}%><%=byvo.Name%></a></dd>
            <%byvo_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int zxNX_index=1;
List<Lebi_Currency> zxNXs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency zxNX in zxNXs){%>
            <dd <%if (zxNX_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=zxNX.id%>,'<%=zxNX.Code%>',<%=zxNX.ExchangeRate%>,'<%=zxNX.Msige%>','<%=zxNX.DecimalLength%>');"><%=zxNX.Code%></a></dd>
            <%zxNX_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hOJj_index=1;
List<Lebi_Searchkey> hOJjs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hOJj in hOJjs){%><%=Lang(hOJj.Name)%><%hOJj_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int npmU_index=1;
List<Lebi_Searchkey> npmUs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey npmU in npmUs){%>
<%if (npmU.Type==1){ %><a href="<%=URL("P_Search",""+Lang(npmU.Name)+"") %>"><%}else{ %><a href="<%=npmU.URL%>" target="_blank"><%} %><span><%=Lang(npmU.Name)%></span></a>
<%npmU_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int cSKw_index=1;
List<Lebi_Page> cSKws = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page cSKw in cSKws){%>
<a class="menu" href="<%=URL("","",cSKw.url)%>"><span><%=cSKw.Name%></span></a>
<%cSKw_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int YyOJ_index=1;
List<Lebi_Page> YyOJs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page YyOJ in YyOJs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",YyOJ.id,YyOJ.url)%>" target="<%=YyOJ.target%>" title="<%=YyOJ.Name%>"><span style="color:<%=YyOJ.NameColor%>"><%=YyOJ.Name%></span></a></li>
        <%YyOJ_index++;}%>
    </ul>
</div>
</div>
...
<div id="filter">
	<div class="fore1">
		<dl class="order">
			<dt>排序：</dt>
			<dd <%if(sort == "" || sort == "1" || sort == "1a"){ %>class="curr <%if(sort == "" || sort == "1"){ %>up<%}else if(sort == "1a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_GroupPurchase",""+list+",1"+ordertmp+"") %>">銷量</a><b></b></dd>
			<dd <%if(sort == "2" || sort == "2a"){ %>class="curr <%if(sort == "2"){ %>up<%}else if(sort == "2a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_GroupPurchase",""+list+",2"+ordertmp+"") %>">價格</a><b></b></dd>
			<dd <%if(sort == "3" || sort == "3a"){ %>class="curr <%if(sort == "3"){ %>up<%}else if(sort == "3a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_GroupPurchase",""+list+",3"+ordertmp+"") %>">評價數</a><b></b></dd>
			<dd <%if(sort == "4" || sort == "4a"){ %>class="curr <%if(sort == "4"){ %>up<%}else if(sort == "4a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_GroupPurchase",""+list+",4"+ordertmp+"") %>">上架時間</a><b></b></dd>
		</dl>
        <div class="list-cutover">
			...
			<a id="grid-unselected"  href="<%=URL("P_GroupPurchase","1,"+ sort) %>" class="modfiy-url grid-unselected <%=(list=="1" || list=="")?"grid-curr":"" %>"><b></b>網格</a>
		</div>
...
<div class="mc clearfix">
    <div class="productlist limitbuylist loadlist"><ul>
        ...
        <li class="grid loadli">
            ...
            <div class="proinfo">
            <div class="buyprice"><i>團購價：</i><strong><%=FormatMoney(pro.Price_Sale) %></strong><em><%=Discount(pro.Price_Market,pro.Price_Sale) %>% 折</em></div>
            <div class="marketprice"><i>銷售價：</i><strong><%=FormatMoney(pro.Price) %></strong></div>
            <div class="count"><i>已參團：</i><strong><%=pro.Count_Sales_Show%></strong></div>
            <div class="time_expired" endDate="<%if (System.DateTime.Now < pro.Time_Start){ %><%=DateFormat(pro.Time_Start)%><%}else{ %><%=DateFormat(pro.Time_Expired)%><%} %>" Product="<%=pro.id %>" id="time_expired_<%=pro.id %>">
                <%if (System.DateTime.Now < pro.Time_Start){ %>倒計時<%}else{ %>剩餘<%} %>：<span><span id="dayEnd_<%=pro.id %>">0</span><em>天</em><span id="hourEnd_<%=pro.id %>">0</span><em>小時</em><span id="minEnd_<%=pro.id %>">0</span><em>分鐘</em><span id="secEnd_<%=pro.id %>">0</span><em>秒</em></span>
            </div>
            <div class="probtn">
            <%if (System.DateTime.Now < pro.Time_Start){ %>
            <strong>開始：<%= FormatTime(pro.Time_Start) %></strong>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int QUkR_index=1;
List<Lebi_FriendLink> QUkRs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink QUkR in QUkRs){%>
            
                <% if (QUkR.Logo != "" && QUkR.IsPic == 1){ %><a href="<%=QUkR.Url%>" target="_blank"><img src="<%=Image(QUkR.Logo) %>" alt="<%=QUkR.Name%>" /></a><%}else{%><a href="<%=QUkR.Url%>" target="_blank"><%=QUkR.Name%></a><%} %>     
