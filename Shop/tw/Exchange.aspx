...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_Exchange"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> OcHes=Languages();RecordCount=OcHes.Count;int OcHe_index=1;
foreach (Shop.Model.Lebi_Language OcHe in OcHes){%>
            <dd <%if (OcHe_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=OcHe.id%>,'<%=OcHe.Code%>','<%=OcHe.Path%>');"><%if (OcHe.ImageUrl!=""){%><img src="<%=Image(OcHe.ImageUrl) %>" /><%}%><%=OcHe.Name%></a></dd>
            <%OcHe_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int MvfY_index=1;
List<Lebi_Currency> MvfYs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency MvfY in MvfYs){%>
            <dd <%if (MvfY_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=MvfY.id%>,'<%=MvfY.Code%>',<%=MvfY.ExchangeRate%>,'<%=MvfY.Msige%>','<%=MvfY.DecimalLength%>');"><%=MvfY.Code%></a></dd>
            <%MvfY_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int HdSg_index=1;
List<Lebi_Searchkey> HdSgs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey HdSg in HdSgs){%><%=Lang(HdSg.Name)%><%HdSg_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int gpjN_index=1;
List<Lebi_Searchkey> gpjNs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey gpjN in gpjNs){%>
<%if (gpjN.Type==1){ %><a href="<%=URL("P_Search",""+Lang(gpjN.Name)+"") %>"><%}else{ %><a href="<%=gpjN.URL%>" target="_blank"><%} %><span><%=Lang(gpjN.Name)%></span></a>
<%gpjN_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int xnIT_index=1;
List<Lebi_Page> xnITs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page xnIT in xnITs){%>
<a class="menu" href="<%=URL("","",xnIT.url)%>"><span><%=xnIT.Name%></span></a>
<%xnIT_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int nruz_index=1;
List<Lebi_Page> nruzs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page nruz in nruzs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",nruz.id,nruz.url)%>" target="<%=nruz.target%>" title="<%=nruz.Name%>"><span style="color:<%=nruz.NameColor%>"><%=nruz.Name%></span></a></li>
        <%nruz_index++;}%>
    </ul>
</div>
</div>
...
<div id="filter">
	<div class="fore1">
		<dl class="order">
			<dt>排序：</dt>
			<dd <%if(sort == "" || sort == "1" || sort == "1a"){ %>class="curr <%if(sort == "" || sort == "1"){ %>up<%}else if(sort == "1a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Exchange",""+list+",1"+ordertmp+"") %>">銷量</a><b></b></dd>
			<dd <%if(sort == "2" || sort == "2a"){ %>class="curr <%if(sort == "2"){ %>up<%}else if(sort == "2a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Exchange",""+list+",2"+ordertmp+"") %>">積分</a><b></b></dd>
			<dd <%if(sort == "3" || sort == "3a"){ %>class="curr <%if(sort == "3"){ %>up<%}else if(sort == "3a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Exchange",""+list+",3"+ordertmp+"") %>">評價數</a><b></b></dd>
			<dd <%if(sort == "4" || sort == "4a"){ %>class="curr <%if(sort == "4"){ %>up<%}else if(sort == "4a"){ %>down<%} %>"<%} %>><a href="<%=URL("P_Exchange",""+list+",4"+ordertmp+"") %>">上架時間</a><b></b></dd>
		</dl>
        <div class="list-cutover">
			...
			<a id="grid-unselected"  href="<%=URL("P_Exchange","1,"+ sort) %>" class="modfiy-url grid-unselected <%=(list=="1" || list=="")?"grid-curr":"" %>"><b></b>網格</a>
		</div>
...
<div class="mc clearfix">
    <div class="productlist limitbuylist loadlist"><ul>
        ...
        <li class="grid loadli">
            ...
            <div class="proinfo">
            <div class="buyprice"><i>積分：</i><strong><%=pro.Price_Sale %></strong></div>
            <div class="probtn">
            <%if(ProductStock(pro)<1){ %>
            <strong>換光了</strong>
            <%}else{ %>
            <a href="javascript:void(0)" onclick="UserProduct_Edit(<%=pro.id%>,142,$('#pro_num<%=pro.id%>').val());" class="buy btn btn-12"><s></s>兌換</a>
            <%} %>
            </div>
            </div>
        </li>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int fmNd_index=1;
List<Lebi_FriendLink> fmNds = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink fmNd in fmNds){%>
            
                <% if (fmNd.Logo != "" && fmNd.IsPic == 1){ %><a href="<%=fmNd.Url%>" target="_blank"><img src="<%=Image(fmNd.Logo) %>" alt="<%=fmNd.Name%>" /></a><%}else{%><a href="<%=fmNd.Url%>" target="_blank"><%=fmNd.Name%></a><%} %>     
