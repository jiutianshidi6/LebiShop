...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_Inquiry"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> PWhCs=Languages();RecordCount=PWhCs.Count;int PWhC_index=1;
foreach (Shop.Model.Lebi_Language PWhC in PWhCs){%>
            <dd <%if (PWhC_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=PWhC.id%>,'<%=PWhC.Code%>','<%=PWhC.Path%>');"><%if (PWhC.ImageUrl!=""){%><img src="<%=Image(PWhC.ImageUrl) %>" /><%}%><%=PWhC.Name%></a></dd>
            <%PWhC_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int EwCb_index=1;
List<Lebi_Currency> EwCbs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency EwCb in EwCbs){%>
            <dd <%if (EwCb_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=EwCb.id%>,'<%=EwCb.Code%>',<%=EwCb.ExchangeRate%>,'<%=EwCb.Msige%>','<%=EwCb.DecimalLength%>');"><%=EwCb.Code%></a></dd>
            <%EwCb_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int IpxF_index=1;
List<Lebi_Searchkey> IpxFs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey IpxF in IpxFs){%><%=Lang(IpxF.Name)%><%IpxF_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int FFKC_index=1;
List<Lebi_Searchkey> FFKCs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey FFKC in FFKCs){%>
<%if (FFKC.Type==1){ %><a href="<%=URL("P_Search",""+Lang(FFKC.Name)+"") %>"><%}else{ %><a href="<%=FFKC.URL%>" target="_blank"><%} %><span><%=Lang(FFKC.Name)%></span></a>
<%FFKC_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int aOUH_index=1;
List<Lebi_Page> aOUHs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page aOUH in aOUHs){%>
<a class="menu" href="<%=URL("","",aOUH.url)%>"><span><%=aOUH.Name%></span></a>
<%aOUH_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int CVLX_index=1;
List<Lebi_FriendLink> CVLXs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink CVLX in CVLXs){%>
            
                <% if (CVLX.Logo != "" && CVLX.IsPic == 1){ %><a href="<%=CVLX.Url%>" target="_blank"><img src="<%=Image(CVLX.Logo) %>" alt="<%=CVLX.Name%>" /></a><%}else{%><a href="<%=CVLX.Url%>" target="_blank"><%=CVLX.Name%></a><%} %>     
