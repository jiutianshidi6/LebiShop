...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_News"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> QuFIs=Languages();RecordCount=QuFIs.Count;int QuFI_index=1;
foreach (Shop.Model.Lebi_Language QuFI in QuFIs){%>
            <dd <%if (QuFI_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=QuFI.id%>,'<%=QuFI.Code%>','<%=QuFI.Path%>');"><%if (QuFI.ImageUrl!=""){%><img src="<%=Image(QuFI.ImageUrl) %>" /><%}%><%=QuFI.Name%></a></dd>
            <%QuFI_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Lvcc_index=1;
List<Lebi_Currency> Lvccs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Lvcc in Lvccs){%>
            <dd <%if (Lvcc_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Lvcc.id%>,'<%=Lvcc.Code%>',<%=Lvcc.ExchangeRate%>,'<%=Lvcc.Msige%>','<%=Lvcc.DecimalLength%>');"><%=Lvcc.Code%></a></dd>
            <%Lvcc_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int YnzX_index=1;
List<Lebi_Searchkey> YnzXs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey YnzX in YnzXs){%><%=Lang(YnzX.Name)%><%YnzX_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int QeHF_index=1;
List<Lebi_Searchkey> QeHFs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey QeHF in QeHFs){%>
<%if (QeHF.Type==1){ %><a href="<%=URL("P_Search",""+Lang(QeHF.Name)+"") %>"><%}else{ %><a href="<%=QeHF.URL%>" target="_blank"><%} %><span><%=Lang(QeHF.Name)%></span></a>
<%QeHF_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int eIRl_index=1;
List<Lebi_Page> eIRls = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page eIRl in eIRls){%>
<a class="menu" href="<%=URL("","",eIRl.url)%>"><span><%=eIRl.Name%></span></a>
<%eIRl_index++;}%>
...
<ul class="text">
    <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int GyLv_index=1;
List<Lebi_Brand> GyLvs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand GyLv in GyLvs){%>
    <li><a href="<%=URL("P_Brand",GyLv.id)%>" title="<%=Lang(GyLv.Name) %>"><%=Lang(GyLv.Name) %></a></li>
    <%GyLv_index++;}%>
</ul>
</div>
</div>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int nulm_index=1;
List<Lebi_Page> nulms = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page nulm in nulms){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",nulm.id,nulm.url)%>" target="<%=nulm.target%>" title="<%=nulm.Name%>"><span style="color:<%=nulm.NameColor%>"><%=nulm.Name%></span></a></li>
        <%nulm_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int SeYz_index=1;
List<Lebi_FriendLink> SeYzs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink SeYz in SeYzs){%>
            
                <% if (SeYz.Logo != "" && SeYz.IsPic == 1){ %><a href="<%=SeYz.Url%>" target="_blank"><img src="<%=Image(SeYz.Logo) %>" alt="<%=SeYz.Name%>" /></a><%}else{%><a href="<%=SeYz.Url%>" target="_blank"><%=SeYz.Name%></a><%} %>     
