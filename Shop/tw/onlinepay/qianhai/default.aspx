...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_qianhaiPay"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> aGaws=Languages();RecordCount=aGaws.Count;int aGaw_index=1;
foreach (Shop.Model.Lebi_Language aGaw in aGaws){%>
            <dd <%if (aGaw_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=aGaw.id%>,'<%=aGaw.Code%>','<%=aGaw.Path%>');"><%if (aGaw.ImageUrl!=""){%><img src="<%=Image(aGaw.ImageUrl) %>" /><%}%><%=aGaw.Name%></a></dd>
            <%aGaw_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int euRU_index=1;
List<Lebi_Currency> euRUs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency euRU in euRUs){%>
            <dd <%if (euRU_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=euRU.id%>,'<%=euRU.Code%>',<%=euRU.ExchangeRate%>,'<%=euRU.Msige%>','<%=euRU.DecimalLength%>');"><%=euRU.Code%></a></dd>
            <%euRU_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int UFOs_index=1;
List<Lebi_Searchkey> UFOss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey UFOs in UFOss){%><%=Lang(UFOs.Name)%><%UFOs_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int jcJQ_index=1;
List<Lebi_Searchkey> jcJQs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey jcJQ in jcJQs){%>
<%if (jcJQ.Type==1){ %><a href="<%=URL("P_Search",""+Lang(jcJQ.Name)+"") %>"><%}else{ %><a href="<%=jcJQ.URL%>" target="_blank"><%} %><span><%=Lang(jcJQ.Name)%></span></a>
<%jcJQ_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Mtvu_index=1;
List<Lebi_Page> Mtvus = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Mtvu in Mtvus){%>
<a class="menu" href="<%=URL("","",Mtvu.url)%>"><span><%=Mtvu.Name%></span></a>
<%Mtvu_index++;}%>
    ...
    <div class="user">
        <div class="reg clearfix">
            <div class="mt clearfix">
                <h2>訂單號碼：<%=order.Code %></h2>
            </div>
            <div class="mc clearfix">
            <div class="dl-table clearfix">
            <dl>
                <dt><font color="red">*</font>金額：</dt>
                <dd>
                    <%=currency %><%=Money %>
                </dd>
            </dl>
            ...
            <dl>
                <dt><font color="red">*</font>持卡人的郵箱：</dt>
            ...
            <dl>
                <dt><font color="red">*</font>持卡人的電話：</dt>
            ...
            <dl>
                <dt><font color="red">*</font>持卡人賬單國家：</dt>
            ...
            <dl>
             ...
             <dl>
                <dt><font color="red">*</font>持卡人的詳細地址：</dt>
            ...
            <dl>
                <dt><font color="red">*</font>持卡人的郵編：</dt>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int fdDk_index=1;
List<Lebi_FriendLink> fdDks = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink fdDk in fdDks){%>
            
                <% if (fdDk.Logo != "" && fdDk.IsPic == 1){ %><a href="<%=fdDk.Url%>" target="_blank"><img src="<%=Image(fdDk.Logo) %>" alt="<%=fdDk.Name%>" /></a><%}else{%><a href="<%=fdDk.Url%>" target="_blank"><%=fdDk.Name%></a><%} %>     
