...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_FindPassword"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> OmYts=Languages();RecordCount=OmYts.Count;int OmYt_index=1;
foreach (Shop.Model.Lebi_Language OmYt in OmYts){%>
            <dd <%if (OmYt_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=OmYt.id%>,'<%=OmYt.Code%>','<%=OmYt.Path%>');"><%if (OmYt.ImageUrl!=""){%><img src="<%=Image(OmYt.ImageUrl) %>" /><%}%><%=OmYt.Name%></a></dd>
            <%OmYt_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int rNXB_index=1;
List<Lebi_Currency> rNXBs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency rNXB in rNXBs){%>
            <dd <%if (rNXB_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=rNXB.id%>,'<%=rNXB.Code%>',<%=rNXB.ExchangeRate%>,'<%=rNXB.Msige%>','<%=rNXB.DecimalLength%>');"><%=rNXB.Code%></a></dd>
            <%rNXB_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int mzPH_index=1;
List<Lebi_Searchkey> mzPHs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey mzPH in mzPHs){%><%=Lang(mzPH.Name)%><%mzPH_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int fKtt_index=1;
List<Lebi_Searchkey> fKtts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey fKtt in fKtts){%>
<%if (fKtt.Type==1){ %><a href="<%=URL("P_Search",""+Lang(fKtt.Name)+"") %>"><%}else{ %><a href="<%=fKtt.URL%>" target="_blank"><%} %><span><%=Lang(fKtt.Name)%></span></a>
<%fKtt_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int rtoS_index=1;
List<Lebi_Page> rtoSs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page rtoS in rtoSs){%>
<a class="menu" href="<%=URL("","",rtoS.url)%>"><span><%=rtoS.Name%></span></a>
<%rtoS_index++;}%>
...
<script type="text/javascript">
    function submit() {
        ...
        if (Answer1 == "") {
            CheckNO("Answer1", "請回答答案");
            return false;
        }
        if (Answer2 == "") {
            CheckNO("Answer2", "請回答答案");
            return false;
        }
        <%} %>
        if (PWD == "") {
        ...
        if (PWD != RPWD) {
            CheckNO("RPWD", "兩次輸入的密碼不一致");
            return false;
        }
        var postData = GetFormJsonData("shop");
        var url = path+"/ajax/ajax_user.aspx?__Action=User_resetpwd&v=<%=v%>&type=<%=type %>";
        RequestAjax(url, postData, function () { MsgBox(1, "密碼修改成功，請登錄", "<%=URL("P_Login","")%>") });
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int ecAn_index=1;
List<Lebi_FriendLink> ecAns = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink ecAn in ecAns){%>
            
                <% if (ecAn.Logo != "" && ecAn.IsPic == 1){ %><a href="<%=ecAn.Url%>" target="_blank"><img src="<%=Image(ecAn.Logo) %>" alt="<%=ecAn.Name%>" /></a><%}else{%><a href="<%=ecAn.Url%>" target="_blank"><%=ecAn.Name%></a><%} %>     
