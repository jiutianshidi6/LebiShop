...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_AllProductCategories"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> rzUNs=Languages();RecordCount=rzUNs.Count;int rzUN_index=1;
foreach (Shop.Model.Lebi_Language rzUN in rzUNs){%>
            <dd <%if (rzUN_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=rzUN.id%>,'<%=rzUN.Code%>','<%=rzUN.Path%>');"><%if (rzUN.ImageUrl!=""){%><img src="<%=Image(rzUN.ImageUrl) %>" /><%}%><%=rzUN.Name%></a></dd>
            <%rzUN_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int JFaI_index=1;
List<Lebi_Currency> JFaIs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency JFaI in JFaIs){%>
            <dd <%if (JFaI_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=JFaI.id%>,'<%=JFaI.Code%>',<%=JFaI.ExchangeRate%>,'<%=JFaI.Msige%>','<%=JFaI.DecimalLength%>');"><%=JFaI.Code%></a></dd>
            <%JFaI_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int rihh_index=1;
List<Lebi_Searchkey> rihhs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey rihh in rihhs){%><%=Lang(rihh.Name)%><%rihh_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int kHbh_index=1;
List<Lebi_Searchkey> kHbhs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey kHbh in kHbhs){%>
<%if (kHbh.Type==1){ %><a href="<%=URL("P_Search",""+Lang(kHbh.Name)+"") %>"><%}else{ %><a href="<%=kHbh.URL%>" target="_blank"><%} %><span><%=Lang(kHbh.Name)%></span></a>
<%kHbh_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int tXJQ_index=1;
List<Lebi_Page> tXJQs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page tXJQ in tXJQs){%>
<a class="menu" href="<%=URL("","",tXJQ.url)%>"><span><%=tXJQ.Name%></span></a>
<%tXJQ_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int lnjF_index=1;
List<Lebi_FriendLink> lnjFs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink lnjF in lnjFs){%>
            
                <% if (lnjF.Logo != "" && lnjF.IsPic == 1){ %><a href="<%=lnjF.Url%>" target="_blank"><img src="<%=Image(lnjF.Logo) %>" alt="<%=lnjF.Name%>" /></a><%}else{%><a href="<%=lnjF.Url%>" target="_blank"><%=lnjF.Name%></a><%} %>     
