...
<% LoadPage("wangdayu523@163.com_37",1,"tcn","P_ProductCommentIndex"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> SjSLs=Languages();RecordCount=SjSLs.Count;int SjSL_index=1;
foreach (Shop.Model.Lebi_Language SjSL in SjSLs){%>
            <dd <%if (SjSL_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=SjSL.id%>,'<%=SjSL.Code%>','<%=SjSL.Path%>');"><%if (SjSL.ImageUrl!=""){%><img src="<%=Image(SjSL.ImageUrl) %>" /><%}%><%=SjSL.Name%></a></dd>
            <%SjSL_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int sYkT_index=1;
List<Lebi_Currency> sYkTs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency sYkT in sYkTs){%>
            <dd <%if (sYkT_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=sYkT.id%>,'<%=sYkT.Code%>',<%=sYkT.ExchangeRate%>,'<%=sYkT.Msige%>','<%=sYkT.DecimalLength%>');"><%=sYkT.Code%></a></dd>
            <%sYkT_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ykmp_index=1;
List<Lebi_Searchkey> ykmps = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ykmp in ykmps){%><%=Lang(ykmp.Name)%><%ykmp_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zdUy_index=1;
List<Lebi_Searchkey> zdUys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zdUy in zdUys){%>
<%if (zdUy.Type==1){ %><a href="<%=URL("P_Search",""+Lang(zdUy.Name)+"") %>"><%}else{ %><a href="<%=zdUy.URL%>" target="_blank"><%} %><span><%=Lang(zdUy.Name)%></span></a>
<%zdUy_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int KfMY_index=1;
List<Lebi_Page> KfMYs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page KfMY in KfMYs){%>
<a class="menu" href="<%=URL("","",KfMY.url)%>"><span><%=KfMY.Name%></span></a>
<%KfMY_index++;}%>
...
<div class="body clearfix">
  <div class="location"><div class="path"><%=path%></div></div>
    ...
    <div id="load">
        <div id="status"></div>
        <a href="javascript:void(0)" onclick="LoadMore();" class="btn btn-11"><s></s>加載更多</a>
    </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int FkiN_index=1;
List<Lebi_FriendLink> FkiNs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink FkiN in FkiNs){%>
            
                <% if (FkiN.Logo != "" && FkiN.IsPic == 1){ %><a href="<%=FkiN.Url%>" target="_blank"><img src="<%=Image(FkiN.Logo) %>" alt="<%=FkiN.Name%>" /></a><%}else{%><a href="<%=FkiN.Url%>" target="_blank"><%=FkiN.Name%></a><%} %>     
