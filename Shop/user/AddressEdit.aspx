...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserAddressEdit"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> Oyvcs=Languages();RecordCount=Oyvcs.Count;int Oyvc_index=1;
foreach (Shop.Model.Lebi_Language Oyvc in Oyvcs){%>
            <dd <%if (Oyvc_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Oyvc.id%>,'<%=Oyvc.Code%>','<%=Oyvc.Path%>');"><%if (Oyvc.ImageUrl!=""){%><img src="<%=Image(Oyvc.ImageUrl) %>" /><%}%><%=Oyvc.Name%></a></dd>
            <%Oyvc_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int uwmG_index=1;
List<Lebi_Currency> uwmGs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency uwmG in uwmGs){%>
            <dd <%if (uwmG_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=uwmG.id%>,'<%=uwmG.Code%>',<%=uwmG.ExchangeRate%>,'<%=uwmG.Msige%>','<%=uwmG.DecimalLength%>');"><%=uwmG.Code%></a></dd>
            <%uwmG_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Mqqw_index=1;
List<Lebi_Searchkey> Mqqws = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Mqqw in Mqqws){%><%=Lang(Mqqw.Name)%><%Mqqw_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int THYv_index=1;
List<Lebi_Searchkey> THYvs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey THYv in THYvs){%>
<%if (THYv.Type==1){ %><a href="<%=URL("P_Search",""+Lang(THYv.Name)+"") %>"><%}else{ %><a href="<%=THYv.URL%>" target="_blank"><%} %><span><%=Lang(THYv.Name)%></span></a>
<%THYv_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int piYV_index=1;
List<Lebi_Page> piYVs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page piYV in piYVs){%>
<a class="menu" href="<%=URL("","",piYV.url)%>"><span><%=piYV.Name%></span></a>
<%piYV_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Kjry_index=1;
List<Lebi_FriendLink> Kjrys = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Kjry in Kjrys){%>
            
                <% if (Kjry.Logo != "" && Kjry.IsPic == 1){ %><a href="<%=Kjry.Url%>" target="_blank"><img src="<%=Image(Kjry.Logo) %>" alt="<%=Kjry.Name%>" /></a><%}else{%><a href="<%=Kjry.Url%>" target="_blank"><%=Kjry.Name%></a><%} %>     
