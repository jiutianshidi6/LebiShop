...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserAddressEdit"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> BAhHs=Languages();RecordCount=BAhHs.Count;int BAhH_index=1;
foreach (Shop.Model.Lebi_Language BAhH in BAhHs){%>
            <dd <%if (BAhH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=BAhH.id%>,'<%=BAhH.Code%>','<%=BAhH.Path%>');"><%if (BAhH.ImageUrl!=""){%><img src="<%=Image(BAhH.ImageUrl) %>" /><%}%><%=BAhH.Name%></a></dd>
            <%BAhH_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int pzzF_index=1;
List<Lebi_Searchkey> pzzFs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey pzzF in pzzFs){%><%=Lang(pzzF.Name)%><%pzzF_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ayAs_index=1;
List<Lebi_Searchkey> ayAss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ayAs in ayAss){%>
<%if (ayAs.Type==1){ %><a href="<%=URL("P_Search",""+Lang(ayAs.Name)+"") %>"><%}else{ %><a href="<%=ayAs.URL%>" target="_blank"><%} %><span><%=Lang(ayAs.Name)%></span></a>
<%ayAs_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int SNwI_index=1;
List<Lebi_Brand> SNwIs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand SNwI in SNwIs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",SNwI.id)%>"><img alt="<%=Lang(SNwI.Name) %>" src="<%=Image(SNwI.ImageUrl) %>"><span><%=Lang(SNwI.Name) %></span></a></li>
                                        <%SNwI_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int tOnH_index=1;
List<Lebi_Page> tOnHs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page tOnH in tOnHs){%>
<a class="menu" href="<%=URL("","",tOnH.url)%>"><span><%=tOnH.Name%></span></a>
<%tOnH_index++;}%>
