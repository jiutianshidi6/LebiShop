...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_Recharge"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> WTWcs=Languages();RecordCount=WTWcs.Count;int WTWc_index=1;
foreach (Shop.Model.Lebi_Language WTWc in WTWcs){%>
            <dd <%if (WTWc_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=WTWc.id%>,'<%=WTWc.Code%>','<%=WTWc.Path%>');"><%if (WTWc.ImageUrl!=""){%><img src="<%=Image(WTWc.ImageUrl) %>" /><%}%><%=WTWc.Name%></a></dd>
            <%WTWc_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int yFvg_index=1;
List<Lebi_Searchkey> yFvgs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey yFvg in yFvgs){%><%=Lang(yFvg.Name)%><%yFvg_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vGOi_index=1;
List<Lebi_Searchkey> vGOis = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vGOi in vGOis){%>
<%if (vGOi.Type==1){ %><a href="<%=URL("P_Search",""+Lang(vGOi.Name)+"") %>"><%}else{ %><a href="<%=vGOi.URL%>" target="_blank"><%} %><span><%=Lang(vGOi.Name)%></span></a>
<%vGOi_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int OmCw_index=1;
List<Lebi_Brand> OmCws = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand OmCw in OmCws){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",OmCw.id)%>"><img alt="<%=Lang(OmCw.Name) %>" src="<%=Image(OmCw.ImageUrl) %>"><span><%=Lang(OmCw.Name) %></span></a></li>
                                        <%OmCw_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int xUmb_index=1;
List<Lebi_Page> xUmbs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page xUmb in xUmbs){%>
<a class="menu" href="<%=URL("","",xUmb.url)%>"><span><%=xUmb.Name%></span></a>
<%xUmb_index++;}%>
