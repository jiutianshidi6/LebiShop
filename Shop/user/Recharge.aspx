...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_Recharge"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> ynews=Languages();RecordCount=ynews.Count;int ynew_index=1;
foreach (Shop.Model.Lebi_Language ynew in ynews){%>
            <dd <%if (ynew_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=ynew.id%>,'<%=ynew.Code%>','<%=ynew.Path%>');"><%if (ynew.ImageUrl!=""){%><img src="<%=Image(ynew.ImageUrl) %>" /><%}%><%=ynew.Name%></a></dd>
            <%ynew_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int NWDy_index=1;
List<Lebi_Currency> NWDys = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency NWDy in NWDys){%>
            <dd <%if (NWDy_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=NWDy.id%>,'<%=NWDy.Code%>',<%=NWDy.ExchangeRate%>,'<%=NWDy.Msige%>','<%=NWDy.DecimalLength%>');"><%=NWDy.Code%></a></dd>
            <%NWDy_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int KAhr_index=1;
List<Lebi_Searchkey> KAhrs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey KAhr in KAhrs){%><%=Lang(KAhr.Name)%><%KAhr_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int eQSx_index=1;
List<Lebi_Searchkey> eQSxs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey eQSx in eQSxs){%>
<%if (eQSx.Type==1){ %><a href="<%=URL("P_Search",""+Lang(eQSx.Name)+"") %>"><%}else{ %><a href="<%=eQSx.URL%>" target="_blank"><%} %><span><%=Lang(eQSx.Name)%></span></a>
<%eQSx_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int RmNV_index=1;
List<Lebi_Page> RmNVs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page RmNV in RmNVs){%>
<a class="menu" href="<%=URL("","",RmNV.url)%>"><span><%=RmNV.Name%></span></a>
<%RmNV_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int VsSf_index=1;
List<Lebi_FriendLink> VsSfs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink VsSf in VsSfs){%>
            
                <% if (VsSf.Logo != "" && VsSf.IsPic == 1){ %><a href="<%=VsSf.Url%>" target="_blank"><img src="<%=Image(VsSf.Logo) %>" alt="<%=VsSf.Name%>" /></a><%}else{%><a href="<%=VsSf.Url%>" target="_blank"><%=VsSf.Name%></a><%} %>     
