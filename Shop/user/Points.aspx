...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserPoint"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> oHLPs=Languages();RecordCount=oHLPs.Count;int oHLP_index=1;
foreach (Shop.Model.Lebi_Language oHLP in oHLPs){%>
            <dd <%if (oHLP_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=oHLP.id%>,'<%=oHLP.Code%>','<%=oHLP.Path%>');"><%if (oHLP.ImageUrl!=""){%><img src="<%=Image(oHLP.ImageUrl) %>" /><%}%><%=oHLP.Name%></a></dd>
            <%oHLP_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int BWVl_index=1;
List<Lebi_Currency> BWVls = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency BWVl in BWVls){%>
            <dd <%if (BWVl_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=BWVl.id%>,'<%=BWVl.Code%>',<%=BWVl.ExchangeRate%>,'<%=BWVl.Msige%>','<%=BWVl.DecimalLength%>');"><%=BWVl.Code%></a></dd>
            <%BWVl_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int FlWI_index=1;
List<Lebi_Searchkey> FlWIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey FlWI in FlWIs){%><%=Lang(FlWI.Name)%><%FlWI_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int OKko_index=1;
List<Lebi_Searchkey> OKkos = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey OKko in OKkos){%>
<%if (OKko.Type==1){ %><a href="<%=URL("P_Search",""+Lang(OKko.Name)+"") %>"><%}else{ %><a href="<%=OKko.URL%>" target="_blank"><%} %><span><%=Lang(OKko.Name)%></span></a>
<%OKko_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int VQcG_index=1;
List<Lebi_Page> VQcGs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page VQcG in VQcGs){%>
<a class="menu" href="<%=URL("","",VQcG.url)%>"><span><%=VQcG.Name%></span></a>
<%VQcG_index++;}%>
...
<div class="user">
    <div class="searchbox clearfix">
        ...
        <a href="<%=URL("P_UserPointToMoney","") %>" class="btn btn-12"><s></s>积分转换</a>
        <%} %>
    </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int mRQi_index=1;
List<Lebi_FriendLink> mRQis = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink mRQi in mRQis){%>
            
                <% if (mRQi.Logo != "" && mRQi.IsPic == 1){ %><a href="<%=mRQi.Url%>" target="_blank"><img src="<%=Image(mRQi.Logo) %>" alt="<%=mRQi.Name%>" /></a><%}else{%><a href="<%=mRQi.Url%>" target="_blank"><%=mRQi.Name%></a><%} %>     
