...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserPoint"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> JnuKs=Languages();RecordCount=JnuKs.Count;int JnuK_index=1;
foreach (Shop.Model.Lebi_Language JnuK in JnuKs){%>
            <dd <%if (JnuK_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=JnuK.id%>,'<%=JnuK.Code%>','<%=JnuK.Path%>');"><%if (JnuK.ImageUrl!=""){%><img src="<%=Image(JnuK.ImageUrl) %>" /><%}%><%=JnuK.Name%></a></dd>
            <%JnuK_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zdEx_index=1;
List<Lebi_Searchkey> zdExs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zdEx in zdExs){%><%=Lang(zdEx.Name)%><%zdEx_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int bpCO_index=1;
List<Lebi_Searchkey> bpCOs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey bpCO in bpCOs){%>
<%if (bpCO.Type==1){ %><a href="<%=URL("P_Search",""+Lang(bpCO.Name)+"") %>"><%}else{ %><a href="<%=bpCO.URL%>" target="_blank"><%} %><span><%=Lang(bpCO.Name)%></span></a>
<%bpCO_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int jTJx_index=1;
List<Lebi_Brand> jTJxs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand jTJx in jTJxs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",jTJx.id)%>"><img alt="<%=Lang(jTJx.Name) %>" src="<%=Image(jTJx.ImageUrl) %>"><span><%=Lang(jTJx.Name) %></span></a></li>
                                        <%jTJx_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int WfBh_index=1;
List<Lebi_Page> WfBhs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page WfBh in WfBhs){%>
<a class="menu" href="<%=URL("","",WfBh.url)%>"><span><%=WfBh.Name%></span></a>
<%WfBh_index++;}%>
