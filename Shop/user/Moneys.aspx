...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserMoney"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> kmDks=Languages();RecordCount=kmDks.Count;int kmDk_index=1;
foreach (Shop.Model.Lebi_Language kmDk in kmDks){%>
            <dd <%if (kmDk_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=kmDk.id%>,'<%=kmDk.Code%>','<%=kmDk.Path%>');"><%if (kmDk.ImageUrl!=""){%><img src="<%=Image(kmDk.ImageUrl) %>" /><%}%><%=kmDk.Name%></a></dd>
            <%kmDk_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int ITGW_index=1;
List<Lebi_Currency> ITGWs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency ITGW in ITGWs){%>
            <dd <%if (ITGW_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=ITGW.id%>,'<%=ITGW.Code%>',<%=ITGW.ExchangeRate%>,'<%=ITGW.Msige%>','<%=ITGW.DecimalLength%>');"><%=ITGW.Code%></a></dd>
            <%ITGW_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ARVo_index=1;
List<Lebi_Searchkey> ARVos = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ARVo in ARVos){%><%=Lang(ARVo.Name)%><%ARVo_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int fCBg_index=1;
List<Lebi_Searchkey> fCBgs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey fCBg in fCBgs){%>
<%if (fCBg.Type==1){ %><a href="<%=URL("P_Search",""+Lang(fCBg.Name)+"") %>"><%}else{ %><a href="<%=fCBg.URL%>" target="_blank"><%} %><span><%=Lang(fCBg.Name)%></span></a>
<%fCBg_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int rGjM_index=1;
List<Lebi_Page> rGjMs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page rGjM in rGjMs){%>
<a class="menu" href="<%=URL("","",rGjM.url)%>"><span><%=rGjM.Name%></span></a>
<%rGjM_index++;}%>
...
<div class="user">
    <div class="searchbox clearfix">
        ...
        <a href="<%=URL("P_UserCash","") %>"  class="btn btn-12"><s></s>提现</a>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int NIpM_index=1;
List<Lebi_FriendLink> NIpMs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink NIpM in NIpMs){%>
            
                <% if (NIpM.Logo != "" && NIpM.IsPic == 1){ %><a href="<%=NIpM.Url%>" target="_blank"><img src="<%=Image(NIpM.Logo) %>" alt="<%=NIpM.Name%>" /></a><%}else{%><a href="<%=NIpM.Url%>" target="_blank"><%=NIpM.Name%></a><%} %>     
