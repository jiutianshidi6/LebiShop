...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserMoney"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> UxOzs=Languages();RecordCount=UxOzs.Count;int UxOz_index=1;
foreach (Shop.Model.Lebi_Language UxOz in UxOzs){%>
            <dd <%if (UxOz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=UxOz.id%>,'<%=UxOz.Code%>','<%=UxOz.Path%>');"><%if (UxOz.ImageUrl!=""){%><img src="<%=Image(UxOz.ImageUrl) %>" /><%}%><%=UxOz.Name%></a></dd>
            <%UxOz_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int FEBO_index=1;
List<Lebi_Searchkey> FEBOs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey FEBO in FEBOs){%><%=Lang(FEBO.Name)%><%FEBO_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int foeG_index=1;
List<Lebi_Searchkey> foeGs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey foeG in foeGs){%>
<%if (foeG.Type==1){ %><a href="<%=URL("P_Search",""+Lang(foeG.Name)+"") %>"><%}else{ %><a href="<%=foeG.URL%>" target="_blank"><%} %><span><%=Lang(foeG.Name)%></span></a>
<%foeG_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int NzoH_index=1;
List<Lebi_Brand> NzoHs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand NzoH in NzoHs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",NzoH.id)%>"><img alt="<%=Lang(NzoH.Name) %>" src="<%=Image(NzoH.ImageUrl) %>"><span><%=Lang(NzoH.Name) %></span></a></li>
                                        <%NzoH_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int xrAW_index=1;
List<Lebi_Page> xrAWs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page xrAW in xrAWs){%>
<a class="menu" href="<%=URL("","",xrAW.url)%>"><span><%=xrAW.Name%></span></a>
<%xrAW_index++;}%>
