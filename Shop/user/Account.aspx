...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserAccount"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> EiCRs=Languages();RecordCount=EiCRs.Count;int EiCR_index=1;
foreach (Shop.Model.Lebi_Language EiCR in EiCRs){%>
            <dd <%if (EiCR_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=EiCR.id%>,'<%=EiCR.Code%>','<%=EiCR.Path%>');"><%if (EiCR.ImageUrl!=""){%><img src="<%=Image(EiCR.ImageUrl) %>" /><%}%><%=EiCR.Name%></a></dd>
            <%EiCR_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int iVwI_index=1;
List<Lebi_Currency> iVwIs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency iVwI in iVwIs){%>
            <dd <%if (iVwI_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=iVwI.id%>,'<%=iVwI.Code%>',<%=iVwI.ExchangeRate%>,'<%=iVwI.Msige%>','<%=iVwI.DecimalLength%>');"><%=iVwI.Code%></a></dd>
            <%iVwI_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MLyh_index=1;
List<Lebi_Searchkey> MLyhs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MLyh in MLyhs){%><%=Lang(MLyh.Name)%><%MLyh_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int DVeT_index=1;
List<Lebi_Searchkey> DVeTs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey DVeT in DVeTs){%>
<%if (DVeT.Type==1){ %><a href="<%=URL("P_Search",""+Lang(DVeT.Name)+"") %>"><%}else{ %><a href="<%=DVeT.URL%>" target="_blank"><%} %><span><%=Lang(DVeT.Name)%></span></a>
<%DVeT_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int LvYf_index=1;
List<Lebi_Page> LvYfs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page LvYf in LvYfs){%>
<a class="menu" href="<%=URL("","",LvYf.url)%>"><span><%=LvYf.Name%></span></a>
<%LvYf_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int vVwY_index=1;
List<Lebi_FriendLink> vVwYs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink vVwY in vVwYs){%>
            
                <% if (vVwY.Logo != "" && vVwY.IsPic == 1){ %><a href="<%=vVwY.Url%>" target="_blank"><img src="<%=Image(vVwY.Logo) %>" alt="<%=vVwY.Name%>" /></a><%}else{%><a href="<%=vVwY.Url%>" target="_blank"><%=vVwY.Name%></a><%} %>     
