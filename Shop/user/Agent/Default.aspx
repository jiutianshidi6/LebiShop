...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserAgent"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> HOFys=Languages();RecordCount=HOFys.Count;int HOFy_index=1;
foreach (Shop.Model.Lebi_Language HOFy in HOFys){%>
            <dd <%if (HOFy_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=HOFy.id%>,'<%=HOFy.Code%>','<%=HOFy.Path%>');"><%if (HOFy.ImageUrl!=""){%><img src="<%=Image(HOFy.ImageUrl) %>" /><%}%><%=HOFy.Name%></a></dd>
            <%HOFy_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int pLlG_index=1;
List<Lebi_Currency> pLlGs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency pLlG in pLlGs){%>
            <dd <%if (pLlG_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=pLlG.id%>,'<%=pLlG.Code%>',<%=pLlG.ExchangeRate%>,'<%=pLlG.Msige%>','<%=pLlG.DecimalLength%>');"><%=pLlG.Code%></a></dd>
            <%pLlG_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int jiAY_index=1;
List<Lebi_Searchkey> jiAYs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey jiAY in jiAYs){%><%=Lang(jiAY.Name)%><%jiAY_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int mszc_index=1;
List<Lebi_Searchkey> mszcs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey mszc in mszcs){%>
<%if (mszc.Type==1){ %><a href="<%=URL("P_Search",""+Lang(mszc.Name)+"") %>"><%}else{ %><a href="<%=mszc.URL%>" target="_blank"><%} %><span><%=Lang(mszc.Name)%></span></a>
<%mszc_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int tzTX_index=1;
List<Lebi_Page> tzTXs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page tzTX in tzTXs){%>
<a class="menu" href="<%=URL("","",tzTX.url)%>"><span><%=tzTX.Name%></span></a>
<%tzTX_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int mtQX_index=1;
List<Lebi_FriendLink> mtQXs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink mtQX in mtQXs){%>
            
                <% if (mtQX.Logo != "" && mtQX.IsPic == 1){ %><a href="<%=mtQX.Url%>" target="_blank"><img src="<%=Image(mtQX.Logo) %>" alt="<%=mtQX.Name%>" /></a><%}else{%><a href="<%=mtQX.Url%>" target="_blank"><%=mtQX.Name%></a><%} %>     
