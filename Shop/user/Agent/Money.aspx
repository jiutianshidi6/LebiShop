...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserAgentMoney"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> SWYgs=Languages();RecordCount=SWYgs.Count;int SWYg_index=1;
foreach (Shop.Model.Lebi_Language SWYg in SWYgs){%>
            <dd <%if (SWYg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=SWYg.id%>,'<%=SWYg.Code%>','<%=SWYg.Path%>');"><%if (SWYg.ImageUrl!=""){%><img src="<%=Image(SWYg.ImageUrl) %>" /><%}%><%=SWYg.Name%></a></dd>
            <%SWYg_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int JKiy_index=1;
List<Lebi_Currency> JKiys = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency JKiy in JKiys){%>
            <dd <%if (JKiy_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=JKiy.id%>,'<%=JKiy.Code%>',<%=JKiy.ExchangeRate%>,'<%=JKiy.Msige%>','<%=JKiy.DecimalLength%>');"><%=JKiy.Code%></a></dd>
            <%JKiy_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int HCFl_index=1;
List<Lebi_Searchkey> HCFls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey HCFl in HCFls){%><%=Lang(HCFl.Name)%><%HCFl_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int fVuC_index=1;
List<Lebi_Searchkey> fVuCs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey fVuC in fVuCs){%>
<%if (fVuC.Type==1){ %><a href="<%=URL("P_Search",""+Lang(fVuC.Name)+"") %>"><%}else{ %><a href="<%=fVuC.URL%>" target="_blank"><%} %><span><%=Lang(fVuC.Name)%></span></a>
<%fVuC_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Qtvw_index=1;
List<Lebi_Page> Qtvws = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Qtvw in Qtvws){%>
<a class="menu" href="<%=URL("","",Qtvw.url)%>"><span><%=Qtvw.Name%></span></a>
<%Qtvw_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int WDqe_index=1;
List<Lebi_FriendLink> WDqes = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink WDqe in WDqes){%>
            
                <% if (WDqe.Logo != "" && WDqe.IsPic == 1){ %><a href="<%=WDqe.Url%>" target="_blank"><img src="<%=Image(WDqe.Logo) %>" alt="<%=WDqe.Name%>" /></a><%}else{%><a href="<%=WDqe.Url%>" target="_blank"><%=WDqe.Name%></a><%} %>     
