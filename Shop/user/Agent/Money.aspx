<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.P_UserAgentMoney" validateRequest="false"%>


<%@ Import Namespace="Shop.Bussiness" %>
<%@ Import Namespace="Shop.Model" %>
<%@ Import Namespace="System.Collections.Generic" %>
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserAgentMoney"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> WhGJs=Languages();RecordCount=WhGJs.Count;int WhGJ_index=1;
foreach (Shop.Model.Lebi_Language WhGJ in WhGJs){%>
            <dd <%if (WhGJ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=WhGJ.id%>,'<%=WhGJ.Code%>','<%=WhGJ.Path%>');"><%if (WhGJ.ImageUrl!=""){%><img src="<%=Image(WhGJ.ImageUrl) %>" /><%}%><%=WhGJ.Name%></a></dd>
            <%WhGJ_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int QxYs_index=1;
List<Lebi_Searchkey> QxYss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey QxYs in QxYss){%><%=Lang(QxYs.Name)%><%QxYs_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int fKMl_index=1;
List<Lebi_Searchkey> fKMls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey fKMl in fKMls){%>
<%if (fKMl.Type==1){ %><a href="<%=URL("P_Search",""+Lang(fKMl.Name)+"") %>"><%}else{ %><a href="<%=fKMl.URL%>" target="_blank"><%} %><span><%=Lang(fKMl.Name)%></span></a>
<%fKMl_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int uaxe_index=1;
List<Lebi_Brand> uaxes = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand uaxe in uaxes){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",uaxe.id)%>"><img alt="<%=Lang(uaxe.Name) %>" src="<%=Image(uaxe.ImageUrl) %>"><span><%=Lang(uaxe.Name) %></span></a></li>
                                        <%uaxe_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int SziV_index=1;
List<Lebi_Page> SziVs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page SziV in SziVs){%>
<a class="menu" href="<%=URL("","",SziV.url)%>"><span><%=SziV.Name%></span></a>
<%SziV_index++;}%>
