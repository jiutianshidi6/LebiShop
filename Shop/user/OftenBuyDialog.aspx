...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserOftenBuyDialog"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> tiCPs=Languages();RecordCount=tiCPs.Count;int tiCP_index=1;
foreach (Shop.Model.Lebi_Language tiCP in tiCPs){%>
            <dd <%if (tiCP_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=tiCP.id%>,'<%=tiCP.Code%>','<%=tiCP.Path%>');"><%if (tiCP.ImageUrl!=""){%><img src="<%=Image(tiCP.ImageUrl) %>" /><%}%><%=tiCP.Name%></a></dd>
            <%tiCP_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int argE_index=1;
List<Lebi_Currency> argEs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency argE in argEs){%>
            <dd <%if (argE_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=argE.id%>,'<%=argE.Code%>',<%=argE.ExchangeRate%>,'<%=argE.Msige%>','<%=argE.DecimalLength%>');"><%=argE.Code%></a></dd>
            <%argE_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int KdhM_index=1;
List<Lebi_Searchkey> KdhMs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey KdhM in KdhMs){%><%=Lang(KdhM.Name)%><%KdhM_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MyzI_index=1;
List<Lebi_Searchkey> MyzIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MyzI in MyzIs){%>
<%if (MyzI.Type==1){ %><a href="<%=URL("P_Search",""+Lang(MyzI.Name)+"") %>"><%}else{ %><a href="<%=MyzI.URL%>" target="_blank"><%} %><span><%=Lang(MyzI.Name)%></span></a>
<%MyzI_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int LPwf_index=1;
List<Lebi_Page> LPwfs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page LPwf in LPwfs){%>
<a class="menu" href="<%=URL("","",LPwf.url)%>"><span><%=LPwf.Name%></span></a>
<%LPwf_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Bbiz_index=1;
List<Lebi_FriendLink> Bbizs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Bbiz in Bbizs){%>
            
                <% if (Bbiz.Logo != "" && Bbiz.IsPic == 1){ %><a href="<%=Bbiz.Url%>" target="_blank"><img src="<%=Image(Bbiz.Logo) %>" alt="<%=Bbiz.Name%>" /></a><%}else{%><a href="<%=Bbiz.Url%>" target="_blank"><%=Bbiz.Name%></a><%} %>     
