...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserOftenBuyDialog"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> SACGs=Languages();RecordCount=SACGs.Count;int SACG_index=1;
foreach (Shop.Model.Lebi_Language SACG in SACGs){%>
            <dd <%if (SACG_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=SACG.id%>,'<%=SACG.Code%>','<%=SACG.Path%>');"><%if (SACG.ImageUrl!=""){%><img src="<%=Image(SACG.ImageUrl) %>" /><%}%><%=SACG.Name%></a></dd>
            <%SACG_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int NDPd_index=1;
List<Lebi_Searchkey> NDPds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey NDPd in NDPds){%><%=Lang(NDPd.Name)%><%NDPd_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int lFwu_index=1;
List<Lebi_Searchkey> lFwus = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey lFwu in lFwus){%>
<%if (lFwu.Type==1){ %><a href="<%=URL("P_Search",""+Lang(lFwu.Name)+"") %>"><%}else{ %><a href="<%=lFwu.URL%>" target="_blank"><%} %><span><%=Lang(lFwu.Name)%></span></a>
<%lFwu_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int hzQw_index=1;
List<Lebi_Brand> hzQws = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand hzQw in hzQws){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",hzQw.id)%>"><img alt="<%=Lang(hzQw.Name) %>" src="<%=Image(hzQw.ImageUrl) %>"><span><%=Lang(hzQw.Name) %></span></a></li>
                                        <%hzQw_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int JMBz_index=1;
List<Lebi_Page> JMBzs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page JMBz in JMBzs){%>
<a class="menu" href="<%=URL("","",JMBz.url)%>"><span><%=JMBz.Name%></span></a>
<%JMBz_index++;}%>
