...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserReturnApply"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> kgrMs=Languages();RecordCount=kgrMs.Count;int kgrM_index=1;
foreach (Shop.Model.Lebi_Language kgrM in kgrMs){%>
            <dd <%if (kgrM_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=kgrM.id%>,'<%=kgrM.Code%>','<%=kgrM.Path%>');"><%if (kgrM.ImageUrl!=""){%><img src="<%=Image(kgrM.ImageUrl) %>" /><%}%><%=kgrM.Name%></a></dd>
            <%kgrM_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int xwoB_index=1;
List<Lebi_Currency> xwoBs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency xwoB in xwoBs){%>
            <dd <%if (xwoB_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=xwoB.id%>,'<%=xwoB.Code%>',<%=xwoB.ExchangeRate%>,'<%=xwoB.Msige%>','<%=xwoB.DecimalLength%>');"><%=xwoB.Code%></a></dd>
            <%xwoB_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int PKNr_index=1;
List<Lebi_Searchkey> PKNrs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey PKNr in PKNrs){%><%=Lang(PKNr.Name)%><%PKNr_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Jnmm_index=1;
List<Lebi_Searchkey> Jnmms = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Jnmm in Jnmms){%>
<%if (Jnmm.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Jnmm.Name)+"") %>"><%}else{ %><a href="<%=Jnmm.URL%>" target="_blank"><%} %><span><%=Lang(Jnmm.Name)%></span></a>
<%Jnmm_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int gCCG_index=1;
List<Lebi_Page> gCCGs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page gCCG in gCCGs){%>
<a class="menu" href="<%=URL("","",gCCG.url)%>"><span><%=gCCG.Name%></span></a>
<%gCCG_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int FWjI_index=1;
List<Lebi_FriendLink> FWjIs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink FWjI in FWjIs){%>
            
                <% if (FWjI.Logo != "" && FWjI.IsPic == 1){ %><a href="<%=FWjI.Url%>" target="_blank"><img src="<%=Image(FWjI.Logo) %>" alt="<%=FWjI.Name%>" /></a><%}else{%><a href="<%=FWjI.Url%>" target="_blank"><%=FWjI.Name%></a><%} %>     
