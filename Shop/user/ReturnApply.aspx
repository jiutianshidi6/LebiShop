...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserReturnApply"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> YBYys=Languages();RecordCount=YBYys.Count;int YBYy_index=1;
foreach (Shop.Model.Lebi_Language YBYy in YBYys){%>
            <dd <%if (YBYy_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=YBYy.id%>,'<%=YBYy.Code%>','<%=YBYy.Path%>');"><%if (YBYy.ImageUrl!=""){%><img src="<%=Image(YBYy.ImageUrl) %>" /><%}%><%=YBYy.Name%></a></dd>
            <%YBYy_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int yXgE_index=1;
List<Lebi_Searchkey> yXgEs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey yXgE in yXgEs){%><%=Lang(yXgE.Name)%><%yXgE_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int OcKa_index=1;
List<Lebi_Searchkey> OcKas = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey OcKa in OcKas){%>
<%if (OcKa.Type==1){ %><a href="<%=URL("P_Search",""+Lang(OcKa.Name)+"") %>"><%}else{ %><a href="<%=OcKa.URL%>" target="_blank"><%} %><span><%=Lang(OcKa.Name)%></span></a>
<%OcKa_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int kaam_index=1;
List<Lebi_Brand> kaams = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand kaam in kaams){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",kaam.id)%>"><img alt="<%=Lang(kaam.Name) %>" src="<%=Image(kaam.ImageUrl) %>"><span><%=Lang(kaam.Name) %></span></a></li>
                                        <%kaam_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int PHNg_index=1;
List<Lebi_Page> PHNgs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page PHNg in PHNgs){%>
<a class="menu" href="<%=URL("","",PHNg.url)%>"><span><%=PHNg.Name%></span></a>
<%PHNg_index++;}%>
