...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserProfile"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> TsPEs=Languages();RecordCount=TsPEs.Count;int TsPE_index=1;
foreach (Shop.Model.Lebi_Language TsPE in TsPEs){%>
            <dd <%if (TsPE_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=TsPE.id%>,'<%=TsPE.Code%>','<%=TsPE.Path%>');"><%if (TsPE.ImageUrl!=""){%><img src="<%=Image(TsPE.ImageUrl) %>" /><%}%><%=TsPE.Name%></a></dd>
            <%TsPE_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Vxaj_index=1;
List<Lebi_Currency> Vxajs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Vxaj in Vxajs){%>
            <dd <%if (Vxaj_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Vxaj.id%>,'<%=Vxaj.Code%>',<%=Vxaj.ExchangeRate%>,'<%=Vxaj.Msige%>','<%=Vxaj.DecimalLength%>');"><%=Vxaj.Code%></a></dd>
            <%Vxaj_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int pmPR_index=1;
List<Lebi_Searchkey> pmPRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey pmPR in pmPRs){%><%=Lang(pmPR.Name)%><%pmPR_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int mSYs_index=1;
List<Lebi_Searchkey> mSYss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey mSYs in mSYss){%>
<%if (mSYs.Type==1){ %><a href="<%=URL("P_Search",""+Lang(mSYs.Name)+"") %>"><%}else{ %><a href="<%=mSYs.URL%>" target="_blank"><%} %><span><%=Lang(mSYs.Name)%></span></a>
<%mSYs_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int UJKE_index=1;
List<Lebi_Page> UJKEs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page UJKE in UJKEs){%>
<a class="menu" href="<%=URL("","",UJKE.url)%>"><span><%=UJKE.Name%></span></a>
<%UJKE_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int vrGh_index=1;
List<Lebi_FriendLink> vrGhs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink vrGh in vrGhs){%>
            
                <% if (vrGh.Logo != "" && vrGh.IsPic == 1){ %><a href="<%=vrGh.Url%>" target="_blank"><img src="<%=Image(vrGh.Logo) %>" alt="<%=vrGh.Name%>" /></a><%}else{%><a href="<%=vrGh.Url%>" target="_blank"><%=vrGh.Name%></a><%} %>     
