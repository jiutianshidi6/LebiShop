...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserProfile"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> jVKPs=Languages();RecordCount=jVKPs.Count;int jVKP_index=1;
foreach (Shop.Model.Lebi_Language jVKP in jVKPs){%>
            <dd <%if (jVKP_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=jVKP.id%>,'<%=jVKP.Code%>','<%=jVKP.Path%>');"><%if (jVKP.ImageUrl!=""){%><img src="<%=Image(jVKP.ImageUrl) %>" /><%}%><%=jVKP.Name%></a></dd>
            <%jVKP_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int FsLg_index=1;
List<Lebi_Searchkey> FsLgs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey FsLg in FsLgs){%><%=Lang(FsLg.Name)%><%FsLg_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int uAvV_index=1;
List<Lebi_Searchkey> uAvVs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey uAvV in uAvVs){%>
<%if (uAvV.Type==1){ %><a href="<%=URL("P_Search",""+Lang(uAvV.Name)+"") %>"><%}else{ %><a href="<%=uAvV.URL%>" target="_blank"><%} %><span><%=Lang(uAvV.Name)%></span></a>
<%uAvV_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int yicR_index=1;
List<Lebi_Brand> yicRs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand yicR in yicRs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",yicR.id)%>"><img alt="<%=Lang(yicR.Name) %>" src="<%=Image(yicR.ImageUrl) %>"><span><%=Lang(yicR.Name) %></span></a></li>
                                        <%yicR_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int XifG_index=1;
List<Lebi_Page> XifGs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page XifG in XifGs){%>
<a class="menu" href="<%=URL("","",XifG.url)%>"><span><%=XifG.Name%></span></a>
<%XifG_index++;}%>
