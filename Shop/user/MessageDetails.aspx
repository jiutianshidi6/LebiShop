...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserMessageDetails"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> nBMFs=Languages();RecordCount=nBMFs.Count;int nBMF_index=1;
foreach (Shop.Model.Lebi_Language nBMF in nBMFs){%>
            <dd <%if (nBMF_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=nBMF.id%>,'<%=nBMF.Code%>','<%=nBMF.Path%>');"><%if (nBMF.ImageUrl!=""){%><img src="<%=Image(nBMF.ImageUrl) %>" /><%}%><%=nBMF.Name%></a></dd>
            <%nBMF_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int vCoe_index=1;
List<Lebi_Currency> vCoes = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency vCoe in vCoes){%>
            <dd <%if (vCoe_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=vCoe.id%>,'<%=vCoe.Code%>',<%=vCoe.ExchangeRate%>,'<%=vCoe.Msige%>','<%=vCoe.DecimalLength%>');"><%=vCoe.Code%></a></dd>
            <%vCoe_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int YNCW_index=1;
List<Lebi_Searchkey> YNCWs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey YNCW in YNCWs){%><%=Lang(YNCW.Name)%><%YNCW_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zLSI_index=1;
List<Lebi_Searchkey> zLSIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zLSI in zLSIs){%>
<%if (zLSI.Type==1){ %><a href="<%=URL("P_Search",""+Lang(zLSI.Name)+"") %>"><%}else{ %><a href="<%=zLSI.URL%>" target="_blank"><%} %><span><%=Lang(zLSI.Name)%></span></a>
<%zLSI_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int OrOR_index=1;
List<Lebi_Page> OrORs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page OrOR in OrORs){%>
<a class="menu" href="<%=URL("","",OrOR.url)%>"><span><%=OrOR.Name%></span></a>
<%OrOR_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int lIuV_index=1;
List<Lebi_FriendLink> lIuVs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink lIuV in lIuVs){%>
            
                <% if (lIuV.Logo != "" && lIuV.IsPic == 1){ %><a href="<%=lIuV.Url%>" target="_blank"><img src="<%=Image(lIuV.Logo) %>" alt="<%=lIuV.Name%>" /></a><%}else{%><a href="<%=lIuV.Url%>" target="_blank"><%=lIuV.Name%></a><%} %>     
