...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserComment"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> GlKjs=Languages();RecordCount=GlKjs.Count;int GlKj_index=1;
foreach (Shop.Model.Lebi_Language GlKj in GlKjs){%>
            <dd <%if (GlKj_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=GlKj.id%>,'<%=GlKj.Code%>','<%=GlKj.Path%>');"><%if (GlKj.ImageUrl!=""){%><img src="<%=Image(GlKj.ImageUrl) %>" /><%}%><%=GlKj.Name%></a></dd>
            <%GlKj_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int shFt_index=1;
List<Lebi_Searchkey> shFts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey shFt in shFts){%><%=Lang(shFt.Name)%><%shFt_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int xRRq_index=1;
List<Lebi_Searchkey> xRRqs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey xRRq in xRRqs){%>
<%if (xRRq.Type==1){ %><a href="<%=URL("P_Search",""+Lang(xRRq.Name)+"") %>"><%}else{ %><a href="<%=xRRq.URL%>" target="_blank"><%} %><span><%=Lang(xRRq.Name)%></span></a>
<%xRRq_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int myUM_index=1;
List<Lebi_Brand> myUMs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand myUM in myUMs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",myUM.id)%>"><img alt="<%=Lang(myUM.Name) %>" src="<%=Image(myUM.ImageUrl) %>"><span><%=Lang(myUM.Name) %></span></a></li>
                                        <%myUM_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int IMon_index=1;
List<Lebi_Page> IMons = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page IMon in IMons){%>
<a class="menu" href="<%=URL("","",IMon.url)%>"><span><%=IMon.Name%></span></a>
<%IMon_index++;}%>
