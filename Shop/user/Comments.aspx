...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserComment"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> kfdSs=Languages();RecordCount=kfdSs.Count;int kfdS_index=1;
foreach (Shop.Model.Lebi_Language kfdS in kfdSs){%>
            <dd <%if (kfdS_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=kfdS.id%>,'<%=kfdS.Code%>','<%=kfdS.Path%>');"><%if (kfdS.ImageUrl!=""){%><img src="<%=Image(kfdS.ImageUrl) %>" /><%}%><%=kfdS.Name%></a></dd>
            <%kfdS_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int yJta_index=1;
List<Lebi_Currency> yJtas = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency yJta in yJtas){%>
            <dd <%if (yJta_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=yJta.id%>,'<%=yJta.Code%>',<%=yJta.ExchangeRate%>,'<%=yJta.Msige%>','<%=yJta.DecimalLength%>');"><%=yJta.Code%></a></dd>
            <%yJta_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int VLjY_index=1;
List<Lebi_Searchkey> VLjYs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey VLjY in VLjYs){%><%=Lang(VLjY.Name)%><%VLjY_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int SnkO_index=1;
List<Lebi_Searchkey> SnkOs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey SnkO in SnkOs){%>
<%if (SnkO.Type==1){ %><a href="<%=URL("P_Search",""+Lang(SnkO.Name)+"") %>"><%}else{ %><a href="<%=SnkO.URL%>" target="_blank"><%} %><span><%=Lang(SnkO.Name)%></span></a>
<%SnkO_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int vuWN_index=1;
List<Lebi_Page> vuWNs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page vuWN in vuWNs){%>
<a class="menu" href="<%=URL("","",vuWN.url)%>"><span><%=vuWN.Name%></span></a>
<%vuWN_index++;}%>
...
<div class="user">
    ...
    {
    %>
    <tr><td colspan="5">客服回复：<%=creply.Content%><em><%=creply.Time_Add%></em></td></tr>
    <%} %>
    </table>
    <%} %>
    <%}else{ %>
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        ...
        <tr>
            ...
            <td style="width:100px;"><a href="<%=URL("P_UserCommentWrite",op.id) %>">发表评价</a></td>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int mFjj_index=1;
List<Lebi_FriendLink> mFjjs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink mFjj in mFjjs){%>
            
                <% if (mFjj.Logo != "" && mFjj.IsPic == 1){ %><a href="<%=mFjj.Url%>" target="_blank"><img src="<%=Image(mFjj.Logo) %>" alt="<%=mFjj.Name%>" /></a><%}else{%><a href="<%=mFjj.Url%>" target="_blank"><%=mFjj.Name%></a><%} %>     
