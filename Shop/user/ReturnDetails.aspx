...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserReturnDetails"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> rXuHs=Languages();RecordCount=rXuHs.Count;int rXuH_index=1;
foreach (Shop.Model.Lebi_Language rXuH in rXuHs){%>
            <dd <%if (rXuH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=rXuH.id%>,'<%=rXuH.Code%>','<%=rXuH.Path%>');"><%if (rXuH.ImageUrl!=""){%><img src="<%=Image(rXuH.ImageUrl) %>" /><%}%><%=rXuH.Name%></a></dd>
            <%rXuH_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int xRaX_index=1;
List<Lebi_Currency> xRaXs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency xRaX in xRaXs){%>
            <dd <%if (xRaX_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=xRaX.id%>,'<%=xRaX.Code%>',<%=xRaX.ExchangeRate%>,'<%=xRaX.Msige%>','<%=xRaX.DecimalLength%>');"><%=xRaX.Code%></a></dd>
            <%xRaX_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Juqh_index=1;
List<Lebi_Searchkey> Juqhs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Juqh in Juqhs){%><%=Lang(Juqh.Name)%><%Juqh_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hLvv_index=1;
List<Lebi_Searchkey> hLvvs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hLvv in hLvvs){%>
<%if (hLvv.Type==1){ %><a href="<%=URL("P_Search",""+Lang(hLvv.Name)+"") %>"><%}else{ %><a href="<%=hLvv.URL%>" target="_blank"><%} %><span><%=Lang(hLvv.Name)%></span></a>
<%hLvv_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int CAtn_index=1;
List<Lebi_Page> CAtns = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page CAtn in CAtns){%>
<a class="menu" href="<%=URL("","",CAtn.url)%>"><span><%=CAtn.Name%></span></a>
<%CAtn_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int xmKD_index=1;
List<Lebi_FriendLink> xmKDs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink xmKD in xmKDs){%>
            
                <% if (xmKD.Logo != "" && xmKD.IsPic == 1){ %><a href="<%=xmKD.Url%>" target="_blank"><img src="<%=Image(xmKD.Logo) %>" alt="<%=xmKD.Name%>" /></a><%}else{%><a href="<%=xmKD.Url%>" target="_blank"><%=xmKD.Name%></a><%} %>     
