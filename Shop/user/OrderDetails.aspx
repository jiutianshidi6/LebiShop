...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserOrderDetails"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> rqRTs=Languages();RecordCount=rqRTs.Count;int rqRT_index=1;
foreach (Shop.Model.Lebi_Language rqRT in rqRTs){%>
            <dd <%if (rqRT_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=rqRT.id%>,'<%=rqRT.Code%>','<%=rqRT.Path%>');"><%if (rqRT.ImageUrl!=""){%><img src="<%=Image(rqRT.ImageUrl) %>" /><%}%><%=rqRT.Name%></a></dd>
            <%rqRT_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int TDYE_index=1;
List<Lebi_Currency> TDYEs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency TDYE in TDYEs){%>
            <dd <%if (TDYE_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=TDYE.id%>,'<%=TDYE.Code%>',<%=TDYE.ExchangeRate%>,'<%=TDYE.Msige%>','<%=TDYE.DecimalLength%>');"><%=TDYE.Code%></a></dd>
            <%TDYE_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int EaVX_index=1;
List<Lebi_Searchkey> EaVXs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey EaVX in EaVXs){%><%=Lang(EaVX.Name)%><%EaVX_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int EXjw_index=1;
List<Lebi_Searchkey> EXjws = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey EXjw in EXjws){%>
<%if (EXjw.Type==1){ %><a href="<%=URL("P_Search",""+Lang(EXjw.Name)+"") %>"><%}else{ %><a href="<%=EXjw.URL%>" target="_blank"><%} %><span><%=Lang(EXjw.Name)%></span></a>
<%EXjw_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int uuLv_index=1;
List<Lebi_Page> uuLvs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page uuLv in uuLvs){%>
<a class="menu" href="<%=URL("","",uuLv.url)%>"><span><%=uuLv.Name%></span></a>
<%uuLv_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int THRK_index=1;
List<Lebi_FriendLink> THRKs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink THRK in THRKs){%>
            
                <% if (THRK.Logo != "" && THRK.IsPic == 1){ %><a href="<%=THRK.Url%>" target="_blank"><img src="<%=Image(THRK.Logo) %>" alt="<%=THRK.Name%>" /></a><%}else{%><a href="<%=THRK.Url%>" target="_blank"><%=THRK.Name%></a><%} %>     
