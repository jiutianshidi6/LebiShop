...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserLike"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> tNXLs=Languages();RecordCount=tNXLs.Count;int tNXL_index=1;
foreach (Shop.Model.Lebi_Language tNXL in tNXLs){%>
            <dd <%if (tNXL_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=tNXL.id%>,'<%=tNXL.Code%>','<%=tNXL.Path%>');"><%if (tNXL.ImageUrl!=""){%><img src="<%=Image(tNXL.ImageUrl) %>" /><%}%><%=tNXL.Name%></a></dd>
            <%tNXL_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int jCOv_index=1;
List<Lebi_Currency> jCOvs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency jCOv in jCOvs){%>
            <dd <%if (jCOv_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=jCOv.id%>,'<%=jCOv.Code%>',<%=jCOv.ExchangeRate%>,'<%=jCOv.Msige%>','<%=jCOv.DecimalLength%>');"><%=jCOv.Code%></a></dd>
            <%jCOv_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int tbUK_index=1;
List<Lebi_Searchkey> tbUKs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey tbUK in tbUKs){%><%=Lang(tbUK.Name)%><%tbUK_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int GXWd_index=1;
List<Lebi_Searchkey> GXWds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey GXWd in GXWds){%>
<%if (GXWd.Type==1){ %><a href="<%=URL("P_Search",""+Lang(GXWd.Name)+"") %>"><%}else{ %><a href="<%=GXWd.URL%>" target="_blank"><%} %><span><%=Lang(GXWd.Name)%></span></a>
<%GXWd_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int LjwU_index=1;
List<Lebi_Page> LjwUs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page LjwU in LjwUs){%>
<a class="menu" href="<%=URL("","",LjwU.url)%>"><span><%=LjwU.Name%></span></a>
<%LjwU_index++;}%>
...
<div class="user">
    ...
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>删除</a><a href="javascript:void(0)" onclick="LikeToBasket();" class="btn btn-7"><s></s>加入购物车</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript" >
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int zpsY_index=1;
List<Lebi_FriendLink> zpsYs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink zpsY in zpsYs){%>
            
                <% if (zpsY.Logo != "" && zpsY.IsPic == 1){ %><a href="<%=zpsY.Url%>" target="_blank"><img src="<%=Image(zpsY.Logo) %>" alt="<%=zpsY.Name%>" /></a><%}else{%><a href="<%=zpsY.Url%>" target="_blank"><%=zpsY.Name%></a><%} %>     
