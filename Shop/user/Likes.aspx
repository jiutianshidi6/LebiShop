...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserLike"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> ecNns=Languages();RecordCount=ecNns.Count;int ecNn_index=1;
foreach (Shop.Model.Lebi_Language ecNn in ecNns){%>
            <dd <%if (ecNn_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=ecNn.id%>,'<%=ecNn.Code%>','<%=ecNn.Path%>');"><%if (ecNn.ImageUrl!=""){%><img src="<%=Image(ecNn.ImageUrl) %>" /><%}%><%=ecNn.Name%></a></dd>
            <%ecNn_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int xOJs_index=1;
List<Lebi_Searchkey> xOJss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey xOJs in xOJss){%><%=Lang(xOJs.Name)%><%xOJs_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int XVMp_index=1;
List<Lebi_Searchkey> XVMps = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey XVMp in XVMps){%>
<%if (XVMp.Type==1){ %><a href="<%=URL("P_Search",""+Lang(XVMp.Name)+"") %>"><%}else{ %><a href="<%=XVMp.URL%>" target="_blank"><%} %><span><%=Lang(XVMp.Name)%></span></a>
<%XVMp_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int FDqR_index=1;
List<Lebi_Brand> FDqRs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand FDqR in FDqRs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",FDqR.id)%>"><img alt="<%=Lang(FDqR.Name) %>" src="<%=Image(FDqR.ImageUrl) %>"><span><%=Lang(FDqR.Name) %></span></a></li>
                                        <%FDqR_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int prRP_index=1;
List<Lebi_Page> prRPs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page prRP in prRPs){%>
<a class="menu" href="<%=URL("","",prRP.url)%>"><span><%=prRP.Name%></span></a>
<%prRP_index++;}%>
