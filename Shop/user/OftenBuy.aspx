...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserOftenBuy"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> WnQEs=Languages();RecordCount=WnQEs.Count;int WnQE_index=1;
foreach (Shop.Model.Lebi_Language WnQE in WnQEs){%>
            <dd <%if (WnQE_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=WnQE.id%>,'<%=WnQE.Code%>','<%=WnQE.Path%>');"><%if (WnQE.ImageUrl!=""){%><img src="<%=Image(WnQE.ImageUrl) %>" /><%}%><%=WnQE.Name%></a></dd>
            <%WnQE_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int XPkI_index=1;
List<Lebi_Searchkey> XPkIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey XPkI in XPkIs){%><%=Lang(XPkI.Name)%><%XPkI_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int iQuu_index=1;
List<Lebi_Searchkey> iQuus = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey iQuu in iQuus){%>
<%if (iQuu.Type==1){ %><a href="<%=URL("P_Search",""+Lang(iQuu.Name)+"") %>"><%}else{ %><a href="<%=iQuu.URL%>" target="_blank"><%} %><span><%=Lang(iQuu.Name)%></span></a>
<%iQuu_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int KVxy_index=1;
List<Lebi_Brand> KVxys = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand KVxy in KVxys){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",KVxy.id)%>"><img alt="<%=Lang(KVxy.Name) %>" src="<%=Image(KVxy.ImageUrl) %>"><span><%=Lang(KVxy.Name) %></span></a></li>
                                        <%KVxy_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int BDHF_index=1;
List<Lebi_Page> BDHFs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page BDHF in BDHFs){%>
<a class="menu" href="<%=URL("","",BDHF.url)%>"><span><%=BDHF.Name%></span></a>
<%BDHF_index++;}%>
