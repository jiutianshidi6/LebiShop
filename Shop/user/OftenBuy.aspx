...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserOftenBuy"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> wNeWs=Languages();RecordCount=wNeWs.Count;int wNeW_index=1;
foreach (Shop.Model.Lebi_Language wNeW in wNeWs){%>
            <dd <%if (wNeW_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=wNeW.id%>,'<%=wNeW.Code%>','<%=wNeW.Path%>');"><%if (wNeW.ImageUrl!=""){%><img src="<%=Image(wNeW.ImageUrl) %>" /><%}%><%=wNeW.Name%></a></dd>
            <%wNeW_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int kHAr_index=1;
List<Lebi_Currency> kHArs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency kHAr in kHArs){%>
            <dd <%if (kHAr_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=kHAr.id%>,'<%=kHAr.Code%>',<%=kHAr.ExchangeRate%>,'<%=kHAr.Msige%>','<%=kHAr.DecimalLength%>');"><%=kHAr.Code%></a></dd>
            <%kHAr_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int uYDY_index=1;
List<Lebi_Searchkey> uYDYs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey uYDY in uYDYs){%><%=Lang(uYDY.Name)%><%uYDY_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int jGqO_index=1;
List<Lebi_Searchkey> jGqOs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey jGqO in jGqOs){%>
<%if (jGqO.Type==1){ %><a href="<%=URL("P_Search",""+Lang(jGqO.Name)+"") %>"><%}else{ %><a href="<%=jGqO.URL%>" target="_blank"><%} %><span><%=Lang(jGqO.Name)%></span></a>
<%jGqO_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int mxeN_index=1;
List<Lebi_Page> mxeNs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page mxeN in mxeNs){%>
<a class="menu" href="<%=URL("","",mxeN.url)%>"><span><%=mxeN.Name%></span></a>
<%mxeN_index++;}%>
...
<div class="user">
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        <tr>
            ...
            <th style="width:80px;">购买周期</th>
            <th style="width:120px;">预购时间</th>
        </tr>
    ...
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>删除</a><a href="javascript:void(0)" onclick="Update();" class="btn btn-11"><s></s>保存</a><a href="javascript:void(0)" onclick="LikeToBasket();" class="btn btn-7"><s></s>加入购物车</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript" >
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int pftS_index=1;
List<Lebi_FriendLink> pftSs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink pftS in pftSs){%>
            
                <% if (pftS.Logo != "" && pftS.IsPic == 1){ %><a href="<%=pftS.Url%>" target="_blank"><img src="<%=Image(pftS.Logo) %>" alt="<%=pftS.Name%>" /></a><%}else{%><a href="<%=pftS.Url%>" target="_blank"><%=pftS.Name%></a><%} %>     
