...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserPointToMoney"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> ctDds=Languages();RecordCount=ctDds.Count;int ctDd_index=1;
foreach (Shop.Model.Lebi_Language ctDd in ctDds){%>
            <dd <%if (ctDd_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=ctDd.id%>,'<%=ctDd.Code%>','<%=ctDd.Path%>');"><%if (ctDd.ImageUrl!=""){%><img src="<%=Image(ctDd.ImageUrl) %>" /><%}%><%=ctDd.Name%></a></dd>
            <%ctDd_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int JYXQ_index=1;
List<Lebi_Currency> JYXQs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency JYXQ in JYXQs){%>
            <dd <%if (JYXQ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=JYXQ.id%>,'<%=JYXQ.Code%>',<%=JYXQ.ExchangeRate%>,'<%=JYXQ.Msige%>','<%=JYXQ.DecimalLength%>');"><%=JYXQ.Code%></a></dd>
            <%JYXQ_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int wreI_index=1;
List<Lebi_Searchkey> wreIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey wreI in wreIs){%><%=Lang(wreI.Name)%><%wreI_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int eVqa_index=1;
List<Lebi_Searchkey> eVqas = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey eVqa in eVqas){%>
<%if (eVqa.Type==1){ %><a href="<%=URL("P_Search",""+Lang(eVqa.Name)+"") %>"><%}else{ %><a href="<%=eVqa.URL%>" target="_blank"><%} %><span><%=Lang(eVqa.Name)%></span></a>
<%eVqa_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int PQlm_index=1;
List<Lebi_Page> PQlms = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page PQlm in PQlms){%>
<a class="menu" href="<%=URL("","",PQlm.url)%>"><span><%=PQlm.Name%></span></a>
<%PQlm_index++;}%>
...
<div class="userbox">
    ...
    %>
    <dl>
        <dt><input type="radio" name="ptom" value="<%=v %>" checked /><%=val[0] %>积分：</dt>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int dYkb_index=1;
List<Lebi_FriendLink> dYkbs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink dYkb in dYkbs){%>
            
                <% if (dYkb.Logo != "" && dYkb.IsPic == 1){ %><a href="<%=dYkb.Url%>" target="_blank"><img src="<%=Image(dYkb.Logo) %>" alt="<%=dYkb.Name%>" /></a><%}else{%><a href="<%=dYkb.Url%>" target="_blank"><%=dYkb.Name%></a><%} %>     
