...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserPointToMoney"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> lEjzs=Languages();RecordCount=lEjzs.Count;int lEjz_index=1;
foreach (Shop.Model.Lebi_Language lEjz in lEjzs){%>
            <dd <%if (lEjz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=lEjz.id%>,'<%=lEjz.Code%>','<%=lEjz.Path%>');"><%if (lEjz.ImageUrl!=""){%><img src="<%=Image(lEjz.ImageUrl) %>" /><%}%><%=lEjz.Name%></a></dd>
            <%lEjz_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ScIB_index=1;
List<Lebi_Searchkey> ScIBs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ScIB in ScIBs){%><%=Lang(ScIB.Name)%><%ScIB_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int BcmQ_index=1;
List<Lebi_Searchkey> BcmQs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey BcmQ in BcmQs){%>
<%if (BcmQ.Type==1){ %><a href="<%=URL("P_Search",""+Lang(BcmQ.Name)+"") %>"><%}else{ %><a href="<%=BcmQ.URL%>" target="_blank"><%} %><span><%=Lang(BcmQ.Name)%></span></a>
<%BcmQ_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int rFDc_index=1;
List<Lebi_Brand> rFDcs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand rFDc in rFDcs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",rFDc.id)%>"><img alt="<%=Lang(rFDc.Name) %>" src="<%=Image(rFDc.ImageUrl) %>"><span><%=Lang(rFDc.Name) %></span></a></li>
                                        <%rFDc_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int kEnO_index=1;
List<Lebi_Page> kEnOs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page kEnO in kEnOs){%>
<a class="menu" href="<%=URL("","",kEnO.url)%>"><span><%=kEnO.Name%></span></a>
<%kEnO_index++;}%>
