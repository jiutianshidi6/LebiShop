...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserCard"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> RDvas=Languages();RecordCount=RDvas.Count;int RDva_index=1;
foreach (Shop.Model.Lebi_Language RDva in RDvas){%>
            <dd <%if (RDva_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=RDva.id%>,'<%=RDva.Code%>','<%=RDva.Path%>');"><%if (RDva.ImageUrl!=""){%><img src="<%=Image(RDva.ImageUrl) %>" /><%}%><%=RDva.Name%></a></dd>
            <%RDva_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int dqBQ_index=1;
List<Lebi_Currency> dqBQs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency dqBQ in dqBQs){%>
            <dd <%if (dqBQ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=dqBQ.id%>,'<%=dqBQ.Code%>',<%=dqBQ.ExchangeRate%>,'<%=dqBQ.Msige%>','<%=dqBQ.DecimalLength%>');"><%=dqBQ.Code%></a></dd>
            <%dqBQ_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int IIKH_index=1;
List<Lebi_Searchkey> IIKHs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey IIKH in IIKHs){%><%=Lang(IIKH.Name)%><%IIKH_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int nrit_index=1;
List<Lebi_Searchkey> nrits = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey nrit in nrits){%>
<%if (nrit.Type==1){ %><a href="<%=URL("P_Search",""+Lang(nrit.Name)+"") %>"><%}else{ %><a href="<%=nrit.URL%>" target="_blank"><%} %><span><%=Lang(nrit.Name)%></span></a>
<%nrit_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int hmhe_index=1;
List<Lebi_Page> hmhes = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page hmhe in hmhes){%>
<a class="menu" href="<%=URL("","",hmhe.url)%>"><span><%=hmhe.Name%></span></a>
<%hmhe_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int MClg_index=1;
List<Lebi_FriendLink> MClgs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink MClg in MClgs){%>
            
                <% if (MClg.Logo != "" && MClg.IsPic == 1){ %><a href="<%=MClg.Url%>" target="_blank"><img src="<%=Image(MClg.Logo) %>" alt="<%=MClg.Name%>" /></a><%}else{%><a href="<%=MClg.Url%>" target="_blank"><%=MClg.Name%></a><%} %>     
