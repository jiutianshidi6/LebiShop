...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserCard"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> WwTQs=Languages();RecordCount=WwTQs.Count;int WwTQ_index=1;
foreach (Shop.Model.Lebi_Language WwTQ in WwTQs){%>
            <dd <%if (WwTQ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=WwTQ.id%>,'<%=WwTQ.Code%>','<%=WwTQ.Path%>');"><%if (WwTQ.ImageUrl!=""){%><img src="<%=Image(WwTQ.ImageUrl) %>" /><%}%><%=WwTQ.Name%></a></dd>
            <%WwTQ_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Kybg_index=1;
List<Lebi_Searchkey> Kybgs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Kybg in Kybgs){%><%=Lang(Kybg.Name)%><%Kybg_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int WASf_index=1;
List<Lebi_Searchkey> WASfs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey WASf in WASfs){%>
<%if (WASf.Type==1){ %><a href="<%=URL("P_Search",""+Lang(WASf.Name)+"") %>"><%}else{ %><a href="<%=WASf.URL%>" target="_blank"><%} %><span><%=Lang(WASf.Name)%></span></a>
<%WASf_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int tomy_index=1;
List<Lebi_Brand> tomys = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand tomy in tomys){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",tomy.id)%>"><img alt="<%=Lang(tomy.Name) %>" src="<%=Image(tomy.ImageUrl) %>"><span><%=Lang(tomy.Name) %></span></a></li>
                                        <%tomy_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int JfRB_index=1;
List<Lebi_Page> JfRBs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page JfRB in JfRBs){%>
<a class="menu" href="<%=URL("","",JfRB.url)%>"><span><%=JfRB.Name%></span></a>
<%JfRB_index++;}%>
