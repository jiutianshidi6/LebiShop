...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserBank"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> YICLs=Languages();RecordCount=YICLs.Count;int YICL_index=1;
foreach (Shop.Model.Lebi_Language YICL in YICLs){%>
            <dd <%if (YICL_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=YICL.id%>,'<%=YICL.Code%>','<%=YICL.Path%>');"><%if (YICL.ImageUrl!=""){%><img src="<%=Image(YICL.ImageUrl) %>" /><%}%><%=YICL.Name%></a></dd>
            <%YICL_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int erqL_index=1;
List<Lebi_Searchkey> erqLs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey erqL in erqLs){%><%=Lang(erqL.Name)%><%erqL_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int KkwD_index=1;
List<Lebi_Searchkey> KkwDs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey KkwD in KkwDs){%>
<%if (KkwD.Type==1){ %><a href="<%=URL("P_Search",""+Lang(KkwD.Name)+"") %>"><%}else{ %><a href="<%=KkwD.URL%>" target="_blank"><%} %><span><%=Lang(KkwD.Name)%></span></a>
<%KkwD_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int NesS_index=1;
List<Lebi_Brand> NesSs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand NesS in NesSs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",NesS.id)%>"><img alt="<%=Lang(NesS.Name) %>" src="<%=Image(NesS.ImageUrl) %>"><span><%=Lang(NesS.Name) %></span></a></li>
                                        <%NesS_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Jmii_index=1;
List<Lebi_Page> Jmiis = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Jmii in Jmiis){%>
<a class="menu" href="<%=URL("","",Jmii.url)%>"><span><%=Jmii.Name%></span></a>
<%Jmii_index++;}%>
