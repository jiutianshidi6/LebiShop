...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserBank"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> jryzs=Languages();RecordCount=jryzs.Count;int jryz_index=1;
foreach (Shop.Model.Lebi_Language jryz in jryzs){%>
            <dd <%if (jryz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=jryz.id%>,'<%=jryz.Code%>','<%=jryz.Path%>');"><%if (jryz.ImageUrl!=""){%><img src="<%=Image(jryz.ImageUrl) %>" /><%}%><%=jryz.Name%></a></dd>
            <%jryz_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int xLhu_index=1;
List<Lebi_Currency> xLhus = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency xLhu in xLhus){%>
            <dd <%if (xLhu_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=xLhu.id%>,'<%=xLhu.Code%>',<%=xLhu.ExchangeRate%>,'<%=xLhu.Msige%>','<%=xLhu.DecimalLength%>');"><%=xLhu.Code%></a></dd>
            <%xLhu_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MYgR_index=1;
List<Lebi_Searchkey> MYgRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MYgR in MYgRs){%><%=Lang(MYgR.Name)%><%MYgR_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ICTG_index=1;
List<Lebi_Searchkey> ICTGs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ICTG in ICTGs){%>
<%if (ICTG.Type==1){ %><a href="<%=URL("P_Search",""+Lang(ICTG.Name)+"") %>"><%}else{ %><a href="<%=ICTG.URL%>" target="_blank"><%} %><span><%=Lang(ICTG.Name)%></span></a>
<%ICTG_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int owrL_index=1;
List<Lebi_Page> owrLs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page owrL in owrLs){%>
<a class="menu" href="<%=URL("","",owrL.url)%>"><span><%=owrL.Name%></span></a>
<%owrL_index++;}%>
...
<div class="user">
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        ...
        <tr>
            ...
            <td><%=ub.Code%></td>
            <td><%=ub.Name%></td>
            <td><a href="<%=URL("P_UserBankEdit",ub.id,"")%>">编辑</a></td>
        </tr>
        <%} %>
    </table>
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>删除</a><a href="<%=URL("P_UserBankEdit","0","")%>" class="btn btn-7"><s></s>添加</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript">
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int pjPW_index=1;
List<Lebi_FriendLink> pjPWs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink pjPW in pjPWs){%>
            
                <% if (pjPW.Logo != "" && pjPW.IsPic == 1){ %><a href="<%=pjPW.Url%>" target="_blank"><img src="<%=Image(pjPW.Logo) %>" alt="<%=pjPW.Name%>" /></a><%}else{%><a href="<%=pjPW.Url%>" target="_blank"><%=pjPW.Name%></a><%} %>     
