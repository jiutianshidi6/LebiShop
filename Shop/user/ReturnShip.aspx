...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserReturnShip"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> QXKhs=Languages();RecordCount=QXKhs.Count;int QXKh_index=1;
foreach (Shop.Model.Lebi_Language QXKh in QXKhs){%>
            <dd <%if (QXKh_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=QXKh.id%>,'<%=QXKh.Code%>','<%=QXKh.Path%>');"><%if (QXKh.ImageUrl!=""){%><img src="<%=Image(QXKh.ImageUrl) %>" /><%}%><%=QXKh.Name%></a></dd>
            <%QXKh_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int AKDu_index=1;
List<Lebi_Currency> AKDus = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency AKDu in AKDus){%>
            <dd <%if (AKDu_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=AKDu.id%>,'<%=AKDu.Code%>',<%=AKDu.ExchangeRate%>,'<%=AKDu.Msige%>','<%=AKDu.DecimalLength%>');"><%=AKDu.Code%></a></dd>
            <%AKDu_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zSLK_index=1;
List<Lebi_Searchkey> zSLKs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zSLK in zSLKs){%><%=Lang(zSLK.Name)%><%zSLK_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int crzu_index=1;
List<Lebi_Searchkey> crzus = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey crzu in crzus){%>
<%if (crzu.Type==1){ %><a href="<%=URL("P_Search",""+Lang(crzu.Name)+"") %>"><%}else{ %><a href="<%=crzu.URL%>" target="_blank"><%} %><span><%=Lang(crzu.Name)%></span></a>
<%crzu_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int ETeE_index=1;
List<Lebi_Page> ETeEs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page ETeE in ETeEs){%>
<a class="menu" href="<%=URL("","",ETeE.url)%>"><span><%=ETeE.Name%></span></a>
<%ETeE_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int jBwF_index=1;
List<Lebi_FriendLink> jBwFs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink jBwF in jBwFs){%>
            
                <% if (jBwF.Logo != "" && jBwF.IsPic == 1){ %><a href="<%=jBwF.Url%>" target="_blank"><img src="<%=Image(jBwF.Logo) %>" alt="<%=jBwF.Name%>" /></a><%}else{%><a href="<%=jBwF.Url%>" target="_blank"><%=jBwF.Name%></a><%} %>     
