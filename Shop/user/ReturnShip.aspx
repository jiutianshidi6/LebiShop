...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserReturnShip"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> jPXMs=Languages();RecordCount=jPXMs.Count;int jPXM_index=1;
foreach (Shop.Model.Lebi_Language jPXM in jPXMs){%>
            <dd <%if (jPXM_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=jPXM.id%>,'<%=jPXM.Code%>','<%=jPXM.Path%>');"><%if (jPXM.ImageUrl!=""){%><img src="<%=Image(jPXM.ImageUrl) %>" /><%}%><%=jPXM.Name%></a></dd>
            <%jPXM_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int GQOg_index=1;
List<Lebi_Searchkey> GQOgs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey GQOg in GQOgs){%><%=Lang(GQOg.Name)%><%GQOg_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int qhxg_index=1;
List<Lebi_Searchkey> qhxgs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey qhxg in qhxgs){%>
<%if (qhxg.Type==1){ %><a href="<%=URL("P_Search",""+Lang(qhxg.Name)+"") %>"><%}else{ %><a href="<%=qhxg.URL%>" target="_blank"><%} %><span><%=Lang(qhxg.Name)%></span></a>
<%qhxg_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int aiQT_index=1;
List<Lebi_Brand> aiQTs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand aiQT in aiQTs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",aiQT.id)%>"><img alt="<%=Lang(aiQT.Name) %>" src="<%=Image(aiQT.ImageUrl) %>"><span><%=Lang(aiQT.Name) %></span></a></li>
                                        <%aiQT_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int ghPz_index=1;
List<Lebi_Page> ghPzs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page ghPz in ghPzs){%>
<a class="menu" href="<%=URL("","",ghPz.url)%>"><span><%=ghPz.Name%></span></a>
<%ghPz_index++;}%>
