...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserMessage"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> tSNYs=Languages();RecordCount=tSNYs.Count;int tSNY_index=1;
foreach (Shop.Model.Lebi_Language tSNY in tSNYs){%>
            <dd <%if (tSNY_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=tSNY.id%>,'<%=tSNY.Code%>','<%=tSNY.Path%>');"><%if (tSNY.ImageUrl!=""){%><img src="<%=Image(tSNY.ImageUrl) %>" /><%}%><%=tSNY.Name%></a></dd>
            <%tSNY_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int TjIl_index=1;
List<Lebi_Searchkey> TjIls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey TjIl in TjIls){%><%=Lang(TjIl.Name)%><%TjIl_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int lCyj_index=1;
List<Lebi_Searchkey> lCyjs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey lCyj in lCyjs){%>
<%if (lCyj.Type==1){ %><a href="<%=URL("P_Search",""+Lang(lCyj.Name)+"") %>"><%}else{ %><a href="<%=lCyj.URL%>" target="_blank"><%} %><span><%=Lang(lCyj.Name)%></span></a>
<%lCyj_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int Nhbb_index=1;
List<Lebi_Brand> Nhbbs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand Nhbb in Nhbbs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",Nhbb.id)%>"><img alt="<%=Lang(Nhbb.Name) %>" src="<%=Image(Nhbb.ImageUrl) %>"><span><%=Lang(Nhbb.Name) %></span></a></li>
                                        <%Nhbb_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Imzv_index=1;
List<Lebi_Page> Imzvs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Imzv in Imzvs){%>
<a class="menu" href="<%=URL("","",Imzv.url)%>"><span><%=Imzv.Name%></span></a>
<%Imzv_index++;}%>
