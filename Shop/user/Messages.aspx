...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserMessage"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> GXyHs=Languages();RecordCount=GXyHs.Count;int GXyH_index=1;
foreach (Shop.Model.Lebi_Language GXyH in GXyHs){%>
            <dd <%if (GXyH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=GXyH.id%>,'<%=GXyH.Code%>','<%=GXyH.Path%>');"><%if (GXyH.ImageUrl!=""){%><img src="<%=Image(GXyH.ImageUrl) %>" /><%}%><%=GXyH.Name%></a></dd>
            <%GXyH_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int GXpt_index=1;
List<Lebi_Currency> GXpts = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency GXpt in GXpts){%>
            <dd <%if (GXpt_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=GXpt.id%>,'<%=GXpt.Code%>',<%=GXpt.ExchangeRate%>,'<%=GXpt.Msige%>','<%=GXpt.DecimalLength%>');"><%=GXpt.Code%></a></dd>
            <%GXpt_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int PKvs_index=1;
List<Lebi_Searchkey> PKvss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey PKvs in PKvss){%><%=Lang(PKvs.Name)%><%PKvs_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zUJc_index=1;
List<Lebi_Searchkey> zUJcs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zUJc in zUJcs){%>
<%if (zUJc.Type==1){ %><a href="<%=URL("P_Search",""+Lang(zUJc.Name)+"") %>"><%}else{ %><a href="<%=zUJc.URL%>" target="_blank"><%} %><span><%=Lang(zUJc.Name)%></span></a>
<%zUJc_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int GpiC_index=1;
List<Lebi_Page> GpiCs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page GpiC in GpiCs){%>
<a class="menu" href="<%=URL("","",GpiC.url)%>"><span><%=GpiC.Name%></span></a>
<%GpiC_index++;}%>
...
<div class="user">
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        <tr>
            <th style="width:30px;" class="selectAll"><a href="javascript:void(0);" onclick="$('input[name=\'id\']').attr('checked',!$(this).attr('checked'));$(this).attr('checked',!$(this).attr('checked'));">全选</a></th>
        ...
        <tr class="list">
            ...
            <td style="text-align:center"><%if (model.IsRead == 0){%><img src="/Theme/system/images/icon/email.png" title="未读" /><%}else{ %><img src="/Theme/system/images/icon/email_open.png" title="已读" /><%} %></td>
            <td><%=FormatTime(model.Time_Add) %></td>
            <td><a href="<%=URL("P_UserMessageDetails",model.id)%>"><%=model.Title %></a></td>
        </tr>
        <%} %>
    </table>
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>删除</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript">
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int SMEv_index=1;
List<Lebi_FriendLink> SMEvs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink SMEv in SMEvs){%>
            
                <% if (SMEv.Logo != "" && SMEv.IsPic == 1){ %><a href="<%=SMEv.Url%>" target="_blank"><img src="<%=Image(SMEv.Logo) %>" alt="<%=SMEv.Name%>" /></a><%}else{%><a href="<%=SMEv.Url%>" target="_blank"><%=SMEv.Name%></a><%} %>     
