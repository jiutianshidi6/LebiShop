...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserMessageWrite"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> Jisls=Languages();RecordCount=Jisls.Count;int Jisl_index=1;
foreach (Shop.Model.Lebi_Language Jisl in Jisls){%>
            <dd <%if (Jisl_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Jisl.id%>,'<%=Jisl.Code%>','<%=Jisl.Path%>');"><%if (Jisl.ImageUrl!=""){%><img src="<%=Image(Jisl.ImageUrl) %>" /><%}%><%=Jisl.Name%></a></dd>
            <%Jisl_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int FuPd_index=1;
List<Lebi_Currency> FuPds = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency FuPd in FuPds){%>
            <dd <%if (FuPd_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=FuPd.id%>,'<%=FuPd.Code%>',<%=FuPd.ExchangeRate%>,'<%=FuPd.Msige%>','<%=FuPd.DecimalLength%>');"><%=FuPd.Code%></a></dd>
            <%FuPd_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int djPu_index=1;
List<Lebi_Searchkey> djPus = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey djPu in djPus){%><%=Lang(djPu.Name)%><%djPu_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int VILN_index=1;
List<Lebi_Searchkey> VILNs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey VILN in VILNs){%>
<%if (VILN.Type==1){ %><a href="<%=URL("P_Search",""+Lang(VILN.Name)+"") %>"><%}else{ %><a href="<%=VILN.URL%>" target="_blank"><%} %><span><%=Lang(VILN.Name)%></span></a>
<%VILN_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Vozm_index=1;
List<Lebi_Page> Vozms = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Vozm in Vozms){%>
<a class="menu" href="<%=URL("","",Vozm.url)%>"><span><%=Vozm.Name%></span></a>
<%Vozm_index++;}%>
...
<div class="user">
    <ul class="tablist">
        <li><a href="<%=URL("P_UserMessage","0","")%>"><span>收件箱<em>(<%=Count_Message(0) %>)</em></span></a></li>
        <li><a href="<%=URL("P_UserMessage","1","")%>"><span>发件箱</span></a></li>
        <li class="selected"><a href="<%=URL("P_UserMessageWrite","","")%>"><span>发信息</span></a></li>
    </ul>
    ...
    <dl class="dl-btn">
        <dt></dt>
        <dd><a href="javascript:void(0)" onclick="submit();" class="btn btn-11"><s></s>发送</a></dd>
    </dl>
    </div>
    </div>
</div>
...
<script type="text/javascript">
    function submit() {
        ...
        RequestAjax(url, postData, function () { MsgBox(1, "发送成功", "<%=URL("P_UserMessage","1","")%>") });
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int oWKl_index=1;
List<Lebi_FriendLink> oWKls = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink oWKl in oWKls){%>
            
                <% if (oWKl.Logo != "" && oWKl.IsPic == 1){ %><a href="<%=oWKl.Url%>" target="_blank"><img src="<%=Image(oWKl.Logo) %>" alt="<%=oWKl.Name%>" /></a><%}else{%><a href="<%=oWKl.Url%>" target="_blank"><%=oWKl.Name%></a><%} %>     
