...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserMessageWrite"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> PlmLs=Languages();RecordCount=PlmLs.Count;int PlmL_index=1;
foreach (Shop.Model.Lebi_Language PlmL in PlmLs){%>
            <dd <%if (PlmL_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=PlmL.id%>,'<%=PlmL.Code%>','<%=PlmL.Path%>');"><%if (PlmL.ImageUrl!=""){%><img src="<%=Image(PlmL.ImageUrl) %>" /><%}%><%=PlmL.Name%></a></dd>
            <%PlmL_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int aYtf_index=1;
List<Lebi_Searchkey> aYtfs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey aYtf in aYtfs){%><%=Lang(aYtf.Name)%><%aYtf_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int lKFN_index=1;
List<Lebi_Searchkey> lKFNs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey lKFN in lKFNs){%>
<%if (lKFN.Type==1){ %><a href="<%=URL("P_Search",""+Lang(lKFN.Name)+"") %>"><%}else{ %><a href="<%=lKFN.URL%>" target="_blank"><%} %><span><%=Lang(lKFN.Name)%></span></a>
<%lKFN_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int SSzP_index=1;
List<Lebi_Brand> SSzPs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand SSzP in SSzPs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",SSzP.id)%>"><img alt="<%=Lang(SSzP.Name) %>" src="<%=Image(SSzP.ImageUrl) %>"><span><%=Lang(SSzP.Name) %></span></a></li>
                                        <%SSzP_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int FPDx_index=1;
List<Lebi_Page> FPDxs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page FPDx in FPDxs){%>
<a class="menu" href="<%=URL("","",FPDx.url)%>"><span><%=FPDx.Name%></span></a>
<%FPDx_index++;}%>
