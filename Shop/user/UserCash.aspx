...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserCash"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> fjXfs=Languages();RecordCount=fjXfs.Count;int fjXf_index=1;
foreach (Shop.Model.Lebi_Language fjXf in fjXfs){%>
            <dd <%if (fjXf_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=fjXf.id%>,'<%=fjXf.Code%>','<%=fjXf.Path%>');"><%if (fjXf.ImageUrl!=""){%><img src="<%=Image(fjXf.ImageUrl) %>" /><%}%><%=fjXf.Name%></a></dd>
            <%fjXf_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int pmGS_index=1;
List<Lebi_Currency> pmGSs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency pmGS in pmGSs){%>
            <dd <%if (pmGS_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=pmGS.id%>,'<%=pmGS.Code%>',<%=pmGS.ExchangeRate%>,'<%=pmGS.Msige%>','<%=pmGS.DecimalLength%>');"><%=pmGS.Code%></a></dd>
            <%pmGS_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int YSqX_index=1;
List<Lebi_Searchkey> YSqXs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey YSqX in YSqXs){%><%=Lang(YSqX.Name)%><%YSqX_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ERxn_index=1;
List<Lebi_Searchkey> ERxns = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ERxn in ERxns){%>
<%if (ERxn.Type==1){ %><a href="<%=URL("P_Search",""+Lang(ERxn.Name)+"") %>"><%}else{ %><a href="<%=ERxn.URL%>" target="_blank"><%} %><span><%=Lang(ERxn.Name)%></span></a>
<%ERxn_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int WAWN_index=1;
List<Lebi_Page> WAWNs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page WAWN in WAWNs){%>
<a class="menu" href="<%=URL("","",WAWN.url)%>"><span><%=WAWN.Name%></span></a>
<%WAWN_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int tfBQ_index=1;
List<Lebi_FriendLink> tfBQs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink tfBQ in tfBQs){%>
            
                <% if (tfBQ.Logo != "" && tfBQ.IsPic == 1){ %><a href="<%=tfBQ.Url%>" target="_blank"><img src="<%=Image(tfBQ.Logo) %>" alt="<%=tfBQ.Name%>" /></a><%}else{%><a href="<%=tfBQ.Url%>" target="_blank"><%=tfBQ.Name%></a><%} %>     
