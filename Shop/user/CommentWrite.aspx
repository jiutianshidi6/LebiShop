...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserCommentWrite"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> Fvlfs=Languages();RecordCount=Fvlfs.Count;int Fvlf_index=1;
foreach (Shop.Model.Lebi_Language Fvlf in Fvlfs){%>
            <dd <%if (Fvlf_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Fvlf.id%>,'<%=Fvlf.Code%>','<%=Fvlf.Path%>');"><%if (Fvlf.ImageUrl!=""){%><img src="<%=Image(Fvlf.ImageUrl) %>" /><%}%><%=Fvlf.Name%></a></dd>
            <%Fvlf_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int wxgk_index=1;
List<Lebi_Currency> wxgks = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency wxgk in wxgks){%>
            <dd <%if (wxgk_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=wxgk.id%>,'<%=wxgk.Code%>',<%=wxgk.ExchangeRate%>,'<%=wxgk.Msige%>','<%=wxgk.DecimalLength%>');"><%=wxgk.Code%></a></dd>
            <%wxgk_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int wzNL_index=1;
List<Lebi_Searchkey> wzNLs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey wzNL in wzNLs){%><%=Lang(wzNL.Name)%><%wzNL_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int YILS_index=1;
List<Lebi_Searchkey> YILSs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey YILS in YILSs){%>
<%if (YILS.Type==1){ %><a href="<%=URL("P_Search",""+Lang(YILS.Name)+"") %>"><%}else{ %><a href="<%=YILS.URL%>" target="_blank"><%} %><span><%=Lang(YILS.Name)%></span></a>
<%YILS_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int SNsA_index=1;
List<Lebi_Page> SNsAs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page SNsA in SNsAs){%>
<a class="menu" href="<%=URL("","",SNsA.url)%>"><span><%=SNsA.Name%></span></a>
<%SNsA_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int TfUY_index=1;
List<Lebi_FriendLink> TfUYs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink TfUY in TfUYs){%>
            
                <% if (TfUY.Logo != "" && TfUY.IsPic == 1){ %><a href="<%=TfUY.Url%>" target="_blank"><img src="<%=Image(TfUY.Logo) %>" alt="<%=TfUY.Name%>" /></a><%}else{%><a href="<%=TfUY.Url%>" target="_blank"><%=TfUY.Name%></a><%} %>     
