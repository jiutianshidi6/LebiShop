...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserCommentWrite"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> iRIjs=Languages();RecordCount=iRIjs.Count;int iRIj_index=1;
foreach (Shop.Model.Lebi_Language iRIj in iRIjs){%>
            <dd <%if (iRIj_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=iRIj.id%>,'<%=iRIj.Code%>','<%=iRIj.Path%>');"><%if (iRIj.ImageUrl!=""){%><img src="<%=Image(iRIj.ImageUrl) %>" /><%}%><%=iRIj.Name%></a></dd>
            <%iRIj_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int gNDs_index=1;
List<Lebi_Searchkey> gNDss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey gNDs in gNDss){%><%=Lang(gNDs.Name)%><%gNDs_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Pzom_index=1;
List<Lebi_Searchkey> Pzoms = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Pzom in Pzoms){%>
<%if (Pzom.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Pzom.Name)+"") %>"><%}else{ %><a href="<%=Pzom.URL%>" target="_blank"><%} %><span><%=Lang(Pzom.Name)%></span></a>
<%Pzom_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int YMmH_index=1;
List<Lebi_Brand> YMmHs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand YMmH in YMmHs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",YMmH.id)%>"><img alt="<%=Lang(YMmH.Name) %>" src="<%=Image(YMmH.ImageUrl) %>"><span><%=Lang(YMmH.Name) %></span></a></li>
                                        <%YMmH_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int IMRn_index=1;
List<Lebi_Page> IMRns = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page IMRn in IMRns){%>
<a class="menu" href="<%=URL("","",IMRn.url)%>"><span><%=IMRn.Name%></span></a>
<%IMRn_index++;}%>
