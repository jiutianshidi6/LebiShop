...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserAddress"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> XarHs=Languages();RecordCount=XarHs.Count;int XarH_index=1;
foreach (Shop.Model.Lebi_Language XarH in XarHs){%>
            <dd <%if (XarH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=XarH.id%>,'<%=XarH.Code%>','<%=XarH.Path%>');"><%if (XarH.ImageUrl!=""){%><img src="<%=Image(XarH.ImageUrl) %>" /><%}%><%=XarH.Name%></a></dd>
            <%XarH_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int nzRP_index=1;
List<Lebi_Searchkey> nzRPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey nzRP in nzRPs){%><%=Lang(nzRP.Name)%><%nzRP_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int fEab_index=1;
List<Lebi_Searchkey> fEabs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey fEab in fEabs){%>
<%if (fEab.Type==1){ %><a href="<%=URL("P_Search",""+Lang(fEab.Name)+"") %>"><%}else{ %><a href="<%=fEab.URL%>" target="_blank"><%} %><span><%=Lang(fEab.Name)%></span></a>
<%fEab_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int JeXe_index=1;
List<Lebi_Brand> JeXes = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand JeXe in JeXes){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",JeXe.id)%>"><img alt="<%=Lang(JeXe.Name) %>" src="<%=Image(JeXe.ImageUrl) %>"><span><%=Lang(JeXe.Name) %></span></a></li>
                                        <%JeXe_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int ziWL_index=1;
List<Lebi_Page> ziWLs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page ziWL in ziWLs){%>
<a class="menu" href="<%=URL("","",ziWL.url)%>"><span><%=ziWL.Name%></span></a>
<%ziWL_index++;}%>
