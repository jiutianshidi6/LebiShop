...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserAddress"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> MKqPs=Languages();RecordCount=MKqPs.Count;int MKqP_index=1;
foreach (Shop.Model.Lebi_Language MKqP in MKqPs){%>
            <dd <%if (MKqP_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=MKqP.id%>,'<%=MKqP.Code%>','<%=MKqP.Path%>');"><%if (MKqP.ImageUrl!=""){%><img src="<%=Image(MKqP.ImageUrl) %>" /><%}%><%=MKqP.Name%></a></dd>
            <%MKqP_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Iopq_index=1;
List<Lebi_Currency> Iopqs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Iopq in Iopqs){%>
            <dd <%if (Iopq_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Iopq.id%>,'<%=Iopq.Code%>',<%=Iopq.ExchangeRate%>,'<%=Iopq.Msige%>','<%=Iopq.DecimalLength%>');"><%=Iopq.Code%></a></dd>
            <%Iopq_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int JYpJ_index=1;
List<Lebi_Searchkey> JYpJs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey JYpJ in JYpJs){%><%=Lang(JYpJ.Name)%><%JYpJ_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int aoOc_index=1;
List<Lebi_Searchkey> aoOcs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey aoOc in aoOcs){%>
<%if (aoOc.Type==1){ %><a href="<%=URL("P_Search",""+Lang(aoOc.Name)+"") %>"><%}else{ %><a href="<%=aoOc.URL%>" target="_blank"><%} %><span><%=Lang(aoOc.Name)%></span></a>
<%aoOc_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int DveB_index=1;
List<Lebi_Page> DveBs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page DveB in DveBs){%>
<a class="menu" href="<%=URL("","",DveB.url)%>"><span><%=DveB.Name%></span></a>
<%DveB_index++;}%>
    ...
    <table cellpadding="0" cellspacing="0" width="100%" class="tablelist box">
        <tr>
            <th style="width:30px;" class="selectAll">
                <a href="javascript:void(0);" onclick="$('input[name=\'id\']').attr('checked',!$(this).attr('checked'));$(this).attr('checked',!$(this).attr('checked'));">全选</a>
            </th>
        ...
        <tr>
            ...
            <td><%if (CurrentUser.User_Address_id == ua.id){ %>默认<%}else{ %>&nbsp;<%} %></td>
            <td><a href="<%=URL("P_UserAddressEdit",ua.id,"")%>">编辑</a>
            </td>
        </tr>
        <%} %>
    </table>
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>删除</a><a href="<%=URL("P_UserAddressEdit","0","")%>" class="btn btn-7"><s></s>添加</a></div><div class="right"><%=PageString%></div></div>
</div>
</div>
<script type="text/javascript">
    function search_() {
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int lcli_index=1;
List<Lebi_FriendLink> lclis = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink lcli in lclis){%>
            
                <% if (lcli.Logo != "" && lcli.IsPic == 1){ %><a href="<%=lcli.Url%>" target="_blank"><img src="<%=Image(lcli.Logo) %>" alt="<%=lcli.Name%>" /></a><%}else{%><a href="<%=lcli.Url%>" target="_blank"><%=lcli.Name%></a><%} %>     
