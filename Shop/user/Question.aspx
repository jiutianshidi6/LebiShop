...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserQuestion"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> FCdzs=Languages();RecordCount=FCdzs.Count;int FCdz_index=1;
foreach (Shop.Model.Lebi_Language FCdz in FCdzs){%>
            <dd <%if (FCdz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=FCdz.id%>,'<%=FCdz.Code%>','<%=FCdz.Path%>');"><%if (FCdz.ImageUrl!=""){%><img src="<%=Image(FCdz.ImageUrl) %>" /><%}%><%=FCdz.Name%></a></dd>
            <%FCdz_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Eskd_index=1;
List<Lebi_Currency> Eskds = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Eskd in Eskds){%>
            <dd <%if (Eskd_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Eskd.id%>,'<%=Eskd.Code%>',<%=Eskd.ExchangeRate%>,'<%=Eskd.Msige%>','<%=Eskd.DecimalLength%>');"><%=Eskd.Code%></a></dd>
            <%Eskd_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int mceQ_index=1;
List<Lebi_Searchkey> mceQs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey mceQ in mceQs){%><%=Lang(mceQ.Name)%><%mceQ_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int lTyN_index=1;
List<Lebi_Searchkey> lTyNs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey lTyN in lTyNs){%>
<%if (lTyN.Type==1){ %><a href="<%=URL("P_Search",""+Lang(lTyN.Name)+"") %>"><%}else{ %><a href="<%=lTyN.URL%>" target="_blank"><%} %><span><%=Lang(lTyN.Name)%></span></a>
<%lTyN_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int dScp_index=1;
List<Lebi_Page> dScps = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page dScp in dScps){%>
<a class="menu" href="<%=URL("","",dScp.url)%>"><span><%=dScp.Name%></span></a>
<%dScp_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int aOwN_index=1;
List<Lebi_FriendLink> aOwNs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink aOwN in aOwNs){%>
            
                <% if (aOwN.Logo != "" && aOwN.IsPic == 1){ %><a href="<%=aOwN.Url%>" target="_blank"><img src="<%=Image(aOwN.Logo) %>" alt="<%=aOwN.Name%>" /></a><%}else{%><a href="<%=aOwN.Url%>" target="_blank"><%=aOwN.Name%></a><%} %>     
