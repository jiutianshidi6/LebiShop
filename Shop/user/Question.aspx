...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserQuestion"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> ULxxs=Languages();RecordCount=ULxxs.Count;int ULxx_index=1;
foreach (Shop.Model.Lebi_Language ULxx in ULxxs){%>
            <dd <%if (ULxx_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=ULxx.id%>,'<%=ULxx.Code%>','<%=ULxx.Path%>');"><%if (ULxx.ImageUrl!=""){%><img src="<%=Image(ULxx.ImageUrl) %>" /><%}%><%=ULxx.Name%></a></dd>
            <%ULxx_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hFyg_index=1;
List<Lebi_Searchkey> hFygs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hFyg in hFygs){%><%=Lang(hFyg.Name)%><%hFyg_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int dEDG_index=1;
List<Lebi_Searchkey> dEDGs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey dEDG in dEDGs){%>
<%if (dEDG.Type==1){ %><a href="<%=URL("P_Search",""+Lang(dEDG.Name)+"") %>"><%}else{ %><a href="<%=dEDG.URL%>" target="_blank"><%} %><span><%=Lang(dEDG.Name)%></span></a>
<%dEDG_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int KiWz_index=1;
List<Lebi_Brand> KiWzs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand KiWz in KiWzs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",KiWz.id)%>"><img alt="<%=Lang(KiWz.Name) %>" src="<%=Image(KiWz.ImageUrl) %>"><span><%=Lang(KiWz.Name) %></span></a></li>
                                        <%KiWz_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int CDac_index=1;
List<Lebi_Page> CDacs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page CDac in CDacs){%>
<a class="menu" href="<%=URL("","",CDac.url)%>"><span><%=CDac.Name%></span></a>
<%CDac_index++;}%>
