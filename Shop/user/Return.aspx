...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserReturn"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> EFPms=Languages();RecordCount=EFPms.Count;int EFPm_index=1;
foreach (Shop.Model.Lebi_Language EFPm in EFPms){%>
            <dd <%if (EFPm_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=EFPm.id%>,'<%=EFPm.Code%>','<%=EFPm.Path%>');"><%if (EFPm.ImageUrl!=""){%><img src="<%=Image(EFPm.ImageUrl) %>" /><%}%><%=EFPm.Name%></a></dd>
            <%EFPm_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int AMRu_index=1;
List<Lebi_Currency> AMRus = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency AMRu in AMRus){%>
            <dd <%if (AMRu_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=AMRu.id%>,'<%=AMRu.Code%>',<%=AMRu.ExchangeRate%>,'<%=AMRu.Msige%>','<%=AMRu.DecimalLength%>');"><%=AMRu.Code%></a></dd>
            <%AMRu_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int DQao_index=1;
List<Lebi_Searchkey> DQaos = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey DQao in DQaos){%><%=Lang(DQao.Name)%><%DQao_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int PiFW_index=1;
List<Lebi_Searchkey> PiFWs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey PiFW in PiFWs){%>
<%if (PiFW.Type==1){ %><a href="<%=URL("P_Search",""+Lang(PiFW.Name)+"") %>"><%}else{ %><a href="<%=PiFW.URL%>" target="_blank"><%} %><span><%=Lang(PiFW.Name)%></span></a>
<%PiFW_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int SaeT_index=1;
List<Lebi_Page> SaeTs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page SaeT in SaeTs){%>
<a class="menu" href="<%=URL("","",SaeT.url)%>"><span><%=SaeT.Name%></span></a>
<%SaeT_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int EpIn_index=1;
List<Lebi_FriendLink> EpIns = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink EpIn in EpIns){%>
            
                <% if (EpIn.Logo != "" && EpIn.IsPic == 1){ %><a href="<%=EpIn.Url%>" target="_blank"><img src="<%=Image(EpIn.Logo) %>" alt="<%=EpIn.Name%>" /></a><%}else{%><a href="<%=EpIn.Url%>" target="_blank"><%=EpIn.Name%></a><%} %>     
