...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserReturn"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> CCHls=Languages();RecordCount=CCHls.Count;int CCHl_index=1;
foreach (Shop.Model.Lebi_Language CCHl in CCHls){%>
            <dd <%if (CCHl_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=CCHl.id%>,'<%=CCHl.Code%>','<%=CCHl.Path%>');"><%if (CCHl.ImageUrl!=""){%><img src="<%=Image(CCHl.ImageUrl) %>" /><%}%><%=CCHl.Name%></a></dd>
            <%CCHl_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int UhEw_index=1;
List<Lebi_Searchkey> UhEws = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey UhEw in UhEws){%><%=Lang(UhEw.Name)%><%UhEw_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Pvdx_index=1;
List<Lebi_Searchkey> Pvdxs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Pvdx in Pvdxs){%>
<%if (Pvdx.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Pvdx.Name)+"") %>"><%}else{ %><a href="<%=Pvdx.URL%>" target="_blank"><%} %><span><%=Lang(Pvdx.Name)%></span></a>
<%Pvdx_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int ugzu_index=1;
List<Lebi_Brand> ugzus = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand ugzu in ugzus){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",ugzu.id)%>"><img alt="<%=Lang(ugzu.Name) %>" src="<%=Image(ugzu.ImageUrl) %>"><span><%=Lang(ugzu.Name) %></span></a></li>
                                        <%ugzu_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int HzeM_index=1;
List<Lebi_Page> HzeMs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page HzeM in HzeMs){%>
<a class="menu" href="<%=URL("","",HzeM.url)%>"><span><%=HzeM.Name%></span></a>
<%HzeM_index++;}%>
