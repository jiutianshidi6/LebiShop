...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserCardGet"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> AYPbs=Languages();RecordCount=AYPbs.Count;int AYPb_index=1;
foreach (Shop.Model.Lebi_Language AYPb in AYPbs){%>
            <dd <%if (AYPb_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=AYPb.id%>,'<%=AYPb.Code%>','<%=AYPb.Path%>');"><%if (AYPb.ImageUrl!=""){%><img src="<%=Image(AYPb.ImageUrl) %>" /><%}%><%=AYPb.Name%></a></dd>
            <%AYPb_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int CGks_index=1;
List<Lebi_Searchkey> CGkss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey CGks in CGkss){%><%=Lang(CGks.Name)%><%CGks_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hxOb_index=1;
List<Lebi_Searchkey> hxObs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hxOb in hxObs){%>
<%if (hxOb.Type==1){ %><a href="<%=URL("P_Search",""+Lang(hxOb.Name)+"") %>"><%}else{ %><a href="<%=hxOb.URL%>" target="_blank"><%} %><span><%=Lang(hxOb.Name)%></span></a>
<%hxOb_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int Yile_index=1;
List<Lebi_Brand> Yiles = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand Yile in Yiles){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",Yile.id)%>"><img alt="<%=Lang(Yile.Name) %>" src="<%=Image(Yile.ImageUrl) %>"><span><%=Lang(Yile.Name) %></span></a></li>
                                        <%Yile_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int tSCO_index=1;
List<Lebi_Page> tSCOs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page tSCO in tSCOs){%>
<a class="menu" href="<%=URL("","",tSCO.url)%>"><span><%=tSCO.Name%></span></a>
<%tSCO_index++;}%>
