...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserCardGet"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> DWcxs=Languages();RecordCount=DWcxs.Count;int DWcx_index=1;
foreach (Shop.Model.Lebi_Language DWcx in DWcxs){%>
            <dd <%if (DWcx_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=DWcx.id%>,'<%=DWcx.Code%>','<%=DWcx.Path%>');"><%if (DWcx.ImageUrl!=""){%><img src="<%=Image(DWcx.ImageUrl) %>" /><%}%><%=DWcx.Name%></a></dd>
            <%DWcx_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int XyOB_index=1;
List<Lebi_Currency> XyOBs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency XyOB in XyOBs){%>
            <dd <%if (XyOB_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=XyOB.id%>,'<%=XyOB.Code%>',<%=XyOB.ExchangeRate%>,'<%=XyOB.Msige%>','<%=XyOB.DecimalLength%>');"><%=XyOB.Code%></a></dd>
            <%XyOB_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int qGWI_index=1;
List<Lebi_Searchkey> qGWIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey qGWI in qGWIs){%><%=Lang(qGWI.Name)%><%qGWI_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int dcBc_index=1;
List<Lebi_Searchkey> dcBcs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey dcBc in dcBcs){%>
<%if (dcBc.Type==1){ %><a href="<%=URL("P_Search",""+Lang(dcBc.Name)+"") %>"><%}else{ %><a href="<%=dcBc.URL%>" target="_blank"><%} %><span><%=Lang(dcBc.Name)%></span></a>
<%dcBc_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int yGnw_index=1;
List<Lebi_Page> yGnws = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page yGnw in yGnws){%>
<a class="menu" href="<%=URL("","",yGnw.url)%>"><span><%=yGnw.Name%></span></a>
<%yGnw_index++;}%>
...
class="input" /> <img class="img_c" align="absmiddle" style="width: 80px; height: 25px;" title="点击重新获取验证码" id="img1" src="<%=WebPath%>/code.aspx" language="javascript" onclick="this.src=this.src+'?'"></dd>
        </dl>
        <dl class="dl-btn">
            <dt></dt>
            <dd><a href="javascript:void(0)" onclick="submit();" class="btn btn-11"><s></s>领取</a></dd>
        </dl>
        </div>
    </div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int akup_index=1;
List<Lebi_FriendLink> akups = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink akup in akups){%>
            
                <% if (akup.Logo != "" && akup.IsPic == 1){ %><a href="<%=akup.Url%>" target="_blank"><img src="<%=Image(akup.Logo) %>" alt="<%=akup.Name%>" /></a><%}else{%><a href="<%=akup.Url%>" target="_blank"><%=akup.Name%></a><%} %>     
