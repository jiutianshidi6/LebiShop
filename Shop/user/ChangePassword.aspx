...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserChangePassword"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> FqpHs=Languages();RecordCount=FqpHs.Count;int FqpH_index=1;
foreach (Shop.Model.Lebi_Language FqpH in FqpHs){%>
            <dd <%if (FqpH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=FqpH.id%>,'<%=FqpH.Code%>','<%=FqpH.Path%>');"><%if (FqpH.ImageUrl!=""){%><img src="<%=Image(FqpH.ImageUrl) %>" /><%}%><%=FqpH.Name%></a></dd>
            <%FqpH_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int DTLu_index=1;
List<Lebi_Searchkey> DTLus = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey DTLu in DTLus){%><%=Lang(DTLu.Name)%><%DTLu_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int EjdJ_index=1;
List<Lebi_Searchkey> EjdJs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey EjdJ in EjdJs){%>
<%if (EjdJ.Type==1){ %><a href="<%=URL("P_Search",""+Lang(EjdJ.Name)+"") %>"><%}else{ %><a href="<%=EjdJ.URL%>" target="_blank"><%} %><span><%=Lang(EjdJ.Name)%></span></a>
<%EjdJ_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int tUAx_index=1;
List<Lebi_Brand> tUAxs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand tUAx in tUAxs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",tUAx.id)%>"><img alt="<%=Lang(tUAx.Name) %>" src="<%=Image(tUAx.ImageUrl) %>"><span><%=Lang(tUAx.Name) %></span></a></li>
                                        <%tUAx_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int pdwa_index=1;
List<Lebi_Page> pdwas = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page pdwa in pdwas){%>
<a class="menu" href="<%=URL("","",pdwa.url)%>"><span><%=pdwa.Name%></span></a>
<%pdwa_index++;}%>
