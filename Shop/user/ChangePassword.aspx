...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserChangePassword"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> DMpns=Languages();RecordCount=DMpns.Count;int DMpn_index=1;
foreach (Shop.Model.Lebi_Language DMpn in DMpns){%>
            <dd <%if (DMpn_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=DMpn.id%>,'<%=DMpn.Code%>','<%=DMpn.Path%>');"><%if (DMpn.ImageUrl!=""){%><img src="<%=Image(DMpn.ImageUrl) %>" /><%}%><%=DMpn.Name%></a></dd>
            <%DMpn_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int duoe_index=1;
List<Lebi_Currency> duoes = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency duoe in duoes){%>
            <dd <%if (duoe_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=duoe.id%>,'<%=duoe.Code%>',<%=duoe.ExchangeRate%>,'<%=duoe.Msige%>','<%=duoe.DecimalLength%>');"><%=duoe.Code%></a></dd>
            <%duoe_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zbyi_index=1;
List<Lebi_Searchkey> zbyis = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zbyi in zbyis){%><%=Lang(zbyi.Name)%><%zbyi_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int RXAP_index=1;
List<Lebi_Searchkey> RXAPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey RXAP in RXAPs){%>
<%if (RXAP.Type==1){ %><a href="<%=URL("P_Search",""+Lang(RXAP.Name)+"") %>"><%}else{ %><a href="<%=RXAP.URL%>" target="_blank"><%} %><span><%=Lang(RXAP.Name)%></span></a>
<%RXAP_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int YMBA_index=1;
List<Lebi_Page> YMBAs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page YMBA in YMBAs){%>
<a class="menu" href="<%=URL("","",YMBA.url)%>"><span><%=YMBA.Name%></span></a>
<%YMBA_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int RpRC_index=1;
List<Lebi_FriendLink> RpRCs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink RpRC in RpRCs){%>
            
                <% if (RpRC.Logo != "" && RpRC.IsPic == 1){ %><a href="<%=RpRC.Url%>" target="_blank"><img src="<%=Image(RpRC.Logo) %>" alt="<%=RpRC.Name%>" /></a><%}else{%><a href="<%=RpRC.Url%>" target="_blank"><%=RpRC.Name%></a><%} %>     
