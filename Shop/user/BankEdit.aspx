...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserBankEdit"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> bQAUs=Languages();RecordCount=bQAUs.Count;int bQAU_index=1;
foreach (Shop.Model.Lebi_Language bQAU in bQAUs){%>
            <dd <%if (bQAU_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=bQAU.id%>,'<%=bQAU.Code%>','<%=bQAU.Path%>');"><%if (bQAU.ImageUrl!=""){%><img src="<%=Image(bQAU.ImageUrl) %>" /><%}%><%=bQAU.Name%></a></dd>
            <%bQAU_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int dAyf_index=1;
List<Lebi_Currency> dAyfs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency dAyf in dAyfs){%>
            <dd <%if (dAyf_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=dAyf.id%>,'<%=dAyf.Code%>',<%=dAyf.ExchangeRate%>,'<%=dAyf.Msige%>','<%=dAyf.DecimalLength%>');"><%=dAyf.Code%></a></dd>
            <%dAyf_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int HFjv_index=1;
List<Lebi_Searchkey> HFjvs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey HFjv in HFjvs){%><%=Lang(HFjv.Name)%><%HFjv_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Jhkm_index=1;
List<Lebi_Searchkey> Jhkms = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Jhkm in Jhkms){%>
<%if (Jhkm.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Jhkm.Name)+"") %>"><%}else{ %><a href="<%=Jhkm.URL%>" target="_blank"><%} %><span><%=Lang(Jhkm.Name)%></span></a>
<%Jhkm_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int QeUP_index=1;
List<Lebi_Page> QeUPs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page QeUP in QeUPs){%>
<a class="menu" href="<%=URL("","",QeUP.url)%>"><span><%=QeUP.Name%></span></a>
<%QeUP_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int ubnR_index=1;
List<Lebi_FriendLink> ubnRs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink ubnR in ubnRs){%>
            
                <% if (ubnR.Logo != "" && ubnR.IsPic == 1){ %><a href="<%=ubnR.Url%>" target="_blank"><img src="<%=Image(ubnR.Logo) %>" alt="<%=ubnR.Name%>" /></a><%}else{%><a href="<%=ubnR.Url%>" target="_blank"><%=ubnR.Name%></a><%} %>     
