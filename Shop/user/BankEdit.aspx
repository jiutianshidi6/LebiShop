...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserBankEdit"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> MuKes=Languages();RecordCount=MuKes.Count;int MuKe_index=1;
foreach (Shop.Model.Lebi_Language MuKe in MuKes){%>
            <dd <%if (MuKe_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=MuKe.id%>,'<%=MuKe.Code%>','<%=MuKe.Path%>');"><%if (MuKe.ImageUrl!=""){%><img src="<%=Image(MuKe.ImageUrl) %>" /><%}%><%=MuKe.Name%></a></dd>
            <%MuKe_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int rNWR_index=1;
List<Lebi_Searchkey> rNWRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey rNWR in rNWRs){%><%=Lang(rNWR.Name)%><%rNWR_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int VhNT_index=1;
List<Lebi_Searchkey> VhNTs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey VhNT in VhNTs){%>
<%if (VhNT.Type==1){ %><a href="<%=URL("P_Search",""+Lang(VhNT.Name)+"") %>"><%}else{ %><a href="<%=VhNT.URL%>" target="_blank"><%} %><span><%=Lang(VhNT.Name)%></span></a>
<%VhNT_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int IJcA_index=1;
List<Lebi_Brand> IJcAs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand IJcA in IJcAs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",IJcA.id)%>"><img alt="<%=Lang(IJcA.Name) %>" src="<%=Image(IJcA.ImageUrl) %>"><span><%=Lang(IJcA.Name) %></span></a></li>
                                        <%IJcA_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int UcPK_index=1;
List<Lebi_Page> UcPKs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page UcPK in UcPKs){%>
<a class="menu" href="<%=URL("","",UcPK.url)%>"><span><%=UcPK.Name%></span></a>
<%UcPK_index++;}%>
