...
<% LoadPage("ljq@lebi.cn_49",1,"CN","P_UserAsk"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> XLxps=Languages();RecordCount=XLxps.Count;int XLxp_index=1;
foreach (Shop.Model.Lebi_Language XLxp in XLxps){%>
            <dd <%if (XLxp_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=XLxp.id%>,'<%=XLxp.Code%>','<%=XLxp.Path%>');"><%if (XLxp.ImageUrl!=""){%><img src="<%=Image(XLxp.ImageUrl) %>" /><%}%><%=XLxp.Name%></a></dd>
            <%XLxp_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ewpg_index=1;
List<Lebi_Searchkey> ewpgs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ewpg in ewpgs){%><%=Lang(ewpg.Name)%><%ewpg_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int BItw_index=1;
List<Lebi_Searchkey> BItws = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey BItw in BItws){%>
<%if (BItw.Type==1){ %><a href="<%=URL("P_Search",""+Lang(BItw.Name)+"") %>"><%}else{ %><a href="<%=BItw.URL%>" target="_blank"><%} %><span><%=Lang(BItw.Name)%></span></a>
<%BItw_index++;}%>
</div>
                        ...
                        <div class="menu-classify">
                            ...
                            <div class="menu-classify-part right">
                                <div class="menu-classify-mode">
                                    ...
                                    <ul class="brand-ul">
                                        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and ','+Pro_Type_id+',' like '%,"+c0.id+",%'";Order="Sort desc";PageSize=16;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int Osbq_index=1;
List<Lebi_Brand> Osbqs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand Osbq in Osbqs){%>
                                            <li class="brand-box first"><a target="_blank" href="<%=URL("P_Brand",Osbq.id)%>"><img alt="<%=Lang(Osbq.Name) %>" src="<%=Image(Osbq.ImageUrl) %>"><span><%=Lang(Osbq.Name) %></span></a></li>
                                        <%Osbq_index++;}%> 
                                    </ul>
                                </div>
                            </div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int aIgV_index=1;
List<Lebi_Page> aIgVs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page aIgV in aIgVs){%>
<a class="menu" href="<%=URL("","",aIgV.url)%>"><span><%=aIgV.Name%></span></a>
<%aIgV_index++;}%>
