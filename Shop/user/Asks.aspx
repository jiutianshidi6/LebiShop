...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_UserAsk"); %>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int oSUR_index=1;
List<Lebi_Currency> oSURs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency oSUR in oSURs){%>
            <dd <%if (oSUR_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=oSUR.id%>,'<%=oSUR.Code%>',<%=oSUR.ExchangeRate%>,'<%=oSUR.Msige%>','<%=oSUR.DecimalLength%>');"><%=oSUR.Code%></a></dd>
            <%oSUR_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int XFFc_index=1;
List<Lebi_Searchkey> XFFcs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey XFFc in XFFcs){%><%=Lang(XFFc.Name)%><%XFFc_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int WIPT_index=1;
List<Lebi_Searchkey> WIPTs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey WIPT in WIPTs){%>
<%if (WIPT.Type==1){ %><a href="<%=URL("P_Search",""+Lang(WIPT.Name)+"") %>"><%}else{ %><a href="<%=WIPT.URL%>" target="_blank"><%} %><span><%=Lang(WIPT.Name)%></span></a>
<%WIPT_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int qjdr_index=1;
List<Lebi_Page> qjdrs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page qjdr in qjdrs){%>
<a class="menu" href="<%=URL("","",qjdr.url)%>"><span><%=qjdr.Name%></span></a>
<%qjdr_index++;}%>
...
<div class="user">
    ...
    <tr><td colspan="5">内容：<%=c.Content.Replace("\r\n", "<br/>")%></td></tr>
    <%
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int tlEj_index=1;
List<Lebi_FriendLink> tlEjs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink tlEj in tlEjs){%>
            
                <% if (tlEj.Logo != "" && tlEj.IsPic == 1){ %><a href="<%=tlEj.Url%>" target="_blank"><img src="<%=Image(tlEj.Logo) %>" alt="<%=tlEj.Name%>" /></a><%}else{%><a href="<%=tlEj.Url%>" target="_blank"><%=tlEj.Name%></a><%} %>     
