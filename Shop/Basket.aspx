...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_Basket"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> MONDs=Languages();RecordCount=MONDs.Count;int MOND_index=1;
foreach (Shop.Model.Lebi_Language MOND in MONDs){%>
            <dd <%if (MOND_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=MOND.id%>,'<%=MOND.Code%>','<%=MOND.Path%>');"><%if (MOND.ImageUrl!=""){%><img src="<%=Image(MOND.ImageUrl) %>" /><%}%><%=MOND.Name%></a></dd>
            <%MOND_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int HOBO_index=1;
List<Lebi_Currency> HOBOs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency HOBO in HOBOs){%>
            <dd <%if (HOBO_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=HOBO.id%>,'<%=HOBO.Code%>',<%=HOBO.ExchangeRate%>,'<%=HOBO.Msige%>','<%=HOBO.DecimalLength%>');"><%=HOBO.Code%></a></dd>
            <%HOBO_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int idzU_index=1;
List<Lebi_Searchkey> idzUs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey idzU in idzUs){%><%=Lang(idzU.Name)%><%idzU_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Ykou_index=1;
List<Lebi_Searchkey> Ykous = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Ykou in Ykous){%>
<%if (Ykou.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Ykou.Name)+"") %>"><%}else{ %><a href="<%=Ykou.URL%>" target="_blank"><%} %><span><%=Lang(Ykou.Name)%></span></a>
<%Ykou_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int cQYc_index=1;
List<Lebi_Page> cQYcs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page cQYc in cQYcs){%>
<a class="menu" href="<%=URL("","",cQYc.url)%>"><span><%=cQYc.Name%></span></a>
<%cQYc_index++;}%>
...
<ul>
	<li class="current"><span>加入购物车</span></li>
	<li class="over"><span class="con">填写订单信息</span></li>
	<li class="over"><span class="con">成功提交订单</span></li>
...
<div class="basketlist">
    <table align="center" cellpadding="0" cellspacing="0">
        ...
        <tr valign="middle">
            ...
            <td class="list" style="text-align: center">
                <a href="javascript:void(0)" onclick="UserProduct_Del(<%=p.Product_id %>,142);" class="btn btn-12"><s></s>移除购物车</a>
            </td>
        </tr>
        <%} %>  
        </table>
    ...
    <%if (shop.Products.Count > 0 || shop.FreeProducts.Count > 0)
      {%>
    <div class="money">
        <%if (shop.Money_Market > 0){ %>市场价：<span><%=FormatMoney(shop.Money_Market)%></span>&nbsp;&nbsp;<%} %>
        ...
        <%} %>
        <%if (shop.Money_Market - shop.Money_Product - shop.Money_Cut >0){ %>节省：<s><span><%=FormatMoney(shop.Money_Market - shop.Money_Product-shop.Money_Cut)%></span></s>&nbsp;&nbsp;<%} %>
        <%if(shop.Point_Buy>0){ %>
...
<%if (basket.Shops.Count > 1){%>
    <div class="money">
        <%if (basket.Money_Market > 0){ %><p>市场价：<span><%=FormatMoney(basket.Money_Market)%></span></p><%} %>
        ...
        <%} %>
        <%if (basket.Money_Market - basket.Money_Product - basket.Money_Cut >0){ %><p>节省：<s><span><%=FormatMoney(basket.Money_Market - basket.Money_Product-basket.Money_Cut)%></span></s></p><%} %>
...
<%if(basket.Money_Refund_VAT>0){ %>
    <div class="money">
        ...
        <p>退税总额：<span><%=FormatMoney(basket.Money_Refund)%></span></p>
        <%} %>
        <%if(basket.Money_Refund>basket.OtherSite_Money_Refund && basket.OtherSite_Money_Refund>0){ %>
        <p>高街退税额：<span><%=FormatMoney(basket.OtherSite_Money_Refund)%></span></p>
        <%} %>
        <%if(basket.Money_Refund - basket.OtherSite_Money_Refund>0){ %>
        <p>预计本次购物您多得退税额：<span><%=FormatMoney(basket.Money_Refund - basket.OtherSite_Money_Refund)%></span></p>
        <%} %>
    </div>
<%} %>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int eLkd_index=1;
List<Lebi_FriendLink> eLkds = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink eLkd in eLkds){%>
            
                <% if (eLkd.Logo != "" && eLkd.IsPic == 1){ %><a href="<%=eLkd.Url%>" target="_blank"><img src="<%=Image(eLkd.Logo) %>" alt="<%=eLkd.Name%>" /></a><%}else{%><a href="<%=eLkd.Url%>" target="_blank"><%=eLkd.Name%></a><%} %>     
