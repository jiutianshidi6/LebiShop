...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_ForgetPassword"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> MBXKs=Languages();RecordCount=MBXKs.Count;int MBXK_index=1;
foreach (Shop.Model.Lebi_Language MBXK in MBXKs){%>
            <dd <%if (MBXK_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=MBXK.id%>,'<%=MBXK.Code%>','<%=MBXK.Path%>');"><%if (MBXK.ImageUrl!=""){%><img src="<%=Image(MBXK.ImageUrl) %>" /><%}%><%=MBXK.Name%></a></dd>
            <%MBXK_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int cOgq_index=1;
List<Lebi_Currency> cOgqs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency cOgq in cOgqs){%>
            <dd <%if (cOgq_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=cOgq.id%>,'<%=cOgq.Code%>',<%=cOgq.ExchangeRate%>,'<%=cOgq.Msige%>','<%=cOgq.DecimalLength%>');"><%=cOgq.Code%></a></dd>
            <%cOgq_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zLco_index=1;
List<Lebi_Searchkey> zLcos = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zLco in zLcos){%><%=Lang(zLco.Name)%><%zLco_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int AUUJ_index=1;
List<Lebi_Searchkey> AUUJs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey AUUJ in AUUJs){%>
<%if (AUUJ.Type==1){ %><a href="<%=URL("P_Search",""+Lang(AUUJ.Name)+"") %>"><%}else{ %><a href="<%=AUUJ.URL%>" target="_blank"><%} %><span><%=Lang(AUUJ.Name)%></span></a>
<%AUUJ_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Ackn_index=1;
List<Lebi_Page> Ackns = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Ackn in Ackns){%>
<a class="menu" href="<%=URL("","",Ackn.url)%>"><span><%=Ackn.Name%></span></a>
<%Ackn_index++;}%>
...
<div class="userbox">
    ...
    <dl>
        <dt>验证码：</dt>
        <dd><input name="verifycode" type="text" shop="true" id="verifycode" shop="true" size="6" class="input" /><img class="verifycode" id="img1" src="<%=WebPath%>/code.aspx" /><img src="/Theme/system/images/reload.gif" class="refresh" title="点击刷新验证码" onclick="refresh('img1')" /></dd>
    </dl>
    <%} %>
    <dl class="dl-btn">
        <dt></dt>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int hGEf_index=1;
List<Lebi_FriendLink> hGEfs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink hGEf in hGEfs){%>
            
                <% if (hGEf.Logo != "" && hGEf.IsPic == 1){ %><a href="<%=hGEf.Url%>" target="_blank"><img src="<%=Image(hGEf.Logo) %>" alt="<%=hGEf.Name%>" /></a><%}else{%><a href="<%=hGEf.Url%>" target="_blank"><%=hGEf.Name%></a><%} %>     
