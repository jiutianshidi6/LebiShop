...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_FriendLink"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> XvBas=Languages();RecordCount=XvBas.Count;int XvBa_index=1;
foreach (Shop.Model.Lebi_Language XvBa in XvBas){%>
            <dd <%if (XvBa_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=XvBa.id%>,'<%=XvBa.Code%>','<%=XvBa.Path%>');"><%if (XvBa.ImageUrl!=""){%><img src="<%=Image(XvBa.ImageUrl) %>" /><%}%><%=XvBa.Name%></a></dd>
            <%XvBa_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int nNMg_index=1;
List<Lebi_Currency> nNMgs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency nNMg in nNMgs){%>
            <dd <%if (nNMg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=nNMg.id%>,'<%=nNMg.Code%>',<%=nNMg.ExchangeRate%>,'<%=nNMg.Msige%>','<%=nNMg.DecimalLength%>');"><%=nNMg.Code%></a></dd>
            <%nNMg_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int oGiq_index=1;
List<Lebi_Searchkey> oGiqs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey oGiq in oGiqs){%><%=Lang(oGiq.Name)%><%oGiq_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int VGUF_index=1;
List<Lebi_Searchkey> VGUFs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey VGUF in VGUFs){%>
<%if (VGUF.Type==1){ %><a href="<%=URL("P_Search",""+Lang(VGUF.Name)+"") %>"><%}else{ %><a href="<%=VGUF.URL%>" target="_blank"><%} %><span><%=Lang(VGUF.Name)%></span></a>
<%VGUF_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int pEhc_index=1;
List<Lebi_Page> pEhcs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page pEhc in pEhcs){%>
<a class="menu" href="<%=URL("","",pEhc.url)%>"><span><%=pEhc.Name%></span></a>
<%pEhc_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Fwjq_index=1;
List<Lebi_Page> Fwjqs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Fwjq in Fwjqs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",Fwjq.id,Fwjq.url)%>" target="<%=Fwjq.target%>" title="<%=Fwjq.Name%>"><span style="color:<%=Fwjq.NameColor%>"><%=Fwjq.Name%></span></a></li>
        <%Fwjq_index++;}%>
    </ul>
</div>
</div>
...
<div class="friendlinklist">
    ...
    <div class="mc clearfix">
        <ul class="image">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and IsPic=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_FriendLink.Counts(Where);int Jlmi_index=1;
List<Lebi_FriendLink> Jlmis = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Jlmi in Jlmis){%>
            <li>
                <p><a href="<%=Jlmi.Url%>" target="_blank"><img src="<%=Jlmi.Logo%>" alt="<%=Jlmi.Name%>" /></a></p>     
            </li>
            <%Jlmi_index++;}%>
        </ul>
        <div class="clear"></div>
    ...
    <div class="mc clearfix">
        <ul class="text">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and IsPic=0 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_FriendLink.Counts(Where);int duzg_index=1;
List<Lebi_FriendLink> duzgs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink duzg in duzgs){%>
            <li>
                <a href="<%=duzg.Url%>" target="_blank"><%=duzg.Name%></a>     
            </li>
            <%duzg_index++;}%>
        </ul>
        <div class="clear"></div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int RVOJ_index=1;
List<Lebi_FriendLink> RVOJs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink RVOJ in RVOJs){%>
            
                <% if (RVOJ.Logo != "" && RVOJ.IsPic == 1){ %><a href="<%=RVOJ.Url%>" target="_blank"><img src="<%=Image(RVOJ.Logo) %>" alt="<%=RVOJ.Name%>" /></a><%}else{%><a href="<%=RVOJ.Url%>" target="_blank"><%=RVOJ.Name%></a><%} %>     
