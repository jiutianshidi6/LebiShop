...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_Inquiry"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> MtACs=Languages();RecordCount=MtACs.Count;int MtAC_index=1;
foreach (Shop.Model.Lebi_Language MtAC in MtACs){%>
            <dd <%if (MtAC_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=MtAC.id%>,'<%=MtAC.Code%>','<%=MtAC.Path%>');"><%if (MtAC.ImageUrl!=""){%><img src="<%=Image(MtAC.ImageUrl) %>" /><%}%><%=MtAC.Name%></a></dd>
            <%MtAC_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int cbrC_index=1;
List<Lebi_Currency> cbrCs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency cbrC in cbrCs){%>
            <dd <%if (cbrC_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=cbrC.id%>,'<%=cbrC.Code%>',<%=cbrC.ExchangeRate%>,'<%=cbrC.Msige%>','<%=cbrC.DecimalLength%>');"><%=cbrC.Code%></a></dd>
            <%cbrC_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int tPfM_index=1;
List<Lebi_Searchkey> tPfMs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey tPfM in tPfMs){%><%=Lang(tPfM.Name)%><%tPfM_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int OHbd_index=1;
List<Lebi_Searchkey> OHbds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey OHbd in OHbds){%>
<%if (OHbd.Type==1){ %><a href="<%=URL("P_Search",""+Lang(OHbd.Name)+"") %>"><%}else{ %><a href="<%=OHbd.URL%>" target="_blank"><%} %><span><%=Lang(OHbd.Name)%></span></a>
<%OHbd_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int SgEU_index=1;
List<Lebi_Page> SgEUs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page SgEU in SgEUs){%>
<a class="menu" href="<%=URL("","",SgEU.url)%>"><span><%=SgEU.Name%></span></a>
<%SgEU_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int CJtX_index=1;
List<Lebi_FriendLink> CJtXs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink CJtX in CJtXs){%>
            
                <% if (CJtX.Logo != "" && CJtX.IsPic == 1){ %><a href="<%=CJtX.Url%>" target="_blank"><img src="<%=Image(CJtX.Logo) %>" alt="<%=CJtX.Name%>" /></a><%}else{%><a href="<%=CJtX.Url%>" target="_blank"><%=CJtX.Name%></a><%} %>     
