...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_SendFriend"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> BAYUs=Languages();RecordCount=BAYUs.Count;int BAYU_index=1;
foreach (Shop.Model.Lebi_Language BAYU in BAYUs){%>
            <dd <%if (BAYU_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=BAYU.id%>,'<%=BAYU.Code%>','<%=BAYU.Path%>');"><%if (BAYU.ImageUrl!=""){%><img src="<%=Image(BAYU.ImageUrl) %>" /><%}%><%=BAYU.Name%></a></dd>
            <%BAYU_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int RbDw_index=1;
List<Lebi_Currency> RbDws = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency RbDw in RbDws){%>
            <dd <%if (RbDw_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=RbDw.id%>,'<%=RbDw.Code%>',<%=RbDw.ExchangeRate%>,'<%=RbDw.Msige%>','<%=RbDw.DecimalLength%>');"><%=RbDw.Code%></a></dd>
            <%RbDw_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int IcRm_index=1;
List<Lebi_Searchkey> IcRms = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey IcRm in IcRms){%><%=Lang(IcRm.Name)%><%IcRm_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int cOxS_index=1;
List<Lebi_Searchkey> cOxSs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey cOxS in cOxSs){%>
<%if (cOxS.Type==1){ %><a href="<%=URL("P_Search",""+Lang(cOxS.Name)+"") %>"><%}else{ %><a href="<%=cOxS.URL%>" target="_blank"><%} %><span><%=Lang(cOxS.Name)%></span></a>
<%cOxS_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int bRHw_index=1;
List<Lebi_Page> bRHws = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page bRHw in bRHws){%>
<a class="menu" href="<%=URL("","",bRHw.url)%>"><span><%=bRHw.Name%></span></a>
<%bRHw_index++;}%>
...
<script type="text/javascript">
    function submit() {
        ...
        RequestAjax(url, postData, function () { MsgBox(1, "发送成功", "<%=URL("P_Product",id) %>");});
    }
</script>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int piPY_index=1;
List<Lebi_FriendLink> piPYs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink piPY in piPYs){%>
            
                <% if (piPY.Logo != "" && piPY.IsPic == 1){ %><a href="<%=piPY.Url%>" target="_blank"><img src="<%=Image(piPY.Logo) %>" alt="<%=piPY.Name%>" /></a><%}else{%><a href="<%=piPY.Url%>" target="_blank"><%=piPY.Name%></a><%} %>     
