...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_News"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> sCYQs=Languages();RecordCount=sCYQs.Count;int sCYQ_index=1;
foreach (Shop.Model.Lebi_Language sCYQ in sCYQs){%>
            <dd <%if (sCYQ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=sCYQ.id%>,'<%=sCYQ.Code%>','<%=sCYQ.Path%>');"><%if (sCYQ.ImageUrl!=""){%><img src="<%=Image(sCYQ.ImageUrl) %>" /><%}%><%=sCYQ.Name%></a></dd>
            <%sCYQ_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int fVDn_index=1;
List<Lebi_Currency> fVDns = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency fVDn in fVDns){%>
            <dd <%if (fVDn_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=fVDn.id%>,'<%=fVDn.Code%>',<%=fVDn.ExchangeRate%>,'<%=fVDn.Msige%>','<%=fVDn.DecimalLength%>');"><%=fVDn.Code%></a></dd>
            <%fVDn_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int SQTC_index=1;
List<Lebi_Searchkey> SQTCs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey SQTC in SQTCs){%><%=Lang(SQTC.Name)%><%SQTC_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int nJtQ_index=1;
List<Lebi_Searchkey> nJtQs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey nJtQ in nJtQs){%>
<%if (nJtQ.Type==1){ %><a href="<%=URL("P_Search",""+Lang(nJtQ.Name)+"") %>"><%}else{ %><a href="<%=nJtQ.URL%>" target="_blank"><%} %><span><%=Lang(nJtQ.Name)%></span></a>
<%nJtQ_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int bgUm_index=1;
List<Lebi_Page> bgUms = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page bgUm in bgUms){%>
<a class="menu" href="<%=URL("","",bgUm.url)%>"><span><%=bgUm.Name%></span></a>
<%bgUm_index++;}%>
...
<ul class="text">
    <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int tmxL_index=1;
List<Lebi_Brand> tmxLs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand tmxL in tmxLs){%>
    <li><a href="<%=URL("P_Brand",tmxL.id)%>" title="<%=Lang(tmxL.Name) %>"><%=Lang(tmxL.Name) %></a></li>
    <%tmxL_index++;}%>
</ul>
</div>
</div>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int EkSz_index=1;
List<Lebi_Page> EkSzs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page EkSz in EkSzs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",EkSz.id,EkSz.url)%>" target="<%=EkSz.target%>" title="<%=EkSz.Name%>"><span style="color:<%=EkSz.NameColor%>"><%=EkSz.Name%></span></a></li>
        <%EkSz_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int iiSn_index=1;
List<Lebi_FriendLink> iiSns = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink iiSn in iiSns){%>
            
                <% if (iiSn.Logo != "" && iiSn.IsPic == 1){ %><a href="<%=iiSn.Url%>" target="_blank"><img src="<%=Image(iiSn.Logo) %>" alt="<%=iiSn.Name%>" /></a><%}else{%><a href="<%=iiSn.Url%>" target="_blank"><%=iiSn.Name%></a><%} %>     
