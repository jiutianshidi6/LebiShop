<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.inc.CheckOut_Pay" validateRequest="false"%><%LoadPage();%>
    ...
    <dl>
        ...
        <dd><em><%=Lang(model.Description)%></em>
            ...
            <% 
                ...
                %>
                <dl>
                    <dt><label><input type="radio" name="onlinepay_id" value="<%=sononpay.id %>" <%=(sononpay.id==CurrentUser.OnlinePay_id||sononpay.parentid==CurrentUser.OnlinePay_id)?"checked":"" %> order="true" /> <%if (onpay.Logo != ""){ %><img src="<%=Image(sononpay.Logo) %>" style="height:40px;"/><br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%}%><%=Lang(sononpay.Name)%>(<%=onpay.Currency_Code %>)</label></dt>
                    <dd><%=Lang(sononpay.Description)%><%if (sononpay.FeeRate > 0){ %><%=Tag("手续费")%>：<%=sononpay.FeeRate%> %<%} %></dd>
                </dl>
                <%} %>
            </div>
            </div>
