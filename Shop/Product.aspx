...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_Product"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> XTpls=Languages();RecordCount=XTpls.Count;int XTpl_index=1;
foreach (Shop.Model.Lebi_Language XTpl in XTpls){%>
            <dd <%if (XTpl_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=XTpl.id%>,'<%=XTpl.Code%>','<%=XTpl.Path%>');"><%if (XTpl.ImageUrl!=""){%><img src="<%=Image(XTpl.ImageUrl) %>" /><%}%><%=XTpl.Name%></a></dd>
            <%XTpl_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int LPWv_index=1;
List<Lebi_Currency> LPWvs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency LPWv in LPWvs){%>
            <dd <%if (LPWv_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=LPWv.id%>,'<%=LPWv.Code%>',<%=LPWv.ExchangeRate%>,'<%=LPWv.Msige%>','<%=LPWv.DecimalLength%>');"><%=LPWv.Code%></a></dd>
            <%LPWv_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hdPS_index=1;
List<Lebi_Searchkey> hdPSs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hdPS in hdPSs){%><%=Lang(hdPS.Name)%><%hdPS_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Jshi_index=1;
List<Lebi_Searchkey> Jshis = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Jshi in Jshis){%>
<%if (Jshi.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Jshi.Name)+"") %>"><%}else{ %><a href="<%=Jshi.URL%>" target="_blank"><%} %><span><%=Lang(Jshi.Name)%></span></a>
<%Jshi_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int XSOw_index=1;
List<Lebi_Page> XSOws = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page XSOw in XSOws){%>
<a class="menu" href="<%=URL("","",XSOw.url)%>"><span><%=XSOw.Name%></span></a>
<%XSOw_index++;}%>
...
<div class="mc clearfix">
    <ul class="text">
        <%Table="Lebi_Page";Where="Node_id="+Node("News").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Time_Add desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int KCVt_index=1;
List<Lebi_Page> KCVts = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page KCVt in KCVts){%>
        <li><a href="<%if (KCVt.url == ""){Response.Write(URL("P_NewsDetails",KCVt.id));}else{Response.Write(KCVt.url);} %>" title="<%=KCVt.Name%>"><em><%=KCVt.Time_Add.ToString("MM-dd") %></em><span style="color:<%=KCVt.NameColor%>"><%=KCVt.Name%></span></a></li>
        <%KCVt_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int TxET_index=1;
List<Lebi_FriendLink> TxETs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink TxET in TxETs){%>
            
                <% if (TxET.Logo != "" && TxET.IsPic == 1){ %><a href="<%=TxET.Url%>" target="_blank"><img src="<%=Image(TxET.Logo) %>" alt="<%=TxET.Name%>" /></a><%}else{%><a href="<%=TxET.Url%>" target="_blank"><%=TxET.Name%></a><%} %>     
