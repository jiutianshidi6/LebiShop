...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_AllProductCategories"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> ldnxs=Languages();RecordCount=ldnxs.Count;int ldnx_index=1;
foreach (Shop.Model.Lebi_Language ldnx in ldnxs){%>
            <dd <%if (ldnx_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=ldnx.id%>,'<%=ldnx.Code%>','<%=ldnx.Path%>');"><%if (ldnx.ImageUrl!=""){%><img src="<%=Image(ldnx.ImageUrl) %>" /><%}%><%=ldnx.Name%></a></dd>
            <%ldnx_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int nyMQ_index=1;
List<Lebi_Currency> nyMQs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency nyMQ in nyMQs){%>
            <dd <%if (nyMQ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=nyMQ.id%>,'<%=nyMQ.Code%>',<%=nyMQ.ExchangeRate%>,'<%=nyMQ.Msige%>','<%=nyMQ.DecimalLength%>');"><%=nyMQ.Code%></a></dd>
            <%nyMQ_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int geJl_index=1;
List<Lebi_Searchkey> geJls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey geJl in geJls){%><%=Lang(geJl.Name)%><%geJl_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int KRvW_index=1;
List<Lebi_Searchkey> KRvWs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey KRvW in KRvWs){%>
<%if (KRvW.Type==1){ %><a href="<%=URL("P_Search",""+Lang(KRvW.Name)+"") %>"><%}else{ %><a href="<%=KRvW.URL%>" target="_blank"><%} %><span><%=Lang(KRvW.Name)%></span></a>
<%KRvW_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int PlCI_index=1;
List<Lebi_Page> PlCIs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page PlCI in PlCIs){%>
<a class="menu" href="<%=URL("","",PlCI.url)%>"><span><%=PlCI.Name%></span></a>
<%PlCI_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int mVec_index=1;
List<Lebi_FriendLink> mVecs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink mVec in mVecs){%>
            
                <% if (mVec.Logo != "" && mVec.IsPic == 1){ %><a href="<%=mVec.Url%>" target="_blank"><img src="<%=Image(mVec.Logo) %>" alt="<%=mVec.Name%>" /></a><%}else{%><a href="<%=mVec.Url%>" target="_blank"><%=mVec.Name%></a><%} %>     
