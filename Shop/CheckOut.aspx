...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_CheckOut"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> VOuIs=Languages();RecordCount=VOuIs.Count;int VOuI_index=1;
foreach (Shop.Model.Lebi_Language VOuI in VOuIs){%>
            <dd <%if (VOuI_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=VOuI.id%>,'<%=VOuI.Code%>','<%=VOuI.Path%>');"><%if (VOuI.ImageUrl!=""){%><img src="<%=Image(VOuI.ImageUrl) %>" /><%}%><%=VOuI.Name%></a></dd>
            <%VOuI_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int TfIy_index=1;
List<Lebi_Currency> TfIys = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency TfIy in TfIys){%>
            <dd <%if (TfIy_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=TfIy.id%>,'<%=TfIy.Code%>',<%=TfIy.ExchangeRate%>,'<%=TfIy.Msige%>','<%=TfIy.DecimalLength%>');"><%=TfIy.Code%></a></dd>
            <%TfIy_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int bTek_index=1;
List<Lebi_Searchkey> bTeks = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey bTek in bTeks){%><%=Lang(bTek.Name)%><%bTek_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int gRTh_index=1;
List<Lebi_Searchkey> gRThs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey gRTh in gRThs){%>
<%if (gRTh.Type==1){ %><a href="<%=URL("P_Search",""+Lang(gRTh.Name)+"") %>"><%}else{ %><a href="<%=gRTh.URL%>" target="_blank"><%} %><span><%=Lang(gRTh.Name)%></span></a>
<%gRTh_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int uCid_index=1;
List<Lebi_Page> uCids = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page uCid in uCids){%>
<a class="menu" href="<%=URL("","",uCid.url)%>"><span><%=uCid.Name%></span></a>
<%uCid_index++;}%>
...
<ul>
	...
	<li class="current"><span class="con">填写订单信息</span></li>
    ...
    <div id="orderinfo">
        ...
        <div class="ordertips">
            <div class="ordertotal">
                <%if (basket.Money_Market >0){ %>市场价：<font color="#ff3333"><%=FormatMoney(basket.Money_Market)%></font><%} %>
                <%if(basket.Point_Buy>0){ %>
                &nbsp;&nbsp;&nbsp;&nbsp;所需积分：<font color="#ff3333"><%=basket.Point_Buy%></font>
                <%} %>
                <%if(basket.Point>0){ %>
                &nbsp;&nbsp;&nbsp;&nbsp;获得积分：<font color="#ff3333"><%=basket.Point%></font>
                <%} %>
                <%if(basket.Weight>0){ %>
                &nbsp;&nbsp;&nbsp;&nbsp;总重量：<font color="#ff3333"><%=basket.Weight%></font> KG
                <%} %>
                <%if(basket.Volume>0){ %>
                &nbsp;&nbsp;&nbsp;&nbsp;体积：<font color="#ff3333"><%=(basket.Volume/1000000).ToString("0.00")%></font> m&sup3;
                <%} %>
            </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int stgY_index=1;
List<Lebi_FriendLink> stgYs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink stgY in stgYs){%>
            
                <% if (stgY.Logo != "" && stgY.IsPic == 1){ %><a href="<%=stgY.Url%>" target="_blank"><img src="<%=Image(stgY.Logo) %>" alt="<%=stgY.Name%>" /></a><%}else{%><a href="<%=stgY.Url%>" target="_blank"><%=stgY.Name%></a><%} %>     
