...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> sceDs=Languages();RecordCount=sceDs.Count;int sceD_index=1;
foreach (Shop.Model.Lebi_Language sceD in sceDs){%>
            <dd <%if (sceD_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=sceD.id%>,'<%=sceD.Code%>','<%=sceD.Path%>');"><%if (sceD.ImageUrl!=""){%><img src="<%=Image(sceD.ImageUrl) %>" /><%}%><%=sceD.Name%></a></dd>
            <%sceD_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Vias_index=1;
List<Lebi_Currency> Viass = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Vias in Viass){%>
            <dd <%if (Vias_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Vias.id%>,'<%=Vias.Code%>',<%=Vias.ExchangeRate%>,'<%=Vias.Msige%>','<%=Vias.DecimalLength%>');"><%=Vias.Code%></a></dd>
            <%Vias_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Wsav_index=1;
List<Lebi_Searchkey> Wsavs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Wsav in Wsavs){%><%=Lang(Wsav.Name)%><%Wsav_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int LVuu_index=1;
List<Lebi_Searchkey> LVuus = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey LVuu in LVuus){%>
<%if (LVuu.Type==1){ %><a href="<%=URL("P_Search",""+Lang(LVuu.Name)+"") %>"><%}else{ %><a href="<%=LVuu.URL%>" target="_blank"><%} %><span><%=Lang(LVuu.Name)%></span></a>
<%LVuu_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int UsoS_index=1;
List<Lebi_Page> UsoSs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page UsoS in UsoSs){%>
<a class="menu" href="<%=URL("","",UsoS.url)%>"><span><%=UsoS.Name%></span></a>
<%UsoS_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int PNHR_index=1;
List<Lebi_FriendLink> PNHRs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink PNHR in PNHRs){%>
            
                <% if (PNHR.Logo != "" && PNHR.IsPic == 1){ %><a href="<%=PNHR.Url%>" target="_blank"><img src="<%=Image(PNHR.Logo) %>" alt="<%=PNHR.Name%>" /></a><%}else{%><a href="<%=PNHR.Url%>" target="_blank"><%=PNHR.Name%></a><%} %>     
