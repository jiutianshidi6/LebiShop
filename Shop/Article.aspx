...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> QQKjs=Languages();RecordCount=QQKjs.Count;int QQKj_index=1;
foreach (Shop.Model.Lebi_Language QQKj in QQKjs){%>
            <dd <%if (QQKj_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=QQKj.id%>,'<%=QQKj.Code%>','<%=QQKj.Path%>');"><%if (QQKj.ImageUrl!=""){%><img src="<%=Image(QQKj.ImageUrl) %>" /><%}%><%=QQKj.Name%></a></dd>
            <%QQKj_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int BIWJ_index=1;
List<Lebi_Currency> BIWJs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency BIWJ in BIWJs){%>
            <dd <%if (BIWJ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=BIWJ.id%>,'<%=BIWJ.Code%>',<%=BIWJ.ExchangeRate%>,'<%=BIWJ.Msige%>','<%=BIWJ.DecimalLength%>');"><%=BIWJ.Code%></a></dd>
            <%BIWJ_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int FMPy_index=1;
List<Lebi_Searchkey> FMPys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey FMPy in FMPys){%><%=Lang(FMPy.Name)%><%FMPy_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zTBJ_index=1;
List<Lebi_Searchkey> zTBJs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zTBJ in zTBJs){%>
<%if (zTBJ.Type==1){ %><a href="<%=URL("P_Search",""+Lang(zTBJ.Name)+"") %>"><%}else{ %><a href="<%=zTBJ.URL%>" target="_blank"><%} %><span><%=Lang(zTBJ.Name)%></span></a>
<%zTBJ_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int nFBx_index=1;
List<Lebi_Page> nFBxs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page nFBx in nFBxs){%>
<a class="menu" href="<%=URL("","",nFBx.url)%>"><span><%=nFBx.Name%></span></a>
<%nFBx_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int FNAO_index=1;
List<Lebi_FriendLink> FNAOs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink FNAO in FNAOs){%>
            
                <% if (FNAO.Logo != "" && FNAO.IsPic == 1){ %><a href="<%=FNAO.Url%>" target="_blank"><img src="<%=Image(FNAO.Logo) %>" alt="<%=FNAO.Name%>" /></a><%}else{%><a href="<%=FNAO.Url%>" target="_blank"><%=FNAO.Name%></a><%} %>     
