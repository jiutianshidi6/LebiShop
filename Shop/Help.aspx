...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> bbAqs=Languages();RecordCount=bbAqs.Count;int bbAq_index=1;
foreach (Shop.Model.Lebi_Language bbAq in bbAqs){%>
            <dd <%if (bbAq_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=bbAq.id%>,'<%=bbAq.Code%>','<%=bbAq.Path%>');"><%if (bbAq.ImageUrl!=""){%><img src="<%=Image(bbAq.ImageUrl) %>" /><%}%><%=bbAq.Name%></a></dd>
            <%bbAq_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int MHoM_index=1;
List<Lebi_Currency> MHoMs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency MHoM in MHoMs){%>
            <dd <%if (MHoM_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=MHoM.id%>,'<%=MHoM.Code%>',<%=MHoM.ExchangeRate%>,'<%=MHoM.Msige%>','<%=MHoM.DecimalLength%>');"><%=MHoM.Code%></a></dd>
            <%MHoM_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int AErl_index=1;
List<Lebi_Searchkey> AErls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey AErl in AErls){%><%=Lang(AErl.Name)%><%AErl_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zRjU_index=1;
List<Lebi_Searchkey> zRjUs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zRjU in zRjUs){%>
<%if (zRjU.Type==1){ %><a href="<%=URL("P_Search",""+Lang(zRjU.Name)+"") %>"><%}else{ %><a href="<%=zRjU.URL%>" target="_blank"><%} %><span><%=Lang(zRjU.Name)%></span></a>
<%zRjU_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int lEaO_index=1;
List<Lebi_Page> lEaOs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page lEaO in lEaOs){%>
<a class="menu" href="<%=URL("","",lEaO.url)%>"><span><%=lEaO.Name%></span></a>
<%lEaO_index++;}%>
...
<div class="mc">
    ...
    if (type !=""){
        ...
        %>
    <%Table="Lebi_Page";Where=""+strwhere+"";PageSize=999;pageindex=Rint("page");RecordCount=B_Lebi_Page.Counts(Where);int fkwI_index=1;
List<Lebi_Page> fkwIs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page fkwI in fkwIs){%>
    <div class="nodeitem clearfix" id="help<%=fkwI.id%>">
    <div class="mt">
        <h2><font style="color:<%=fkwI.NameColor%>"><%=fkwI.Name%></font></h2>
    </div>
    <%if (fkwI.Description != ""){ %><div class="tips"><%=fkwI.Description%></div><%} %>
    <div class="content">
    <%=ImgSrc(fkwI.Content,CurrentLanguage.IsLazyLoad) %>
    </div>
    </div>
    <%fkwI_index++;}%>
    <%} %>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int zyFr_index=1;
List<Lebi_FriendLink> zyFrs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink zyFr in zyFrs){%>
            
                <% if (zyFr.Logo != "" && zyFr.IsPic == 1){ %><a href="<%=zyFr.Url%>" target="_blank"><img src="<%=Image(zyFr.Logo) %>" alt="<%=zyFr.Name%>" /></a><%}else{%><a href="<%=zyFr.Url%>" target="_blank"><%=zyFr.Name%></a><%} %>     
