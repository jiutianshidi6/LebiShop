...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_Exchange"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_Exchange","title")%></h2>
...
$(function () {
    blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MMEe_index=1;
List<Lebi_Searchkey> MMEes = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MMEe in MMEes){%><%=Lang(MMEe.Name)%><%MMEe_index++;}%><%} %>');
...
<div id="keywords">
    ...
    <div class="mc clearfix">
    <ul class="text">
        <%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=18;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int RyjI_index=1;
List<Lebi_Searchkey> RyjIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey RyjI in RyjIs){%>
        <li><%if (RyjI.Type==1){ %><a href="<%=URL("P_Search",""+Lang(RyjI.Name)+"") %>"><%}else{ %><a href="<%=RyjI.URL%>"><%} %><%=Lang(RyjI.Name)%></a></li>
        <%RyjI_index++;}%>
    </ul>
    </div>
    </div>
    ...
    <div class="mc clearfix">
    <ul class="text">
        <%Table="Lebi_Brand";Where="IsRecommend=1";Order="Sort desc,id desc";PageSize=18;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int kvIw_index=1;
List<Lebi_Brand> kvIws = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand kvIw in kvIws){%>
        <li><a href="<%=URL("P_Brand",kvIw.id)%>" title="<%=Lang(kvIw.Name) %>"><%=Lang(kvIw.Name) %></a></li>
        <%kvIw_index++;}%>
    </ul>
    </div>
    </div>
...
<div class="mc clearfix">
   ...
   <%} %>
	<%Table="Lebi_Page";Where="Node_id="+Node("FootMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int WmUo_index=1;
List<Lebi_Page> WmUos = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page WmUo in WmUos){%>
   <div class="item"><span class="itemname"><h3><a href="<%=URL("","",WmUo.url)%>"><%=WmUo.Name%></a></h3><s></s></span></div>
	<%WmUo_index++;}%>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> shrbs=Languages();RecordCount=shrbs.Count;int shrb_index=1;
foreach (Shop.Model.Lebi_Language shrb in shrbs){%>
<a <%if (shrb_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=shrb.id%>,'<%=shrb.Code%>','<%=shrb.Path%>');"><img src="<%=Image(shrb.ImageUrl) %>" title="<%=shrb.Name%>" /><%=shrb.Name%></a>
<%shrb_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int bklp_index=1;
List<Lebi_Currency> bklps = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency bklp in bklps){%>
<dd <%if (bklp_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=bklp.id%>,'<%=bklp.Code%>',<%=bklp.ExchangeRate%>,'<%=bklp.Msige%>');"><%=bklp.Code%></a></dd>
<%bklp_index++;}%>
</dl></li></ul></div>
