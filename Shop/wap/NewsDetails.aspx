...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_NewsDetails"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_NewsDetails","title")%></h2>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> pexhs=Languages();RecordCount=pexhs.Count;int pexh_index=1;
foreach (Shop.Model.Lebi_Language pexh in pexhs){%>
<a <%if (pexh_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=pexh.id%>,'<%=pexh.Code%>','<%=pexh.Path%>');"><img src="<%=Image(pexh.ImageUrl) %>" title="<%=pexh.Name%>" /><%=pexh.Name%></a>
<%pexh_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int WqVT_index=1;
List<Lebi_Currency> WqVTs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency WqVT in WqVTs){%>
<dd <%if (WqVT_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=WqVT.id%>,'<%=WqVT.Code%>',<%=WqVT.ExchangeRate%>,'<%=WqVT.Msige%>');"><%=WqVT.Code%></a></dd>
<%WqVT_index++;}%>
</dl></li></ul></div>
