...
<%} %>
    <%if (CurrentUser.NickName != "") { Response.Write(CurrentUser.NickName); } else { Response.Write(CurrentUser.UserName); } %></a><a href="<%=URL("P_UserMessage","0")%>" title="<%=Tag("收件箱")%>"><span>(<%=Count_Message(0) %>)</span></a> ┊ <a href="<%=URL("P_UserAsk","")%>"><%=Tag("商品咨询")%><span>(<%=Count_ProductAsk(0) %>)</span></a> ┊ <a href="javascript:LoginOut();"><%=Tag("退出登录")%></a>
<%}
  else
  { %>
