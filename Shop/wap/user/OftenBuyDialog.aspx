...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_UserOftenBuyDialog"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_UserOftenBuyDialog","title")%></h2>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> Nhwws=Languages();RecordCount=Nhwws.Count;int Nhww_index=1;
foreach (Shop.Model.Lebi_Language Nhww in Nhwws){%>
<a <%if (Nhww_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=Nhww.id%>,'<%=Nhww.Code%>','<%=Nhww.Path%>');"><img src="<%=Image(Nhww.ImageUrl) %>" title="<%=Nhww.Name%>" /><%=Nhww.Name%></a>
<%Nhww_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Gvvn_index=1;
List<Lebi_Currency> Gvvns = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Gvvn in Gvvns){%>
<dd <%if (Gvvn_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=Gvvn.id%>,'<%=Gvvn.Code%>',<%=Gvvn.ExchangeRate%>,'<%=Gvvn.Msige%>');"><%=Gvvn.Code%></a></dd>
<%Gvvn_index++;}%>
</dl></li></ul></div>
