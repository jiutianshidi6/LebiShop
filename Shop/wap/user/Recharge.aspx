...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_Recharge"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_Recharge","title")%></h2>
...
<script type="text/javascript">
    function submit() {
        ...
        var postData = { "RMoney": RMoney,"onlinepay_id":onlinepay_id};
...
<div class="language">
<%List<Shop.Model.Lebi_Language> eOkHs=Languages();RecordCount=eOkHs.Count;int eOkH_index=1;
foreach (Shop.Model.Lebi_Language eOkH in eOkHs){%>
<a <%if (eOkH_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=eOkH.id%>,'<%=eOkH.Code%>','<%=eOkH.Path%>');"><img src="<%=Image(eOkH.ImageUrl) %>" title="<%=eOkH.Name%>" /><%=eOkH.Name%></a>
<%eOkH_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int NdUQ_index=1;
List<Lebi_Currency> NdUQs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency NdUQ in NdUQs){%>
<dd <%if (NdUQ_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=NdUQ.id%>,'<%=NdUQ.Code%>',<%=NdUQ.ExchangeRate%>,'<%=NdUQ.Msige%>');"><%=NdUQ.Code%></a></dd>
<%NdUQ_index++;}%>
</dl></li></ul></div>
