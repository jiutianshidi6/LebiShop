...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_UserBank"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_UserBank","title")%></h2>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> taOAs=Languages();RecordCount=taOAs.Count;int taOA_index=1;
foreach (Shop.Model.Lebi_Language taOA in taOAs){%>
<a <%if (taOA_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=taOA.id%>,'<%=taOA.Code%>','<%=taOA.Path%>');"><img src="<%=Image(taOA.ImageUrl) %>" title="<%=taOA.Name%>" /><%=taOA.Name%></a>
<%taOA_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int dcBH_index=1;
List<Lebi_Currency> dcBHs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency dcBH in dcBHs){%>
<dd <%if (dcBH_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=dcBH.id%>,'<%=dcBH.Code%>',<%=dcBH.ExchangeRate%>,'<%=dcBH.Msige%>');"><%=dcBH.Code%></a></dd>
<%dcBH_index++;}%>
</dl></li></ul></div>
