...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_UserCard"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_UserCard","title")%></h2>
...
<div class="user">
    ...
    <div class="table-list">
        ...
        <tr>
            <th>代码：<%=uc.Code%></th>
            <th colspan="2">名称：<%=CardName(uc.CardOrder_id) %></th>
        </tr>
        <tr>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> sEGjs=Languages();RecordCount=sEGjs.Count;int sEGj_index=1;
foreach (Shop.Model.Lebi_Language sEGj in sEGjs){%>
<a <%if (sEGj_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=sEGj.id%>,'<%=sEGj.Code%>','<%=sEGj.Path%>');"><img src="<%=Image(sEGj.ImageUrl) %>" title="<%=sEGj.Name%>" /><%=sEGj.Name%></a>
<%sEGj_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int buXr_index=1;
List<Lebi_Currency> buXrs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency buXr in buXrs){%>
<dd <%if (buXr_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=buXr.id%>,'<%=buXr.Code%>',<%=buXr.ExchangeRate%>,'<%=buXr.Msige%>');"><%=buXr.Code%></a></dd>
<%buXr_index++;}%>
</dl></li></ul></div>
