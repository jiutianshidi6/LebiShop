...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_UserBankEdit"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_UserBankEdit","title")%></h2>
...
<div class="userbox">
    ...
    <dl>
        <dt>收款方式：</dt>
        <dd>
        <label><input type="radio" name="PayType" value="bank" shop="true" onclick="SetType();" <%=model.PayType == "alipay"?"":"checked"%> />银行转账</label>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> oceIs=Languages();RecordCount=oceIs.Count;int oceI_index=1;
foreach (Shop.Model.Lebi_Language oceI in oceIs){%>
<a <%if (oceI_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=oceI.id%>,'<%=oceI.Code%>','<%=oceI.Path%>');"><img src="<%=Image(oceI.ImageUrl) %>" title="<%=oceI.Name%>" /><%=oceI.Name%></a>
<%oceI_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int oOAS_index=1;
List<Lebi_Currency> oOASs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency oOAS in oOASs){%>
<dd <%if (oOAS_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=oOAS.id%>,'<%=oOAS.Code%>',<%=oOAS.ExchangeRate%>,'<%=oOAS.Msige%>');"><%=oOAS.Code%></a></dd>
<%oOAS_index++;}%>
</dl></li></ul></div>
