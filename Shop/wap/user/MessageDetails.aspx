...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_UserMessageDetails"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_UserMessageDetails","title")%></h2>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> Irers=Languages();RecordCount=Irers.Count;int Irer_index=1;
foreach (Shop.Model.Lebi_Language Irer in Irers){%>
<a <%if (Irer_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=Irer.id%>,'<%=Irer.Code%>','<%=Irer.Path%>');"><img src="<%=Image(Irer.ImageUrl) %>" title="<%=Irer.Name%>" /><%=Irer.Name%></a>
<%Irer_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int jpCo_index=1;
List<Lebi_Currency> jpCos = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency jpCo in jpCos){%>
<dd <%if (jpCo_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=jpCo.id%>,'<%=jpCo.Code%>',<%=jpCo.ExchangeRate%>,'<%=jpCo.Msige%>');"><%=jpCo.Code%></a></dd>
<%jpCo_index++;}%>
</dl></li></ul></div>
