...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_UserReturnShip"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_UserReturnShip","title")%></h2>
...
<div class="user">
    ...
    <tr>
        <td style="width:50px" valign="top"><a href="<%=URL("P_Product",o_p.Product_id) %>"><img src="<%=Image(o_p.ImageSmall) %>" style="width:50px;height:50px;" /></a></td>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> tDqYs=Languages();RecordCount=tDqYs.Count;int tDqY_index=1;
foreach (Shop.Model.Lebi_Language tDqY in tDqYs){%>
<a <%if (tDqY_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=tDqY.id%>,'<%=tDqY.Code%>','<%=tDqY.Path%>');"><img src="<%=Image(tDqY.ImageUrl) %>" title="<%=tDqY.Name%>" /><%=tDqY.Name%></a>
<%tDqY_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int aysr_index=1;
List<Lebi_Currency> aysrs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency aysr in aysrs){%>
<dd <%if (aysr_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=aysr.id%>,'<%=aysr.Code%>',<%=aysr.ExchangeRate%>,'<%=aysr.Msige%>');"><%=aysr.Code%></a></dd>
<%aysr_index++;}%>
</dl></li></ul></div>
