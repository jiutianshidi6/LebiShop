...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_UserAgentMoney"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_UserAgentMoney","title")%></h2>
...
<div class="user">
    ...
    <div class="table-list">
        ...
        <tr>
            <th style="width:150px"><%=Tag("订单")%>：<%=model.Order_Code%></th>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> QeXjs=Languages();RecordCount=QeXjs.Count;int QeXj_index=1;
foreach (Shop.Model.Lebi_Language QeXj in QeXjs){%>
<a <%if (QeXj_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=QeXj.id%>,'<%=QeXj.Code%>','<%=QeXj.Path%>');"><img src="<%=Image(QeXj.ImageUrl) %>" title="<%=QeXj.Name%>" /><%=QeXj.Name%></a>
<%QeXj_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Nsvp_index=1;
List<Lebi_Currency> Nsvps = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Nsvp in Nsvps){%>
<dd <%if (Nsvp_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=Nsvp.id%>,'<%=Nsvp.Code%>',<%=Nsvp.ExchangeRate%>,'<%=Nsvp.Msige%>');"><%=Nsvp.Code%></a></dd>
<%Nsvp_index++;}%>
</dl></li></ul></div>
