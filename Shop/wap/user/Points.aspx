...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_UserPoint"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_UserPoint","title")%></h2>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> zxAHs=Languages();RecordCount=zxAHs.Count;int zxAH_index=1;
foreach (Shop.Model.Lebi_Language zxAH in zxAHs){%>
<a <%if (zxAH_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=zxAH.id%>,'<%=zxAH.Code%>','<%=zxAH.Path%>');"><img src="<%=Image(zxAH.ImageUrl) %>" title="<%=zxAH.Name%>" /><%=zxAH.Name%></a>
<%zxAH_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int RLsb_index=1;
List<Lebi_Currency> RLsbs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency RLsb in RLsbs){%>
<dd <%if (RLsb_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=RLsb.id%>,'<%=RLsb.Code%>',<%=RLsb.ExchangeRate%>,'<%=RLsb.Msige%>');"><%=RLsb.Code%></a></dd>
<%RLsb_index++;}%>
</dl></li></ul></div>
