...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_UserLike"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_UserLike","title")%></h2>
...
<div class="user">
    ...
    <div class="table-list">
        ...
        <tr>
            <th style="width:50px" valign="top" rowspan="2"><% if (product.ImageSmall == ""){ %>&nbsp;<%}else{ %><a href="<%=URL("P_Product",u_p.Product_id) %>"><img style="height: 50px;width: 50px;vertical-align:middle" src="<%=Image(product.ImageSmall) %>" /></a><%} %></th>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> eYkRs=Languages();RecordCount=eYkRs.Count;int eYkR_index=1;
foreach (Shop.Model.Lebi_Language eYkR in eYkRs){%>
<a <%if (eYkR_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=eYkR.id%>,'<%=eYkR.Code%>','<%=eYkR.Path%>');"><img src="<%=Image(eYkR.ImageUrl) %>" title="<%=eYkR.Name%>" /><%=eYkR.Name%></a>
<%eYkR_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int HVCL_index=1;
List<Lebi_Currency> HVCLs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency HVCL in HVCLs){%>
<dd <%if (HVCL_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=HVCL.id%>,'<%=HVCL.Code%>',<%=HVCL.ExchangeRate%>,'<%=HVCL.Msige%>');"><%=HVCL.Code%></a></dd>
<%HVCL_index++;}%>
</dl></li></ul></div>
