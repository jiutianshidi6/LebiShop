...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_UserMessageWrite"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_UserMessageWrite","title")%></h2>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> cnbbs=Languages();RecordCount=cnbbs.Count;int cnbb_index=1;
foreach (Shop.Model.Lebi_Language cnbb in cnbbs){%>
<a <%if (cnbb_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=cnbb.id%>,'<%=cnbb.Code%>','<%=cnbb.Path%>');"><img src="<%=Image(cnbb.ImageUrl) %>" title="<%=cnbb.Name%>" /><%=cnbb.Name%></a>
<%cnbb_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int fGIi_index=1;
List<Lebi_Currency> fGIis = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency fGIi in fGIis){%>
<dd <%if (fGIi_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=fGIi.id%>,'<%=fGIi.Code%>',<%=fGIi.ExchangeRate%>,'<%=fGIi.Msige%>');"><%=fGIi.Code%></a></dd>
<%fGIi_index++;}%>
</dl></li></ul></div>
