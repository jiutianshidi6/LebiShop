...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_UserProfile"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_UserProfile","title")%></h2>
...
<div class="userbox">
    ...
    <dl>
        <dt>出生日期：</dt>
        <dd><input type="text" name="Birthday" size="30" value="<%=FormatDate(CurrentUser.Birthday) %>" id="Birthday" class="input" shop="true" onClick="WdatePicker()" /></dd>
    </dl>
    <dl>
        <dt>性别：</dt>
    ...
    <dl>
        <dt>手机号码：</dt>
        <dd><input type="text" name="MobilePhone" id="MobilePhone" size="30" value="<%=CurrentUser.MobilePhone %>" class="input" shop="true" /></dd>
    </dl>
    <dl>
        <dt>电话号码：</dt>
    ...
    <dl>
        <dt>Email地址：</dt>
        <dd><input type="text" name="Email" id="Email" size="30" value="<%=CurrentUser.Email %>" class="input" shop="true" /></dd>
    </dl>
    <dl>
        <dt>QQ号码：</dt>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> YlvLs=Languages();RecordCount=YlvLs.Count;int YlvL_index=1;
foreach (Shop.Model.Lebi_Language YlvL in YlvLs){%>
<a <%if (YlvL_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=YlvL.id%>,'<%=YlvL.Code%>','<%=YlvL.Path%>');"><img src="<%=Image(YlvL.ImageUrl) %>" title="<%=YlvL.Name%>" /><%=YlvL.Name%></a>
<%YlvL_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int zMyR_index=1;
List<Lebi_Currency> zMyRs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency zMyR in zMyRs){%>
<dd <%if (zMyR_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=zMyR.id%>,'<%=zMyR.Code%>',<%=zMyR.ExchangeRate%>,'<%=zMyR.Msige%>');"><%=zMyR.Code%></a></dd>
<%zMyR_index++;}%>
</dl></li></ul></div>
