...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_UserComment"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_UserComment","title")%></h2>
...
<div class="user">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $("area[rel^='prettyPhoto<%=c.id %>']").prettyPhoto();
            $(".piclist<%=c.id %>:first a[rel^='prettyPhoto<%=c.id %>']").prettyPhoto({ animation_speed: 'fast', slideshow: 10000, hideflash: true });
        })
    </script>
    <table cellpadding="0" cellspacing="0" width="100%">
    <tr>
        <th style="width:50px" valign="top" rowspan="2"><% if (GetProduct(c.Keyid).ImageSmall == ""){ %>&nbsp;<%}else{ %><a href="<%=URL("P_Product",c.Keyid) %>"><img style="height: 50px;width: 50px;vertical-align:middle" src="<%=Image(GetProduct(c.Keyid).ImageSmall) %>" /></a><%} %></th>
    ...
    <ul class="piclist<%=c.id %>">
        ...
        <li class="imagespreviewlist">
            <span class="image"><a href="<%=WebPath+bigs[i] %>" rel="prettyPhoto<%=c.id %>[]" target="_blank"><img src="<%=WebPath+images[i] %>" /></a></span>
        </li>
            <%} %>
    </ul>
    </td></tr>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> OhUbs=Languages();RecordCount=OhUbs.Count;int OhUb_index=1;
foreach (Shop.Model.Lebi_Language OhUb in OhUbs){%>
<a <%if (OhUb_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=OhUb.id%>,'<%=OhUb.Code%>','<%=OhUb.Path%>');"><img src="<%=Image(OhUb.ImageUrl) %>" title="<%=OhUb.Name%>" /><%=OhUb.Name%></a>
<%OhUb_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int oIRn_index=1;
List<Lebi_Currency> oIRns = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency oIRn in oIRns){%>
<dd <%if (oIRn_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=oIRn.id%>,'<%=oIRn.Code%>',<%=oIRn.ExchangeRate%>,'<%=oIRn.Msige%>');"><%=oIRn.Code%></a></dd>
<%oIRn_index++;}%>
</dl></li></ul></div>
