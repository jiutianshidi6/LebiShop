...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_UserAddress"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_UserAddress","title")%></h2>
...
<div class="user">
    ...
    <div class="table-list">
        ...
        <tr>
            <td colspan="2"><%if (CurrentUser.User_Address_id == ua.id){ %>[默认]&nbsp;<%} %><%=Shop.Bussiness.EX_Area.GetAreaName(ua.Area_id,2) %> <%=ua.Address%></td>
        </tr>
        <tr>
            <td colspan="2"><a href="<%=URL("P_UserAddressEdit",ua.id,"")%>">编辑</a></td>
        </tr>
        </table>
        <%} %>
    </div>
    <div class="bottom clearfix"><div class="left"><a href="javascript:void(0);" onclick="$('input[name=\'id\']').attr('checked',!$(this).attr('checked'));$(this).attr('checked',!$(this).attr('checked'));" class="selectAll">全选</a>&nbsp;&nbsp;<a href="javascript:void(0)" onclick="Delete();" class="btn btn-11"><s></s>删除</a><a href="<%=URL("P_UserAddressEdit","0","")%>" class="btn btn-7"><s></s>添加</a></div></div>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> HXuvs=Languages();RecordCount=HXuvs.Count;int HXuv_index=1;
foreach (Shop.Model.Lebi_Language HXuv in HXuvs){%>
<a <%if (HXuv_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=HXuv.id%>,'<%=HXuv.Code%>','<%=HXuv.Path%>');"><img src="<%=Image(HXuv.ImageUrl) %>" title="<%=HXuv.Name%>" /><%=HXuv.Name%></a>
<%HXuv_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int gJrz_index=1;
List<Lebi_Currency> gJrzs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency gJrz in gJrzs){%>
<dd <%if (gJrz_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=gJrz.id%>,'<%=gJrz.Code%>',<%=gJrz.ExchangeRate%>,'<%=gJrz.Msige%>');"><%=gJrz.Code%></a></dd>
<%gJrz_index++;}%>
</dl></li></ul></div>
