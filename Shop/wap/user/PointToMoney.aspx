...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_UserPointToMoney"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_UserPointToMoney","title")%></h2>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> aAhgs=Languages();RecordCount=aAhgs.Count;int aAhg_index=1;
foreach (Shop.Model.Lebi_Language aAhg in aAhgs){%>
<a <%if (aAhg_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=aAhg.id%>,'<%=aAhg.Code%>','<%=aAhg.Path%>');"><img src="<%=Image(aAhg.ImageUrl) %>" title="<%=aAhg.Name%>" /><%=aAhg.Name%></a>
<%aAhg_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int bDGH_index=1;
List<Lebi_Currency> bDGHs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency bDGH in bDGHs){%>
<dd <%if (bDGH_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=bDGH.id%>,'<%=bDGH.Code%>',<%=bDGH.ExchangeRate%>,'<%=bDGH.Msige%>');"><%=bDGH.Code%></a></dd>
<%bDGH_index++;}%>
</dl></li></ul></div>
