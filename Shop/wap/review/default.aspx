...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_ProductCommentIndex"); %>
...
<script src="http://192.168.1.188/Theme/system/wap/js/jquery.masonry.min.js" type="text/javascript"></script>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_ProductCommentIndex","title")%></h2>
...
<script type="text/javascript">
$(function () {
    blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int oQUC_index=1;
List<Lebi_Searchkey> oQUCs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey oQUC in oQUCs){%><%=Lang(oQUC.Name)%><%oQUC_index++;}%><%} %>');
...
<div id="keywords">
    ...
    <ul class="text">
        <%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=18;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int kEhh_index=1;
List<Lebi_Searchkey> kEhhs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey kEhh in kEhhs){%>
        <li><%if (kEhh.Type==1){ %><a href="<%=URL("P_Search",""+Lang(kEhh.Name)+"") %>"><%}else{ %><a href="<%=kEhh.URL%>"><%} %><%=Lang(kEhh.Name)%></a></li>
        <%kEhh_index++;}%>
    </ul>
    </div>
    </div>
    ...
    <div class="mc clearfix">
    <ul class="text">
        <%Table="Lebi_Brand";Where="IsRecommend=1";Order="Sort desc,id desc";PageSize=18;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int hFHN_index=1;
List<Lebi_Brand> hFHNs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand hFHN in hFHNs){%>
        <li><a href="<%=URL("P_Brand",hFHN.id)%>" title="<%=Lang(hFHN.Name) %>"><%=Lang(hFHN.Name) %></a></li>
        <%hFHN_index++;}%>
    </ul>
    </div>
    </div>
...
<div class="mc clearfix">
   ...
   <%} %>
	<%Table="Lebi_Page";Where="Node_id="+Node("FootMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int NGIQ_index=1;
List<Lebi_Page> NGIQs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page NGIQ in NGIQs){%>
   <div class="item"><span class="itemname"><h3><a href="<%=URL("","",NGIQ.url)%>"><%=NGIQ.Name%></a></h3><s></s></span></div>
	<%NGIQ_index++;}%>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> yVJms=Languages();RecordCount=yVJms.Count;int yVJm_index=1;
foreach (Shop.Model.Lebi_Language yVJm in yVJms){%>
<a <%if (yVJm_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=yVJm.id%>,'<%=yVJm.Code%>','<%=yVJm.Path%>');"><img src="<%=Image(yVJm.ImageUrl) %>" title="<%=yVJm.Name%>" /><%=yVJm.Name%></a>
<%yVJm_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int snHX_index=1;
List<Lebi_Currency> snHXs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency snHX in snHXs){%>
<dd <%if (snHX_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=snHX.id%>,'<%=snHX.Code%>',<%=snHX.ExchangeRate%>,'<%=snHX.Msige%>');"><%=snHX.Code%></a></dd>
<%snHX_index++;}%>
</dl></li></ul></div>
