...
<% LoadPage("ljq@lebi.cn_43",8,"CN","P_LimitBuy"); %>
...
<div class="mhead clearfix">
	...
	<h2><%=ThemePageMeta("P_LimitBuy","title")%></h2>
...
$(function () {
    blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vOmA_index=1;
List<Lebi_Searchkey> vOmAs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vOmA in vOmAs){%><%=Lang(vOmA.Name)%><%vOmA_index++;}%><%} %>');
...
<div id="keywords">
    ...
    <div class="mc clearfix">
    <ul class="text">
        <%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=18;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int DJOt_index=1;
List<Lebi_Searchkey> DJOts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey DJOt in DJOts){%>
        <li><%if (DJOt.Type==1){ %><a href="<%=URL("P_Search",""+Lang(DJOt.Name)+"") %>"><%}else{ %><a href="<%=DJOt.URL%>"><%} %><%=Lang(DJOt.Name)%></a></li>
        <%DJOt_index++;}%>
    </ul>
    </div>
    </div>
    ...
    <div class="mc clearfix">
    <ul class="text">
        <%Table="Lebi_Brand";Where="IsRecommend=1";Order="Sort desc,id desc";PageSize=18;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int PMYz_index=1;
List<Lebi_Brand> PMYzs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand PMYz in PMYzs){%>
        <li><a href="<%=URL("P_Brand",PMYz.id)%>" title="<%=Lang(PMYz.Name) %>"><%=Lang(PMYz.Name) %></a></li>
        <%PMYz_index++;}%>
    </ul>
    </div>
    </div>
...
<div class="mc clearfix">
   ...
   <%} %>
	<%Table="Lebi_Page";Where="Node_id="+Node("FootMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int IWQs_index=1;
List<Lebi_Page> IWQss = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page IWQs in IWQss){%>
   <div class="item"><span class="itemname"><h3><a href="<%=URL("","",IWQs.url)%>"><%=IWQs.Name%></a></h3><s></s></span></div>
	<%IWQs_index++;}%>
...
<div class="language">
<%List<Shop.Model.Lebi_Language> RwPMs=Languages();RecordCount=RwPMs.Count;int RwPM_index=1;
foreach (Shop.Model.Lebi_Language RwPM in RwPMs){%>
<a <%if (RwPM_index==RecordCount){%>class="last"<%} %> href="javascript:SetLanguage(<%=RwPM.id%>,'<%=RwPM.Code%>','<%=RwPM.Path%>');"><img src="<%=Image(RwPM.ImageUrl) %>" title="<%=RwPM.Name%>" /><%=RwPM.Name%></a>
<%RwPM_index++;}%>
</div>
...
<%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int NVOi_index=1;
List<Lebi_Currency> NVOis = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency NVOi in NVOis){%>
<dd <%if (NVOi_index==RecordCount){%>class="last"<%} %>><a href="javascript:SetCurrency(<%=NVOi.id%>,'<%=NVOi.Code%>',<%=NVOi.ExchangeRate%>,'<%=NVOi.Msige%>');"><%=NVOi.Code%></a></dd>
<%NVOi_index++;}%>
</dl></li></ul></div>
