<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.Supplier.Config.ServicePanel_Group" validateRequest="false"%>
...
<div id="sidecontent" class="clear">
    ...
    <li class="name"><span id="navIgation"><%=Tag("当前位置")%>：<a href="<%=site.AdminPath %>/Ajax/ajax_supplier.aspx?__Action=MenuJump&pid=0"><%=Tag("管理首页")%></a> > <a href="<%=site.AdminPath %>/Config/ServicePanel.aspx"><%=Tag("客服面板")%></a> > <%=Tag("部门管理")%></span></li>
    </ul>
    </div>
    ...
    <table cellpadding="0" cellspacing="0" width="100%" border="0" class="datalist">
        ...
        <%foreach (Shop.Model.Lebi_ServicePanel_Group model in models)
          {%>
    ...
    <script type="text/javascript">
        ...
        function Update() {
            ...
            var url = "<%=site.AdminPath %>/ajax/ajax_config.aspx?__Action=ServicePanel_Group_Update";
        ...
        function DEL() {
            ...
            var url = "<%=site.AdminPath %>/ajax/ajax_config.aspx?__Action=ServicePanel_Group_Del";
            RequestAjax(url,postData,function(){MsgBox(1, "<%=Tag("操作成功")%>", "?")});
        }
    </script>
