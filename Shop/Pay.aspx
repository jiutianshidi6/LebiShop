...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> zmhes=Languages();RecordCount=zmhes.Count;int zmhe_index=1;
foreach (Shop.Model.Lebi_Language zmhe in zmhes){%>
            <dd <%if (zmhe_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=zmhe.id%>,'<%=zmhe.Code%>','<%=zmhe.Path%>');"><%if (zmhe.ImageUrl!=""){%><img src="<%=Image(zmhe.ImageUrl) %>" /><%}%><%=zmhe.Name%></a></dd>
            <%zmhe_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int oRTi_index=1;
List<Lebi_Currency> oRTis = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency oRTi in oRTis){%>
            <dd <%if (oRTi_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=oRTi.id%>,'<%=oRTi.Code%>',<%=oRTi.ExchangeRate%>,'<%=oRTi.Msige%>','<%=oRTi.DecimalLength%>');"><%=oRTi.Code%></a></dd>
            <%oRTi_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int JJsp_index=1;
List<Lebi_Searchkey> JJsps = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey JJsp in JJsps){%><%=Lang(JJsp.Name)%><%JJsp_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int veQt_index=1;
List<Lebi_Searchkey> veQts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey veQt in veQts){%>
<%if (veQt.Type==1){ %><a href="<%=URL("P_Search",""+Lang(veQt.Name)+"") %>"><%}else{ %><a href="<%=veQt.URL%>" target="_blank"><%} %><span><%=Lang(veQt.Name)%></span></a>
<%veQt_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int XkzC_index=1;
List<Lebi_Page> XkzCs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page XkzC in XkzCs){%>
<a class="menu" href="<%=URL("","",XkzC.url)%>"><span><%=XkzC.Name%></span></a>
<%XkzC_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int qRgJ_index=1;
List<Lebi_FriendLink> qRgJs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink qRgJ in qRgJs){%>
            
                <% if (qRgJ.Logo != "" && qRgJ.IsPic == 1){ %><a href="<%=qRgJ.Url%>" target="_blank"><img src="<%=Image(qRgJ.Logo) %>" alt="<%=qRgJ.Name%>" /></a><%}else{%><a href="<%=qRgJ.Url%>" target="_blank"><%=qRgJ.Name%></a><%} %>     
