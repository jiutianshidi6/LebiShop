...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_About"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> QKbis=Languages();RecordCount=QKbis.Count;int QKbi_index=1;
foreach (Shop.Model.Lebi_Language QKbi in QKbis){%>
            <dd <%if (QKbi_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=QKbi.id%>,'<%=QKbi.Code%>','<%=QKbi.Path%>');"><%if (QKbi.ImageUrl!=""){%><img src="<%=Image(QKbi.ImageUrl) %>" /><%}%><%=QKbi.Name%></a></dd>
            <%QKbi_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int WeQi_index=1;
List<Lebi_Currency> WeQis = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency WeQi in WeQis){%>
            <dd <%if (WeQi_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=WeQi.id%>,'<%=WeQi.Code%>',<%=WeQi.ExchangeRate%>,'<%=WeQi.Msige%>','<%=WeQi.DecimalLength%>');"><%=WeQi.Code%></a></dd>
            <%WeQi_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Xmfd_index=1;
List<Lebi_Searchkey> Xmfds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Xmfd in Xmfds){%><%=Lang(Xmfd.Name)%><%Xmfd_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int okXx_index=1;
List<Lebi_Searchkey> okXxs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey okXx in okXxs){%>
<%if (okXx.Type==1){ %><a href="<%=URL("P_Search",""+Lang(okXx.Name)+"") %>"><%}else{ %><a href="<%=okXx.URL%>" target="_blank"><%} %><span><%=Lang(okXx.Name)%></span></a>
<%okXx_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int eBmL_index=1;
List<Lebi_Page> eBmLs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page eBmL in eBmLs){%>
<a class="menu" href="<%=URL("","",eBmL.url)%>"><span><%=eBmL.Name%></span></a>
<%eBmL_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int MlUE_index=1;
List<Lebi_FriendLink> MlUEs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink MlUE in MlUEs){%>
            
                <% if (MlUE.Logo != "" && MlUE.IsPic == 1){ %><a href="<%=MlUE.Url%>" target="_blank"><img src="<%=Image(MlUE.Logo) %>" alt="<%=MlUE.Name%>" /></a><%}else{%><a href="<%=MlUE.Url%>" target="_blank"><%=MlUE.Name%></a><%} %>     
