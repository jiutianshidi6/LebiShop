...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_ProductCommentIndex"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> iAYgs=Languages();RecordCount=iAYgs.Count;int iAYg_index=1;
foreach (Shop.Model.Lebi_Language iAYg in iAYgs){%>
            <dd <%if (iAYg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=iAYg.id%>,'<%=iAYg.Code%>','<%=iAYg.Path%>');"><%if (iAYg.ImageUrl!=""){%><img src="<%=Image(iAYg.ImageUrl) %>" /><%}%><%=iAYg.Name%></a></dd>
            <%iAYg_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int nJzw_index=1;
List<Lebi_Currency> nJzws = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency nJzw in nJzws){%>
            <dd <%if (nJzw_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=nJzw.id%>,'<%=nJzw.Code%>',<%=nJzw.ExchangeRate%>,'<%=nJzw.Msige%>','<%=nJzw.DecimalLength%>');"><%=nJzw.Code%></a></dd>
            <%nJzw_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int crXq_index=1;
List<Lebi_Searchkey> crXqs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey crXq in crXqs){%><%=Lang(crXq.Name)%><%crXq_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zjay_index=1;
List<Lebi_Searchkey> zjays = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zjay in zjays){%>
<%if (zjay.Type==1){ %><a href="<%=URL("P_Search",""+Lang(zjay.Name)+"") %>"><%}else{ %><a href="<%=zjay.URL%>" target="_blank"><%} %><span><%=Lang(zjay.Name)%></span></a>
<%zjay_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int lBTK_index=1;
List<Lebi_Page> lBTKs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page lBTK in lBTKs){%>
<a class="menu" href="<%=URL("","",lBTK.url)%>"><span><%=lBTK.Name%></span></a>
<%lBTK_index++;}%>
...
<div class="body clearfix">
  <div class="location"><div class="path"><%=path%></div></div>
    ...
    <div id="load">
        <div id="status"></div>
        <a href="javascript:void(0)" onclick="LoadMore();" class="btn btn-11"><s></s>加载更多</a>
    </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int oJee_index=1;
List<Lebi_FriendLink> oJees = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink oJee in oJees){%>
            
                <% if (oJee.Logo != "" && oJee.IsPic == 1){ %><a href="<%=oJee.Url%>" target="_blank"><img src="<%=Image(oJee.Logo) %>" alt="<%=oJee.Name%>" /></a><%}else{%><a href="<%=oJee.Url%>" target="_blank"><%=oJee.Name%></a><%} %>     
