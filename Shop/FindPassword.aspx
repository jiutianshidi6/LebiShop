...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_FindPassword"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> XPeXs=Languages();RecordCount=XPeXs.Count;int XPeX_index=1;
foreach (Shop.Model.Lebi_Language XPeX in XPeXs){%>
            <dd <%if (XPeX_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=XPeX.id%>,'<%=XPeX.Code%>','<%=XPeX.Path%>');"><%if (XPeX.ImageUrl!=""){%><img src="<%=Image(XPeX.ImageUrl) %>" /><%}%><%=XPeX.Name%></a></dd>
            <%XPeX_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int JTsV_index=1;
List<Lebi_Currency> JTsVs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency JTsV in JTsVs){%>
            <dd <%if (JTsV_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=JTsV.id%>,'<%=JTsV.Code%>',<%=JTsV.ExchangeRate%>,'<%=JTsV.Msige%>','<%=JTsV.DecimalLength%>');"><%=JTsV.Code%></a></dd>
            <%JTsV_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int uDVm_index=1;
List<Lebi_Searchkey> uDVms = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey uDVm in uDVms){%><%=Lang(uDVm.Name)%><%uDVm_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zXyk_index=1;
List<Lebi_Searchkey> zXyks = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zXyk in zXyks){%>
<%if (zXyk.Type==1){ %><a href="<%=URL("P_Search",""+Lang(zXyk.Name)+"") %>"><%}else{ %><a href="<%=zXyk.URL%>" target="_blank"><%} %><span><%=Lang(zXyk.Name)%></span></a>
<%zXyk_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int ifve_index=1;
List<Lebi_Page> ifves = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page ifve in ifves){%>
<a class="menu" href="<%=URL("","",ifve.url)%>"><span><%=ifve.Name%></span></a>
<%ifve_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int hmmU_index=1;
List<Lebi_FriendLink> hmmUs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink hmmU in hmmUs){%>
            
                <% if (hmmU.Logo != "" && hmmU.IsPic == 1){ %><a href="<%=hmmU.Url%>" target="_blank"><img src="<%=Image(hmmU.Logo) %>" alt="<%=hmmU.Name%>" /></a><%}else{%><a href="<%=hmmU.Url%>" target="_blank"><%=hmmU.Name%></a><%} %>     
