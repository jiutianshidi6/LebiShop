...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_AddToBasket"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> Eygzs=Languages();RecordCount=Eygzs.Count;int Eygz_index=1;
foreach (Shop.Model.Lebi_Language Eygz in Eygzs){%>
            <dd <%if (Eygz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Eygz.id%>,'<%=Eygz.Code%>','<%=Eygz.Path%>');"><%if (Eygz.ImageUrl!=""){%><img src="<%=Image(Eygz.ImageUrl) %>" /><%}%><%=Eygz.Name%></a></dd>
            <%Eygz_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int TDht_index=1;
List<Lebi_Currency> TDhts = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency TDht in TDhts){%>
            <dd <%if (TDht_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=TDht.id%>,'<%=TDht.Code%>',<%=TDht.ExchangeRate%>,'<%=TDht.Msige%>','<%=TDht.DecimalLength%>');"><%=TDht.Code%></a></dd>
            <%TDht_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int gmId_index=1;
List<Lebi_Searchkey> gmIds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey gmId in gmIds){%><%=Lang(gmId.Name)%><%gmId_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int rlIK_index=1;
List<Lebi_Searchkey> rlIKs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey rlIK in rlIKs){%>
<%if (rlIK.Type==1){ %><a href="<%=URL("P_Search",""+Lang(rlIK.Name)+"") %>"><%}else{ %><a href="<%=rlIK.URL%>" target="_blank"><%} %><span><%=Lang(rlIK.Name)%></span></a>
<%rlIK_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int icRI_index=1;
List<Lebi_Page> icRIs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page icRI in icRIs){%>
<a class="menu" href="<%=URL("","",icRI.url)%>"><span><%=icRI.Name%></span></a>
<%icRI_index++;}%>
...
<div class="tag-tab">
    <ul class="tablist">
    <%Table="Lebi_Pro_Tag";Where="1=1";Order="Sort desc";PageSize=4;pageindex=1;RecordCount=B_Lebi_Pro_Tag.Counts(Where);int Jtnp_index=1;
List<Lebi_Pro_Tag> Jtnps = B_Lebi_Pro_Tag.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Pro_Tag Jtnp in Jtnps){%>
        <li <%=Jtnp_index==1?" class=\"selected\"":"" %>><a href="<%=URL("P_ProductCategory","$,$,$,$,$,"+Jtnp.id+"")%>" target="_blank"><span><%=Lang(Jtnp.Name) %></span></a></li>
    <%Jtnp_index++;}%>
    </ul>
</div>
...
List<Lebi_Product> tab_tag_pros = B_Lebi_Product.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Product tab_tag_pro in tab_tag_pros){%>
        <li>
            ...
            <div class="proprice"><%if (tab_tag_pro.Price_Market>0){ %><div class="marketprice"><i>市场价：</i><strong><%=FormatMoney(tab_tag_pro.Price_Market) %></strong></div><%} %><div class="buyprice"><i><%=Lang(CurrentUserLevel.PriceName) %>：</i><strong><%=FormatMoney(ProductPrice(tab_tag_pro)) %></strong></div></div>                    
        </li>
        <%tab_tag_pro_index++;}%>
        </ul></div>
    </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int qvEs_index=1;
List<Lebi_FriendLink> qvEss = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink qvEs in qvEss){%>
            
                <% if (qvEs.Logo != "" && qvEs.IsPic == 1){ %><a href="<%=qvEs.Url%>" target="_blank"><img src="<%=Image(qvEs.Logo) %>" alt="<%=qvEs.Name%>" /></a><%}else{%><a href="<%=qvEs.Url%>" target="_blank"><%=qvEs.Name%></a><%} %>     
