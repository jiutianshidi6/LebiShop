...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<Shop.Model.Lebi_Language> suRes=Languages();RecordCount=suRes.Count;int suRe_index=1;
foreach (Shop.Model.Lebi_Language suRe in suRes){%>
            <dd <%if (suRe_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=suRe.id%>,'<%=suRe.Code%>','<%=suRe.Path%>');"><%if (suRe.ImageUrl!=""){%><img src="<%=Image(suRe.ImageUrl) %>" /><%}%><%=suRe.Name%></a></dd>
            <%suRe_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Jwze_index=1;
List<Lebi_Currency> Jwzes = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Jwze in Jwzes){%>
            <dd <%if (Jwze_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Jwze.id%>,'<%=Jwze.Code%>',<%=Jwze.ExchangeRate%>,'<%=Jwze.Msige%>','<%=Jwze.DecimalLength%>');"><%=Jwze.Code%></a></dd>
            <%Jwze_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vUpC_index=1;
List<Lebi_Searchkey> vUpCs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vUpC in vUpCs){%><%=Lang(vUpC.Name)%><%vUpC_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int bQXt_index=1;
List<Lebi_Searchkey> bQXts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey bQXt in bQXts){%>
<%if (bQXt.Type==1){ %><a href="<%=URL("P_Search",""+Lang(bQXt.Name)+"") %>"><%}else{ %><a href="<%=bQXt.URL%>" target="_blank"><%} %><span><%=Lang(bQXt.Name)%></span></a>
<%bQXt_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int CGJa_index=1;
List<Lebi_Page> CGJas = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page CGJa in CGJas){%>
<a class="menu" href="<%=URL("","",CGJa.url)%>"><span><%=CGJa.Name%></span></a>
<%CGJa_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int AQRa_index=1;
List<Lebi_FriendLink> AQRas = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink AQRa in AQRas){%>
            
                <% if (AQRa.Logo != "" && AQRa.IsPic == 1){ %><a href="<%=AQRa.Url%>" target="_blank"><img src="<%=Image(AQRa.Logo) %>" alt="<%=AQRa.Name%>" /></a><%}else{%><a href="<%=AQRa.Url%>" target="_blank"><%=AQRa.Name%></a><%} %>     
