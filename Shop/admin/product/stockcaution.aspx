<%@ Page Language="C#" AutoEventWireup="true" Inherits="Lebi.ERP.Bussiness.pagebase.StockCaution" validateRequest="false"%>
...
<%
    if (!Shop.Bussiness.EX_Admin.Power("log_view", "查看操作日志"))
    {
        PageNoPower();
        return;
    }
 %>
    <div class="searchbox">
        <input type="text" name="dateFrom" id="dateFrom" size="12" class="input-calendar" value="<%=dateFrom %>" onClick="WdatePicker()" /> - <input type="text" name="dateTo" id="dateTo" size="12" class="input-calendar" value="<%=dateTo %>" onClick="    WdatePicker()" /><input type="button" id="btnSou" class="btn-query" onclick="    search_();" align="absmiddle" />
    </div>
    ...
    <table class="datalist">
        ...
        <tr class="list" ondblclick="View(<%=model.id %>);">
            ...
            <td>
                <div class="piclist<%=model.id %>"><% if (GetProduct(model.Keyid).ImageSmall == "")
                   { %>
                &nbsp;
                <%}else
                   { %>
                <a href="<%=Image(GetProduct(model.Keyid).ImageOriginal,"big") %>" rel="prettyPhoto<%=model.id %>[]" target="_blank"><img style="height: 30px;width: 30px;vertical-align:middle" src="<%=Image(GetProduct(model.Keyid).ImageOriginal,50,50) %>" /></a>
                <%} %></div>
            </td>
            <td>
    ...
    <script type="text/javascript">
        ...
        function searchproduct(id)
        {
            var dateFrom = $("#dateFrom").val();
            var dateTo = $("#dateTo").val();
            window.location = "?dateFrom=" + dateFrom + "&dateTo=" + dateTo+"&tid="+id ;
        }
    </script>
