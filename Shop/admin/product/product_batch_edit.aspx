...
<div id="sidecontent" class="clear">
    ...
    <li class="edit"><a href="javascript:void(0);" onclick="Product_limit_Edit_muti();"><b></b><span><%=Tag("权限")%></span></a></li>
    ...
    <table id="list" class="datalist">
        ...
        <script type="text/javascript">
            ...
            shop="true" value="<%=model.Price_Market %>" style="width: 70px;" onkeyup="value=value.replace(/[^\d\.]/g,'')" />
                </td>
                <td>
                    <input type="text" name="Price<%=model.id %>" id="Price<%=model.id %>" class="input"
            shop="true" value="<%=model.Price %>" style="width: 80px;" onkeyup="value=value.replace(/[^\d\.]/g,'')" />
                </td>
                <%if (Shop.Bussiness.EX_Admin.CheckPower("product_price_cost")){ %>
                <td>
                    <input type="text" name="Price_Cost<%=model.id %>" id="Price_Cost<%=model.id %>" class="input"
            shop="true" value="<%=model.Price_Cost %>" style="width: 80px;" onkeyup="value=value.replace(/[^\d\.]/g,'')" />
                </td>
                <%} %>
                <%if (Shop.LebiAPI.Service.Instanse.Check("plugin_product_price")){ %>
                    <%foreach(Shop.Model.Lebi_UserLevel ul in userlevels){ %>
                ...
                shop="true" value="<%=GetUserLevelPrice(ulps,ul.id) %>" style="width: 80px;" onkeyup="value=value.replace(/[^\d\.]/g,'')" />
                    </td>
                    <%} %>
                <%} %>
                <td>
            ...
            shop="true" value="<%=model.Count_Stock %>" style="width: 80px;" onkeyup="value=value.replace(/[^\d\.]/g,'')" />
                    <%}else{ %>
                    <input type="hidden" name="Count_Stock<%=model.id %>" id="Hidden1" shop="true" value="<%=model.Count_Stock %>" /><%=model.Count_Stock %>
                    <%} %>
                </td>
                <td>
    ...
    <script type="text/javascript">
        ...
        function Product_limit_Edit_muti(){
            var title_ = "<%=Tag("商品权限")%>"; 
            var height_ = 'auto';
            var url_ = "product_limit_edit_window.aspx";
            var width_ = 400;
            var modal_ = true;
            EditWindow(title_, url_, width_, height_, modal_);
        }
        ...
        {
            $("#Pro_Type_id").find("option[value='"+id+"']").attr("selected",true);
            search_();
        }
    </script>
