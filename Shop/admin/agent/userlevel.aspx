<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.Admin.agent.UserLevel" validateRequest="false"%>
...
<meta name="author" content="LebiShop" />
    ...
    <title><%=Tag("会员分组")%>-<%=Tag("代理")%>-<%=site.title%></title>
...
<div id="sidecontent" class="clear">
    ...
    <table class="datalist">
        ...
        <tr class="list" ondblclick="Edit(<%=model.id %>);">
            ...
            <td>
                <a href="user.aspx?level=<%=model.id %>"><%=Shop.Bussiness.EX_User.UserCount(model.id)%></a>
            </td>
            <td>
                <%=Shop.Bussiness.Agent.UserLevel_Commission(model.id).Angent1_Commission %> %
            </td>
            <td>
                <%=Shop.Bussiness.Agent.UserLevel_Commission(model.id).Angent2_Commission %> %
            </td>
            <td>
                <%=Shop.Bussiness.Agent.UserLevel_Commission(model.id).Angent3_Commission %> %
            </td>
            <td>
...
<script type="text/javascript">
    function Edit(id) {
        var title_ = "<%=Tag("编辑")%>";
        var url_ = "userlevel_edit_window.aspx?id=" + id;
        var width_ = 400;
        var height_ = 200;
        var modal_ = true;
