...
string id = Shop.Tools.RequestTool.RequestSafeString("id");
int loop = Shop.Tools.RequestTool.RequestInt("loop",0);
string where = "";
int Count = 1;
if (!id.Contains(",")){
    where = "id = lbsql{"+id+"}";
...
<body>
    <div class="print-btn">
        <div class="tools">
            <ul>
                <%if ((loop+1) < Count){ %>
                    <li class="tip"><a href="?id=<%=id%>&loop=<%=loop+1 %>"><b></b><span><%=Tag("下一个")%>(<%=Tag("剩余")%>：<%=Count-loop-1 %>)</span></a></li>
                <%} %>
    ...
    <div class="order-print">
        ...
        <table cellspacing="0" cellpadding="0" align="center">
            ...
            <tr>
                <td colspan="3" valign="top">
                    <%=Shop.Bussiness.Language.Tag("订单留言", UserLanguage)%>：<%int i =0;foreach (Shop.Model.Lebi_Comment comm in comms){ %><%=comm.Content%><%if (i>0){Response.Write("<br/>");}i++;} %>
                </td>
            </tr>
        </table>
        <table cellspacing="0" cellpadding="0" align="center">
