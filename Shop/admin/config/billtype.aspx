<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.Admin.storeConfig.BillType" validateRequest="false"%>
...
<style type="text/css">.mainbody_top{display:none;height:0;overflow:hidden}</style>
    ...
    <%}%>
    <li class="name"><span id="navIgation"><%=Tag("当前位置")%>：<a href="<%=site.AdminPath %>/Ajax/ajax_admin.aspx?__Action=MenuJump&pid=0"><%=Tag("管理首页")%></a> > <%=Tag("发票类型")%></span></li>
    </ul>
    </div>
    ...
    <table id="list" class="datalist">
        ...
        <%foreach (Shop.Model.Lebi_BillType model in models)
          {%>
