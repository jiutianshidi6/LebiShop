<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.statis.category" validateRequest="false"%>
...
<div id="sidecontent" class="clear">
    ...
    subtitle: {text: ''},
    xAxis: {categories: [<%
    List<Shop.Model.Lebi_Pro_Type> nodes = Shop.Bussiness.EX_Product.Types(0);
    int i=1; 
    int count=nodes.Count;
    foreach (Shop.Model.Lebi_Pro_Type node in nodes)
    {
    ...
    series: [{name: '<%=Tag("商品数量") %>',data: [<%foreach (Shop.Model.Lebi_Pro_Type node in nodes){%><%=Shop.Bussiness.EX_Product.TypeProductCount(node.id)%>,<%} %>]},{name: '<%=Tag("收藏数量") %>',data: [<%foreach (Shop.Model.Lebi_Pro_Type node in nodes){%><%=Shop.Bussiness.EX_Product.LikeProductCount(node.id)%>,<%} %>]},{name: '<%=Tag("销量") %>',data: [<%foreach (Shop.Model.Lebi_Pro_Type node in nodes){%><%=Shop.Bussiness.EX_Product.SalesProductCount(node.id)%>,<%} %>]},{name: '<%=Tag("浏览量") %>',data: [<%foreach (Shop.Model.Lebi_Pro_Type node in nodes){%><%=Shop.Bussiness.EX_Product.ViewsProductCount(node.id)%>,<%} %>]}]
    });});
    </script>
