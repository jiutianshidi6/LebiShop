<%@ Page Language="C#" AutoEventWireup="true" Inherits="Shop.statis.userlevel" validateRequest="false"%>
...
<div id="sidecontent" class="clear">
    ...
    series: [{name: '<%=Tag("会员数量") %>',data: [<%foreach (Shop.Model.Lebi_UserLevel model in models){%><%=Shop.Bussiness.EX_User.UserCount(model.id)%>,<%} %>]},{name: '<%=Tag("余额") %>',data: [<%foreach (Shop.Model.Lebi_UserLevel model in models){%><%=Shop.Bussiness.EX_User.MoneyCount(model.id)%>,<%} %>]},{name: '<%=Tag("积分") %>',data: [<%foreach (Shop.Model.Lebi_UserLevel model in models){%><%=Shop.Bussiness.EX_User.PointCount(model.id)%>,<%} %>]},{name: '<%=Tag("订单") %>',data: [<%foreach (Shop.Model.Lebi_UserLevel model in models){%><%=Shop.Bussiness.EX_User.OrderCount(model.id)%>,<%} %>]},{name: '<%=Tag("消费") %>',data: [<%foreach (Shop.Model.Lebi_UserLevel model in models){%><%=Shop.Bussiness.EX_User.Money_xiaofeiCount(model.id)%>,<%} %>]}]
    });});
    </script>
